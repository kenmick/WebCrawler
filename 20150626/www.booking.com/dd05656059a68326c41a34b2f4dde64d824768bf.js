...
a=function(e,d){booking.track.stage(e,d)
};
...
}}function h(P,Q,R){if(k[R]){return false
}e.push([P,Q,R]);
k[R]=1;
...
}function l(P){return !!(b.r[M(P)]!==undefined&&!G["e"+P])
}function K(P){return b.r[M(P)]||b.f[M(P)]||0
}function o(P){if(l(P)){G["e"+P]=true;
f({hash:P})
}return K(P)
...
f({custom:Q,num:P})
}function L(Q,P){if(!Q||!P||!/^[1-9]$/.test(P)||G["s"+Q+"-"+P]){return
}G["s"+Q+"-"+P]=true;
f({hash:Q,num:P});
return true
}function z(P,Q,T){if(window.jQuery){if(!H){H=jQuery(window)
}var S=jQuery(Q).eq(0);
if(!S.length){if(O){console.log("Element not present on the page for exp: "+T)
}return
}if(P==="view"){h(P,S,T);
return
...
if(O){console.log("Set event handler [ "+P+", "+T+"]")
}S.bind(P,R)
}else{}}function I(P,Q){return function(R){if(l(R)){z(P,Q,R)
}else{if(O){console.log((b.f[M(R)]?"Full-on":"Not running")+" experiment: "+R)
}}}
}function t(P,Q){var R=I(P,Q);
return{track:R,exp:R}
...
}}}function f(P){if(!x){x=J(["b_action","b_sid","b_aid","b_lang","pageview_id","b_site_type"])
}var Q;
if(P){c(P)
}if(r){if(O){console.log("Defering js_tracking")
}return
}else{if(Q=i()){r=true;
$.ajax({url:"/js_tracking?ver=2;"+Q+";ref="+x.b_action+";sid="+x.b_sid+";aid="+x.b_aid+";lang="+x.b_lang+";pid="+x.pageview_id+";stype="+x.b_site_type_id,complete:p});
c({flush:1})
}}}function E(){}E.prototype.variant=K;
E.prototype.track=o;
E.prototype.track_stage=L;
E.prototype.goal=v;
