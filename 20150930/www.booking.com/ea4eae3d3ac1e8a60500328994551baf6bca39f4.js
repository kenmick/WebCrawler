...
},_renderMenu:function(w,z){var y=this,e=false,k=b("#frm");
y.isSynonymTracked=false;
var I,G,E,h,o,m=0,H=["city","airport","district","region","country","hotel","landmark","searchHistory"],C=H.length;
w.removeClass("ui-ac-powered-by-google");
if(z.__did_you_mean__||(z.city&&z.city.length==0)){if(z.city&&z.city.length==0&&z.__upa__&&z.__upa__.length){if(z.__js_upa__){}else{if(!booking.env.b_bbtool_address_search){booking.track.exp("bLYUELMNLNPORLO")
}}w.addClass("ui-ac-powered-by-google");
z.city=z.__upa__
}}if(booking.env.autocomplete_bbtoollocations&&z.bbtoollocations&&z.bbtoollocations.length){z.bbtoollocations.forEach(function(i){if(i.name&&i.address){i.label_highlighted="<b>"+this._escapeHTML(i.name)+"</b>, "+this._escapeHTML(i.address)
}},this);
z.city=z.bbtoollocations.concat(z.city)
}var D=z.theme;
var J=D&&D.length;
var r=z.city&&z.city.length;
if(!r&&!J){w.addClass("ui-ac-empty")
}else{w.removeClass("ui-ac-empty")
}if(z.__did_you_mean__){var x=z.__did_you_mean__,F=x.suggestion,u={value:F,label:F};
var n=k.find(".js-suggestion_clicked"),f=k.find(".js-suggested_term");
if(!n.length){n=b('<input type="hidden" class="js-suggestion_clicked" name="suggestion_clicked" value="0">').appendTo(k);
f=b('<input type="hidden" class="js-suggested_term" name="suggested_term" value="">').appendTo(k)
}n.val("1");
f.val(F);
booking.track.exp("PVdIVMGaOVaMCfFIMNDdWe");
var d=!booking.track.getVariant("PVdIVMGaOVaMCfFIMNDdWe");
if(d){var q=booking.jstmpl.translations("sb_autocomplete_suggest",null,{searchedDestination:F});
var p=booking.jstmpl.translations("sb_autocomplete_suggestions_for",null,{searchedDestination:F});
b("#ac-suggestion").live("click",function(){b("#destination").val(F);
b("#destination").trigger("keydown.autocomplete")
});
b('<li id="ac-suggestion" class="ui-menu-item" role="menuitem" data-suggestion="'+F+'"><a class="partition_item ui-corner-all did-you-mean-item">'+q+"</a></li>").data("item.autocomplete",u).appendTo("ul.ui-autocomplete")
}}else{k.find(".js-suggestion_clicked").val("0");
k.find(".js-suggested_term").val("")
}(function(t){if(!t){return
}var K=b("#suggestion_clicked");
var j=b("#suggested_term");
if(!K.size()){K=b('<input type="hidden" id="suggestion_clicked" name="suggestion_clicked" value="0">');
...
if(z.__suggestions__&&z.__suggestions__.updated_suggestion){var i=z.__suggestions__.updated_suggestion,L={value:i,label:i};
if(z.__suggestions__.has_results){B.track.stage("PAWdLYVJXIJdJQSVWe",1)
}if(t===2){var M=booking.jstmpl.translations("search_box_opt_in_alternative_search",null,{search_term2:z.__suggestions__.initial_term});
var l=booking.jstmpl.translations("search_box_result_your_search",null,{user_searched_term:z.__suggestions__.searched_term});
K.val(1);
b('<li id="ac-suggestion" class="ui-menu-item search-spellcheck" role="menuitem" data-suggestion="'+i+'"><span class="spellcheck-notlink">'+l+'</span><br /><a class="partition_item ui-corner-all did-you-mean-item spellcheck-islink">'+M+"</a></li>").data("item.autocomplete",L).appendTo("ul.ui-autocomplete");
if(!j.val()&&z.__suggestions__.updated_suggestion){j.val(z.__suggestions__.updated_suggestion)
}b("#ac-suggestion.search-spellcheck a").bind("click",function(){y._search(z.__suggestions__.initial_term,{v_out:0});
b("ul.ui-autocomplete").delegate(".partition_item","click.search_rewrite",function(){K.val(0)
})
})
...
for(var v=0;
v<A;
v++){if(D[v]["match_start"]){s.push(D[v])
}}if(A!=s.length){z.theme=s
}if(B.env.b_action=="index"){var g=H.shift();
H.unshift("theme");
H.unshift(g);
C=H.length
}else{delete z.theme
...
I<C;
I++){o=H[I];
if(z[o]){h=y._normalize(z[o]);
for(G=0;
G<h.length;
G++){y._renderItem(w,h[G],G,h.length)
