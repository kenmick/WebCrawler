...
break;
case o.ENTER:case o.NUMPAD_ENTER:if(b("#ac-suggestion.ac-suggestion-opt-out a.ui-state-hover").length){b("#ac-suggestion.search-spellcheck a").trigger("click");
return false
...
},selected:function(o,q){if(typeof(q.item)!=="undefined"&&q.item!==null){var n=q.item.data("item.autocomplete");
var m=l.previous;
if(l.element[0]!==k.activeElement){l.element.focus();
l.previous=m;
setTimeout(function(){l.previous=m
},1)
}if(false!==l._trigger("select",o,{item:n})){if(typeof(n)!=="undefined"){var p="<div>"+n.value+"</div>";
...
}},_initSource:function(){var f=this,j,g,e,h=true;
if(b.isArray(this.options.source)){j=this.options.source;
this.source=function(k,l){l(b.ui.autocomplete.filter(j,k.term))
}
}else{if(typeof this.options.source==="string"){g=this.options.source;
var d=B.require("search/destination/service"),i=B.require("search/destination/service-booking");
i.on("beforeajax",function(k,l){booking.eventEmitter.trigger("AUTOCOMPLETE:beforeAjax",{params:l,ui:f})
});
i.on("afterajax",function(k,l){booking.eventEmitter.trigger("AUTOCOMPLETE:afterAjax",{data:l,ui:f})
});
this.source=function(k,l){if(this.menu.searchInstance){this.menu.searchInstance.abort()
}k.ss=k.term;
k.gpf=true;
k.gpf_encode=true;
delete k.term;
this.menu.searchInstance=d.search(k,function(m,n){if(m){n={city:[]}
}l(n)
});
...
if((!G.__new_popular__&&D(G.dest_id,l))||(G.__new_popular__&&G.flags&&G.flags.popular)){result.find("a").append("<strong class='ac_badge ac_popular_ufi'>"+s+"</strong>");
e=true
...
e.one("keyup",function(){var h=B.require("search/destination/service-atlas");
h.init()
});
...
var b=200;
var c=2;
if(booking.track.getVariant("PAHBYROdVaC")===1&&booking.env.b_lang_for_url.match(/^(zh|ja|ko)/)&&booking.track.getVariant("PVdIGbGWZSBVaHEEKBNKe")===1){c=1
}e.autocomplete({source:a,extraParams:booking.env.search_autocomplete_params,minLength:c,delay:booking.env.search_autocomplete_delay||b}).bind("autocompleteselect",function(p,o){if(o.item){if(B.env.b_can_run_sau){B.track.custom_goal("PVdIESZHO",1)
}d=true;
...
}}}}).bind("autocompleteopen",function(h,i){if($("#calendar_popup").length&&booking[sNSStartup].calendar&&typeof booking[sNSStartup].calendar.closePopupCalendar==="function"){booking[sNSStartup].calendar.closePopupCalendar()
...
if(B.env.b_can_run_sau){B.track.custom_goal("PVdIESZHO",3)
