...
if(i.MD(a[15])){y+=b[28];
if(i.MJ(i.track_experiment_stage("gs_sb_child_age_at_checkout_date",1))){}y+=b[29];
if((i.MJ(i.MC(a[12]))&&i.MJ(i.track_experiment("bHVSfPfECIAEcMfBDZOVHJWe")))){if((i.MJ(i.MB(a[10])==1))){y+=[(f.unshift({date:i.MC(a[11])}),(c=i.ME(b[30],i.MB,i.MN,null)),f.shift(),c),b[2]].join("")
}else{y+=[(f.unshift({date:i.MC(a[11])}),(c=i.ME(b[31],i.MB,i.MN,null)),f.shift(),c),b[2]].join("")
}}else{if((i.MJ(i.MB(a[10])==1))){y+=[i.ME(b[32],i.MB,i.MN,null),b[2]].join("")
}else{y+=[i.ME(b[33],i.MB,i.MN,null),b[2]].join("")
}}y+=b[34];
i.MN("fe_child_i",0);
y+=b[7];
var v=(i.MC(a[1])||[]);
...
u<=n;
u++){f[0]["fe_child_age"]=w=v[u-1];
y+=[b[35],i.F.entities(i.MC(a[13])),b[36]].join("");
var q=u;
var j=v;
var x=n;
var m=o;
...
u<=n;
u++){f[0]["i"]=o=v[u-1];
y+=[b[37],i.F.entities(o.value),b[16]].join("");
if(((i.MJ(w+""!==""))&&(i.MJ(w==o.value)))){y+=b[17]
}y+=[b[18],o.value,b[38]].join("")
}f.shift();
u=q;
v=j;
n=x;
...
B.define("component/search/group/group",function(d,f,c){_i_("378:17");var i=d("component");
var a=d("morphdom");
...
c.exports=i.extend({init:function(){_i_("378:239");this.options=g(this.$el,{modelId:{name:"sb-id",type:String,required:true},gscaacd:{name:"sb-group-gscaacd",type:Boolean}});
...
this.modelInit()
;_r_()},modelInit:function(){_i_("378:240");var k={rooms_count:h.rooms?h.rooms.length:1,adults_count:h.adults||2,children_ages:h.childrenAges||[]};
this.model.group.init(k)
;_r_()},modelChange:function(k){_i_("378:241");if(!k||(!this.options.gscaacd&&k.group!="group")){return _r_();
}this.render()
},searchConfigChange:function(){_i_("378:242");this.model.group.set({key:"rooms_count",value:Array.isArray(h.rooms)?h.rooms.length:1});
...
Array.isArray(h.childrenAges)&&h.childrenAges.forEach(function(l,k){_i_("378:292");this.model.group.set({key:"children_ages",i:k,value:l})
;_r_()}.bind(this))
;_r_()},groupRoomsChange:function(l){_i_("378:243");var k=$(l.currentTarget);
this.model.group.set({key:"rooms_count",value:k.val()})
;_r_()},groupAdultsChange:function(l){_i_("378:244");var k=$(l.currentTarget);
this.model.group.set({key:"adults_count",value:k.val()})
;_r_()},groupChildrenChange:function(l){_i_("378:245");var k=$(l.currentTarget);
this.model.group.set({key:"children_count",value:k.val()});
if(k.val()){j.stage("bHVSfPfECIAEcMfBDZOVHJWe",1)
};_r_()},groupChildAgeChange:function(m){_i_("378:246");var l=$(m.currentTarget);
var k=l.attr("data-group-child-age");
this.model.group.set({key:"children_ages",i:+k,value:l.val()})
;_r_()},render:function(){_i_("378:247");var m=this.model.group.get();
