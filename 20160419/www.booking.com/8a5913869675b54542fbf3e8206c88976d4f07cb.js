...
function a(){_i_("378:29");if(!b){b=true;
...
}}};_r_()}function g(m){_i_("378:30");var k=Object.keys(e),l={},j,i=false;
if(k.length){j=m.target||m.srcElement;
while(j){k.forEach(function(n){_i_("378:274");if(e[n].element===j){l[n]=true;
if(k.length==1){i=true
}};_r_()});
if(i){break
}j=j.parentNode
}}k.forEach(function(n){_i_("378:87");if(!l[n]&&e[n]){e[n].callback.call(false);
h(n)
};_r_()})
;_r_()}function h(i){_i_("378:31");if(e[i]){delete e[i]
};_r_()}c.addMonitor=function(i,k){_i_("378:63");a();
var j=Math.random();
if(typeof i.jquery==="string"){i=i[0]
}setTimeout(function(){_i_("378:268");e[j]={element:i,callback:k}
;_r_()},4);
return _r_( j);
};
c.removeMonitor=h
;_r_()});
B.define("read-data-options",function(b,a,c){_i_("378:2");c.exports=function(f,e){_i_("378:64");var d={};
if(f.jquery){f=f[0]
}Object.keys(e).forEach(function(g){_i_("378:269");var h=e[g];
var j="data-"+(h.name||g);
var i=null;
...
d.exports=b.extend({init:function(){_i_("378:164");this.highlightedIndex=-1;
...
;_r_()},addInputEventListeners:function(){_i_("378:165");var f=this.$input.component("keyboard");
...
;_r_()},addListEventListeners:function(){_i_("378:166");this.$list.mousedown(this.listMouseDown.bind(this));
this.$list.mouseup(this.listMouseUp.bind(this));
this.$list.delegate('[data-list-item=""]',"click",this.listItemClick.bind(this))
;_r_()},addItemsEventListeners:function(){_i_("378:167");this.$items.click(this.listItemClick.bind(this))
;_r_()},addClickOutEventListener:function(){_i_("378:168");this.clickOutMonitorId=this.clickOutController.addMonitor(this.el,this.clickOut.bind(this))
;_r_()},removeClickOutEventListener:function(){_i_("378:169");this.clickOutController.removeMonitor(this.clickOutMonitorId)
;_r_()},calculateListHeight:function(){_i_("378:170");this.listHeight=this.$list.outerHeight()
;_r_()},inputFocus:function(f){_i_("378:171");if(this.options.openFocus){this.listShow();
this.autocomplete({fromFocus:true})
};_r_()},inputBlur:function(f){_i_("378:172");setTimeout(this.inputBlurSync.bind(this),4)
;_r_()},inputBlurSync:function(){_i_("378:173");if(!this.preventBlurClose&&this.listVisible){this.highlightReset();
this.reset()
};_r_()},inputKeyDown:function(h){_i_("378:174");var f=h.ctrlKey||h.metaKey||h.shiftKey||h.altKey,g=h.which==8||h.which==46;
this.isDeletion=g;
this.taintedKeyDown=f
;_r_()},inputInput:function(f){_i_("378:175");this.autocomplete()
;_r_()},inputKeyUp:function(h){_i_("378:176");var g=this.taintedKeyDown||h.ctrlKey||h.metaKey||h.shiftKey||h.altKey||(h.which>=16&&h.which<=18)||h.which==91,f=h.which==32||h.which>=48,i=h.which>=37&&h.which<=40;
if(!g){if(i||f){this.listShow()
}if((this.isDeletion||f)&&!this.supportsInputEvent){this.autocomplete()
}};_r_()},inputEscape:function(f){_i_("378:177");if(this.listVisible){f.originalEvent.preventDefault();
this.highlightReset();
this.reset()
};_r_()},inputArrowUp:function(f){_i_("378:178");f.originalEvent.preventDefault();
if(this.listVisible){this.highlightPrevious()
}else{this.listShow()
};_r_()},inputArrowDown:function(f){_i_("378:179");f.originalEvent.preventDefault();
if(this.listVisible){this.highlightNext()
}else{this.listShow()
};_r_()},inputEnter:function(f){_i_("378:180");if(this.listVisible&&this.highlightedIndex!=-1){f.originalEvent.preventDefault();
this.choose(this.highlightedIndex)
};_r_()},inputTab:function(f){_i_("378:181");this.inputEnter.apply(this,arguments)
;_r_()},listItemClick:function(g){_i_("378:182");var f=$(g.currentTarget);
this.choose(f.data("i"))
;_r_()},listMouseDown:function(f){_i_("378:183");this.preventBlurClose=true
;_r_()},listMouseUp:function(f){_i_("378:184");this.preventBlurClose=false
;_r_()},clickOut:function(){_i_("378:185");if(this.listVisible){this.highlightReset();
this.reset()
};_r_()},setValue:function(f){_i_("378:186");this.input.value=f
;_r_()},listShow:function(){_i_("378:187");if(!this.listVisible){this.$list.addClass("-visible");
this.listVisible=true;
this.addClickOutEventListener();
this.$list[0].scrollTop=1;
setTimeout(function(){_i_("378:309");this.$list[0].scrollTop=0
;_r_()}.bind(this),10)
};_r_()},listHide:function(){_i_("378:188");if(this.listVisible){this.$list.removeClass("-visible");
this.resetFlags();
this.removeClickOutEventListener()
};_r_()},highlightPrevious:function(){_i_("378:189");var f=this.highlightedIndex;
do{if(f==-1){f=this.items.length-1
}else{f--
}}while(f!=-1&&this.items[f].hidden);
this.highlightFromNavigation(f)
;_r_()},highlightNext:function(){_i_("378:190");var f=this.highlightedIndex;
do{if(f==this.items.length-1){f=-1
}else{f++
}}while(f!=-1&&this.items[f].hidden);
this.highlightFromNavigation(f)
;_r_()},highlightFromNavigation:function(h,j){_i_("378:191");if(this.highlightedIndex==-1||j){this.valueBuffer=this.input.value
...
;_r_()},highlightFromAutocomplete:function(){_i_("378:192");var g=this.input.value,f=this.getListFirstIndex();
...
};_r_()},highlight:function(f){_i_("378:193");this.listShow();
...
;_r_()},highlightReset:function(f){_i_("378:194");f=f||{};
if(this.valueBuffer!==null&&!f.preserveInputValue){this.setValue(this.valueBuffer);
if(f.preserveFocus){this.fixIEFocusLoss()
}}if(this.highlightedIndex!=-1){this.$items.eq(this.highlightedIndex).removeClass("-highlighted")
}this.highlightedIndex=-1
;_r_()},highlightScroll:function(){_i_("378:195");var f=false,h=this.$list[0].scrollTop;
...
};_r_()},getListLength:function(){_i_("378:196");return _r_( this.items.length);
},getListFirstIndex:function(){_i_("378:197");if(this.items.length===0){return _r_( -1);
}else{return _r_( 0);
}},autocomplete:function(){_i_("378:198");this.postAutocomplete()
;_r_()},postAutocomplete:function(){_i_("378:199");if(this.getListLength()===0){this.listHide()
}else{this.listShow();
this.calculateListHeight();
this.highlightFromAutocomplete()
};_r_()},choose:function(f){_i_("378:200");this.selectedIndex=f;
this.selectedValue=this.items[f].value;
this.setValue(this.selectedValue);
this.reset()
;_r_()},reset:function(f){_i_("378:201");f=f||{};
if(this.highlightedIndex!=-1){this.$items.eq(this.highlightedIndex).removeClass("-highlighted")
}this.listHide();
...
this.selectedValue=null;
this.setValue("")
};_r_()},resetFlags:function(){_i_("378:202");this.highlightedIndex=-1;
this.isDeletion=false;
this.keyDownCount=0;
this.keysDown={};
this.listVisible=false;
this.preventBlurClose=false;
this.valueBuffer=null
;_r_()},fixIEFocusLoss:function(){_i_("378:203");if(this.input.createTextRange){var f=this.input.createTextRange();
...
var e=c("read-data-options");
return _r_( b.extend({init:function(){_i_("378:88");if(this.browserSupported()){return _r_();
...
}f.filter(function(h,i){_i_("378:291");return _r_( !!$(i).attr("placeholder"));
}).each(function(h,i){_i_("378:290");this.setupPlaceholder($(i))
;_r_()}.bind(this))
},browserSupported:function(){_i_("378:89");var f=$('input[type="text"]')[0]||$('<input type="text">')[0];
return _r_( !!("placeholder" in f));
},setupPlaceholder:function(f){_i_("378:90");var g=$("<label />").text(f.attr("placeholder")).css({width:f.width()+"px",height:f.height()+"px",paddingTop:f.css("padding-top"),paddingLeft:f.css("padding-left"),paddingRight:f.css("padding-right"),fontFamily:f.css("font-family"),fontSize:f.css("font-size"),color:this.options.placeholderColor,overflow:"hidden",cursor:"text",position:"absolute"});
if(this.options.placeholderClass){g.addClass(this.options.placeholderClass)
}g.insertBefore(f);
g.click(function(){_i_("378:275");f.focus()
;_r_()});
f.focus(function(){_i_("378:292");g.hide()
;_r_()}).blur(function(){_i_("378:276");g[f.val().length?"hide":"show"]()
;_r_()});
...
function f(i,h){_i_("378:32");return _r_( function(k){_i_("378:85");var j="";
...
m+=b[24];
if(((h.MJ(h.MC(a[4])["history_today"])||h.MJ(h.MC(a[4])["history_yesterday"]))||h.MJ(h.MC(a[4])["history_recent"]))){m+=[b[25],h.ME(b[26],h.MB,h.MN,null),b[27],h.F.entities(h.MC(a[8])),b[28],h.F.entities(h.MC(a[9])),b[29],h.F.entities(h.MC(a[3])),b[30]].join("")
}m+=b[24];
if((((h.MJ(h.MB(a[11])+""!=="combined_theme"))&&(h.MJ(h.MB(a[3])+""!=="theme")))&&h.MJ(h.track_experiment("PVdITfNZSTRe")))){m+=b[24];
h.MN(a[10],(h.MI(h.MB(a[10]))+h.MI(1)));
m+=[b[31],h.MC(a[10]),b[32]].join("")
}m+=b[24];
if((h.MJ(h.MB(a[11])+""==="combined_theme"))){m+=[b[33],h.MC(a[12]),b[24]].join("")
}else{if((h.MJ(h.MB(a[3])+""==="theme"))){m+=b[34];
m+=b[35];
m+=[b[36],h.F.entities(h.MC(a[9])),b[37]].join("");
if(h.MJ(h.track_experiment("PVdIOHHABWIccIKDACYO"))){m+=b[24];
h.MN(a[13],b[38]);
m+=b[24];
h.MN(a[14],b[39]);
m+=b[24];
h.MN(a[15],h.F.entities(h.MC(a[16])));
m+=b[24];
h.MN(a[17],h.MC(a[18]));
m+=[b[24],h.ME(b[40],h.MB,h.MN,null),b[24]].join("")
}else{m+=[b[38],h.F.entities(h.MC(a[16])),b[39]].join("")
}m+=b[41]
}else{m+=b[24];
...
if((h.MJ(n==1))){m+=b[24];
if((h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET")==2))){m+=b[43];
if((h.MJ(h.MB(a[3])+""==="hotel"))){m+=[b[24],h.ME(b[44],h.MB,h.MN,null),b[24]].join("")
}else{m+=[b[24],h.F.entities(h.ME(b[45],h.MB,h.MN,null)),b[24]].join("")
}m+=b[46]
}m+=b[34];
m+=b[47];
if((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))){m+=b[48]
}m+=b[23]
}m+=[b[24],h.F.entities(h.MB(a[19])),b[24]].join("");
if(h.MD(a[20])){m+=[b[49],h.F.entities(h.MB(a[20])),b[50]].join("")
}m+=b[24];
if((h.MJ(n==1))){m+=b[24];
if(h.MJ(h.MG((h.MC(a[4])||{})["popular"]))){m+=b[24];
if(((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))&&h.MJ(h.track_experiment_stage("PVdIGbGWZSXKbdRNYYdVTbLWZHOaO",1)))){}m+=b[51];
m+=b[52];
if((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))){m+=b[48]
}m+=[b[23],h.F.entities(h.ME(b[53],h.MB,h.MN,null)),b[54]].join("")
}m+=b[55]
}m+=b[24]
}f.shift();
n=l;
...
m+=b[24];
if(h.MD(a[22])){m+=b[56]
}m+=[b[24],h.F.entities(h.MC(a[19])),b[24]].join("");
if(h.MD(a[22])){m+=b[54]
}m+=b[24]
}f.shift();
n=l;
...
o=k;
m+=b[24]
}else{if(h.MD(a[23])){m+=[b[24],h.MC(a[23]),b[24]].join("")
}else{if(h.MD(a[16])){m+=[b[24],h.F.entities(h.MC(a[16])),b[24]].join("")
}}}}m+=b[24];
if(h.MD(a[24])){m+=[b[57],h.MC(a[24]),b[54]].join("")
}m+=b[24];
if(!((h.MJ(h.MB(a[1]))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET"))))){m+=b[24];
if(h.MJ(h.MG((h.MC(a[4])||{})["trending"]))){m+=[b[58],h.F.entities(h.ME(b[59],h.MB,h.MN,null)),b[54]].join("")
}m+=b[24];
if(h.MJ(h.MG((h.MC(a[4])||{})["popular"]))){m+=b[24];
if(((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))&&h.MJ(h.track_experiment_stage("PVdIGbGWZSXKbdRNYYdVTbLWZHOaO",1)))){}m+=[b[60],h.F.entities(h.ME(b[53],h.MB,h.MN,null)),b[54]].join("")
}m+=b[24];
if(h.MJ(h.MG((h.MC(a[4])||{})["history_today"]))){m+=[b[61],h.F.entities(h.ME(b[62],h.MB,h.MN,null)),b[54]].join("")
}else{if(h.MJ(h.MG((h.MC(a[4])||{})["history_yesterday"]))){m+=[b[61],h.F.entities(h.ME(b[63],h.MB,h.MN,null)),b[54]].join("")
}else{if(h.MJ(h.MG((h.MC(a[4])||{})["history_recent"]))){m+=[b[61],h.F.entities(h.ME(b[64],h.MB,h.MN,null)),b[54]].join("")
}}}m+=b[24]
}m+=b[24]
}}m+=b[24];
...
e.exports=b.extend({init:function(){_i_("378:205");this.optionsSpec={delay:{name:"delay",type:Number,defaultValue:300},modelId:{name:"sb-id",type:String,required:true},filterDestType:{name:"filter-dest-type",type:String},gpf:{name:"gpf",type:Boolean},gpOnly:{name:"gp-only",type:Boolean},minLength:{name:"minlength",type:Number,defaultValue:2},required:{name:"required",type:Boolean},multipleThemes:{name:"multiple-themes",type:Boolean},e_sarshf:{name:"esarshf",type:Number},e_saasb:{name:"esaasb",type:Boolean},e_saas:{name:"esaas",type:Boolean},e_saaac:{name:"esaaac",type:Boolean}};
$.extend(this.optionsSpec,this.extendOptions());
this.$errors=this.$el.find("[data-error-id]");
this.$clear=this.$el.find('[data-clear=""]');
this.$historyIcon=this.$el.find('[data-history-icon=""]');
...
this.toggleClear();
this.modelInit()
;_r_()},addInputEventListeners:function(){_i_("378:206");b.prototype.addInputEventListeners.apply(this,arguments);
if(this.$clear.length){this.$clear.on("click",this.clearClick.bind(this))
};_r_()},modelInit:function(){_i_("378:207");var g={ss:this.input.value};
this.model.destination.init(g)
;_r_()},modelChange:function(h){_i_("378:208");if(h.group=="destination"){var g=this.model.destination.get();
if(g.ss!=this.input.value){this.setValue(g.ss)
}if(h.onlyCurrentCountry){this.$input.focus();
this.autocomplete()
}if(h.history){this.autocomplete({fromFocus:true})
}};_r_()},modelValidation:function(g){_i_("378:209");if(g&&g.group=="destination"){this.$errors.filter(".-visible").removeClass("-visible");
...
}};_r_()},inputInput:function(){_i_("378:210");this.model.destination.setSearchString(this.input.value);
b.prototype.inputInput.apply(this,arguments)
;_r_()},inputFocus:function(){_i_("378:211");a.send("[interaction] search input focus");
b.prototype.inputFocus.apply(this,arguments)
;_r_()},inputBlur:function(){_i_("378:212");var g=this.model.destination.get().ss_raw;
if(g){var h=g.split(/\s+/).length;
a.sendOnce("[interaction] autocomplete tokens typed ["+h+"]")
}b.prototype.inputBlur.apply(this,arguments)
;_r_()},inputEnter:function(){_i_("378:213");a.send("[interaction] search input pressed enter");
b.prototype.inputEnter.apply(this,arguments)
;_r_()},clearClick:function(g){_i_("378:214");g.preventDefault();
this.reset({hard:true});
this.input.focus()
;_r_()},setValue:function(g){_i_("378:215");b.prototype.setValue.apply(this,arguments);
this.toggleClear()
;_r_()},toggleClear:function(){_i_("378:216");if(this.$clear.length){this.$clear.toggleClass("-visible",this.input.value!=="")
};_r_()},listShow:function(){_i_("378:217");if(this.items.length>0&&this.input.value.length>=this.options.minLength){b.prototype.listShow.apply(this,arguments)
}else{if(this.items.length>0&&this.options.e_sarshf==2){b.prototype.listShow.apply(this,arguments)
}};_r_()},autocomplete:function(g){_i_("378:218");g=g||{};
this.highlightReset({preserveInputValue:true});
clearTimeout(this.timer);
this.model.destination.abortPredictions();
this.model.destination.abortHistory();
this.toggleClear();
...
}this.timer=setTimeout(function(){_i_("378:312");if(this.input.value.length>=this.options.minLength){this.model.destination.getPredictions(this.query(),this.render.bind(this))
};_r_()}.bind(this),this.options.delay)
}else{if(this.options.e_sarshf&&this.input.value.length==0){this.model.destination.getHistory(this.render.bind(this))
}};_r_()},query:function(){_i_("378:219");return _r_( this.input.value);
},extendOptions:function(){_i_("378:220");return _r_({});
},render:function(g,h){_i_("378:221");this.results=[];
this.items=[];
this.$items=$({});
this._esarshf=!g&&h._esarshf&&this.options.e_sarshf==2;
if(!g&&h._esarshf){B.et.stage("PVdIXCeZVJRbYWVWOIILZHe",1);
if(this.options.e_sarshf!=2){return _r_();
}}this._loading=false;
...
}if(!g&&this.options.e_sarshf==2){h.sb_id=this.options.modelId
...
this.items=h.results.map(function(i){_i_("378:302");return _r_({value:i.label,valueLowerCase:i.label.toLowerCase()});
});
...
}}if(!g&&this.options.e_sarshf==2){this.$historyIcon.toggleClass("-visible",!!h.has_suggestions);
this.$list.loadComponents()
}this.postAutocomplete()
},choose:function(g){_i_("378:222");if(g>=5){B.et.customGoal("PVdITUbERVUOKe",1)
...
if(this._esarshf){B.et.customGoal("PVdIXCeZVJRbYWVWOIILZHe",1)
}if(this.results[g].dest_type=="theme"||this.results[g].result_type=="theme_dest"){this.model.submit()
};_r_()},reset:function(){_i_("378:223");if(!this.selectedValue){this.model.destination.reset()
...
function e(f){_i_("378:33");this.model=f;
this.data={ss:""};
this.valid=true;
...
;_r_()}$.extend(e.prototype,{init:function(f){_i_("378:92");this.data=f;
this.emit("init")
;_r_()},configure:function(f,g){_i_("378:93");if(g!==undefined){this.options[f]=g
}else{if(typeof f=="object"){$.extend(this.options,f)
}};_r_()},getOption:function(f){_i_("378:94");return _r_( this.options[f]);
},get:function(){_i_("378:95");return _r_( this.data);
},set:function(f){_i_("378:96");if(this.searchInstance&&this.searchInstance.getResultDetails&&!f._details){this.searchInstance.getResultDetails(f,function(h,g){_i_("378:303");g=g||{};
g._details=true;
this.set(g);
...
},setSearchString:function(f){_i_("378:97");this.data.ss=f;
this.data.ss_raw=f;
if(!this.valid){this.validate()
};_r_()},reset:function(){_i_("378:98");this.data.dest_id=null;
...
this.emit("change")
;_r_()},validate:function(){_i_("378:99");var f=true;
var g="";
...
},getHistory:function(h){_i_("378:100");if(this.historyCache.results.length){h(null,this.historyCache);
return _r_();
}var f=function(i,j){_i_("378:277");if(!i){j._esarshf=true;
this.historyCache=j
}h(i,j)
;_r_()};
var g=B.require("search/destination/service-booking");
this.historyInstance=g.getHistory(f.bind(this))
},removeHistory:function(g,f,i){_i_("378:101");var h=B.require("search/destination/service-booking");
h.removeHistory(g,f,i);
this.historyCache.results=this.historyCache.results.filter(function(j){_i_("378:280");return _r_( j.dest_type!=g||j.dest_id!=f);
});
B.et.customGoal("PVdIXCeZVJRbYWVWOIILZHe",2);
this.historyCache.has_suggestions=this.historyCache.results.length!=0;
this.emit("change",{history:true})
;_r_()},abortHistory:function(){_i_("378:102");if(this.historyInstance&&this.historyInstance.abort){this.historyInstance.abort()
};_r_()},initPredictions:function(){_i_("378:103");if(this.options.gpf){var f=B.require("search/destination/service-atlas");
f.init()
};_r_()},configurePredictions:function(f){_i_("378:104");$.extend(this.predicitonParameters,f)
;_r_()},getPredictions:function(h,i){_i_("378:105");var g=$.extend({},B.env.search_autocomplete_params,this.predicitonParameters);
if(this.searchInstance){this.searchInstance.abort()
}function f(j,k){_i_("378:270");if(k){this.model.setFields("destination",k._ef);
...
;_r_()},abortPredictions:function(){_i_("378:106");if(this.searchInstance){this.searchInstance.abort()
};_r_()},initOnlyCurrentCountry:function(f){_i_("378:107");this.options.onlyCurrentCountry=!!f
;_r_()},getOnlyCurrentCountry:function(){_i_("378:108");return _r_( !!this.options.onlyCurrentCountry);
},setOnlyCurrentCountry:function(f){_i_("378:109");this.options.onlyCurrentCountry=!!f;
this.emit("change",{onlyCurrentCountry:true})
;_r_()},emit:function(f,g){_i_("378:110");g=g||{};
...
function h(o,w,u){_i_("378:34");var r,t,x;
var m=[];
var n=[];
var q=[];
var s=0;
var v=function(z,A,C){_i_("378:111");s++;
...
var p=function(){_i_("378:112");for(r=0;
r<m.length;
r++){m[r].abort()
};_r_()};
for(r=0;
...
}function e(p){_i_("378:35");var n=6,o=3;
var m={results:[],_ef:[],_efd:[]};
p.forEach(function(q){_i_("378:113");if(!q){return _r_();
...
}}["_ef","_efd"].forEach(function(s){_i_("378:278");if(q[s]){m[s]=m[s].concat(q[s].filter(c(m[s])))
};_r_()});
m.is_powered_by_google=m.is_powered_by_google||(q.results.length&&q.is_powered_by_google)
});
m.results.forEach(function(q,r){_i_("378:114");q._ef=j(q._ef,"ac_position",r)
;_r_()});
m.city=m.results;
m.__js_upa__=m.is_powered_by_google;
return _r_( m);
}function c(m){_i_("378:36");var n={};
m.forEach(function(o){_i_("378:115");n[o.name]=true
;_r_()});
return _r_( function(o){_i_("378:86");return _r_( !n[o.name]);
});
}function j(m,n,o){_i_("378:37");m=m.filter(function(p){_i_("378:224");return _r_( p.name!==n);
});
m.push({name:n,value:o});
return _r_( m);
}function i(m){_i_("378:38");return _r_(["route","postal_code","street_address"].indexOf(m.place_type)!=-1);
}function g(n,m,t){_i_("378:39");var o;
var s=[];
var q=[];
var p=function(u,v){_i_("378:116");if(s.length){if(u){q.push(u);
...
var r=function(){_i_("378:117");for(o=0;
o<s.length;
o++){s[o].abort()
};_r_()};
for(o=0;
...
}function f(o,n,t){_i_("378:40");var m;
var s={};
var q=function(u,v){_i_("378:118");setTimeout(function(){_i_("378:279");if(u&&o.length){p()
}else{t.call(null,u,v)
};_r_()},0)
;_r_()};
var p=function(){_i_("378:119");var u=o.shift();
m=u.search.call(u,$.extend({},n),q);
s.getResultDetails=m.getResultDetails
;_r_()};
var r=function(){_i_("378:120");if(m){m.abort()
};_r_()};
p();
s.abort=r;
return _r_( s);
}a.exports={search:function(m,n){_i_("378:121");if(!m||!m.ss||typeof n!="function"){throw new Error("Invalid arguments")
...
function i(){_i_("378:41");return _r_( !!B.atlas);
}function j(q,s,t){_i_("378:42");var r={city:[]};
r.__upa__=s.filter(m).slice(0,5).map(function(w,v){_i_("378:225");w=w.get("data");
...
t(null,r)
;_r_()}function m(q){_i_("378:43");var r=q.get("data");
return _r_( n.indexOf(r.id)===-1);
}function h(q,r,s){_i_("378:44");q=q||{};
...
s(null,q)
;_r_()}function o(q,r){_i_("378:45");if(!q.place_id){r(null,q);
return _r_();
}this.atlas.getGeocodeDetails({placeId:q.place_id},function(s){_i_("378:122");h(q,s,r)
;_r_()})
}function g(t,w,s){_i_("378:46");var q="";
s=s||{};
if(!w||w.length===0){return _r_( t);
}var v=w.map(function(x){_i_("378:226");return _r_( x.offset);
});
var r=w.map(function(x){_i_("378:227");return _r_( x.offset+x.length-1);
});
for(var u=0;
u<t.length;
...
}function b(r,s){_i_("378:47");var q=[];
if(!s||s.length===0){return _r_([{text:r}]);
}s.forEach(function(v,w){_i_("378:123");var u=v.offset+v.length,t=((s[w+1]&&s[w+1].offset)||r.length)-u;
q.push({highlighted:true,text:r.substr(v.offset,v.length)});
if(t){q.push({text:r.substr(u,t)})
};_r_()});
return _r_( q);
}a.exports=p.extend({init:function(q,u){_i_("378:228");q=q||{};
...
}},search:function(r,w){_i_("378:229");if(!r||!r.ss||typeof w!="function"){throw new Error("service-atlas-invalid-arguments")
}if(!i()){setTimeout(function(){_i_("378:295");w(new Error("service-atlas-no-atlas"),null)
;_r_()},4);
...
u.lon=k;
u.radius=l
}var t=function(x){_i_("378:281");if(!q){if(!x||!Array.isArray(x)){w(new Error("service-atlas-invalid-results"),null)
}else{j(r,x,w)
}};_r_()};
var s=function(x){_i_("378:282");if(!q){w(new Error("service-atlas-error "+x),null)
};_r_()};
this.init(r,function(){_i_("378:296");this.atlas.autocomplete(u,t,s)
;_r_()}.bind(this));
var v=function(){_i_("378:283");q=true
;_r_()};
...
function d(k){_i_("378:48");k.term=k.ss;
delete k.ss;
return _r_( k);
}function h(l,k,m){_i_("378:49");if(!l||!l.city){m(new Error("service-booking-data-invalid"),null)
...
}else{l.city=l.city.map(function(n,o){_i_("378:284");if(!n.label&&n.labels){n.label=f(n.labels)
...
l.results=l.city;
if(l.bbtoollocations){l.results=l.bbtoollocations.map(function(n,o){_i_("378:310");delete n.label_highlighted;
...
}if(l.theme){l.theme=l.theme.map(function(n,o){_i_("378:297");n.ss=n.label;
n._ef=n._ef||[];
...
l._efd=l._efd||[];
m(null,l)
}};_r_()}function f(k){_i_("378:50");return _r_( k.map(function(l){_i_("378:271");return _r_( l.text);
}).join(", "));
}function b(l){_i_("378:51");var k=[];
l.forEach(function(m,n){_i_("378:124");if(n>0){k.push({text:", "})
}k.push({highlighted:!!m.hl,text:m.text})
;_r_()});
return _r_( k);
}a.exports=j.extend({search:function(k,o){_i_("378:230");if(!k||!k.ss||typeof o!="function"){throw new Error("Invalid arguments")
}var l=d(k);
this.emit("beforeajax",k,l);
if(g=="/autocomplete_2"&&B.et.track("PVdIMEHLNRfYO")){g="/autocomplete_nt"
}var m=$.ajax({type:"get",url:g,data:l,success:function(q,p,r){_i_("378:313");this.emit("afterajax",k,q);
h(q,k,o)
;_r_()}.bind(this),error:function(r,p,q){_i_("378:314");if(p!=="abort"){o(new Error("service-booking-error "+q),null)
};_r_()}.bind(this)});
var n=function(){_i_("378:285");if(m&&m.abort){m.abort()
};_r_()};
return _r_({abort:n});
},getHistory:function(n){_i_("378:231");var k=B.env.search_autocomplete_params;
var l=$.ajax({type:"get",url:"/search_suggestions",data:k,success:function(p,o,q){_i_("378:304");n(null,p)
;_r_()},error:function(q,o,p){_i_("378:305");if(o==="abort"){return _r_();
}n(new Error("service-booking-error "+p),null)
}});
var m=function(){_i_("378:286");if(l&&l.abort){l.abort()
};_r_()};
return _r_({abort:m});
},removeHistory:function(m,k,o){_i_("378:232");var l=$.extend({},B.env.search_autocomplete_params);
l.dest_type=m;
l.dest_id=k;
var n=$.ajax({type:"get",url:"/delete_sh_item",data:l,success:function(q,p,r){_i_("378:306");o(null,q)
;_r_()},error:function(r,p,q){_i_("378:307");if(p==="abort"){return _r_();
}o(new Error("service-booking-error "+q),null)
}})
;_r_()}})
;_r_()});
B.define("component/search/destination/history-remove",function(c,a,d){_i_("378:11");var b=c("component");
var e=c("read-data-options");
var f=c("search/searchbox/model");
d.exports=b.extend({init:function(){_i_("378:233");this.options=e(this.$el,{modelId:{name:"sb-id",type:String,required:true},destType:{name:"dest-type",type:String,required:true},destId:{name:"dest-id",type:String,required:true}});
this.model=f.getInstance(this.options.modelId);
this.model.use("destination");
this.$el.on("click",this.click.bind(this))
;_r_()},click:function(g){_i_("378:234");g.stopPropagation();
this.model.destination.removeHistory(this.options.destType,this.options.destId,$.noop)
;_r_()}})
;_r_()});
B.define("morphdom",function(e,h,d){_i_("378:12");var i;
var g=document.body||document.createElement("div");
var c;
if(g.hasAttribute){c=function c(p,o){_i_("378:125");return _r_( p.hasAttribute(o));
}
}else{c=function c(p,o){_i_("378:126");return _r_( p.getAttributeNode(o));
}
}function j(q){_i_("378:52");for(var p in q){if(q.hasOwnProperty(p)){return _r_( false);
}}return _r_( true);
}function f(p){_i_("378:53");if(!i&&document.createRange){i=document.createRange();
...
}var b={OPTION:function(p,o){_i_("378:127");if((p.selected=o.selected)){p.setAttribute("selected","")
}else{p.removeAttribute("selected","")
};_r_()},INPUT:function(p,o){_i_("378:128");p.checked=o.checked;
if(p.value!=o.value){p.value=o.value
}if(!c(o,"checked")){p.removeAttribute("checked")
}if(!c(o,"value")){p.removeAttribute("value")
};_r_()},TEXTAREA:function(q,o){_i_("378:129");var p=o.value;
if(q.value!=p){q.value=p
}if(q.firstChild){q.firstChild.nodeValue=p
};_r_()}};
function n(){}function l(t,s){_i_("378:55");var p=s.attributes;
var r;
var o;
var q;
var u;
...
}}};_r_()}function k(r,q){_i_("378:56");var o=r.firstChild;
while(o){var p=o.nextSibling;
q.appendChild(o);
o=p
}return _r_( q);
}function m(o){_i_("378:57");return _r_( o.id);
}function a(t,D,r){_i_("378:58");if(!r){r={}
...
var M=[];
function A(O,Q){_i_("378:66");var P=H(O);
if(P){J[P]=O
}else{if(!Q){L(O)
}}if(O.nodeType===1){var N=O.firstChild;
while(N){A(N,Q||P);
N=N.nextSibling
}};_r_()}function C(O){_i_("378:67");if(O.nodeType===1){var N=O.firstChild;
while(N){if(!H(N)){L(N);
C(N)
}N=N.nextSibling
}};_r_()}function F(O,N,P){_i_("378:68");if(w(O)===false){return _r_();
}N.removeChild(O);
if(P){if(!H(O)){L(O);
C(O)
}}else{A(O)
}}function I(V,R,Z,S){_i_("378:69");var aa=H(R);
if(aa){delete J[aa]
...
var p=function(P){_i_("378:272");var N=P.firstChild;
...
;_r_()});
booking.jstmpl("search_dates_dates_los",(function(){_i_("378:26");var b=["\n\t",'\n\t\u003cdiv class="sb-dates__los"\u003e\n\t\t\n\t\t',"\n\t\t\t","/private/sbox_num_nights_ar_1/name","\n\t\t","\n\t\t\n\t\t\t","/private/sbox_dates_num_nights_1/name","\n\t\u003c/div\u003e\n","\n"],a=["b_checkin_checkout_interval","b_lang"],e,d,c;
return _r_( function(f){_i_("378:70");var h="",i=this.fn;
function g(j){_i_("378:130");if(i.MJ(i.MC(a[0]))){j+=b[1];
...
B.define("calendar-base",function(b,a,c){_i_("378:13");function d(e){_i_("378:59");e=e||{};
...
;_r_()}d.prototype.getCalendar=function(q,p){_i_("378:71");this.date.setUTCFullYear(q);
...
d.prototype.isDateSelected=function(e){_i_("378:72");if(e.year==this.startDate.year&&e.month==this.startDate.month&&e.day==this.startDate.day){return _r_( true);
...
d.prototype.setStartDate=function(e){_i_("378:73");this.startDate=e
;_r_()};
d.prototype.setEndDate=function(e){_i_("378:74");this.endDate=e
;_r_()};
d.prototype.setDate=d.prototype.setStartDate;
d.diff=function(h,g){_i_("378:75");var f=new Date(1986,9,14,0,0,0),e=new Date(1986,9,14,0,0,0);
...
d.interval=function(f,e){_i_("378:76");return _r_( Math.abs(d.diff(f,e))+1);
};
d.daysInMonth=function(e,f){_i_("378:77");return _r_( new Date(e,f+1,0).getDate());
};
d.isLeapYear=function(e){_i_("378:78");return _r_(((e%4==0)&&(e%100!=0))||(e%400==0));
};
d.calculateWeekNumber=function(e){_i_("378:79");var g=new Date(1986,9,14,0,0,0);
...
;_r_()});
B.define("component/search/calendar/calendar",function(d,b,e){_i_("378:14");var c=d("component");
var g=d("formatting/date");
var a=d("morphdom");
var f=d("read-data-options");
var h=d("search/searchbox/model");
e.exports=c.extend({init:function(){_i_("378:235");this.options=f(this.$el,{modelId:{name:"sb-id",type:String,required:true},mode:{name:"mode",type:String,required:true}});
...
this.modelInit()
;_r_()},modelInit:function(){_i_("378:236");var j=false;
...
};_r_()},modelChange:function(i){_i_("378:237");if(!i||i.group!="dates"){return _r_();
}this.render()
},render:function(){_i_("378:238");var j=this.model.dates.get();
...
return _r_( l);
}function h(l){_i_("378:132");j.MN("fe_has_date",((j.MB(a[5])||((j.MJ(j.MB(a[2])+""==="check-in"))&&j.MC(a[3])))||((j.MJ(j.MB(a[2])+""==="check-out"))&&j.MC(a[4]))));
l+=b[12];
...
if((j.MJ(j.MB(a[2])+""==="check-in"))){l+=[b[18],j.ME(b[19],j.MB,j.MN,null),b[17]].join("")
}else{l+=[b[18],j.ME(b[20],j.MB,j.MN,null),b[17]].join("")
}l+=b[0]
}}}l+=b[21];
if((j.MJ(j.MB(a[9]))&&j.MJ(j.MB(a[8])))){l+=[b[17],(f.unshift({num_nights:j.MB(a[8])}),(c=j.ME(b[22],j.MB,j.MN,"fe_length_of_stay")),f.shift(),c),b[0]].join("")
}l+=b[23];
return _r_( l);
}function g(l){_i_("378:133");l+=b[1];
l=k(l);
l+=b[11];
l=h(l);
l+=b[24];
...
B.define("component/search/dates/dates-los",function(c,a,d){_i_("378:15");var f=c("calendar-base").Calendar;
var b=c("component");
var e=c("read-data-options");
var g=c("search/searchbox/model");
d.exports=b.extend({init:function(){_i_("378:240");this.options=e(this.$el,{modelId:{name:"sb-id",type:String,required:true}});
this.model=g.getInstance(this.options.modelId);
this.model.use("dates");
this.model.on("change",this.modelChange.bind(this));
this.state={};
this.template=B.jstmpl("search_dates_dates_los")
;_r_()},modelChange:function(h){_i_("378:241");if(!h||h.group!="dates"){return _r_();
}this.render()
},render:function(){_i_("378:242");var h=this.model.dates.get();
...
;_r_()}})
;_r_()});
B.define("search/dates/model",function(b,a,c){_i_("378:16");var f=b("calendar-base").Calendar;
var d=b("formatting/date");
function e(h){_i_("378:60");this.model=h;
...
};_r_()}$.extend(e.prototype,{init:function(g){_i_("378:134");var h=this.validateDate("check-in",g.check_in).valid;
...
this.emit("init")
};_r_()},configure:function(g,h){_i_("378:135");if(h!==undefined){this.options[g]=h
}else{if(typeof g=="object"){$.extend(this.options,g)
}};_r_()},getOption:function(g){_i_("378:136");return _r_( this.options[g]);
},get:function(){_i_("378:137");return _r_( this.data);
},setDate:function(k,j,i){_i_("378:138");i=i||{};
var m,h,g;
var l={};
j=this.makeDate(j);
...
},usePreset:function(j){_i_("378:139");var g=new Date(B.env.b_timestamp*1000);
...
;_r_()},setPresets:function(g){_i_("378:140");var h;
this.presets={};
this.data.presets=[];
g.forEach(function(i){_i_("378:293");var j=Math.random();
this.presets[j]={check_in:i.check_in,los:i.los};
this.data.presets.push({name:i.name,id:j})
;_r_()}.bind(this));
this.emit("change",{presets:true})
;_r_()},validate:function(){_i_("378:141");var h=true;
var i="";
...
},validateDate:function(i,h,g){_i_("378:142");g=g||{};
...
},makeDateFromString:function(h){_i_("378:145");var g=h.split("-");
return _r_( this.makeDate(+g[0],+g[1]-1,+g[2]));
},makeStringFromDate:function(i,j,g){_i_("378:146");var h=this.makeDate(i,j,g);
return _r_( h.year+"-"+(h.month+1)+"-"+h.day);
},compareDates:function(h,g){_i_("378:147");if(h.year<g.year){return _r_( -1);
...
},emit:function(g,h){_i_("378:148");h=h||{};
this.model.emit(g,$.extend(h,{group:"dates"}))
;_r_()}});
c.exports=e
;_r_()});
B.define("component/search/destination/country-only",function(c,a,d){_i_("378:17");var b=c("component");
var f=c("search/searchbox/model");
var e=c("read-data-options");
d.exports=b.extend({init:function(){_i_("378:243");this.options=e(this.$el,{modelId:{name:"sb-id",type:String,required:true}});
this.model=f.getInstance(this.options.modelId);
this.model.use("destination");
this.model.on("change",this.modelChange.bind(this));
...
this.modelInit()
;_r_()},modelInit:function(){_i_("378:244");var g=this.$onlyCurrentCountry.is(":checked");
this.model.destination.initOnlyCurrentCountry(g)
;_r_()},modelChange:function(i){_i_("378:245");if(!i||!i.onlyCurrentCountry){return _r_();
...
y+=b[26];
if(i.MJ(i.track_experiment("bHVSfPfECIAEcMfBDZOVHJWe"))){}y+=b[27];
if(i.MJ(i.track_experiment("bHVSfZdfKQZbNcdCIJZYBFYC"))){}y+=b[28];
if(i.MD(a[15])){y+=b[29];
if((i.MJ(i.MC(a[11]))&&i.MJ(i.track_experiment_stage("gs_sb_child_age_at_checkout_date",1)))){}y+=b[30];
if((i.MJ(i.MC(a[11]))&&i.MJ(i.track_experiment("bHVSfPfECIAEcMfBDZOVHJWe")))){if((i.MJ(i.MB(a[10])==1))){y+=[(f.unshift({date:i.MC(a[12])}),(c=i.ME(b[31],i.MB,i.MN,null)),f.shift(),c),b[2]].join("")
}else{y+=[(f.unshift({date:i.MC(a[12])}),(c=i.ME(b[32],i.MB,i.MN,null)),f.shift(),c),b[2]].join("")
}}else{if((i.MJ(i.MB(a[10])==1))){y+=[i.ME(b[33],i.MB,i.MN,null),b[2]].join("")
}else{y+=[i.ME(b[34],i.MB,i.MN,null),b[2]].join("")
}}y+=b[35];
if(i.MJ(i.track_experiment_stage("gs_sb_humanize_age_select_option_0",1))){}y+=b[36];
i.MN("fe_child_i",0);
y+=b[7];
var v=(i.MC(a[1])||[]);
f.unshift({fe_room:null});
...
u<=n;
u++){f[0]["fe_child_age"]=w=v[u-1];
y+=[b[37],i.F.entities(i.MC(a[13])),b[38]].join("");
if(((i.MJ(w!=0))&&i.MJ(i.track_experiment("bHVSfZdfKQZbNcdCIJZYBFYC")))){y+=b[39]
}y+=b[40];
var q=u;
var j=v;
var x=n;
var m=o;
var v=(i.seq(0,i.MB(a[14]))||[]);
f.unshift({i:null});
for(var u=1,n=v.length,o;
u<=n;
u++){f[0]["i"]=o=v[u-1];
y+=[b[41],i.F.entities(o.value),b[16]].join("");
if(((i.MJ(w+""!==""))&&(i.MJ(w==o.value)))){y+=b[17]
}y+=b[42];
if(((i.MJ(o.value==0))&&i.MJ(i.track_experiment("bHVSfZdfKQZbNcdCIJZYBFYC")))){y+=[b[43],i.ME(b[44],i.MB,i.MN,null),b[45]].join("")
}else{y+=[b[43],o.value,b[45]].join("")
}y+=b[46]
}f.shift();
u=q;
v=j;
...
o=m;
y+=b[47];
i.MN("fe_child_i",(i.MI(i.MB(a[13]))+i.MI(1)));
y+=b[30]
}f.shift();
u=r;
v=l;
...
B.define("component/search/group/group",function(d,f,c){_i_("378:18");var i=d("component");
var a=d("morphdom");
var g=d("read-data-options");
var h=d("search-config");
var e=d("formatting/date");
var j=d("et");
var b=d("search/searchbox/model");
c.exports=i.extend({init:function(){_i_("378:247");this.options=g(this.$el,{modelId:{name:"sb-id",type:String,required:true},gscaacd:{name:"sb-group-gscaacd",type:Boolean}});
this.model=b.getInstance(this.options.modelId);
this.model.use("group");
this.model.on("change",this.modelChange.bind(this));
this.model.use("dates");
h.on("change",this.searchConfigChange.bind(this));
...
this.$el.on("focus","[data-group-child-age]",this.groupChildAgeFocus.bind(this));
this.renderEl=this.$el.find('[data-render=""]')[0];
this.template=B.jstmpl("search_group_group_html");
this.modelInit()
;_r_()},modelInit:function(){_i_("378:248");var l={rooms_count:h.rooms?h.rooms.length:1,adults_count:h.adults||2,children_ages:h.childrenAges||[]};
var k={check_in:{day:B.search.dates("checkin").date,month:B.search.dates("checkin").month,year:B.search.dates("checkin").year},check_out:{day:B.search.dates("checkout").date,month:B.search.dates("checkout").month,year:B.search.dates("checkout").year}};
this.model.group.init(l);
this.model.dates.init(k)
;_r_()},modelChange:function(k){_i_("378:249");if(k.checkOut&&k.group==="dates"&&this.model.group.get().children_ages.length){j.stage("bHVSfPfECIAEcMfBDZOVHJWe",1)
}if(!k||(!this.options.gscaacd&&k.group!="group")){return _r_();
}this.render()
},searchConfigChange:function(){_i_("378:250");this.model.group.set({key:"rooms_count",value:Array.isArray(h.rooms)?h.rooms.length:1});
this.model.group.set({key:"adults_count",value:h.adults});
this.model.group.set({key:"children_count",value:Array.isArray(h.childrenAges)?h.childrenAges.length:1});
Array.isArray(h.childrenAges)&&h.childrenAges.forEach(function(l,k){_i_("378:308");this.model.group.set({key:"children_ages",i:k,value:l})
;_r_()}.bind(this))
;_r_()},groupRoomsChange:function(l){_i_("378:251");var k=$(l.currentTarget);
this.model.group.set({key:"rooms_count",value:k.val()})
;_r_()},groupAdultsChange:function(l){_i_("378:252");var k=$(l.currentTarget);
this.model.group.set({key:"adults_count",value:k.val()})
;_r_()},groupChildrenChange:function(l){_i_("378:253");var k=$(l.currentTarget);
this.model.group.set({key:"children_count",value:k.val()});
if(k.val()){if(this.model.dates.get().check_out){j.stage("bHVSfPfECIAEcMfBDZOVHJWe",1)
}j.stage("bHVSfZdfKQZbNcdCIJZYBFYC",1)
};_r_()},groupChildAgeChange:function(m){_i_("378:254");var l=$(m.currentTarget);
var k=l.attr("data-group-child-age");
this.model.group.set({key:"children_ages",i:+k,value:l.val()})
;_r_()},groupChildAgeFocus:function(k){_i_("378:255");j.stage("bHVSfZdfKQZbNcdCIJZYBFYC",2)
;_r_()},render:function(){_i_("378:256");var m=this.model.group.get();
m={b_search_config:{b_nr_rooms_needed:m.rooms_count,b_adults_total:m.adults_count,b_children_total:m.children_ages.length,b_rooms:[{b_adults:m.adults_count,b_children:m.children_ages.length,b_children_ages:m.children_ages,b_room_order:1}]}};
if(this.options.gscaacd){var l=this.model.dates.get();
m.b_checkout_date=l.check_out;
if(l.check_out){m.b_checkout_localized_short_date=e.format(l.check_out,"short_date")
}}var k=this.template.render(m).trim();
a(this.renderEl,k)
;_r_()}})
;_r_()});
B.define("search/group/model",function(c,a,d){_i_("378:19");function b(e){_i_("378:61");this.model=e;
this.data={preset:-1};
this.valid=true;
this.validationError="";
this.options={}
;_r_()}$.extend(b.prototype,{init:function(e){_i_("378:150");this.data=e
;_r_()},configure:function(e,f){_i_("378:151");if(f!==undefined){this.options[e]=f
}else{if(typeof e=="object"){$.extend(this.options,e)
}};_r_()},get:function(){_i_("378:152");return _r_( this.data);
},set:function(f){_i_("378:153");if(!f||!f.key){return _r_( false);
...
return _r_( true);
},validate:function(){_i_("378:154");var e=true;
var f="";
if(this.valid!=e){this.valid=e;
this.validationError=f;
this.emit("validation")
}return _r_( e);
},emit:function(e,f){_i_("378:155");f=f||{};
this.model.emit(e,$.extend(f,{group:"group"}))
;_r_()}});
d.exports=b
;_r_()});
B.define("search/searchbox/analytics",function(c,a,d){_i_("378:20");var b={};
a.send=function(f,e){_i_("378:82");if(!window.ga||!B.et.track("PVSfPATXIebYaO")){return _r_();
}window.ga("send","event","Searchbox",B.env.b_action,f,e)
};
a.sendOnce=function(g,f){_i_("378:83");var e=g+":"+f;
if(!b[e]){a.send(g,f);
b[e]=true
};_r_()}
;_r_()});
B.define("search/searchbox/model",function(c,b,d){_i_("378:21");var e=c("event-emitter");
var f={};
function a(g){_i_("378:62");e.extend(this);
this.id=g;
this.formAction;
this.fieldGroups={track:[]};
this.validators=[]
;_r_()}$.extend(a.prototype,{use:function(j){_i_("378:156");if(this[j]){return _r_();
...
}},submit:function(){_i_("378:157");this.emit("submit")
;_r_()},getFormAction:function(){_i_("378:158");return _r_( this.formAction);
},setFormAction:function(g){_i_("378:159");this.formAction=g
;_r_()},getFields:function(){_i_("378:160");var g=[];
Object.keys(this.fieldGroups).forEach(function(h){_i_("378:294");if(Array.isArray(this.fieldGroups[h])){g=g.concat(this.fieldGroups[h])
};_r_()}.bind(this));
return _r_( g);
},setFields:function(h,g){_i_("378:161");this.fieldGroups[h]=g
;_r_()},validate:function(){_i_("378:162");var g=this.validators.filter(function(h){_i_("378:287");return _r_( h.call());
});
return _r_( g.length==this.validators.length);
}});
d.exports={getInstance:function(g){_i_("378:163");if(!f[g]){f[g]=new a(g)
}return _r_( f[g]);
}}
;_r_()});
B.define("component/search/searchbox/searchbox",function(d,b,e){_i_("378:22");var c=d("component");
var h=d("search/searchbox/model");
var g=B.require("search/destination/service-booking");
var f=d("read-data-options");
var a=["country","dest_id","dest_type","district","iata","landmark","latitude","longitude","place_id","place_id_lat","place_id_lon","region"];
e.exports=c.extend({init:function(){_i_("378:257");this.hiddenFields=[];
...
g.on("beforeajax",function(i,j){_i_("378:288");B.eventEmitter.trigger("AUTOCOMPLETE:beforeAjax",{params:j})
;_r_()});
g.on("afterajax",function(i,j){_i_("378:289");B.eventEmitter.trigger("AUTOCOMPLETE:afterAjax",{data:j})
;_r_()})
;_r_()},modelChange:function(){_i_("378:258");this.updateHiddenFields()
;_r_()},modelSubmit:function(){_i_("378:259");this.$form.submit()
;_r_()},initCurrentCountryField:function(){_i_("378:260");var i=false;
...
};_r_()},onlyCurrentCountryChange:function(){_i_("378:261");this.model.use("destination");
this.model.destination.setOnlyCurrentCountry(this.$onlyCurrentCountry.is(":checked"))
;_r_()},updateHiddenFields:function(){_i_("378:262");var j=this.model.getFormAction();
var l=this.$form.attr("action");
if(!j){j=this.originalAction
}if(l!=j){this.$form.attr("action",j)
}if(this.hiddenFields){this.hiddenFields.forEach(function(n){_i_("378:298");n.$node.remove();
n.$node=null
;_r_()})
...
var k=[];
var i=false;
m.forEach(function(n){_i_("378:299");this.hiddenFields.push({name:n.name,value:n.value});
k.push(n.name);
if(a.indexOf(n.name)!=-1){i=true
};_r_()}.bind(this));
this.$form.find("input").filter(function(){_i_("378:300");return _r_((i&&a.indexOf(this.name)!=-1)||k.indexOf(this.name)!=-1);
}).remove();
this.hiddenFields.forEach(function(n){_i_("378:301");var o=$("<input>");
o.attr("type","hidden");
o.attr("name",n.name);
...
this.$form.append(o)
;_r_()}.bind(this))
;_r_()},formSubmit:function(j){_i_("378:263");var i=this.model.validate();
...
var b=c("component");
d.exports=b.extend({init:function(){_i_("378:264");this.dropdown=false;
this.$form=$("#frm");
this.isPopupAlreadyShown=false;
this.agesSelector='[name="age"]';
this.addEventListeners()
;_r_()},addEventListeners:function(){_i_("378:265");this.$el.change(this.showTooltip.bind(this));
this.$form.delegate(this.agesSelector,"click",this.hideTooltip.bind(this))
;_r_()},showTooltip:function(g){_i_("378:266");var f=parseInt(this.$el.val())>0;
if(f&&!this.isPopupAlreadyShown){setTimeout(function(){_i_("378:311");this.dropdown=$.fly.dropdown.extend({defaults:{position:"bottom left",content:function(){_i_("378:315");return _r_( e("searchbox_children_ages_tooltip").render({}));
},extraClass:"searchbox_children_ages_tooltip"}}).create("#frm "+this.agesSelector);
this.dropdown.show();
this.isPopupAlreadyShown=true
;_r_()}.bind(this),0)
};_r_()},hideTooltip:function(f){_i_("378:267");if(this.dropdown){this.dropdown.hide();
this.dropdown.destroy()
};_r_()}})
;_r_()});
(function(a,b){_i_("378:24");a(function(){_i_("378:84");a(".ge-search-first-checkbox").change(function(){_i_("378:273");var f="cQZJZadTQZGSRaPSWe";
