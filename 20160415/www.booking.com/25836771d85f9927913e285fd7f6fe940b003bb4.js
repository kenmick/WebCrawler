...
b=function(e){booking.track.exp(e);
return booking.track.getVariant(e)
};
a=function(f,e){return booking.track.stage(f,e)
};
c=function(e){return booking.track.getVariant(e)
};
d=function(f,e){return booking.track.custom_goal(f,e)
};
if(booking.env.b_clientside_templates_new_api){b=function(e){return booking.et.track(e)
};
a=function(f,e){return booking.et.stage(f,e)
};
c=function(e){return booking.et.track(e)
};
d=function(f,e){return booking.et.customGoal(f,e)
}
}booking.jstmpl.setup({fn:{track_experiment:b,track_experiment_stage:a,experiment_variant:c,track_custom_goal:d}})
})();
...
(function(c){if(!c.jstmpl){return
}var d,e,b,a;
d=function(k){var j=0,g,h,f;
...
e=function e(j,g,m,i){var f="",k=m?"":document.location.protocol;
if(!c||!c.env||!c.env.static_hostnames||!c.env.static_hostnames.length){f=k+"//q.bstatic.com"
}else{if(c.env.static_hostnames.length===1||!j){f=c.env.static_hostnames[0]
}else{if(!/https?:/.test(j.toString())){var l=d(j.toString()),h=Math.abs(l)%c.env.static_hostnames.length;
f=c.env.static_hostnames[h]
}}}if(m){f=f.replace(/^https?:/,"")
}return f+j
};
a=function a(f){return Array.isArray(f)?f.length:0
};
b=function b(j,l,g,m,i){if(!j){return""
}var h=j.split(" ").slice(0,2),k=(i||"").split(" ").slice(0,2),f={date:h[0],time:h[1],format:l};
if(k.length>=2){f.date_until=k[0];
f.time_until=k[1]
}return B.require("utils/string-interpolate")(B.require("formatting/date").locale_format(f),{begin_marker:g,end_marker:m})
};
c.jstmpl.setup({get_static_hostname:e,get_date:b,fn:{array_length:a}})
}(window.booking));
if(window.booking&&booking.define&&booking.jstmpl){booking.define("jstmpl",function(){return booking.jstmpl
});
...
function e(h,g,f){if(c[h]===undefined){c[h]=g
}else{if(c.b_dev_server){console.warn("SERVER::Trying to override "+h+" with value: "+g)
}if(f){c[h]=g
...
}}d.reset=function(f){for(var g in f){if(f.hasOwnProperty(g)){e(g,f[g],true)
}}};
return d
})
}(B.define,B.require));
...
}var h={experiment:"e",stage:"s",goal:"g",customGoal:"cg"};
var L=(function(){var ad={};
var ag;
var e="";
function af(){var aj=e;
e=Object.keys(ad).join(",");
if(ag||(ag=document.getElementById("req_info"))){if(ag.innerHTML!==aj){ai(ag.innerHTML);
e=Object.keys(ad).join(",")
}ag.innerHTML=e
}}function ai(aj){ad=aj.split(",").reduce(function(ak,al){ak[al]=true;
return ak
},ad)
}function ah(aj){ad[aj]=true
}function ae(aj){ah(aj);
if(typeof aj==="string"){ah(aj);
af()
}else{if(aj instanceof Array){aj.forEach(ah);
af()
}}}return{populate:ae}
}());
var S=(function(){var ad=["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"];
...
var ab=100;
var f=10;
function c(ad){var af=ad.offset();
var e=(typeof af.constructor==="function")?af.constructor:{name:"OtherConstructor"};
var ae;
if(e.name==="ClientRect"){ae=ad.offset();
if(ae){return ae.top
}return false
}return af.top
}function j(ad,af,ae){var e=c(ad);
if(e===false){return false
}return !!(((af+ae)>(e))&&ad.is(":visible"))
}var m=(function(){var ah=false;
var ai;
var ak=[];
var am=[];
var al=0;
function ae(au){var ar=[];
var at=[];
var aq=[];
var av=[];
for(var ap,ao=0,an=au.length;
ao<an;
++ao){ap=au[ao];
switch(ap.what){case h.experiment:ar.push(ap.hash);
break;
case h.stage:av.push(ap.hash+"|"+ap.id);
break;
case h.goal:at.push(ap.hash);
break;
case h.customGoal:aq.push(ap.hash+"|"+ap.id);
break;
default:C.level&&C.report(C.events.UNABLE_TO_STRINGIFY,ap)
}}return"exps="+ar.join(",")+"&what="+at.join(",")+"&custom="+aq.join(",")+"&stage="+av.join(",")
}function aj(ap){var ao=ap.url;
var au=ap.complete||function(){};
var ar=ap.headers||{};
if(!ao){return false
}var an=new XMLHttpRequest();
an.open("GET",ao,true);
var at;
if(ar){for(at in ar){if(ar.hasOwnProperty(at)){an.setRequestHeader(at,typeof ar[at]==="function"?ar[at].call():ar[at])
}}}an.onreadystatechange=function aq(){if(an.readyState===XMLHttpRequest.DONE){au(an,an.status)
}};
an.send()
}function af(){C.level&&C.report(C.events.BEACON_SENT,ak);
ah=false;
al=0;
ai=null;
if(ak.length){e()
...
ai=null;
if(al>=f){am=[]
}else{ak=ak.concat(am);
am=[];
ai=window.setTimeout(e,al*ab)
}}function e(){C.level&&C.report(C.events.SEND_BEACON,ak.slice(0));
ah=true;
am=ak;
var ao=k+"&"+ae(am);
ak=[];
try{if(Y){aj({url:ao,complete:function(ar,aq){if(aq===200){af()
}else{ag()
}},headers:V})
}else{jQuery.ajax({url:ao,complete:function(ar,aq){if(aq==="success"){af()
}else{ag()
}}})
}}catch(ap){var an=new Image();
an.onload=af;
an.onerror=ag;
an.src=k
}}function ad(ao,an,ap){C.level&&C.report(C.events.INIT_BEACON,ao,an,ap);
if(arguments.length===0){}ak.push({what:ao,hash:an,id:ap});
if(ah||ai){C.level&&C.report(C.events.DEFER_BEACON,ak)
}else{ai=window.setTimeout(e,N)
}}return ad
}());
var o={};
var D=300;
...
}function X(ae,ad,af){var e;
if(ae===h.experiment||ae===h.goal){e=[ae,ad]
}else{e=[ae,ad,af]
}return e.join("-")
}function z(ae){if(!aa){return ae
}if(o[ae]){return o[ae]
}var af=2166136261;
for(var e=0,ad=ae.length;
e<ad;
++e){af+=(af<<1)+(af<<4)+(af<<7)+(af<<8)+(af<<24);
af^=ae.charCodeAt(e)
}return(o[ae]=(af>>>0).toString(16))
}function Z(ad,e,ae){C.level&&C.report(C.events.TRACKING_ATTEMPT,{what:ad,hash:e,id:ae,variant:(ad===h.experiment||ad===h.stage)&&x(e)});
if(v(ad,e,ae)){switch(ad){case h.experiment:Q(h.experiment,e);
L.populate(e);
m(h.experiment,e);
break;
case h.stage:Q(h.stage,e,ae);
L.populate(e+"|"+ae);
m(h.stage,e,ae);
break;
case h.goal:Q(h.goal,e);
m(h.goal,e);
break;
case h.customGoal:Q(h.customGoal,e,ae);
m(h.customGoal,e,ae);
break;
default:C.level&&C.report(C.events.TRACK_UNKNOWN_ITEM,ad,e,ae)
}}if(ad===h.experiment){return x(e)
}else{return true
}}function Q(ad,e,ae){T[X(ad,e,ae)]=true
}function v(ag,af,ah){C.level&&C.report(C.events.SHOULD_TRACK,ag,af,ah);
var e={what:ag,hash:af,id:ah,variant:(ag===h.experiment||ag===h.stage)&&x(af)};
if(T[X(ag,af,ah)]){C.level&&C.report(C.events.NOT_TRACKING_WAS_TRACKED,e);
return false
}else{if(ag===h.experiment||ag===h.stage){var ae=1<<(ah||0);
var ad=z(af);
if(d.f[ad]){C.level&&C.report(C.events.NOT_TRACKING_FULLON,e);
return false
}else{if(d.r[ad]===undefined){C.level&&C.report(C.events.NOT_TRACKING_NOT_RUNNING,e);
return false
}else{if(d.t[ad]&ae){Q(ag,af,ah);
C.level&&C.report(C.events.NOT_TRACKING_WAS_TRACKED,e);
return false
}}}}else{if(ag===h.customGoal){var ad=z(af);
if(d.f[ad]){C.level&&C.report(C.events.NOT_TRACKING_FULLON,e);
return false
}else{if(d.r[ad]===undefined){C.level&&C.report(C.events.NOT_TRACKING_NOT_RUNNING,e);
return false
}}}}}return true
}function O(e,ae,ah,ag,ai){C.level&&C.report(C.events.ADD_EVENT_LISTENER,e,ae,ah,ag,ai);
if(!W){W=jQuery(window)
}var af=jQuery(ae).eq(0);
if(n&&e!=="view"){af=jQuery(ae)
}if(!af.length){C.level&&C.report(C.events.NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER,ah,ag,ai);
return
}function ad(){Z(ah,ag,ai)
}if(e==="view"){q(af,ah,ag,ai)
}else{af.one(e,ad)
}}function q(ad,af,ae,ag){C.level&&C.report(C.events.ADD_DEBOUNCED_VIEW_HANDLER,ad,af,ae,ag);
var e=X(af,ae,ag);
if(T[e]){return false
}M.push([ad,af,ae,ag]);
if(!r){C.level&&C.report(C.events.ATTACH_VIEW_LISTENER,M);
W.bind("scroll resize",w);
B&&B.eventEmitter&&B.eventEmitter.bind(S.join(" "),w);
r=true
...
var ah=W.scrollTop();
var ag=W.height();
C.level&&C.report(C.events.CHECK_IF_VISIBLE,M);
var ad;
for(var ae=0,e=M.length;
ae<e;
++ae){ad=M[ae];
if(ad&&j(ad[0],ah,ag)){C.level&&C.report(C.events.ONVIEW_TRACKING_TRIGGERED,ad[1],ad[2],ad[3]);
Z(ad[1],ad[2],ad[3])
}else{af.push(ad)
}}M=af;
C.level&&C.report(C.events.VISIBLE_CHECK_FINISHED,M);
if(M.length===0){r=false;
W.unbind("scroll resize",w);
B&&B.eventEmitter&&B.eventEmitter.unbind(S.join(" "),w);
C.level&&C.report(C.events.DETACH_VIEW_LISTENER)
}window.setTimeout(function(){R=false
},D)
}function i(ai,ae,ak,ag,ad){C.level&&C.report(C.events.ADD_EVENT_LISTENER,ai,ae,ak,ag,ad);
var e=G(ae);
if(e.length>0){if(ai==="view"){u(e[0],ak,ag,ad)
}else{function aj(){Z(ak,ag,ad);
for(var am=0,al=e.length;
am<al;
am++){g(e[am],ai,aj)
}}for(var ah=0,af=e.length;
ah<af;
ah++){p(e[ah],ai,aj)
}}}else{C.level&&C.report(C.events.NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER,ak,ag,ad)
}}function u(ad,af,ae,ag){C.level&&C.report(C.events.ADD_DEBOUNCED_VIEW_HANDLER,ad,af,ae,ag);
var e=X(af,ae,ag);
if(T[e]){return false
}M.push([ad,af,ae,ag]);
if(!r){C.level&&C.report(C.events.ATTACH_VIEW_LISTENER,M);
...
ae<e;
++ae){ad=M[ae];
if(ad&&K(ad[0])){C.level&&C.report(C.events.ONVIEW_TRACKING_TRIGGERED,ad[1],ad[2],ad[3]);
Z(ad[1],ad[2],ad[3])
}else{af.push(ad)
}}M=af;
C.level&&C.report(C.events.VISIBLE_CHECK_FINISHED,M);
...
}function K(e){if(!e){return false
}if(!e.getBoundingClientRect){return true
}var ad=e.getBoundingClientRect();
return ad.top<window.innerHeight
}function G(e){if(jQuery){return jQuery(e).toArray()
}else{if(typeof e==="string"){return[].slice.call(document.querySelectorAll(e))
}else{if(e instanceof HTMLCollection){return[].slice.call(e)
}else{if(e instanceof NodeList){return[].slice.call(e)
}else{if(e instanceof Element){return[e]
}else{if(Object.prototype.toString.call(e)==="[object Array]"){return e
}}}}}}return[]
}function p(ae,ad,e){if(ae.attachEvent){ae["e"+ad+e]=e;
ae[ad+e]=function(){ae["e"+ad+e](window.event)
};
ae.attachEvent("on"+ad,ae[ad+e])
}else{ae.addEventListener(ad,e,false)
}}function g(ae,ad,e){if(ae.detachEvent){ae.detachEvent("on"+ad,ae[ad+e]);
ae[ad+e]=null
}else{ae.removeEventListener(ad,e,false)
}}function l(e,ad,ae){return function(af,ag){if(v(ae,af,ag)){if(window.jQuery&&!U){O(e,ad,ae,af,ag)
}else{i(e,ad,ae,af,ag)
}}else{C.level&&C.report(C.events.WONT_ATTACH_EVENT_TRACKING,e,ad,ae,af,ag)
}}
}function H(e,ad){return{track:l(e,ad,h.experiment),stage:l(e,ad,h.stage),goal:l(e,ad,h.goal),customGoal:l(e,ad,h.customGoal)}
}function t(e,ae){for(var ad in ae){if(ae.hasOwnProperty(ad)){e[ad]=ae[ad]
}}}function E(e){d.f=e.f||{};
t(d.r,e.r||{});
t(d.t,e.t||{})
}function x(e){var ad=z(e);
return d.r[ad]||d.f[ad]||0
}function y(e){if(arguments.length>1){throw"Track only accept one parameter"
}if(!e){return 0
}return Z(h.experiment,e)
}function P(ad,e){if(!ad){return false
}if(e===undefined){return false
}if(e===0){return y(ad)
}if(!/^[1-9]$/.test(e)){return false
}return Z(h.stage,ad,e)
}function I(e){if(!e){return false
}return Z(h.goal,e)
}function J(ad,e){if(!ad||!e||!/^[1-5]$/.test(e)){return false
}return Z(h.customGoal,ad,e)
}function F(e){if(e.url){k=e.url
}if(e.jset){E(e.jset)
}if(typeof e.useFNV!=="undefined"){aa=e.useFNV
}n=!!e.onAllSelectorElements;
if(typeof e.noJqueryOn!=="undefined"){U=e.noJqueryOn
}if(typeof e.noJqueryAjax!=="undefined"){Y=e.noJqueryAjax
}if(typeof e.ajaxHeaders!=="undefined"){V=e.ajaxHeaders
}}function s(){}s.prototype.variant=y;
s.prototype.track=y;
s.prototype.stage=P;
s.prototype.goal=I;
s.prototype.customGoal=J;
s.prototype.on=H;
s.prototype.set=E;
s.prototype.Trackables=h;
s.prototype.configure=F;
return new s()
})
}(B.define,B.require));
B.define("utils/string-interpolate",function(c,a,d){d.exports=function b(f,e){return f.replace(/{([^{}]*)}/g,function(h,g){var i=e[g];
return typeof i==="string"||typeof i==="number"?i:h
})
}
});
B.define("utils/string-pad",function(b,a,c){function d(i,e,h,j){var f=(e||2)-i.toString().length+1;
if(f<=0){return i
}else{var g=new Array(f).join(h||"0");
return j?i+g:g+i
}}d.left=d;
d.right=function(g,e,f){return d(g,e,f,true)
};
c.exports=d
});
B.define("formatting/date",function(d,b,e){var f=B.env.b_date_format,a=B.env.b_month_for_formatted_date,g=B.env.b_weekday_formatted_date,c=B.require("utils/string-interpolate");
g[0]=g[7];
b.format=function(i,q){if(!i||i.year===undefined||i.month===undefined||i.day===undefined){throw new Error("formatting/date: invalid date provided")
}if(!q){throw new Error("formatting/date: invalid format provided")
}i.year=+i.year;
i.month=+i.month;
i.day=+i.day;
if(isNaN(i.year)||isNaN(i.month)||isNaN(i.day)){throw new Error("formatting/date: invalid date provided")
}q=f[q]||q;
var n=new Date(Date.UTC(i.year,i.month,i.day,i.hours||0,i.minutes||0));
var k=n.getUTCMonth()+1;
var m=a[k];
var p=g[n.getUTCDay()];
...
var h=k;
h=h<10?"0"+h:""+h;
var j={weekday:p.name,short_weekday:p["short"],day_of_month:n.getUTCDate(),zero_padded_day_of_month:o,month:k,zero_padded_month:h,month_name:m.name,short_month_name:m.short_name,full_year:n.getUTCFullYear(),time:n.getUTCHours()+":"+l};
return c(q,j)
};
b.compat=function(h,k){var i;
if(h instanceof Date){i=h
}else{if(h&&h.indexOf&&h.indexOf(":")===-1){h=h+" 00:00"
}i=isNaN(Date.parse(h))?false:new Date(h);
if(!i&&typeof h!=="undefined"&&h!==null){h=h.replace(/-/g,"/");
i=isNaN(Date.parse(h))?false:new Date(h)
}if(!i){return false
}}k=k||"short_date";
h={year:i.getFullYear(),month:i.getMonth(),day:i.getDate(),hours:i.getHours(),minutes:i.getMinutes()};
try{return b.format(h,k)
}catch(j){return false
}};
b.locale_format=function(q){var j=typeof q=="object"?q.date:q,h=q.time,k=q.date_until,m=q.time_until;
if(!j){throw new Error("formatting/date: no date provided")
}j=j.replace(/^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2}).*/,function(s,r){return s
});
if(!j.match(/^[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}/)){throw new Error("formatting/date: invalid date provided â "+j)
}if(k){k=k.replace(/^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2}).*/,function(s,r){return s
});
if(!k.match(/^[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}/)){throw new Error("formatting/date: invalid date_until provided â "+k)
}}var o=q.format||"short_date";
if(!f[o]){return""
}var p=j.split(/-0*/),i=k?k.split(/-0*/):void 0,l={};
if(h){var n=B.require("formatting/time").locale_format;
$.extend(l,{time:h?n(h):"",time_until:m?n(m):""})
}[p].concat(k?[i]:[]).forEach(function(s,t){var r={month_name:a[parseInt(s[1])],weekday:g[new Date(t?k:j).getDay()]};
["full_year","month","day_of_month"].forEach(function(v,u){l[v+(t?"_until":"")]=s[u]
});
["%","%_nom","%_in","%_from","%_to","short_%"].forEach(function(u){Object.keys(r).forEach(function(v){l[(u+(t?"_until":"")).replace("%",v)]=r[v][u[0]=="%"||v=="month_name"?u.replace("%","name").replace(/_nom$/,"_only"):"short"]
})
})
});
return c(f[o],l)
...
B.define("formatting/time",function(d,b,e){var a=B.env.b_time_format,c=B.require("utils/string-interpolate"),f=B.require("utils/string-pad");
return{locale_format:function(g){var g=typeof g!="object"?{time:g}:g,h={},i;
...
}g.time.match(/^(?:([0-2]?[0-9])[.: -]?([0-5]?[0-9]))$/).slice(1).forEach(function(k,j){h[j?"minutes":"hour"]=k.replace(/^0/,"")||0
});
...
h.hour_24h_show24=f(h.hour_24h_show24_no0);
h.hour_24h_until24_no0=h[g.until?"hour_24h_show24_no0":"hour_24h_no0"];
h.hour_24h_until24=f(h.hour_24h_until24_no0);
h.hour_12h_no0=h.hour%12||12;
h.hour_12h=f(h.hour_12h_no0);
h.minutes_no0=h.minutes;
h.minutes=f(h.minutes_no0);
h.AM_PM=a[(h.hour<12?"AM":"PM")+"_symbol"].name;
i=c(a.time.name,h).trim();
return g.no_html?i.replace(/\s/g,"&nbsp;"):i
}}
});