...
f.extend(b,{hotel_soldout:j({url:d,w:16,h:28,x:232,y:33,z:501}),hotel:j({url:d,w:16,h:28,x:249,y:33,z:501})})
...
var d={b_id:"20",b_iw_template:"atlas_iw_landmark",b_latitude:B.env.b_latitude,b_longitude:B.env.b_longitude,b_marker_type:"landmark",b_type:"landmark",b_name:B.env.lp_add_marker_to_landmark_lps.name,b_url:"",icon:c.ICONS.landmark};
...
(function(g,n){var b,v=n.env,e,C,u,m=false;
var t=function(D){b=D;
if(b){s(b)
}z();
y()
};
function z(){var E=false,D=g("#searchboxInc");
u=g(".lp_bold_date_picker_wrapper");
C=u.find(".lp_bold_date_picker_checkin");
e=u.find(".lp_bold_date_picker_checkout");
if(u.find(".lp_bold_date_picker_group_container").length){A(u,D)
}u.find(".lp_bold_date_picker_btn").click(function(){var G=p();
if(G.checkin&&G.checkin.type=="valid"&&G.checkout&&G.checkout.type=="valid"){if(G.checkout.dateObject_.getTime()-G.checkin.dateObject_.getTime()>1000*60*60*24*30){E=true;
g(".lp_bold_date_picker_error").fadeIn(300);
return
}}E&&g(".lp_bold_date_picker_error").hide();
g("#frm .b-button_primary").click()
});
u.find(".b-booker-type").click(function(H){var G=g(this).find("input")[0].className;
D.find("."+G).click()
});
var F=p();
if(F.checkin){l("checkin",F.checkin.date,F.checkin.month+1,F.checkin.year)
}if(F.checkout){l("checkout",F.checkout.date,F.checkout.month+1,F.checkout.year)
}}function d(D,E){if(n.env.b_year_months[D+"-"+(E+1)]){return n.env.b_year_months[D+"-"+(E+1)].name
}}function y(){var F=v.sunday_first,I=v.b_simple_weekdays,D=v.b_short_months;
if(F){I.unshift(I.pop())
}n.eventEmitter.bind("SEARCH:date_changed",x);
function H(L,K){if(!L){return
}var J=new Date(L.year,L.month,L.date+K);
return{year:J.getFullYear(),month:J.getMonth(),date:J.getDate()}
}var G={type:"checkin",defaultDate:n.search.dates("checkin"),dayNames:I,monthNames:D,sundayFirst:F,title:v.transl_checkin_title,monthTitle:d,direction:v.rtl?"rtl":"ltr",arrow:true,onDateSelected:function(N,J){var P={year:J.getYear(),month:J.getMonth(),date:J.getDate()};
var M=n.search.dates("checkin"),L=n.search.dates("checkout");
var O=L,Q=j();
if(N.options.type=="checkin"){n.search.dates("checkin",P);
M=n.search.dates("checkin");
if(!L||L.type=="invalid"||(M.toString()>=L.toString())){n.search.dates("checkout",H(M,1));
L=n.search.dates("checkout")
}for(var K=0;
K<Q.length;
K++){if(Q[K].type=="checkout"){Q[K].setOptions({startDate:J});
Q[K].refreshDisabledDays()
}}}else{n.search.dates("checkout",P);
L=n.search.dates("checkout")
}l(N.options.type,J.getDate(),J.getMonth()+1,J.getYear());
if(!M||L.type=="invalid"){P=new Date(J.getYear(),J.getMonth(),J.getDate()-1);
n.search.dates("checkin",P);
M=n.search.dates("checkin")
}if((L&&!M)||((L.type=="valid")&&(M.type=="invalid"))||(M.toString()>=L.toString())){n.search.dates("checkin",H(L,-1));
M=n.search.dates("checkin")
}if(M&&L&&M.type=="valid"&&L.type=="valid"){N.selectRange(n.calendar2.dayId(M.year,M.month,M.date),n.calendar2.dayId(L.year,L.month,L.date))
}if(!O||O.type=="invalid"){l("checkout",L.date,L.month+1,L.year)
}},onHide:function(){g(this.$element.context).removeClass("focus")
},onShow:function(K,J){g(this.$element.context).addClass("focus");
if(K.options.type!="checkout"){var L=n.search.dates("checkin");
if(L&&L.type=="valid"){K.selectDay(n.calendar2.dayId())
}}}};
n.eventEmitter.bind("CALENDAR:opened",q);
var E=g.extend({},G);
E.type="checkout";
E.title=v.transl_checkout_title;
E.defaultDate=n.search.dates("checkout");
E.startDate=o();
E.endDate=w();
C.calendar2(G);
e.calendar2(E)
}function x(I,D){var H=n.search.dates("checkin"),G=n.search.dates("checkout"),F=n.search.dates(D.type),J=j();
if(H){l("checkin",H.date,H.month+1,H.year)
}if(G){l("checkout",G.date,G.month+1,G.year)
}if(F.type==="month"){f("monthSelected",D.type,F)
}else{if(F.type==="valid"){f("dateSelected",D.type,F)
}}for(var E=0;
E<J.length;
E++){if(H&&G&&H.type==="valid"&&G.type==="valid"){J[E].trigger("rangeSelected",{type:D.type,startValue:H,endValue:G})
}}}function q(F,D){var G=j();
for(var E=0;
E<G.length;
E++){if(D.id!==G[E].id()){G[E].trigger("hide")
}}}function f(F,E,G){var H=j();
for(var D=0;
D<H.length;
D++){if(H[D].type()===E){H[D].trigger(F,{type:E,value:G})
}}}function a(){var D=new Date();
return D
}function h(){var D=n.calendar2.today();
D.setFullYear(D.getFullYear()+1);
D.setDate(0);
D.setDate(D.getDate()-1);
return D
}function o(){var D=n.calendar2.today();
D.setDate(D.getDate()+1);
return D
}function w(){var D=n.calendar2.today();
D.setFullYear(D.getFullYear()+1);
D.setDate(0);
return D
}function j(){return n.calendar2.controller.getCalendars()
}function p(){return{checkin:n.search.dates("checkin"),checkout:n.search.dates("checkout")}
}function l(E,D,H,G){var F;
var I;
if(D){I=G+"-"+H+"-"+D;
I=booking.formatter.date(I,"date_with_year")
}if(E=="checkin"){F=C
}else{F=e
}if(D){F.removeClass("placeholder").find("span").text(I)
}else{F.addClass("placeholder").find("span").text(F.data("default"))
}return true
}function A(G,D){var F=G.find(".lp_bold_date_picker_lightbox"),E=F.parent();
G.find(".lp_bold_date_picker_group_container").click(function(K){var J=g(this).find(".main_guests_selector"),I=g(this).find(".lp_bold_date_picker_select");
if(!J.is(":visible")){K.stopPropagation();
J.show();
I.addClass("focus");
g.each(j(),function(){this.trigger("hide")
});
g(document).one("click",function(){J.hide();
I.removeClass("focus")
})
}}).find(".lp_bold_date_picker_group_lists li").click(function(){var I=g(this).data();
if(I.b_number_rooms==0){i(F,n.search.group().value);
E.show()
}else{n.search.group({adults:I.b_number_adults,children:I.b_number_children,childrenAges:[],rooms:I.b_number_rooms});
g(this).closest(".lp_bold_date_picker_group_container").find(".lp_bold_date_picker_select_text").html(g(this).text())
}});
E.click(function(I){if(g(I.target).hasClass("lp_bold_date_picker_dimmer")){E.hide()
}});
F.find(".lp_bold_date_picker_lightbox_secondary_cta, .lp_bold_date_picker_lightbox_close_button").click(function(){E.click()
}).end().find(".lp_bold_date_picker_lightbox_cta").click(function(){var I=c(F);
n.search.group(I);
r(I);
E.click()
});
n.components.require("simple-emitter").bind("spin-button-change",function(I){if(I.data.id=="lp_bold_date_picker_children_selector"){k(I,true)
}});
var H=n.search.group().value;
r(H);
if(H.children){G.find(".lp_bold_date_picker_lightbox_ages").each(function(I){if(typeof H.childrenAges[I]!=="undefined"){g(this).find("input").val(H.childrenAges[I])
}})
}}function r(E){var D=n.jstmpl("lp_bold_date_picker_group_message").render(E);
u.find(".lp_bold_date_picker_group_container").find(".lp_bold_date_picker_select_text").html(D)
}function i(D,E){D.find(".room_guests_selector input").val(E.rooms);
D.find(".adults_guests_selector input").val(E.adults);
D.find(".children_guests_selector input").val(E.children);
k({data:{value:E.children}})
}function k(G,F){var D={show:F?"slideDown":"show",hide:F?"slideUp":"hide"};
if(!G.data.value){u.find(".lp_bold_date_picker_lightbox_ages")[D.hide]().find(".age_guests_selector")[D.hide]()
}else{var E=u.find(".lp_bold_date_picker_lightbox_ages");
if(G.data.value==1){E.find(".singular").show().siblings(".plural").hide()
}else{E.find(".plural").show().siblings(".singular").hide()
}E.slideDown().find(".age_guests_selector:lt("+G.data.value+")").filter(":not(:visible)").val("0").end()[D.show]().end().find(".age_guests_selector:gt("+(G.data.value-1)+")")[D.hide]()
}}function c(D){return{rooms:D.find(".room_guests_selector input").val(),adults:D.find(".adults_guests_selector input").val(),children:D.find(".children_guests_selector input").val(),childrenAges:D.find(".age_guests_selector:visible input").map(function(){return parseInt(g(this).val())
}).toArray()}
}var s=function(E){var D=null;
if(booking.env.lp_bold_date_picker_track_exp!=E){return
}n.eventEmitter.bind("CALENDAR:opened",function(H,F){try{var G=F.instance.options.calendar2Type||F.instance.options.type||"checkin";
D=G;
n.track.goal(G+"_cal_opened");
g(document).click()
}catch(H){}});
n.eventEmitter.bind(n.Search.Events.DATE_CHANGED,function(){if(D){n.track.goal(D+"_date_selected")
}else{n.track.goal("checkin_date_selected");
n.track.goal("checkout_date_selected")
}})
};
booking[sNSStartup]["fESCFGHJPeZBNSGNGSEfWe"]={init:function(){m=true;
t()
}};
booking[sNSExperiments]["fESCFGHJPeZBNSGNET"]={init:function(){if(!m){t("fESCFGHJPeZBNSGNET")
}else{s("fESCFGHJPeZBNSGNET")
}},initElse:function(){s("fESCFGHJPeZBNSGNET")
}};
booking[sNSExperiments]["fESCFGHJPeZBNSGGCQVO"]={init:function(){t("fESCFGHJPeZBNSGGCQVO")
},initElse:function(){s("fESCFGHJPeZBNSGGCQVO")
}}
})(jQuery,booking);
...
(function(b,e){var c="fEJRUcdFVfEQIJceYO";
function a(f){var i=2,h=0,g=0;
heights=[];
f.height("auto").each(function(){heights.push(b(this).height());
h++;
if(h%i==0){var j=Math.max.apply(null,heights);
f.slice(g,h).height(j);
heights=[];
g=h
}})
}function d(){var g=e.track.getVariant(c)==2?b(".lp_cp_top_attractions_cards_wrapper"):b(".lp_cp_top_attractions_box"),f=g.find(".lp_cp_top_attractions_link");
f.click(function(){e.track.stage(c,3)
});
if(e.track.getVariant(c)==2){a(f)
}b(window).resize(function(){a(f)
})
}e[sNSExperiments][c]={init:d}
})(jQuery,booking);
...
B[sNSExperiments]["fEWKRaJHJUeXdVCMFHdZae"]=(function(d){function b(){$("#test_button_i_am_leaving").click(function(){c()
});
$(".dont_leave_close_btn").click(function(){a()
});
$(".dont_leave_dsf_pop_up_darker_bg").click(function(){a()
})
}function c(){$(".dont_leave_dsf_pop_up_darker_bg").removeClass("hide").addClass("show");
$(".dont_leave_dsf_pop_up").removeClass("hide").addClass("show")
}function a(){$(".dont_leave_dsf_pop_up_darker_bg").addClass("hide").removeClass("show");
...
}else{if(!b(i.target).closest("h4").length){h="sr-url";
