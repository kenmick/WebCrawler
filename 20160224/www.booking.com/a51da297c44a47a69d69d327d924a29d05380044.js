...
jQuery.extend(B.atlas,(function(i,d){var b=null,g=null;
function c(){b=null;
g=null
}function j(){var k=d.atlas.SUBSCRIPTIONS;
b={};
g={};
if(!k||!d.jst){return
}i.each(k,function(l,m){i.each(m,function(n,q){try{if(q.name&&q.cond&&d.jst.hasOwnProperty(q.name)&&q.old){b[q.name]=parseInt(d.jst[q.name],10);
...
}function e(l,m){var n=[],k=[],o=d.atlas.SUBSCRIPTIONS;
if(!o||!l||!o.hasOwnProperty(l)){return false
}o[l].forEach(function(p){if(a(p.name)!==false&&!p.touched&&m===p.instanceId){if(p.stage){if(g[p.name]){d.et.stage(p.name,p.stage)
...
}k.forEach(function(p){d.track.stage.apply(null,p)
})
}function h(m,k){var n=d.atlas.SUBSCRIPTIONS,l=n&&n.templates?n.templates:[];
l.some(function(o){if(o.name===m&&a(o.name)!==false){if(g[o.name]){d.et.stage(o.name,k||o.stage)
}else{d.track.stage(m,k||o.stage)
}return true
}return false
});
return true
}function f(l){var m=d.atlas.SUBSCRIPTIONS,k=m&&m.templates?m.templates:[];
k.some(function(n){if(n.name===l&&a(n.name)!==false){if(!n.touched){if(g[n.name]){d.et.track(n.name)
}else{d.track.exp(l)
}n.touched=true
}return true
}return false
...
return a(l)
}d.run("Xfgrwdtafew").onLoad(function(){if(!b){j()
}});
...
B.atlas.define("icons-default",["util-object"],function(e){var f,d="//q-ec.bstatic.com/static/img/map_sprites_omnibus_mk3/44f183b1860aa3035b88da99006cef42c70d8038.png",b={url:d,w:18,h:27,x:33,y:33,z:110};
function c(h){return e.extend({},b,h)
}f={hotel:c({}),hotel_soldout:c({}),hotel_matching:c({x:0,z:110}),hotel_not_matching:c({x:17,z:100}),genius:c({x:136,z:120}),city:c({x:118,z:60}),landmark:c({x:51,z:80}),land_mark:c({x:198,y:0,w:22,h:36,z:85}),airport:c({x:102,z:90}),transport:c({x:68,z:70}),ski_lift:c({x:85,z:70}),hotel_current:c({w:22,h:36,x:198,y:60,z:2000}),hotel_soldout_current:c({w:22,h:36,x:198,y:60,z:2000}),genius_current:c({w:22,h:36,x:176,y:60,z:130}),city_current:c({w:22,h:36,x:154,y:60,z:65}),airport_current:c({w:22,h:36,x:132,y:60,z:95}),landmark_current:c({w:22,h:36,x:66,y:60,z:85}),transport_current:c({w:22,h:36,x:88,y:60,z:75}),ski_lift_current:c({w:22,h:36,x:110,y:60,z:75}),"default":c({})};
function g(h){return(h&&f.hasOwnProperty(h))?f[h]:f["default"]
}function a(j){var h={},i=g(j);
switch(true){case (j==="genius"):case (j==="city"):case (j==="landmark"):case (j==="land_mark"):case (j==="airport"):case (j==="transport"):case (j==="ski_lift"):case (j==="genius_current"):case (j==="city_current"):case (j==="airport_current"):case (j==="landmark_current"):case (j==="transport_current"):case (j==="ski_lift_current"):h=i;
break;
...
}},{evt:"marker-click",name:"atlas_sr_click_ufi",condition:a.sr,filter:function(e){return/city/.test(e.type)&&!/vpm/.test(e.type)
...
B.atlas.define("iw-ajax",["jQuery","util-env","util-array"],function(b,d,j){var c=[],a=[];
function i(l){var k=j.findIndex(c,function(m){return m[0]===l
});
if(k>-1){c[k][1].abort();
c.splice(k);
l.trigger("iw-get-abort",l)
}}function g(l){var k=this.get("markerDetailsURL");
if(!k){k=d.get("markerDetailsURL");
this.set("markerDetailsURL",k)
}return k+";hotel_id="+l
}function e(m){var l,k=this;
l=j.find(a,function(n){return n[0]===k&&n[1]===m
});
return l?l[2]:false
}function h(q,p,k){var o,m=g.call(this,q),l=this,n=e.call(l,q);
i(l);
if(!k&&n&&typeof p==="function"){p.call(l,n);
return n
}l.trigger("iw-get-before",l);
o=b.ajax({url:m,error:function(s,r){if(r!=="abort"){l.trigger("iw-get-fail",l)
}},success:function(r){l.trigger("iw-get-success",r,l);
...
B.atlas.define("mixed-markers-hover",function(){function a(f){var e=this.Interface.getMixedMarker(f);
...
}}function c(g){if(typeof g==="undefined"){this.setIconActive(undefined);
return
...
}}}function b(g){var f=this.Interface.getActiveMixedMarkers();
var e=this;
f.forEach(function(i){var h;
...
}function d(f){var e=this.Interface.getMixedMarker(f);
...
B.atlas.define("markers",["jQuery"],function(g){B.et.track("fBTeecHVPSDDfREFcEXO");
function l(s){var r,q=[];
for(r in s){if(s.hasOwnProperty(r)){q.push(s[r])
}}return q
}function a(s,r){var q=(g.isArray(s))?s:l(s);
this.pruneMarkers(q);
this.addMarkers(q,r)
}function n(){return this.Interface.getActiveMarkers()
}function f(){this.Interface.clearMarkers()
}function e(q){this.Interface.clearMarker(q)
}function i(r){var q=(g.isArray(r))?r:l(r);
this.Interface.pruneOutboundedMarkers();
this.addMarkers(q)
}function o(u){var t,s,r=this.Interface.getActiveMarkers(),q=r.length;
while(q--){t=r[q].b_id;
s=u.length;
while(s--){if(u[s].b_id===t){break
}}if(s<0&&t){this.Interface.clearMarker(t)
}}}function d(r,q){var x,t,y=[],s=0,v=this.Interface,u=(g.isArray(r))?r:l(r),w=u.length;
...
}function j(s){var r,q=this.Interface.getMarker(s);
if(q){r=this.getIcon(q.b_type);
this.Interface.setMarkerType(s,r)
}}function m(q){this.Interface.centerMarker(q,this.get("offset"))
}function c(q){this.Interface.panMarker(q,this.get("offset"))
}function b(r,q){if(q){this.Interface.setMarkerType(r,this.getIcon(q),q)
}}function p(q){return this.Interface.getMarker(q)
}function h(r,q){return this.Interface.setMarkerOptions(r,q)
}function k(){return this.Interface.markersExist()
...
}B.atlas.define("mixed-markers",["util-object","util-array"],function(i,g){function h(v){var u,t=[];
if(g.isArray(v)){return v
}for(u in v){if(v.hasOwnProperty(u)){t.push(v[u])
}}return t
}function j(v,u){var t=h(v);
this.pruneMarkers(t);
this.addMarkers(t,u)
}function r(){return this.Interface.getActiveMixedMarkers()
}function n(){this.Interface.clearMixedMarkers()
}function p(t){this.Interface.clearMixedMarker(t)
}function l(u){var t=h(u);
this.Interface.pruneOutboundedMixedMarkers();
this.addMarkers(t)
}function o(x){var w,v,u=this.Interface.getActiveMixedMarkers(),t=u.length;
while(t--){w=u[t].b_id;
v=x.length;
while(v--){if(x[v].b_id===w){break
}}if(v<0&&w){this.Interface.clearMixedMarker(w)
}}}function a(C,x){var A,w,v=[],y=0,u=h(C),t=u.length;
this.trigger("markers-create",this);
while(t--){A=u[t];
...
}function b(v){var u,t=this.Interface.getMixedMarker(v);
...
}}}function c(t){this.Interface.centerMixedMarker(t,this.get("offset"))
}function k(t){this.Interface.panMixedMarker(t,this.get("offset"))
}function q(v,u){var t=this.Interface.getMixedMarker(v);
if(u&&t){t.b_type=u;
...
}}}function f(t){return this.Interface.getMixedMarker(t)
}function m(u,t){return this.Interface.setMixedMarkerOptions(u,t)
}function e(t){if(typeof this.Interface.animateMarker==="function"){this.Interface.animateMarker(t)
}}function s(){return this.Interface.mixedMarkersExist()
}function d(t){if(typeof this.Interface.getMixedMarkerPosition==="function"){return this.Interface.getMixedMarkerPosition(t)
...
B.atlas.define("markers-ajax",["jQuery","util-env"],function(b,d){var c=[];
function f(k){var j=k.Interface?k.Interface.getBoundingBox():k.get("bounds"),l="";
if(j&&j.SW){l=[j.SW[1],j.SW[0],j.NE[1],j.NE[0]].join(",")
}else{if(j&&j.toMOMString){l=j.toMOMString()
}}return l
}function h(l){var k,j,m;
for(k=0,j=c.length;
k<j;
++k){m=c[k];
if(m[0]===l){m[1].abort();
...
}function a(n){var m,k=this.getMarkersURL(),j=this,l=f(j);
h(j);
if(!l){return
}j.trigger("markers-get-before",j);
m=b.ajax({url:k+";BBOX="+l,error:function(p,o){if(o!=="abort"){j.trigger("markers-get-fail",j)
}},success:function(o){j.trigger("markers-get-success",o,j);
...
c.push([j,m])
}function g(j){return this.set("markersOnMapURL",j||d.get("markersOnMapURL"))
}function e(){return this.get("markersOnMapURL")||d.get("markersOnMapURL")
}function i(j){var k=this.getMarkersURL();
...
B.atlas.define("provider-google-markers",["jQuery","util-array"],function(f,l){function g(o){return l.findIndex(this.markers,function(p){return p.b_id===String(o)
})
}function c(){var s,r=this.markers,q=r.length,p=google.maps.event,o=this.map.getBounds();
while(q--){s=r[q];
if(o&&!o.contains(s.getPosition())&&!s.data.b_persistent){p.clearInstanceListeners(s);
s.setMap(null);
r.splice(q,1)
}}}function e(){var p,o=this.markers.length;
while(o--){p=this.markers[o];
google.maps.event.clearInstanceListeners(p);
p.setMap(null)
}this.markers=[]
}function d(q){var o=g.call(this,q),p=(o>-1)?this.markers[o]:null;
if(p){google.maps.event.clearInstanceListeners(p);
p.setMap(null);
this.markers.splice(o,1)
}}function i(){return this.markers?this.markers.length>0:false
}function n(o){return l.find(this.markers,function(p){return p.b_id===String(o)
})
}function m(){return f.map(this.markers,function(o){return{b_id:o.b_id,b_name:o.b_name,b_type:o.b_type,b_url:o.b_url,b_latitude:o.b_latitude,b_longitude:o.b_longitude,b_persistent:(o.data)?o.data.b_persistent:false,b_behaviors:(o.data)?o.data.b_behaviors:false,b_states:(o.data)?o.data.b_states:false}
})
}function b(t,q,p){var r=google.maps,s=this.getMarker(t),o;
...
}}function k(s,r){var q=this,p=this.getMarker(s),o=q.map.getProjection();
if(p){if(o){q.map.setCenter(this.getOffsetLatLng(p.getPosition(),r))
}else{q.ubber.once("idle",function(){q.map.setCenter(this.getOffsetLatLng(p.getPosition(),r))
})
}}}function a(s,r){var q=this,o=q.map.getProjection(),p=this.getMarker(s);
if(p){if(o){q.map.panTo(q.getOffsetLatLng(p.getPosition(),r))
}else{q.ubber.once("idle",function(){q.map.panTo(q.getOffsetLatLng(p.getPosition(),r))
})
}}}function h(q,p){var o=this.getMarker(q);
if(o){o.setOptions(p)
}}function j(s){var r,u=B.env,p=google.maps,t=p.event,o=this.ubber,w=s.icon,v,q;
...
r.setMap(this.map);
t.addListener(r,"click",function(x){o.trigger("marker-click",{evt:x,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this.data})
});
t.addListener(r,"mouseover",function(x){o.trigger("marker-hover",{evt:x,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this.data})
});
t.addListener(r,"mouseout",function(x){o.trigger("marker-out",{evt:x,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this.data})
});
this.markers.push(r)
...
B.atlas.define("provider-google-mixed-markers",["util-array","util-object","provider-google-overlay-marker"],function(o,d,h){function m(r){return o.findIndex(this.markers,function(s){if(s){return s.b_id===String(r)
}})
}function e(){var v,u=this.markers,t=u.length,s=google.maps.event,r=this.map.getBounds();
while(t--){v=u[t];
if(r&&!r.contains(v.getPosition())&&!v.data.b_persistent){s.clearInstanceListeners(v);
v.setMap(null);
u.splice(t,1)
}}}function i(){var s,r=this.markers.length;
while(r--){s=this.markers[r];
google.maps.event.clearInstanceListeners(s);
s.setMap(null)
}this.markers=[]
}function j(t){var r=m.call(this,t),s=(r>-1)?this.markers[r]:null;
if(s){google.maps.event.clearInstanceListeners(s);
s.setMap(null);
this.markers.splice(r,1)
}}function n(){return this.markers?this.markers.length>0:false
}function g(r){return o.find(this.markers,function(s){if(s){return s.b_id===String(r)
}})
}function q(){var s;
try{s=this.markers.map(function(w){var v={};
Object.keys(w).forEach(function(x){if(/^b_/.test(x)){v[x]=w[x]
}});
return v
})
}catch(u){var t,r;
...
}function c(v,t,s){var u=this.getMixedMarker(v),r;
...
}}}function p(v,u){var s=this,t=this.getMixedMarker(v),r=s.map.getProjection();
if(t){if(r){s.map.setCenter(this.getOffsetLatLng(t.getPosition(),u))
}else{s.ubber.once("idle",function(){s.map.setCenter(this.getOffsetLatLng(t.getPosition(),u))
})
}}}function l(v,u){var s=this,r=s.map.getProjection(),t=this.getMixedMarker(v);
if(t){if(r){s.map.panTo(s.getOffsetLatLng(t.getPosition(),u))
}else{s.ubber.once("idle",function(){s.map.panTo(s.getOffsetLatLng(t.getPosition(),u))
})
}}}function k(t,r){var s=this.getMixedMarker(t);
if(s){s.setOptions(r)
}}function b(t){var v,x=B.env,s=google.maps,w=s.event,r=this.ubber,z,u,y;
...
w.addListener(v,"click",function(A){if(A&&typeof A.preventDefault==="function"){A.preventDefault()
}r.trigger("marker-click",{evt:A,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
});
w.addListener(v,"mouseover",function(A){r.trigger("marker-hover",{evt:A,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
});
w.addListener(v,"mouseout",function(A){r.trigger("marker-out",{evt:A,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
});
this.markers.push(v)
}function a(s){var r=this.getMixedMarker(s);
if(typeof r.setAnimation==="function"){r.setAnimation(google.maps.Animation.DROP)
}}function f(v){var s=new google.maps.OverlayView(),t=this.getMixedMarker(v),r,u;
s.draw=function(){};
s.setMap(this.map);
r=s.getProjection();
...
B.atlas.define("provider-loader",["jQuery","util-env"],function(f,i){var b=i.get("killSwitch"),h={};
function a(k,l){if(h.hasOwnProperty(k)){return h[k]
}this.url=k;
...
this.Interface=l;
h[k]=this
}a.prototype={queue:function g(){var k=[].slice.call(arguments,0);
if(this.isLoaded){this.getInterface.apply(this,k)
}else{this.stack.push(k)
}},get:function d(k){var l=this,m=this.url;
if(b){return
}if(k.get("channel")){m=e(m,"channel",k.get("channel"))
}f.getScript(m,function(){k.trigger("api-load",k);
l.isLoaded=true;
while(l.stack.length>0){l.getInterface.apply(l,l.stack.pop())
}});
this.isLoading=true
},append:function c(n,k){var l=document.createElement("script"),m=this,p=this.url;
if(b){return
}window[n]=function o(){k.trigger("api-load",k);
m.isLoaded=true;
...
this.isLoading=true
},getInterface:function j(m,k,n,l){l=l||this.Interface;
n.call(m,new l(k,m))
}};
function e(k,m,n){var l=new RegExp("\\b("+m+"=).*?(&|$)");
