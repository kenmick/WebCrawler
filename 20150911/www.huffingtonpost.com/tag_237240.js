...
(function() {
	...
	Allpapr.prototype.start = function() {
		...
		framedPam.loadLibrary("sweetlib.js", function() {
			...
			if(that.pmData.c1 === "LEFIGARO") {
				if(!lib.cookie.get("event-dior-capped")) {
					lib.cookie.set("event-dior-capped", 1, 1);
				} else {
					isCapped = true;
					
					that.pmData.allpapr.wallpaper.top = 0;
					that.pmData.allpapr.arch_height = 0;
					//that.pmData.allpapr.video = false;
				}
			}
				...
				var background = document.getElementById("background"),
					...
					ratioWallpaper = pcWidth / (that.pmData.allpapr_custom || that.pmData.allpapr).pc_width;
						...
						closeButtonText.style.top = "8px";
							...
							if(pmData.c1 === "LEMONDE") {
								closeButton.style.display = "block";
								
							} else if(pmData.c1 === "LEFIGARO") {
								closeButton.style.display = "block";
								if(isCapped) {
									closeButtonText.innerHTML = "OUVRIR";
									...
									lib.addEvent(closeButton, "click", function() {
										...
										piximedia.mastheadVideo.play(0);
										closeButton.style.display = "none";
				...
				if(wallpaper.left && wallpaper.right) {
					...
					proceed = function() {
						...
						if(that.pmData.allpapr_video) {
							...
							lib.applyStyle(closeButton, {
								position : "absolute",
								width : assets_video.close_button_width,
								height : assets_video.close_button_height,
								top : parseInt(assets_video.close_button_pos_y, 10) * ratioWallpaper + "px",
								right : parseInt(assets_video.close_button_pos_x, 10) * ratioWallpaper + "px",
								padding : "0px",
								zIndex : "4",
								cursor : "pointer"
							});
							...
							lib.applyStyle(closeButtonBackground, {
								background : assets_video.close_button_background,
								opacity : assets_video.close_button_opacity / 100,
								filter : "alpha(opacity=" + assets_video.close_button_opacity + ")",
								position : "absolute",
								width : "100%",
								height : "100%",
								top : "0px",
							...
							lib.applyStyle(closeButtonText, {
								position : "relative",
								textAlign : "center",
								color : assets_video.close_button_color,
								fontFamily : assets_video.close_button_font,
								fontSize : assets_video.close_button_fontsize,
								//fontWeight : "bold",
								margin : "0px",
								padding : "0px"
							});

							closeButtonText.innerHTML = assets_video.close_button;
