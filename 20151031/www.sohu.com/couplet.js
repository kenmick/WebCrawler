...
define("sjs/matrix/ad/form/couplet", 
	...
	options : {
		...
		init : function(){
			...
			//å·¦è¾¹æ CUBEä¸æ¥
            jQuery("."+adObject.cont_id+"_left").bind("mousedown",function(evt){
            	var oc = /a|img/.test(evt.target.tagName.toLowerCase());
            	if(oc){
            		stat.get("click",adObject.resource);//æçcube-clickçæµ
            ...
            //å³è¾¹æ CUBEä¸æ¥
            jQuery("."+adObject.cont_id+"_right").bind("mousedown",function(evt){
            	var oc = /a|img/.test(evt.target.tagName.toLowerCase());
            	if(oc){
            		stat.get("click",adObject.resource3);//æçcube-clickçæµ
            		if(adobj.resource3.clkm!=void 0)
            		stat.get(adObject.resource3.clkm);//ç¬¬ä¸æ¹cubeçæµ
            	}
            });
			
			stat.get("pv",adObject);//æçcube-pvæå
			//è§¦åå¹¿åç»æäºä»¶
