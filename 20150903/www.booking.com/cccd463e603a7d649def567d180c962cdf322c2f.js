...
d.click(function(h){if(!a(h.target).closest(c).length){var f=d.find(c+":first"),i=f.attr("target")=="_blank",g=f.attr("href")||f.data("href");
...
var c=function(){var f=395,k=false,h=a(".lp_city_sticky_cta_wrapper"),e=a(".lp_bold_date_picker_searchbox"),i="lp_city_sticky_cta_wrapper_visible",j=d.components.require("legacy-emitter");
...
$(function(){if(!B.atlas){return
}B.atlas.define("atlas-common-icons",["jQuery","icons-default","util-env"],function(f,c,h){var l=f.extend({},c.ICONS),b={hotel:j({x:153,y:33,z:2000}),hotel_soldout:j({x:153,y:33,z:2000}),geosearch_place:j({w:28,h:25,x:131,y:96,z:2000})},m={hotel:j({x:215,y:33,z:2000})},k="//q-ec.bstatic.com/static/img/map_sprites_omnibus_mk10/8b553fa5484a0ceb6aaea7eb9db1aa3e109dec1e.png",g={url:k,w:18,h:27,x:33,y:33,z:400};
...
}if(B.atlas.getVariant("eGBUPfBfRXQDXPdRe")===1){var d="//q-ec.bstatic.com/static/img/map_sprites_sr_soldout_red_markers/c139ee4c3f6e318afbf8ac5d578c8f790dddcbf4.png";
...
}if(B.atlas.getVariant("eGfEFGbJEXAEHMbEO")===1){var d="//q-ec.bstatic.com/static/img/map_sprites_sr_soldout_red_markers/c139ee4c3f6e318afbf8ac5d578c8f790dddcbf4.png";
f.extend(l,{hotel:j({x:34,y:33,z:60}),hotel_soldout:j({url:d,w:17,h:28,x:0,y:0})});
f.extend(b,{hotel:j({x:153,y:33,z:2000}),hotel_soldout:j({url:d,w:17,h:28,x:17,y:0,z:501})})
...
}B.atlas.define("atlas-common-markers",a,function(u,z,t){var k,w=u.get("destinationId"),x=false,f=100,l=null,i=false,m=u.get("isMiniIWLandingPages"),e=u.get("isMiniIW");
function d(C,M,A){var N,J,K=35,H=56,D=5,L=27,I=this.ubber.$domNode,G={w:I.width(),h:I.height()},E={w:C.outerWidth(),h:C.outerHeight()};
H=(H>G.w-A.x-D)?D:H;
...
}function j(A){A.setIW({disablePan:true,className:"iw-overlay-lp",engine:"html",getPosition:d,isFixed:true})
}function q(A){A.setIW({getPosition:getIWPositionMini})
}function n(A){A.setIW({getPosition:d})
}function g(F,C,A){if(C&&C.data&&C.data.b_id&&C.data.b_type){var D={};
...
A.getMarkers(function(D){var F=u.get("action"),G,C,E;
D=D||{};
C=[].concat(D.b_hotels||[],D.b_cities||[],D.b_airports||[]);
if(B&&B.env&&B.env.lp_map_background_running){C=[]
}E=C.length;
if(F==="searchresults"||F==="hotel"||B.atlas.getVariant("ebcMMECAFWTYcUNC")||u.get("isLP")&&F!=="city"){B.atlas.require(["map-markers-disperse"],function(H){H.initialize(C,A,w)
})
...
if(k){clearTimeout(k)
}k=setTimeout(function(){C.setIconHover(A.id);
...
C.getIW(A.id,function(D){if(B.atlas.getVariant("eGBUYSaBTbYFbVKIGcTAcHe")){C.trigger("iw-mini-better-price");
...
}function s(C){var F=this,A=F.getMarker(C.id),E=(A&&A.b_url)?A.b_url:"";
...
}}if(A&&A.data&&/^hotel.*/.test(A.data.b_type)&&(B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("eGOQPBRNZRUXe"))){return
...
F.off("marker-out",y);
F.once("iw-close",function(){F.on("marker-hover",b);
F.on("marker-out",y)
})
}if(e||m){F.on("iw-close",function(){if(l){F.setIconOut(l);
l=null
}});
if(l){F.setIconOut(l)
}l=A.b_id;
...
}else{F.getIW(A.b_id,function(H){H.b_map_nights=u.get("checkinCheckoutInterval");
...
}}}function h(H,D){var F=this,A=F.getMarker(D),C,E=3000;
...
}}if(u.get("action")=="hotel"){C=function(){h.call(F,H,D,true);
F.isBoundChangeAutoopen=false
};
F.isBoundChangeAutoopen&&F.once("bounds-change",C);
F.isBoundChangeAutoopen=false
}x=false;
if(!(B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("eGOQPBRNZRUXe"))){F.openIWLoading(D)
...
F.trigger("iw-opened-click",{marker:A,data:A.data});
return
}F.getIW(D,function(I){I.b_marker_type=(A.data)?A.data.b_marker_type:"";
...
}if((B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("eGOQPBRNZRUXe"))&&/hotel.*/.test(A.data.b_type)){window.setTimeout(function(){F.trigger("iw-control-show",{markerId:D,data:A.data})
},0)
}else{F.openIW(D,I)
}if(u.get("action")==="hotel"){E=4000
}k=setTimeout(function(){F.closeIW()
},E)
})
}function o(D,C){var A=u.get("ufi");
D.some(function(E){if(E.b_marker_type==="city"&&E.b_id===A){C.trigger("markers-emphasize-ufi",E);
if(B.atlas.getVariant("ebcBUeEFcJHeBJORe")){C.setMarkerType(E.b_id,E.b_marker_type+"_current")
}return true
}return false
})
}function r(C){try{i=(window.localStorage&&window.sessionStorage?true:false);
if(i){window.localStorage.setItem("btest","1")
}}catch(A){i=false
}if(i){C.on("load",function(){C.trigger("load-dom-storage-supported")
})
}j(C);
...
}if(e||m){$("body").delegate("#marker_close","click",function(){C.closeIW()
})
}}return{init:r,setupIW:j,autoopenIW:h,getIWPosition:d,markerHover:b,markerOut:y,markerClick:s,boundsChange:v}
})
});
$(function(){if(!B.atlas){return
...
$(function(){if(!B.atlas){return
}B.atlas.define("atlas-style",function(){return{setStyle:function(a){this.Interface.setStyle(a)
}}
});
...
}else{if(B.atlas.getVariant("ebcAZTDCMbFDbKDSCFPNSTUMKeKe")==2){b={roadmap:[{featureType:"poi.government",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#000000"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#000000"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#000000"}]}],satellite:[{featureType:"poi.government",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#FFFFFF"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#FFFFFF"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#FFFFFF"}]}]}
}else{if(B.atlas.getVariant("ebcAZTDCMbFDbKDSCFPNSTUMKeKe")==3){b={roadmap:[{featureType:"landscape.man_made",stylers:[{visibility:"off"}]},{featureType:"poi.government",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#000000"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#000000"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#000000"}]}],satellite:[{featureType:"poi.government",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#FFFFFF"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#FFFFFF"}]},{featureType:"poi.attraction",elementType:"labels.text.fill",stylers:[{visibility:"on"},{color:"#FFFFFF"}]}]}
...
if(b.atlas.getVariant("ebcdHZcKNFO")){t.push("atlas-style")
...
}if(b.atlas.getVariant("ebcdHZcKNFO")){b.atlas.require(["styler","styler-theme"],function(w,y){if(typeof y!=="undefined"){var x=new w({map:v,theme:y});
x.init()
}})
...
(function(){var a="fFdHMYdVSfHDVAfSHeae";
booking[sNSExperiments][a]=(function(c,h){var g,b,m,o,p="loc_sb_search_history_list_items",d="loc_sb_search_history_list_items--selected",e={},f={pickSuggestion:1,useKeyboard:2};
function i(){if(j()){q()
}else{k()
...
setTimeout(function(){if(!o){b.removeClass("active_class");
h(document).unbind("click",n)
}},200)
}function n(r){if(!h(r.target).closest(".loc_index_sb_search_history, .recent_search_in_sb").length){k()
}}function j(r){return(r||g).val().length<=1
}function l(r){e[r]=true;
c.track.custom_goal(a,r)
}return{init:function(){g=h(".recent_search_in_sb--active");
...
}).blur(k).bind("click keyup",i);
m=b.find("."+p);
if(m.length){g.on("keydown",function(w){if(!b.is(":visible")){return
}var v=m.filter("."+d),r=function(){if(!e[f.useKeyboard]){l(f.useKeyboard)
}},s,u;
...
}},initElse:function(){if(!h("#loc_index_sb_search_history_has_data").length){return
}var s=h(".recent_search_in_sb").bind("focus click keyup",r);
function r(){if(j(s)){booking.track.stage(a,1);
...
(function(b){var a="bSLSfPTXJMSMTfQHO";
b[sNSExperiments][a]={init:function(){var c=$(".js-b-searchbox-button").find(".b-button__text");
b.eventEmitter.bind(b.Search.Events.GROUP_CHANGED,function(e,d){if(d&&d.children>0){b.track.stage(a,1);
