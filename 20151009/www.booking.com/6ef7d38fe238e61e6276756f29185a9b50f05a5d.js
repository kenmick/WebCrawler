...
}function p(A){A.setIW({getPosition:getIWPositionMini})
...
A.data.b_is_current=(A.id||A.data.b_id)===w;
C.openIW(A.id,{},B.jstmpl("atlas_iw_mini_landmark").render(s(A.data)))
...
C.getIW(A.id,function(D){if(w==A.id){C.set("currentHotel",D)
...
}function r(C){var F=this,A=F.getMarker(C.id),E=(A&&A.b_url)?A.b_url:"";
...
A.data.b_is_current=(A.id||A.data.b_id)===w;
F.openIW(A.b_id,{},B.jstmpl("atlas_iw_landmark").render(s(A.data)));
...
}if((B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("eGOQPBRNZRUXe"))&&/hotel.*/.test(A.data.b_type)){window.setTimeout(function(){F.trigger("autoopen-fixed-iw");
F.trigger("iw-control-show",{markerId:D,data:A.data})
},0)
}else{F.openIW(D,I)
...
}function q(C){try{i=(window.localStorage&&window.sessionStorage?true:false);
...
}}function s(C){if(B.atlas.getVariant("ebcBUaUDGDGae")){var A=$.extend({},C);
A.b_marker_type=A.b_marker_type.replace(/\_current/g,"");
return A
}else{return C
}}return{init:q,setupIW:j,autoopenIW:h,getIWPosition:d,markerHover:b,markerOut:y,markerClick:r,boundsChange:v}
})
});
$(function(){if(!B.atlas){return
}B.atlas.define("atlas-common-overlay",["jQuery","pubsub","util-env"],function(i,e,t,k){var r=".show_map",w="#close_map_lightbox, #close_map, #b_map_back_link",c="#b_map_tiles",v="#b_map_container",x="map_opened",h=new e(),z=B.env,q=(z.b_browser==="msie"&&z.b_browser_version<8),l=z.b_map_center_latitude||z.b_latitude,C=z.b_map_center_longitude||z.b_longitude,a=z.b_map_google_static_thumbnail_zoom_level||14,u=i(v),b=window.location,g=false;
...
i(c).css(D)
}function f(){var E="#b_map_header",D="#b_map_tiles",F,G;
F=u.height();
G=u.find(E).outerHeight();
u.find(D).css({height:F-G,display:"block"})
...
}if((!B.atlas.assert.tdot&&B.env.b_action=="hotel")||B.atlas.getVariant("eGOLcFSBCHC")){u.addClass("b_map_container_fixed").css({top:G,bottom:30})
...
}return{addEvents:s,changeHash:o,open:n,close:j,pubSub:h,getCurrentHotelId:d}
})
});
$(function(){if(!B.atlas){return
...
}if(n.length){m+=h.jstmpl("lp_getaway_deals_on_index_client_side").render({lp_getaway_deals_on_index_data:n});
j.html(m)
...
}}}function a(f){if(!f){b(".lp_bold_date_picker_num_nights_container .b-form-number-of-nights").addClass("visible")
}e.track.stage(c,1)
