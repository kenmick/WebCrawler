...
if(!(B.atlas.getVariant("ebcBUNHTIYSaBNAHBBMC")||B.atlas.getVariant("ebcOQZRUAPBRNcbRYYEO"))){this.map.on("bounds-change",function(){g._close()
})
...
c.prototype.updateRenderLoading=function(h){var g=$.extend({},h,{b_loading:h.b_states.b_is_loaded?false:true});
this.updateRender(g)
};
c.prototype.renderPendingIWCards=function(){var g=B.jstmpl("atlas_iw_hotel_fixed"),h=this.map;
this.$wrapper.find("[data-hotel-load-id]").each(function(k,l){var j=$(l),m=j.data("hotel-load-id");
h.getIW(m,function(o){var i=h.getMarker(o.b_id);
var p=i.b_basic_type;
if(o.b_id===f.get("destinationId")){p+="-current"
}var n=$(g.render($.extend(o,{b_maps_iw_fixed_classes:"iw-overlay-"+p})));
j.replaceWith(n);
h.getMarker(m,function(q){q.b_states.b_is_loaded=true
});
h.trigger("iw-load")
})
})
...
$(".iw-control").animate({opacity:0},0,function(){g&&k.html(i);
if(B.atlas.getVariant("ebcYSaBHCMYQGCIBTAcHe")){h.renderPendingIWCards()
}if(B.atlas.getVariant("ebcBUYSaBcLMeDPISRO")||B.atlas.getVariant("ebcOQPBRNQadeOJATXT")){b(k)
...
c.prototype.isVisible=function(){if(!$(".iw-control").length){return false
...
return r
}function n(r){if((g.MJ(g.MC(a[103]))&&(g.MJ(/^(searchresults|hotel)$/.test(g.ATLAS_ENV("action")))))){r+=b[3];
...
return r
}function m(r){if((g.MJ(g.MC(a[103]))&&(g.MJ(/^(searchresults|hotel)$/.test(g.ATLAS_ENV("action")))))){r+=b[3];
g.MN(a[102],g.MC(a[103]));
r+=b[6]
}r+=b[2];
if((g.MJ(g.MC(a[105]))&&(g.MJ(g.ATLAS_STAGE("ebcBUYSaBaeJBZNSTHT"))&&g.MJ(g.ATLAS_TRACK("ebcBUYSaBaeJBZNSTHT"))))){r+=b[3];
...
}}function e(){var j="/srcompset.{LANG}.html?sid={SID};age={CHILDRENAGES};checkin={CHECKIN};checkout={CHECKOUT};city={UFI};group_adults={ADULTS};group_children={CHILDREN};no_rooms={ROOMS};aid={AID}&limit=6",n=d.search.dates(),m=d.search.group(),o,k;
