...
f.extend(l,{hotel:j({x:34,y:33,z:60}),hotel_soldout:j({url:d,w:17,h:28,x:0,y:0})});
f.extend(b,{hotel:j({x:153,y:33,z:2000}),hotel_soldout:j({url:d,w:17,h:28,x:17,y:0,z:501})})
}if(B.atlas.getVariant("HMDCceGECAFSdfIO")===2){d="//r-ec.bstatic.com/static/img/map/hotel_sr/f0ac72eb6ac10833ef3310bd86375b7ce787ee25.png";
...
if((B.atlas.getVariant("ebcOQPBRYUSPdHcPdDCRe")||B.atlas.getVariant("ebcBUYSaBTbYFYdEFPWHT"))&&!/^(city|airport)/.test(this.config.type)){var E={left:12,top:0};
M=w.x-A+E.left;
...
}function j(w){w.setIW({disablePan:true,className:"iw-overlay-lp",engine:"html",getPosition:c,isFixed:true})
...
y=y||{};
if(B.atlas.getVariant("ebcNHTIYSaBNAHBBMC")){y=w.applyBehaviorsToMarkers(y)
...
if(B.atlas.getVariant("ebcBUcSPWYNHICTae")||B.atlas.getVariant("ebcOQQTDCPZRVMUWe")){g.addPolygon.call(w,y.b_best_areas)
}}else{w.updateMarkers(x);
if(s){while(z--){C=x[z];
if(C&&s===C.b_id){if(!B.atlas.assert.tdot&&B.atlas.assert.hp&&!B.atlas.getVariant("ebcBUHSCQWXXTTWEYcZbMKSGHRIcUZC")){w.setMarkerOptions(C.b_id,{cursor:"default"})
...
if(k){clearTimeout(k)
}k=setTimeout(function(){x.setIconHover(w.id);
...
}else{if(/^(city|airport)/.test(w.type)||(B.atlas.getVariant("fEJMSeCXeLfHHHYbNKe")&&w.type=="landmark")){if(d||m){if(l!==w.id){if(l){x.setIconOut(l);
x.closeIW()
}f(x,w);
w.data.b_is_current=(w.id||w.data.b_id)===s;
x.openIW(w.id,{},B.jstmpl("atlas_iw_mini_landmark").render(p(w.data)))
}}else{x.openIWLoading(w.id);
f(x,w);
x.openIW(w.id,w.data)
}}else{if(d||m){if(l!==w.id){if(l){x.setIconOut(l);
x.closeIW()
}f(x,w);
...
if(k){clearTimeout(k)
}if(!(d||m)){x.setIconOut(w.id)
}if(d||m){if(l!==w.id){x.setIconOut(w.id);
x.setIW({className:"iw-overlay-lp"});
x.closeIW()
}}else{x.setIW({className:"iw-overlay-lp"});
x.closeIW()
}}function o(x){var A=this,w=A.getMarker(x.id),z=(w&&w.b_url)?w.b_url:"";
...
w.data.b_is_current=(w.id||w.data.b_id)===s;
A.openIW(w.b_id,{},B.jstmpl("atlas_iw_landmark").render(p(w.data)));
...
}}}function h(D,y){var A=this,w=A.getMarker(y),x,z=3000;
...
return A.once("markers-show",h,y)
...
}}if(q.get("action")=="hotel"){x=function(){h.call(A,D,y,true);
...
A.trigger("iw-opened-click",{marker:w,data:w.data});
return
}A.getIW(y,function(F){F.b_marker_type=(w.data)?w.data.b_marker_type:"";
F.b_map_nights=q.get("checkinCheckoutInterval");
F.map_price_per_x_nights_txt=q.get("transPricePerXNights");
if(s==y){A.set("currentHotel",F)
}f(A,w);
...
}k=setTimeout(function(){A.closeIW()
},z)
})
}function n(x){try{i=(window.localStorage&&window.sessionStorage?true:false);
if(i){window.localStorage.setItem("btest","1")
}}catch(w){i=false
}if(i){x.on("load",function(){x.trigger("load-dom-storage-supported")
})
}j(x);
...
x.on("open",h,s);
x.on("marker-hover-triggered",function(y){if(s==y.id&&/^hotel/.test(y.type)){x.trigger("current-property-marker-hover")
}});
x.on("marker-click",function(y){if(s==y.id&&/^hotel/.test(y.type)){B.track.custom_goal("ebcBUHSCQWXXTTWEYcZbMKSGHRIcUZC",1)
}});
B.atlas.SUBSCRIPTIONS["marker-iw-inconsistent-group"].forEach(function(y){if(y.cond){x.on("marker-hover",function(A){var D=/^hotel/.test(A.type),z=/soldout/.test(A.type);
if(!D){return
}B.track.stage(y.name,1);
if(!z){x.once("iw-get-success",C)
}function C(G){var F,E;
var H;
if(G&&G.b_hotels&&G.b_hotels[A.id]){H=G.b_hotels[A.id];
E=(H.can_accommodate_group==false);
if(E){x.trigger("marker-iw-inconsistent-group")
}}}})
}});
B.atlas.SUBSCRIPTIONS["marker-iw-inconsistent-non-group"].forEach(function(y){if(y.cond){x.on("marker-hover",function(A){var D=/^hotel/.test(A.type),z=/soldout/.test(A.type);
if(!D){return
}B.track.stage(y.name,1);
if(!z){x.once("iw-get-success",C)
}function C(G){var F,E;
...
if(G&&G.b_hotels&&G.b_hotels[A.id]){H=G.b_hotels[A.id];
F=H.soldout;
if(F){x.trigger("marker-iw-inconsistent-non-group")
}}}})
}});
if(d||m){$("body").delegate("#marker_close","click",function(){x.closeIW()
})
}}function p(x){if(B.atlas.getVariant("ebcBUaUDGDGae")){var w=$.extend({},x);
...
if(B.atlas.getVariant("ebcNHTIYSaBNAHBBMC")){a.push("markers-disperser-filters")
...
var k=false;
if(B.atlas.getVariant("ebcNHTIYSaBNAHBBMC")){var p=new b()
}c.each(n,function(w){var u=new f(w,q,j),x=true;
var v=false,s=false,t;
if(!r.length||w.b_persistent){r.push(w);
l.push(u)
}else{if(!m||r.length<=m){if(B.atlas.getVariant("ebcBUECAFWTYcUNSVGPQJJYJO")||B.atlas.getVariant("ebcOLHMbdIePQNZBaQDbAAPVT")||B.atlas.getVariant("ebcMMfEDXPdLbeMSSdPIFdELSVWe")||B.atlas.getVariant("ebcMbfWWJbVHMbdIePQNZBaQDbAAPVT")||B.atlas.getVariant("ebcNHTIYSaBNAHBBMC")){c.each(l,function(z,y){if(u.intersects(z,o)){if(e(w,r[y])>0&&!B.atlas.getVariant("ebcNHTIYSaBNAHBBMC")){t=y;
v=true;
k=true
}else{if(B.atlas.getVariant("ebcNHTIYSaBNAHBBMC")&&p.check(w,r[y])===0){s=0
}else{if(B.atlas.getVariant("ebcNHTIYSaBNAHBBMC")&&p.check(w,r[y])>0){t=y;
v=true;
k=true
}else{s=true
}}}}});
