...
if(!a(k)){return _r_( k);
}if(k.forEach){k.forEach(function(l,i,m){_i_("96bbb:528");j.call(g,l,i,m)
;_r_()})
}else{for(f=0,h=k.length;
f<h;
++f){j.call(g,k[f],f,k)
}}return _r_( k);
}function c(h,g){_i_("96bbb:72");var f=b(h,function(i){_i_("96bbb:503");return _r_( i===g);
});
return _r_((f>-1)?true:false);
}return _r_({each:d,find:e,findIndex:b,isArray:a,contains:c});
});
B.atlas.define("util-getset",function(){_i_("96bbb:4");function a(b){_i_("96bbb:73");this.set=function(c,d){_i_("96bbb:361");if(c){b[c]=d;
return _r_( d);
}return _r_( undefined);
};
this.get=function(c){_i_("96bbb:362");return _r_( c?b[c]:undefined);
}
;_r_()}return _r_( a);
});
...
}var l=function(p,q,o){_i_("96bbb:363");var n="success";
o.success.call(o.context,p,n,q);
d(n,q,o)
;_r_()};
var j=function(n,p,q,o){_i_("96bbb:364");o.error.call(o.context,q,p,n);
d(p,q,o)
;_r_()};
var d=function(n,p,o){_i_("96bbb:365");o.complete.call(o.context,p,n)
;_r_()};
m.onload=function(){_i_("96bbb:366");var n,o;
...
m.onerror=function(){_i_("96bbb:367");j(null,"error",m,e);
return _r_();
};
m.open(e.type,e.url);
...
function h(){_i_("96bbb:304");if(a.SUBSCRIPTIONS&&b.jst){n={};
j.each(a.SUBSCRIPTIONS,function(o,p){_i_("96bbb:546");j.each(p,function(q,r){_i_("96bbb:584");if(r.name&&r.touched===true){n[r.name]=b.jst[r.name]
};_r_()})
;_r_()})
}return _r_( n);
}function f(){_i_("96bbb:305");j.post(m,{sid:l.b_sid,pid:l.pageview_id,context:"atlas",data:JSON.stringify({metrics:i.metrics,actions:i.actions,experiments:h()})});
i={metrics:[],actions:[]}
;_r_()}function g(q,p){_i_("96bbb:306");var r,o=window.performance&&window.performance.now?window.performance.now():new Date().getTime();
if(c){clearTimeout(c)
}p=p||"";
...
if(!f){return _r_();
}B.atlas.require(["polygons"],function(h){_i_("96bbb:368");h.createPolygon.call(e,{clickable:false,fillColor:"#ff0000",fillOpacity:0.1,strokeColor:"#ff0000",strokeOpacity:0.5,strokeWeight:1,paths:[f.ne,[f.sw[0],f.ne[1]],f.sw,[f.ne[0],f.sw[1]]]});
...
k.set=function(o,p){_i_("96bbb:369");if(o){i[o]=p;
return _r_( p);
}return _r_( f);
};
k.get=function(o){_i_("96bbb:370");return _r_( o?i[o]:f);
};
d.extend(k,new a(k));
k.trigger("pre-init");
k.$domNode=d(j.options.domNode);
function l(o){_i_("96bbb:313");setTimeout(function(){_i_("96bbb:532");n=true;
k.Interface=o;
k.trigger("ready",k)
;_r_()},4)
;_r_()}h.unshift(m,"center","options","analytics","goals");
h.push("debug");
B.atlas.require(h,function(s){_i_("96bbb:371");var p,o,r,q=[].slice.call(arguments,1);
for(p=0,o=q.length;
p<o;
++p){r=q[p];
...
s.init(k,i,l)
;_r_()});
k.done=function g(o){_i_("96bbb:372");if(n){o.call(k)
}else{k.on("ready",o)
};_r_()};
...
k.initialize(l,i,h)
}g.prototype.initialize=function(m,i,h){_i_("96bbb:314");var k=this,j=k.get("isInitialized"),l=this.get("domNode");
if(j){return _r_( false);
}b(l,k);
i.unshift(m);
B.atlas.require(i,function(n){_i_("96bbb:533");c.each([].slice.call(arguments,1),function(o){_i_("96bbb:579");if(o&&o.init){o.init.call(k)
}else{if(o){f.extend(k,o)
}};_r_()});
k.trigger("api-before-load");
n.init(k,h,function(o){_i_("96bbb:580");setTimeout(function(){_i_("96bbb:591");k.set("hasLoaded",true);
k.Interface=o;
k.trigger("ready",k)
;_r_()},4)
;_r_()})
...
g.prototype.done=function(h){_i_("96bbb:315");if(this.get("hasLoaded")){h.call(this)
...
function j(r){_i_("96bbb:316");if(o){console.debug("Atlas event: "+r)
};_r_()}function h(u,t){_i_("96bbb:317");var s=this,r=[];
t=t||null;
j(u);
...
;_r_()})
};_r_()}function i(s,u,r,t){_i_("96bbb:318");s=(s&&typeof s==="string")?[s]:s;
b.each(s,function(w,v){_i_("96bbb:534");if(t){n[v]=n[v]||[];
n[v].push({handler:u,args:r||null})
}else{m[v]=m[v]||[];
m[v].push({handler:u,args:r||null})
};_r_()})
;_r_()}function e(s,t,r){_i_("96bbb:319");s=(s&&typeof s==="string")?[s]:s;
b.each(s,function(v,u){_i_("96bbb:535");m[u]=m[u]||[];
m[u].push({handler:t,args:r||null,once:true})
;_r_()})
;_r_()}function p(r,s){_i_("96bbb:320");if(m.hasOwnProperty(r)&&(!s||s===true)){m[r]=[]
}else{if(m.hasOwnProperty(r)){m[r]=b.grep(m[r],function(t){_i_("96bbb:585");return _r_( t.handler!==s);
})
}};_r_()}function g(r){_i_("96bbb:321");return _r_( m[r]||[]);
}function l(){_i_("96bbb:322");var r=[];
b.each(m,function(s){_i_("96bbb:536");r.push(s)
;_r_()});
return _r_( r);
}function k(r){_i_("96bbb:323");o=r;
return _r_( o);
}i([c.MARKERS_SHOW,c.TILES_LOAD],function(t,s,r){_i_("96bbb:373");if(r===c.MARKERS_SHOW&&q&&!f){h(c.LOAD,d.get("id"))
...
B.atlas.require(l,function(){_i_("96bbb:374");var m=m=[].slice.call(arguments,0);
c.extend.apply(j,[j].concat(m));
h(j)
;_r_()},null,true)
;_r_()}d.prototype.done=function(j){_i_("96bbb:324");j.call(this)
;_r_()};
return _r_( d);
});
jQuery.extend(B.atlas,(function(i,d){_i_("96bbb:64");var b=null,g=null;
function c(){_i_("96bbb:307");b=null;
g=null
;_r_()}function j(){_i_("96bbb:308");var k=d.atlas.SUBSCRIPTIONS;
b={};
g={};
if(!k||!d.jst){return _r_();
}i.each(k,function(l,m){_i_("96bbb:505");i.each(m,function(n,q){_i_("96bbb:573");try{if(q.name&&q.cond&&d.jst.hasOwnProperty(q.name)&&q.old){b[q.name]=parseInt(d.jst[q.name],10);
...
;_r_()})
}function a(k){_i_("96bbb:309");if(!b){j()
}return _r_( b.hasOwnProperty(k)?b[k]:false);
}function e(l,m){_i_("96bbb:310");var n=[],k=[],o=d.atlas.SUBSCRIPTIONS;
if(!o||!l||!o.hasOwnProperty(l)){return _r_( false);
}o[l].forEach(function(p){_i_("96bbb:506");if(a(p.name)!==false&&!p.touched&&m===p.instanceId){if(p.stage){if(g[p.name]){d.et.stage(p.name,p.stage)
...
}k.forEach(function(p){_i_("96bbb:507");d.track.stage.apply(null,p)
;_r_()})
}function h(m,k){_i_("96bbb:311");var n=d.atlas.SUBSCRIPTIONS,l=n&&n.templates?n.templates:[];
l.some(function(o){_i_("96bbb:508");if(o.name===m&&a(o.name)!==false){if(g[o.name]){d.et.stage(o.name,k||o.stage)
}else{d.track.stage(m,k||o.stage)
}return _r_( true);
}return _r_( false);
});
return _r_( true);
}function f(l){_i_("96bbb:312");var m=d.atlas.SUBSCRIPTIONS,k=m&&m.templates?m.templates:[];
k.some(function(n){_i_("96bbb:509");if(n.name===l&&a(n.name)!==false){if(!n.touched){if(g[n.name]){d.et.track(n.name)
}else{d.track.exp(l)
}n.touched=true
}return _r_( true);
}return _r_( false);
...
return _r_( a(l));
}d.run("Xfgrwdtafew").onLoad(function(){_i_("96bbb:357");if(!b){j()
};_r_()});
...
this.extend(f)
};_r_()}a.prototype={extend:function(h){_i_("96bbb:375");var f=d(h),e=this.sw,g=this.ne;
...
},reset:function(){_i_("96bbb:376");this.ne=this.sw=null
;_r_()},getNorthEast:function(){_i_("96bbb:377");return _r_( this.ne||new c(0,0));
},getSouthWest:function(){_i_("96bbb:378");return _r_( this.sw||new c(0,0));
},setNorthEast:function(e){_i_("96bbb:379");this.ne=d(e)
;_r_()},setSouthWest:function(e){_i_("96bbb:380");this.sw=d(e)
;_r_()},getCenter:function(){_i_("96bbb:381");return _r_( new c((this.sw.lat+this.ne.lat)/2,(this.sw.lng+this.ne.lng)/2));
},isEmpty:function(){_i_("96bbb:382");return _r_((!this.sw&&!this.ne)||(this.sw.toString()===this.ne.toString()));
},equals:function(e){_i_("96bbb:383");return _r_( e instanceof a?this.toString()===e.toString():false);
},intersects:function(g){_i_("96bbb:384");var e=this.sw,i=this.ne,f=g.sw,h=g.ne;
return _r_( h.lat>=e.lat||f.lat<=i.lat||h.lng>=e.lng||f.lng<=i.lng);
},union:function(e){_i_("96bbb:385");this.extend(e.getSouthWest());
this.extend(e.getNorthEast());
return _r_( this);
},contains:function(g){_i_("96bbb:386");var i,f,e,h;
...
},toMOMString:function(){_i_("96bbb:387");b(this.sw);
b(this.ne);
return _r_([this.sw.lng,this.sw.lat,this.ne.lng,this.ne.lat].join(","));
},toString:function(){_i_("96bbb:388");b(this.sw);
b(this.ne);
...
this.lng=+b||0
}};_r_()}a.prototype={set:function(c,b){_i_("96bbb:389");this.lat=+c;
this.lng=+b;
return _r_( this);
},clone:function(){_i_("96bbb:390");return _r_( new a(this.lat,this.lng));
},toString:function(){_i_("96bbb:391");return _r_(["[",this.lat,",",this.lng,"]"].join(""));
}};
return _r_( a);
});
...
;_r_()}f.prototype={extend:function(h){_i_("96bbb:392");this.bounds.extend(h);
this.zoom=false;
return _r_( this);
},extrapolate:function(i,l){_i_("96bbb:393");var k=new d(i),m=b.latLngToPoint(k,l),n=new e(this.width/2,this.height/2),h=new e(m.x+n.x,m.y-n.y),j=new e(m.x-n.x,m.y+n.y);
this.setNorthEast(b.pointToLatLng(h,l));
this.setSouthWest(b.pointToLatLng(j,l));
return _r_( this);
},getCenter:function(k){_i_("96bbb:394");var i=this.bounds.getCenter(),j=this.getZoom(),h;
if(k){h=b.latLngToPoint(i,j).subtract(k);
i=b.pointToLatLng(h,j)
}return _r_( i);
},getNorthWest:function(){_i_("96bbb:395");var h=this.bounds.getCenter(),j=this.getZoom(),l=new e(this.width/2,this.height/2),k=b.latLngToPoint(h,j),i=k.subtract(l);
return _r_( b.pointToLatLng(i,j));
},getSouthEast:function(){_i_("96bbb:396");var i=this.bounds.getCenter(),j=this.getZoom(),l=new e(this.width/2,this.height/2),k=b.latLngToPoint(i,j),h=k.add(l);
return _r_( b.pointToLatLng(h,j));
},setNorthEast:function(h){_i_("96bbb:397");this.bounds.setNorthEast(h)
;_r_()},setSouthWest:function(h){_i_("96bbb:398");this.bounds.setSouthWest(h)
;_r_()},getZoom:function(h,m){_i_("96bbb:399");var q=h||g,j=true,i=this.bounds.getNorthEast(),n=this.bounds.getSouthWest(),k=new d(i.lat,n.lng),l=new d(n.lat,i.lng),o,p;
if(!this.zoom){m=m||a;
...
},latLngToPixel:function(j){_i_("96bbb:400");var i=this.getZoom(),h=b.latLngToPoint(this.getNorthWest(),i);
return _r_( b.latLngToPoint(j,i).subtract(h));
},pixelToLatLng:function(h){_i_("96bbb:401");var j=this.getZoom(),i=b.latLngToPoint(this.getNorthWest(),j);
return _r_( b.pointToLatLng(h.add(i),j));
},getDistance:function(i,h){_i_("96bbb:402");return _r_( b.getDistance(i,h));
},toString:function(){_i_("96bbb:403");return _r_( this.bounds.toString());
}};
return _r_( f);
});
B.atlas.define("geometry.point",function(){_i_("96bbb:16");function a(b,c){_i_("96bbb:101");this.x=+b;
this.y=+c
;_r_()}a.prototype={set:function(b,c){_i_("96bbb:404");this.x=+b;
this.y=+c;
return _r_( this);
},clone:function(){_i_("96bbb:405");return _r_( new a(this.x,this.y));
},add:function(b){_i_("96bbb:406");return _r_( new a(this.x+b.x,this.y+b.y));
},subtract:function(b){_i_("96bbb:407");return _r_( new a(this.x-b.x,this.y-b.y));
},toString:function(){_i_("96bbb:408");return _r_(["[",this.x,",",this.y,"]"].join(""));
}};
return _r_( a);
});
...
;_r_()}return _r_({init:function(){_i_("96bbb:358");var e=this;
if(!B.atlas){return _r_();
}d.each(a,function(g,h){_i_("96bbb:547");var f={topic:h.evt,value:h.value||""};
...
b=[{evt:"load",name:"atlas_hp_open",condition:a.hp,filter:function(e){_i_("96bbb:510");if(e==="tdotPOI"){return _r_( false);
}return _r_( true);
}},{evt:"load",name:"atlas_sr_open",condition:a.sr},{evt:"load",name:"atlas_lp_open",condition:a.lp},{evt:"close",name:"atlas_hp_close",condition:a.hp},{evt:"close",name:"atlas_sr_close",condition:a.sr},{evt:"close",name:"atlas_lp_close",condition:a.lp},{evt:"marker-click",name:"atlas_hp_click_property",condition:a.hp,filter:function(e){_i_("96bbb:511");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-click",name:"atlas_sr_click_property",condition:a.sr,filter:function(e){_i_("96bbb:512");return _r_( !/city|airport|vpm/.test(e.type));
}},{evt:"marker-click",name:"atlas_lp_click_property",condition:a.lp,filter:function(e){_i_("96bbb:513");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-click",name:"atlas_hp_click_ufi",condition:a.hp,filter:function(e){_i_("96bbb:514");return _r_(/city/.test(e.type));
}},{evt:"marker-click",name:"atlas_sr_click_ufi",condition:a.sr,filter:function(e){_i_("96bbb:515");return _r_(/city/.test(e.type)&&!/vpm/.test(e.type));
}},{evt:"marker-click",name:"atlas_sr_click_airport",condition:a.sr,filter:function(e){_i_("96bbb:516");return _r_(/airport/.test(e.type));
}},{evt:"marker-click",name:"atlas_lp_click_ufi",condition:a.lp,filter:function(e){_i_("96bbb:517");return _r_(/city/.test(e.type));
}},{evt:"marker-click",name:"atlas_lp_click_airport",condition:a.lp,filter:function(e){_i_("96bbb:518");return _r_(/airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_hp_hover_property",condition:a.hp,filter:function(e){_i_("96bbb:519");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_sr_hover_property",condition:a.sr,filter:function(e){_i_("96bbb:520");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_lp_hover_property",condition:a.lp,filter:function(e){_i_("96bbb:521");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_hp_hover_ufi",condition:a.hp,filter:function(e){_i_("96bbb:522");return _r_(/city/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_sr_hover_ufi",condition:a.sr,filter:function(e){_i_("96bbb:523");return _r_(/city/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_sr_hover_airport",condition:a.sr,filter:function(e){_i_("96bbb:524");return _r_(/airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_lp_hover_ufi",condition:a.lp,filter:function(e){_i_("96bbb:525");return _r_(/city/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_lp_hover_airport",condition:a.lp,filter:function(e){_i_("96bbb:526");return _r_(/airport/.test(e.type));
}}];
...
}return _r_({init:function(){_i_("96bbb:359");var e=this;
if(this.get("id")){return _r_();
}d.each(b,function(g,f){_i_("96bbb:548");if(f.condition){e.on(f.evt,c,f,true)
};_r_()})
}});
});
$(function(){_i_("96bbb:24");if(!B.atlas){return _r_();
}B.atlas.define("iw-ajax",["jQuery","util-env","util-array"],function(b,d,j){_i_("96bbb:325");var c=[],a=[];
function i(n,q){_i_("96bbb:409");var m=j.findIndex(c,function(r){_i_("96bbb:574");return _r_( r[0]===n);
}),p=c[m],o;
if(m>-1){o=p[2];
if(p[1]!=q){o.abort();
c.splice(m);
...
return _r_( l);
}}function k(m){_i_("96bbb:410");var l=j.findIndex(c,function(o){_i_("96bbb:575");return _r_( o[0]===m);
}),n=c[l];
if(typeof n=="object"){n[3].length=0
};_r_()}function g(m){_i_("96bbb:411");var l=this.get("markerDetailsURL");
if(!l){l=d.get("markerDetailsURL");
this.set("markerDetailsURL",l)
}return _r_( l+";hotel_id="+m);
}function e(n){_i_("96bbb:412");var m,l=this;
m=j.find(a,function(o){_i_("96bbb:576");return _r_( o[0]===l&&o[1]==n);
});
return _r_((m)?m[2]:false);
}function h(s,r,l){_i_("96bbb:413");var q,m=g.call(this,s),p=this,n=e.call(p,s),o=i(p,s);
o=o||[];
o[3]=o[3]&&o[3].length>0&&o[1]==s?o[3]:[];
o[3].push(r);
if(!l&&n){j.each(o[3],function(t){_i_("96bbb:581");if(typeof t==="function"){t.apply(p,[n])
};_r_()});
k(p);
return _r_( n);
}if(o[3].length>1){return _r_();
}p.trigger("iw-get-before",p);
q=b.ajax({url:m,error:function(u,t){_i_("96bbb:586");if(t!=="abort"){p.trigger("iw-get-fail",p)
};_r_()},success:function(u){_i_("96bbb:587");if(!u){return _r_();
}var v=j.find(c,function(w){_i_("96bbb:593");return _r_( w[0]===p);
}),t=v[3];
j.each(t,function(w){_i_("96bbb:592");if(typeof w==="function"&&u.b_hotels){w.apply(p,[u.b_hotels[s]])
};_r_()});
k(p);
...
c.push(o);
return _r_( q);
}function f(){_i_("96bbb:414");a.length=0
...
a();
this.on("offset-change",function(p){_i_("96bbb:415");o.IW.OPTIONS.mapOffset=p
;_r_()})
}function a(){_i_("96bbb:122");if(!B.jstmpl){return _r_();
}B.jstmpl.fn.HELPER_FROM_FRICE=function(o){_i_("96bbb:416");return _r_((B.env.map_vars.translation_from.replace("price_placeholder",o)).replace("price_placeholdernbsp;","$ "));
};
B.jstmpl.fn.HELPER_IS_DEFINED=function(o){_i_("96bbb:417");return _r_( !!o||o===0||o===false);
};
B.jstmpl.fn.HELPER_ENV=function(o){_i_("96bbb:418");return _r_((B.env.hasOwnProperty(o))?B.env[o]:"");
};
B.jstmpl.fn.HELPER_IW_B_BLOCKS=function(q,o,p){_i_("96bbb:419");if(!q){return _r_( false);
}if(q[o]&&q[o][p]){return _r_( q[o][p]);
}if(q.b_room_group&&q.b_room_group[o]&&q.b_room_group[o][p]){return _r_( q.b_room_group[o][p]);
}return _r_( false);
};
B.jstmpl.fn.ATLAS_GET_VARIANT=function(o){_i_("96bbb:420");return _r_((B.atlas&&B.atlas.getVariant)?B.atlas.getVariant(o):false);
};
B.jstmpl.fn.ATLAS_TRACK=function(o){_i_("96bbb:421");return _r_((B.atlas&&B.atlas.trackInTemplate)?B.atlas.trackInTemplate(o):false);
};
B.jstmpl.fn.ATLAS_STAGE=function(p,o){_i_("96bbb:422");return _r_((B.atlas&&B.atlas.stageInTemplate)?B.atlas.stageInTemplate(p,o):true);
};
B.jstmpl.fn.ATLAS_ENV=function(o){_i_("96bbb:423");return _r_((o)?b.get(o):false);
};
B.jstmpl.fn.strings=function(q){_i_("96bbb:424");var o="";
try{o=B.strings(q)
...
B.atlas.define("hover",function(){_i_("96bbb:27");function c(){_i_("96bbb:132");this.on("marker-hover",function(d){_i_("96bbb:425");a.call(this,d.data.b_id)
;_r_()});
this.on("marker-out",function(d){_i_("96bbb:426");b.call(this,d.data.b_id)
;_r_()})
...
var e=this;
f.forEach(function(i){_i_("96bbb:427");var h;
...
}B.atlas.define("mixed-markers",["util-object","util-array"],function(h,f){_i_("96bbb:326");function g(u){_i_("96bbb:428");var t,s=[];
if(f.isArray(u)){return _r_( u);
}for(t in u){if(u.hasOwnProperty(t)){s.push(u[t])
}}return _r_( s);
}function i(u,t){_i_("96bbb:429");var s=g(u);
this.pruneMarkers(s);
this.addMarkers(s,t)
;_r_()}function q(){_i_("96bbb:430");return _r_( this.Interface.getActiveMixedMarkers());
}function m(){_i_("96bbb:431");this.Interface.clearMixedMarkers()
;_r_()}function o(s){_i_("96bbb:432");this.Interface.clearMixedMarker(s)
;_r_()}function k(t){_i_("96bbb:433");var s=g(t);
this.Interface.pruneOutboundedMixedMarkers();
this.addMarkers(s)
;_r_()}function n(w){_i_("96bbb:434");var v,u,t=this.Interface.getActiveMixedMarkers(),s=t.length;
while(s--){v=t[s].b_id;
u=w.length;
while(u--){if(w[u].b_id===v){break
}}if(u<0&&v){this.Interface.clearMixedMarker(v)
}};_r_()}function a(A,w){_i_("96bbb:435");var y,v,u=[],x=0,t=g(A),s=t.length;
this.trigger("markers-create",this);
while(s--){y=t[s];
...
;_r_()}function b(u){_i_("96bbb:436");var t,s=this.Interface.getMixedMarker(u);
...
}};_r_()}function c(s){_i_("96bbb:437");this.Interface.centerMixedMarker(s,this.get("offset"))
;_r_()}function j(s){_i_("96bbb:438");this.Interface.panMixedMarker(s,this.get("offset"))
;_r_()}function p(u,t){_i_("96bbb:439");var s=this.Interface.getMixedMarker(u);
if(t&&s){s.b_type=t;
...
}};_r_()}function e(s){_i_("96bbb:440");return _r_( this.Interface.getMixedMarker(s));
}function l(t,s){_i_("96bbb:441");return _r_( this.Interface.setMixedMarkerOptions(t,s));
}function r(){_i_("96bbb:442");return _r_( this.Interface.mixedMarkersExist());
}function d(s){_i_("96bbb:443");if(typeof this.Interface.getMixedMarkerPosition==="function"){return _r_( this.Interface.getMixedMarkerPosition(s));
...
m=b.ajax({url:k+";BBOX="+l,error:function(p,o){_i_("96bbb:549");if(o!=="abort"){j.trigger("markers-get-fail",j)
};_r_()},success:function(o){_i_("96bbb:550");j.trigger("markers-get-success",o,j);
...
c.each(m.reverse(),function(o,p){_i_("96bbb:444");var q;
if(!o){return _r_();
...
n.append(l.children());
f(document.createElement("img")).bind("load",function(){_i_("96bbb:582");l.addClass(j);
k.trigger("load")
...
l=l||i(k);
if(m){h(m,k)
}l.find(b).each(function(n,o){_i_("96bbb:445");k.renderMarker(o)
;_r_()});
k.trigger("render")
...
d.each(h.slice(1),function(i){_i_("96bbb:446");if(i.value){g.push(i.value)
};_r_()});
return _r_( g.join(", "));
...
;_r_()}f.prototype.getDetails=function(i){_i_("96bbb:327");var g=this.get("map"),h=this.get("place_id");
...
if(i.length>0){e.each(i,function(k){_i_("96bbb:538");if(k.place_id&&k.terms){h.push(new g(k,j))
};_r_()})
}return _r_( h);
}function d(j,l,i){_i_("96bbb:188");var h=this,k=f(j,h);
a=a||new google.maps.places.AutocompleteService();
a.getPlacePredictions(k,function(n,m){_i_("96bbb:447");if(m&&m.indexOf("OK")>-1&&n){l(c(n,h))
...
;_r_()}b.prototype={setOptions:function(e){_i_("96bbb:448");var f={};
if(e.center){e.center=d(e.center)
}c.extend(f,e);
this.core.setOptions(f);
return _r_( this);
},getCenter:function(){_i_("96bbb:449");var e=this.core.getCenter();
return _r_([e.lat(),e.lng()]);
},on:function(e,h,g){_i_("96bbb:450");var f=this;
a.push({context:this,evt:e,handler:h,listener:google.maps.event.addListener(this.core,e,function(){_i_("96bbb:588");f.ubber.ubber.trigger("circle-"+e,this);
h.call(f,g)
;_r_()})});
return _r_( this);
},off:function(f,g){_i_("96bbb:451");var i,h,e=a.length;
while(e--){i=a[e];
if(i.context===this&&i.evt===f&&i.handler===g){h=a.splice(e,1);
if(h.length===1&&h[0].listener){h[0].listener.remove()
}break
}}return _r_( this);
},setCenter:function(e){_i_("96bbb:452");this.core.setCenter(d(e));
return _r_( this);
},hide:function(){_i_("96bbb:453");this.core.setVisible(false);
return _r_( this);
},show:function(){_i_("96bbb:454");this.core.setVisible(true);
return _r_( this);
},remove:function(){_i_("96bbb:455");var f,e=a.length;
...
c.addListener(this.map,"dragstart",function(){_i_("96bbb:456");b.trigger("drag-start")
;_r_()});
c.addListener(this.map,"dragend",function(){_i_("96bbb:457");b.trigger("drag")
;_r_()});
c.addListener(this.map,"zoom_changed",function(){_i_("96bbb:458");var e=b.Interface,f=(e&&e.map)?e.map.getZoom():0;
b.trigger("zoom",f)
;_r_()});
c.addListener(this.map,"dblclick",function(){_i_("96bbb:459");b.trigger("doubleclick")
;_r_()});
c.addListener(this.map,"idle",function(){_i_("96bbb:460");b.trigger("idle")
;_r_()});
c.addListener(this.map,"resize",function(){_i_("96bbb:461");b.trigger("resize")
;_r_()});
c.addListener(this.map,"bounds_changed",function(){_i_("96bbb:462");if(d){clearTimeout(d)
}d=setTimeout(function(){_i_("96bbb:577");b.trigger("bounds-change")
;_r_()},150)
;_r_()});
c.addListener(this.map,"tilesloaded",function(){_i_("96bbb:463");b.trigger("tiles-load")
;_r_()});
c.addListener(this.map,"maptypeid_changed",function(){_i_("96bbb:464");b.trigger("map-type-change",this.getMapTypeId())
;_r_()})
...
this.setMap(h)
;_r_()}b.prototype.remove=function(){_i_("96bbb:328");if(this.$domNode){this.$domNode.remove();
this.$domNode=null
}this.setMap(null)
;_r_()};
b.prototype.draw=function(){_i_("96bbb:329");var g=this,h=this.config,f=(h.isFixed)?this.ubber.$domNode:d(this.getPanes().floatPane);
if(!this.$domNode){this.$domNode=d('<div class="'+e+'"/>');
this.$domNode.appendTo(f).bind("mousemove",function(i){_i_("96bbb:583");i.stopPropagation()
...
b.prototype.checkOverlay=function(j,h){_i_("96bbb:330");var i=this.ubber.get("topOverlay"),g=i?i:this.ubber.$domNode,f=this.getPanes().floatPane;
if(j&&h.get(0)!==g.get(0)){h.appendTo(g)
}else{if(!j&&h.get(0)!==f){h.appendTo(d(f))
}};_r_()};
b.prototype.open=function(h,g){_i_("96bbb:331");var k=d.extend(this.config,(h||{})),j=[e,k.className],i=this.$domNode,f=this.getProjection();
...
b.prototype.getPosition=function(g,f){_i_("96bbb:332");var h=this.config;
return _r_({x:f.x+h.offset.x-g.outerWidth()/2,y:f.y+h.offset.y});
};
b.prototype.setPosition=function(){_i_("96bbb:333");var g,m,l,k=this.config,h=this.$domNode,f=this.getProjection(),j=(k.getPosition)||this.getPosition,i=new google.maps.LatLng(k.latLng[0],k.latLng[1]);
if(!f||!this.$domNode){return _r_();
...
b.prototype.close=function(){_i_("96bbb:334");if(this.$domNode){this.isOpen=false;
this.$domNode.addClass(a);
this.$domNode.hide();
this.ubber.trigger("iw-close")
};_r_()};
b.prototype.panMap=function(){_i_("96bbb:335");var h=this.getMap(),g=this.config,n=d(h.getDiv()),i=this.$domNode,p=n.width(),l=n.height(),k=i.outerWidth()/2,o=i.outerHeight(),f=this.getProjection().fromLatLngToContainerPixel(new google.maps.LatLng(g.latLng[0],g.latLng[1])),m=0,j=0;
...
B.atlas.define("provider-google-markers",["jQuery","util-array"],function(f,l){_i_("96bbb:46");function g(o){_i_("96bbb:200");return _r_( l.findIndex(this.markers,function(p){_i_("96bbb:465");return _r_( p.b_id===String(o));
}));
...
}function n(o){_i_("96bbb:205");return _r_( l.find(this.markers,function(p){_i_("96bbb:466");return _r_( p.b_id===String(o));
}));
}function m(){_i_("96bbb:206");return _r_( f.map(this.markers,function(o){_i_("96bbb:467");return _r_({b_id:o.b_id,b_name:o.b_name,b_type:o.b_type,b_url:o.b_url,b_latitude:o.b_latitude,b_longitude:o.b_longitude,b_persistent:(o.data)?o.data.b_persistent:false,b_behaviors:(o.data)?o.data.b_behaviors:false,b_states:(o.data)?o.data.b_states:false});
}));
...
r.setMap(this.map);
t.addListener(r,"click",function(x){_i_("96bbb:468");o.trigger("marker-click",{evt:x,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this.data})
;_r_()});
t.addListener(r,"mouseover",function(x){_i_("96bbb:469");o.trigger("marker-hover",{evt:x,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this.data})
;_r_()});
t.addListener(r,"mouseout",function(x){_i_("96bbb:470");o.trigger("marker-out",{evt:x,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this.data})
;_r_()});
this.markers.push(r)
...
;_r_()}a.prototype.onAdd=function(){_i_("96bbb:336");g(this)
;_r_()};
a.prototype.render=function(j){_i_("96bbb:337");var l=this.options,i=l.items,k=d(j||{});
...
a.prototype.position=function(k,m){_i_("96bbb:338");var j=this.getProjection(),i=google.maps,n,o,l;
...
a.prototype.add=function(j){_i_("96bbb:339");var k=this.options,i=this.options.queue;
...
a.prototype.remove=function(m){_i_("96bbb:340");var i=this.options.items,k=this.options.overlayNode,j,l;
j=f.findIndex(i,function(n){_i_("96bbb:551");return _r_( n.id===m);
});
if(j>-1){l=i.splice(j,1)[0]
}if(k&&l&&k.contains(l.domNode)){k.removeChild(l.domNode)
};_r_()};
a.prototype.removeAll=function(){_i_("96bbb:341");var i=this.options.items,j=this.options.overlayNode,k;
while(j&&i.length){k=i.pop().domNode;
if(j.contains(k)){j.removeChild(k)
}}this.options.items=[]
;_r_()};
a.prototype.draw=function(){_i_("96bbb:342");var j,i=this;
g(this);
j=this.options.items;
j.forEach(function(k){_i_("96bbb:539");i.position(k.options,k.domNode)
;_r_()})
;_r_()};
a.prototype.onRemove=function(){_i_("96bbb:343");var i=this.options.items,j=this.options.overlayNode;
...
d.each(j,function(o){_i_("96bbb:471");l=d.findIndex(m,function(p){_i_("96bbb:578");return _r_( o===p);
});
if(l>-1&&l<k){k=l;
n=m[k]
};_r_()});
...
l.addListener(j,"click",function(m){_i_("96bbb:472");k.trigger("click",m)
;_r_()});
l.addListener(j,"mouseover",function(m){_i_("96bbb:473");k.trigger("mouseover",m)
;_r_()});
l.addListener(j,"mouseout",function(m){_i_("96bbb:474");k.trigger("mouseout",m)
;_r_()})
;_r_()}function h(k){_i_("96bbb:230");var l=google.maps.event,j=this;
l.addListener(k,"closeclick",function(m){_i_("96bbb:475");j.trigger("close",m)
;_r_()})
...
i.atlas=f;
if(i.data){i.data.getImage=function(j){_i_("96bbb:540");return _r_( h.getImage.call(h,j));
}
}d.extend(this,a.call(this));
this.get=function(j){_i_("96bbb:476");return _r_( j?i[j]:i.data);
};
this.set=function(j,k){_i_("96bbb:477");if(j){i[j]=k;
return _r_( k);
}return _r_( undefined);
}
;_r_()}b.prototype.extend=function(f){_i_("96bbb:344");this.set("data",d.extend(this.get("data")||{},f));
return _r_( this);
};
b.prototype.render=function(f){_i_("96bbb:345");if(this.get("atlas").Interface.map){this.renderMarker(f)
}else{this.renderHTML(f)
}this.set("isRendered",true)
;_r_()};
b.prototype.renderMarker=function(g){_i_("96bbb:346");var f=this.get("marker"),j=this.get("atlas").Interface.map,i=google.maps,h=e.normalizeMarker.call(this,g);
if(!j){return _r_();
...
b.prototype.renderHTML=function(f){_i_("96bbb:347");var i=this.get("$domNode"),g=c(this.get("atlas").get("resultsNode")),h=f.className,j=f.content||"";
if(!i){i=this.set("$domNode",c('<div class="'+h+'"/>'));
i.html(j);
g.append(i)
}else{i.html(j)
};_r_()};
b.prototype.remove=function(){_i_("96bbb:348");if(this.get("atlas")&&this.get("atlas").Interface.map){this.removeMarker()
}else{this.removeHTML()
}this.set("isRendered",false)
;_r_()};
b.prototype.removeMarker=function(){_i_("96bbb:349");var f=this.get("marker");
if(f){f.setMap(null);
f.set("marker",null)
};_r_()};
b.prototype.removeHTML=function(){_i_("96bbb:350");c(this.get("$domNode")).remove();
this.set("$domNode",null)
;_r_()};
b.prototype.getImage=function(g){_i_("96bbb:351");var j=this.get("data"),i=g?g.width:80,f=g?g.height:80,h;
...
b.prototype.getDetails=function(h){_i_("96bbb:352");var f=this.get("atlas"),g=this.get("id");
f.getPlaceDetails({placeId:g},h,this)
;_r_()};
b.prototype.getMarkerConfig=function(g){_i_("96bbb:353");var h=this.get("coordinates"),f;
f={b_id:this.get("id"),b_latitude:h.lat,b_longitude:h.lng,data:this.get("data")};
d.extend(f,g);
return _r_( f);
};
b.prototype.open=function(h){_i_("96bbb:354");var g=this.get("iw"),f=this.get("marker"),j=this.get("atlas").Interface.map,i=d.extend({},h||{});
if(!j){return _r_();
...
b.prototype.close=function(){_i_("96bbb:355");var f=this.get("iw");
...
c.each(p,function(q){_i_("96bbb:478");var r=false;
...
l.nearbySearch(o,function(s,r,q){_i_("96bbb:479");var t;
...
m.getDetails(r,function(u,t){_i_("96bbb:480");var s,v="fail";
...
m.geocode(q,function(s,r){_i_("96bbb:481");if(r===google.maps.GeocoderStatus.OK&&c.isArray(s)&&s[0]){p.success.call(n,s[0])
...
function b(e){_i_("96bbb:240");return _r_( d.map(e,function(f){_i_("96bbb:482");return _r_( new google.maps.LatLng(f[0],f[1]));
}));
}function a(e,f){_i_("96bbb:241");var g={};
d.extend(g,e,{paths:b(e.paths),map:f.map});
this.ubber=f;
this.core=new google.maps.Polygon(g)
;_r_()}a.prototype={setOptions:function(e){_i_("96bbb:483");var f={};
if(e.paths){e.paths=b(e.paths)
}d.extend(f,e);
this.core.setOptions(f);
return _r_( this);
},getCenter:function(){_i_("96bbb:484");var e,f=new google.maps.LatLngBounds();
this.core.getPath().forEach(function(g){_i_("96bbb:563");f.extend(g)
;_r_()});
e=f.getCenter();
return _r_([e.lat(),e.lng()]);
},on:function(e,h,g){_i_("96bbb:485");var f=this;
c.push({context:this,evt:e,handler:h,listener:google.maps.event.addListener(this.core,e,function(){_i_("96bbb:589");f.ubber.ubber.trigger("polygon-"+e,this);
h.call(f,g)
;_r_()})});
return _r_( this);
},off:function(f,g){_i_("96bbb:486");var i,h,e=c.length;
while(e--){i=c[e];
if(i.context===this&&i.evt===f&&i.handler===g){h=c.splice(e,1);
if(h.length===1&&h[0].listener){h[0].listener.remove()
}break
}}return _r_( this);
},setPaths:function(e){_i_("96bbb:487");this.core.setPaths(b(e));
return _r_( this);
},hide:function(){_i_("96bbb:488");this.core.setVisible(false);
return _r_( this);
},show:function(){_i_("96bbb:489");this.core.setVisible(true);
return _r_( this);
},remove:function(){_i_("96bbb:490");var f,e=c.length;
...
B.atlas.define("provider-google-style",function(){_i_("96bbb:54");return _r_({setStyle:function(a){_i_("96bbb:360");this.map.setOptions({styles:a})
;_r_()}});
});
...
this._markerWrapper.onselectstart=function(){_i_("96bbb:541");return _r_( false);
};
...
this._draggableListner=google.maps.event.addDomListener(S,"mousedown",function(T){_i_("96bbb:527");R.startDrag(T)
;_r_()});
this._setCursor("draggable")
...
var S=this;
google.maps.event.addDomListener(this._markerContent,"click",function(T){_i_("96bbb:543");google.maps.event.trigger(S,"click",T)
;_r_()});
google.maps.event.addDomListener(this._markerContent,"mouseover",function(T){_i_("96bbb:544");google.maps.event.trigger(S,"mouseover",T)
;_r_()});
google.maps.event.addDomListener(this._markerContent,"mouseout",function(T){_i_("96bbb:545");google.maps.event.trigger(S,"mouseout",T)
;_r_()})
}this._ready=true;
...
d.init=function(){_i_("96bbb:356");if(d._isInitialized){return _r_();
...
B.atlas.define("provider-google-mixed-markers",["util-array","util-object","provider-google-overlay-marker"],function(n,c,g){_i_("96bbb:56");function l(q){_i_("96bbb:282");return _r_( n.findIndex(this.markers,function(r){_i_("96bbb:491");if(r){return _r_( r.b_id===String(q));
}}));
}function d(){_i_("96bbb:283");var u,t=this.markers,s=t.length,r=google.maps.event,q=this.map.getBounds();
while(s--){u=t[s];
if(q&&!q.contains(u.getPosition())&&!u.data.b_persistent){r.clearInstanceListeners(u);
u.setMap(null);
t.splice(s,1)
}};_r_()}function h(){_i_("96bbb:284");var r,q=this.markers.length;
while(q--){r=this.markers[q];
google.maps.event.clearInstanceListeners(r);
r.setMap(null)
}this.markers=[]
;_r_()}function i(s){_i_("96bbb:285");var q=l.call(this,s),r=(q>-1)?this.markers[q]:null;
if(r){google.maps.event.clearInstanceListeners(r);
r.setMap(null);
this.markers.splice(q,1)
};_r_()}function m(){_i_("96bbb:286");return _r_( this.markers?this.markers.length>0:false);
}function f(s){_i_("96bbb:287");var r;
try{r=n.find(this.markers,function(t){_i_("96bbb:554");if(t){return _r_( t.b_id===String(s));
}})
}catch(q){B.reportError&&B.reportError(q,"Error in getMixedMarker")
}return _r_( r);
}function p(){_i_("96bbb:288");var q;
try{q=this.markers.map(function(t){_i_("96bbb:555");var s={};
Object.keys(t).forEach(function(u){_i_("96bbb:590");if(/^b_/.test(u)){s[u]=t[u]
};_r_()});
return _r_( s);
})
}catch(r){B.reportError&&B.reportError(r,"Error in getActiveMixedMarkers")
}return _r_( q||[]);
}function b(u,s,r){_i_("96bbb:289");var t=this.getMixedMarker(u),q;
...
}};_r_()}function o(u,t){_i_("96bbb:290");var r=this,s=this.getMixedMarker(u),q=r.map.getProjection();
if(s){if(q){r.map.setCenter(this.getOffsetLatLng(s.getPosition(),t))
}else{r.ubber.once("idle",function(){_i_("96bbb:569");r.map.setCenter(this.getOffsetLatLng(s.getPosition(),t))
;_r_()})
}};_r_()}function k(u,t){_i_("96bbb:291");var r=this,q=r.map.getProjection(),s=this.getMixedMarker(u);
if(s){if(q){r.map.panTo(r.getOffsetLatLng(s.getPosition(),t))
}else{r.ubber.once("idle",function(){_i_("96bbb:570");r.map.panTo(r.getOffsetLatLng(s.getPosition(),t))
;_r_()})
}};_r_()}function j(s,q){_i_("96bbb:292");var r=this.getMixedMarker(s);
if(r){r.setOptions(q)
};_r_()}function a(s){_i_("96bbb:293");var u,w=B.env,r=google.maps,v=r.event,q=this.ubber,y,t,x;
...
v.addListener(u,"click",function(z){_i_("96bbb:492");if(z&&typeof z.preventDefault==="function"){z.preventDefault()
}q.trigger("marker-click",{evt:z,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
;_r_()});
v.addListener(u,"mouseover",function(z){_i_("96bbb:493");q.trigger("marker-hover",{evt:z,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
;_r_()});
v.addListener(u,"mouseout",function(z){_i_("96bbb:494");q.trigger("marker-out",{evt:z,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
;_r_()});
this.markers.push(u)
}function e(u){_i_("96bbb:294");var r=new google.maps.OverlayView(),s=this.getMixedMarker(u),q,t;
r.draw=function(){};
r.setMap(this.map);
q=r.getProjection();
...
function m(w){_i_("96bbb:295");return _r_((w instanceof Array)?w:false);
}function c(x,y){_i_("96bbb:296");this.markers=[];
...
k=new j(a,c);
function o(x,w,y){_i_("96bbb:297");if(!k.isLoading){k.append("GLOBAL_ATLAS_GOOGLE_MAPS_CALLBACK",x)
}k.queue(x,w,y,c)
...
a=new h(c,b);
function f(j,i,k){_i_("96bbb:299");if(!a.isLoading){a.get(j)
}a.queue(j,i,k,b)
;_r_()}return _r_({init:f});
});
B.atlas.define("provider-loader",["jQuery","util-env"],function(f,i){_i_("96bbb:59");var b=i.get("killSwitch"),h={};
function a(k,l){_i_("96bbb:300");if(h.hasOwnProperty(k)){return _r_( h[k]);
}this.url=k;
...
this.Interface=l;
h[k]=this
}a.prototype={queue:function g(){_i_("96bbb:498");var k=[].slice.call(arguments,0);
if(this.isLoaded){this.getInterface.apply(this,k)
}else{this.stack.push(k)
};_r_()},get:function d(k){_i_("96bbb:499");var l=this,m=this.url;
if(b){return _r_();
...
this.isLoading=true
},append:function c(n,k){_i_("96bbb:500");var l=document.createElement("script"),m=this,p=this.url;
if(b){return _r_();
...
this.isLoading=true
},getInterface:function j(m,k,n,l){_i_("96bbb:501");l=l||this.Interface;
n.call(m,new l(k,m))
;_r_()}};
function e(k,m,n){_i_("96bbb:301");var l=new RegExp("\\b("+m+"=).*?(&|$)");
...
function b(i,k){_i_("96bbb:302");var j=this;
j.ubber=k;
B.atlas.require(["provider-google-places"],function(){_i_("96bbb:502");var l=[].slice.call(arguments,0);
while(l.length>0){g.extend(j,l.pop())
};_r_()})
;_r_()}$.extend(b.prototype,e);
a=new h(c,b);
function f(j,i,k){_i_("96bbb:303");if(!a.isLoading){a.append("GLOBAL_ATLAS_GOOGLE_MAPS_CALLBACK",j)
}a.queue(j,i,k,b)
;_r_()}return _r_({init:f});
});
