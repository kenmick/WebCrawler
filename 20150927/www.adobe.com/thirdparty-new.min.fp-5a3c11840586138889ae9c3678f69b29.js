...
if(ptDownloadFormActive!==null||ptThankyouPageActive!==null){(function(a){a.perpetualTrials={init:function(){perpetualTrials.initVars();
if(ptDownloadFormActive!==null){perpetualTrials.populateForm()
...
perpetualTrials.initialized=true
...
}else{if($(this).text().indexOf("Windows")!=-1&&$(this).text().indexOf("English")!=-1){return $(this).val()
}}}).get()[0];
if(!d){return""
}else{return d
}},setOrgSize:function(){if(trials.apikey.length>0&&trials.endPoint.length>0){var c="https://"+window.location.host+"/api/marketing_get_uds/?api_key="+trials.apikey+"&guid="+adobeIMS.getUserProfile().userId.split("@")[0]+"&rengatoken="+trials.getCookieValueByName("WCDServer")+"&appdomain=ACOM_ECOM"
}else{var c="https://"+window.location.host+"/api/marketing_get_uds/?api_key="+perpetualTrials.apiKey+"&guid="+adobeIMS.getUserProfile().userId.split("@")[0]+"&rengatoken="+trials.getCookieValueByName("WCDServer")+"&appdomain=ACOM_ECOM"
}var d=(trials.getCookieValueByName("testToken").length!=0)?trials.getCookieValueByName("testToken"):adobeIMS.getAccessToken();
var e=$.ajax({type:"GET",url:c,beforeSend:function(f){f.setRequestHeader("Content-Type","application/json");
f.setRequestHeader("Authorization",d)
}});
e.done(function(f,h,g){if(f[0].status==200){perpetualTrials.orgSize=f[0].number_of_employees;
...
if(trials.apikey.length>0&&trials.endPoint.length>0){var c=trials.endPoint+"/?api_key="+trials.apikey
...
},postSubmitSuccess:function(d,e,c){},postSubmitFailure:function(c,e,d){},postSubmitAlways:function(c,e,d){perpetualTrials.redirectToNextPage("thankyou")
...
},initDownload:function(){var g=(trials.getCookieValueByName("testToken").length!=0)?trials.getCookieValueByName("testToken"):adobeIMS.getAccessToken();
var h=$("#productsku").attr("data-basepath").split("/");
var j=h[h.length-2];
var e=h[h.length-1];
var f=$("#productsku").val();
var d=$("#productsku option:selected").attr("data-dlm");
var i=($("#productsku").attr("data-dlmpath").length>0)?$("#productsku").attr("data-dlmpath"):"/tdrc/dlm/dlm.html";
var k=encodeURIComponent($("#ptDownloadForm").attr("data-plabel"));
var c="http://"+window.location.hostname+i+"?product="+j+"&version="+e+"&sku="+f+"&prdLabel="+k+"&prompt=prompt&language="+adobeIMS.getLocale()+"&dlmtype="+d;
document.cookie="popURL="+c+"&retry=true; path=/; domain=.adobe.com";
c=c+"&token="+g;
perpetualTrials.openPopup(c)
},initRedownload:function(){if(perpetualTrials.getCookieValueByName("popURL").length>0){var d=(perpetualTrials.getCookieValueByName("testToken").length!=0)?perpetualTrials.getCookieValueByName("testToken"):adobeIMS.getAccessToken();
var c=perpetualTrials.getCookieValueByName("popURL")+"&token="+d;
perpetualTrials.openPopup(c)
