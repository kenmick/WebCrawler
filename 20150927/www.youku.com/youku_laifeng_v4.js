(function(){
    ...
    function loadLaifeng() {
        ...
        if(ele.scrollTop+800>_top) {
            _isRequest = true;
            xmloadingjs('http://www.laifeng.com/cms/youku/home_channel/v2?_t='+new Date().getTime(),function(){
                readData();
            });
        }
    }
    ...
    function readData() {
        ...
        html+='<div class="yk-head">'+
                '<div class="yk-title">'+
                        '<a target="_blank" href="http://cps.laifeng.com/redirect.html?id=00014089&url=http%3A%2F%2Fwww.laifeng.com%2F" class="yk-drawer-title"><img src="http://static.youku.com/ddshow/img/youku/laifeng.png">æ¥ç¯äºå¨ç´æ­</a>'+
                '</div>'+
            '</div>';
    ...
    function loadConnect(DATA) {
        ...
        for(var i=0;i<Math.min(DATA.length,4);i++) {
                ...
                _html+='</div>'+
                        '<div class="v-link">'+
                        '<a title="'+data.nickName+'" target="_blank" href="'+data.roomUrl+'"></a>'+
                        '</div>'+
                        '<div class="v-meta va">'+
                        '<div class="v-meta-title">'+
                        '<a target="_blank" href="'+data.roomUrl+'">'+data.nickName+'</a>'+
                        '</div>'+
                ...
                if(data.status==1 && !data.livehouse){
                    ...
                    _html+='<a target="_blank" href="'+data.roomUrl+'">æ¥çè¯¦æ</a>';
                }
                _html+='</span></div>'+
                       '</div></div></div>';
            }
    ...
    function loadRank(DATA,n) {
        ...
        for(var i=n;i<Math.min(DATA.length,7);i++) {
            ...
            html+='</div>';
            html+='<div class="v-link"><a href="'+data.roomUrl+'" target="_blank" title="'+data.nickName+'"></a></div>';
            html+='<div class="v-meta">'+
                    '<div class="v-meta-title">'+
                        '<a href="'+data.roomUrl+'" target="_blank">'+data.nickName+'</a>'+
                    '</div>'+
                    '<div class="v-meta-entry"><span>';
            if(data.status==1 && !data.livehouse){
                html+=data.userCount+'äººæ­£å¨è§ç';
            }
