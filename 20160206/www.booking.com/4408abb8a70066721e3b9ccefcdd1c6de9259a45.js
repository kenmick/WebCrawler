...
t.migrateWarnings=[];
t.migrateMute=p.enableRemoveMigrateWarns?false:true;
...
if(!F[J]){F[J]=p.enableRemoveMigrateWarns?false:true;
t.migrateWarnings.push(J);
if(I&&I.warn&&!t.migrateMute){if(J.indexOf("jQuery.fn.live")==-1&&J.indexOf("jQuery.fn.die")==-1&&p.enableRemoveMigrateWarns){return
}I.warn("JQMIGRATE: "+J);
...
t.fn.live=function(I,K,J){h('jQuery.fn.live() is deprecated. Use "$(document).on( types, selector, data, fn );" instead.');
if(d){return d.apply(this,arguments)
}t(this.context).on(I,this.selector,K,J);
return this
};
t.fn.die=function(I,J){h('jQuery.fn.die() is deprecated. Use "$(document).off( types, selector || "**", fn );" instead.');
