...
}else{this.model.setDestination(this.results[f]);
...
B.define("search/destination/service-adaptor",function(c,b,e){var a=c("search/destination/service");
var d=B.env.search_autocomplete_params;
function f(h){h=$.extend({},h,d);
return h
}function g(h){return function(j,k){var i={results:[]};
if(!j&&k.city&&k.city.length){i.results=k.city.map(function(l){var n=l.label_highlighted||l.label;
var m=l.lc==="zh"||l.lc==="xt"||l.lc==="ja"||l.lc==="ko"?l.label_blocks:null;
return{id:l.dest_id,type:l.dest_type,text:l.label,text_blocks:m,text_raw:n,lang:l.lc,position:l.position,flags:l.flags,ef:l.ef,debug:l}
});
if(k.theme&&k.theme.length){i.results=i.results.concat(k.theme.map(function(l){return{id:l.dest_id,type:l.dest_type,text:l.label,text_extra:l.additional_label,ef:l.ef,position:l.position,debug:l}
}))
}if(k.__suggestions__&&k.__suggestions__.has_results){i.results.push({id:"suggestion",type:"suggestion",text:k.__suggestions__.initial_term,suggestedTerm:k.__suggestions__.updated_suggestion,suggestion:k.__suggestions__,ef:false});
i.suggestion=k.__suggestions__
}}else{if(!j&&k.__upa__){i.results=k.__upa__.map(function(l){return{placeId:l.place_id,type:l.dest_type,text:l.label,text_blocks:l.label_blocks,position:l.position,lang:B.env.b_lang,ef:l.ef,debug:l}
});
i.is_powered_by_google=true
}}if(!j){i.ef=k.ef;
i.efd=k.efd
}h(j,i)
}
}e.exports={search:function(h,j){if(!h||!h.ss||typeof j!="function"){throw new Error("Invalid arguments")
}var i=f(h);
return a.search(i,g(j))
}}
});
...
i.init()
},getDestinationPredictions:function(h,k){h=h||{};
var j={};
...
j.gpf=true;
if(this._onlyCurrentCountry){j.e_acf_i=B.env.b_country_id;
j.e_acf_t="country"
}if(h.suggestionOptOut){j.v_out=0;
this._extraFields.optOut=[{name:"outcg",value:2},{name:"suggested_term",value:h.suggestedTerm}]
}else{this._extraFields.optOut=null
}this._searchInstance=f.search(j,i.bind(this));
this._extraFields.destinationOriginal=[{name:"ss_raw",value:j.ss}];
this._extraFields.destinationSelected=null;
this._ssRaw=j.ss
