...
(function(b){if(!b.jstmpl){return
}var c,d,a;
c=function(j){var h=0,f,g,e;
if(!j||j.length===0){return h
}for(f=0,e=j.length;
f<e;
f++){g=j.charCodeAt(f);
h=((h<<5)-h)+g;
...
d=function d(i,f,l,h){var e="",j=l?"":document.location.protocol;
if(!b||!b.env||!b.env.static_hostnames||!b.env.static_hostnames.length){e=j+"//q.bstatic.com"
}else{if(b.env.static_hostnames.length===1||!i){e=b.env.static_hostnames[0]
}else{if(!/https?:/.test(i.toString())){var k=c(i.toString()),g=Math.abs(k)%b.env.static_hostnames.length;
e=b.env.static_hostnames[g]
}}}if(l){e=e.replace(/^https?:/,"")
}return e+i
};
a=function a(e){return Array.isArray(e)?e.length:0
};
b.jstmpl.setup({get_static_hostname:d,fn:{array_length:a}})
}(window.booking));
