...
z=z||{};
if(B.atlas.getVariant("ebcTGPbfRdcC")||B.atlas.getVariant("ebcYSaBHCMYQGCIBTAcHe")){z=x.enforceDataFormat(z,function(E){if(/^hotel/.test(E.b_type)&&B.atlas.getVariant("ebcTGPbfRdcC")){E.b_states.overlay=true
}})
...
}}if(x&&x.data&&((/^hotel.*/.test(x.data.b_type)&&(B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("eGOQPBRNZRUXe")))||(/^city.*/.test(x.data.b_type)&&B.atlas.getVariant("ebcYSaBaUSBCHC")==2))){if(d||m){C.set("hoverMarker","");
...
if((B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("eGOQPBRNZRUXe"))&&/hotel.*/.test(x.data.b_type)){window.setTimeout(function(){C.trigger("autoopen-fixed-iw",G);
C.trigger("iw-control-show",{markerId:z,data:x.data})
},0)
}else{C.openIW(z,G)
...
}function o(F){i(document).on("click",s,function(){var G=i(this);
var H=G.data("source");
if(H==="top_link"){F.trigger("map-open.link")
}else{if(/^map_thumbnail$/.test(H)){F.trigger("map-open.thumbnail")
}}}).on("click",x,function(){F.trigger("map-close.button")
}).keyup(function(){F.trigger("map-close.esc")
});
...
var j=new l();
var e=[{event:"map-open.tab",source:"tab",action:"show_map"},{event:"map-open.thumbnail",source:"thumbnail",action:"show_map"},{event:"map-open.param",source:"param",action:"show_map"},{event:"map-open.hash",source:"hash",action:"show_map"},{event:"map-open.link",source:"address_link",action:"show_map"},{event:"map-close.button",source:"close_button",action:"close_map"},{event:"map-close.outside",source:"lightbox_outside",action:"close_map"},{event:"map-close.esc",source:"esc",action:"close_map"},{event:"map-close.tab",source:"tab",action:"close_map"}];
...
return m
}},{event:"doubleclick",source:"doubleclick",action:"zoomin"},{event:"map-type-change",source:"maptype_control",action:"maptype_change",value:function(m){return m
...
if(b.atlas.getVariant("ebcTGPbfRdcC")){s.push("marker-data-format");
