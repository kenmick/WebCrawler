/* 30,96,81 2015-08-27 11:44:21 */
$(function() {
    $.extend({
        brand: function(main, sub, btn, json) {
            var tempHTML = [ "<li>Ñ¡Æ·ÅÆ</li>" ], temp_id = [], temp_name = [];
            for (var obj in json) {
            ...
            $(main).on("click", "li:not(.sgroup)", function(e) {
                ...
                $(sub).empty().removeAttr("disabled").append("<li>Ñ¡³µÐÍ</li>");
                ...
                $.getJSON("http://data.auto.sina.com.cn/car_compare_new/ajax/getSubs2search.php?bid=" + pid + "&callbk=?", function(json) {
                    ...
                    for (var obj in json) {
                        ...
                        if (temp_id.length == 0) {
                            tempHTML.push('<li value="">ÔÝÎÞÏà¹ØÔÚ²ú³µÐÍ</li>');
                        } else {
            ...
            if (btn && btn != "") $(btn).click(function() {
                var subid = $(sub).attr("value"), bid = $(main).attr("value"), url;
                if (bid != "Ñ¡Æ·ÅÆ" && bid != "" && bid !== "0" && !!bid) {
                    if (subid != "Ñ¡³µÐÍ" && subid != "" && subid !== "0" && !!subid) {
                        url = "http://data.auto.sina.com.cn/" + subid + "/";
                    } else {
                        url = "http://data.auto.sina.com.cn/brands/" + bid + "/";
                    }
                } else {
