...
if(e&&e.b_states&&e.b_states.overlay){e.b_behaviors.hover=true;
this.Interface.setMixedMarkerType(f,{zIndex:this.getMarkerZIndex(e)})
...
if(e&&e.b_states&&e.b_states.overlay){e.b_behaviors.hover=false;
this.Interface.setMixedMarkerType(g,{zIndex:this.getMarkerZIndex(e)})
...
if(e&&e.b_states&&e.b_states.overlay){e.b_behaviors.visited=true;
this.Interface.setMixedMarkerType(f,{content:this.getMarkerContent(e),zIndex:this.getMarkerZIndex(e)})
}else{e.b_behaviors.visited=true;
this.Interface.setMixedMarkerType(f,this.getVisited(e?e.iconType||e.b_type:""))
}}return{setIconHover:a,setIconOut:c,setIconActive:b,setIconVisited:d}
});
B.atlas.define("markers",["jQuery"],function(g){function l(s){var r,q=[];
...
}B.atlas.define("mixed-markers",["util-object","util-array"],function(h,f){function g(u){var t,s=[];
if(f.isArray(u)){return u
}for(t in u){if(u.hasOwnProperty(t)){s.push(u[t])
}}return s
}function i(u,t){var s=g(u);
this.pruneMarkers(s);
this.addMarkers(s,t)
}function q(){return this.Interface.getActiveMixedMarkers()
}function m(){this.Interface.clearMixedMarkers()
}function o(s){this.Interface.clearMixedMarker(s)
}function k(t){var s=g(t);
this.Interface.pruneOutboundedMixedMarkers();
this.addMarkers(s)
}function n(w){var v,u,t=this.Interface.getActiveMixedMarkers(),s=t.length;
while(s--){v=t[s].b_id;
u=w.length;
while(u--){if(w[u].b_id===v){break
}}if(u<0&&v){this.Interface.clearMixedMarker(v)
}}}function a(A,w){var y,v,u=[],x=0,t=g(A),s=t.length;
this.trigger("markers-create",this);
while(s--){y=t[s];
...
}function b(u){var t,s=this.Interface.getMixedMarker(u);
...
}}}function c(s){this.Interface.centerMixedMarker(s,this.get("offset"))
}function j(s){this.Interface.panMixedMarker(s,this.get("offset"))
}function p(u,t){var s=this.Interface.getMixedMarker(u);
if(t&&s){s.b_type=t;
...
}}}function e(s){return this.Interface.getMixedMarker(s)
}function l(t,s){return this.Interface.setMixedMarkerOptions(t,s)
}function d(s){if(typeof this.Interface.animateMarker==="function"){this.Interface.animateMarker(s)
}}function r(){return this.Interface.mixedMarkersExist()
...
B.atlas.define("provider-google-mixed-markers",["util-array","util-object","provider-google-overlay-marker"],function(n,d,g){function l(q){return n.findIndex(this.markers,function(r){return r.b_id===String(q)
})
}function e(){var u,t=this.markers,s=t.length,r=google.maps.event,q=this.map.getBounds();
while(s--){u=t[s];
if(q&&!q.contains(u.getPosition())&&!u.data.b_persistent){r.clearInstanceListeners(u);
u.setMap(null);
t.splice(s,1)
}}}function h(){var r,q=this.markers.length;
while(q--){r=this.markers[q];
google.maps.event.clearInstanceListeners(r);
r.setMap(null)
}this.markers=[]
}function i(s){var q=l.call(this,s),r=(q>-1)?this.markers[q]:null;
if(r){google.maps.event.clearInstanceListeners(r);
r.setMap(null);
this.markers.splice(q,1)
}}function m(){return this.markers?this.markers.length>0:false
}function f(q){return n.find(this.markers,function(r){return r.b_id===String(q)
})
}function p(){return this.markers.map(function(r){var q={};
Object.keys(r).forEach(function(s){if(/^b_/.test(s)){q[s]=r[s]
}});
return q
})
}function c(u,s,r){var t=this.getMixedMarker(u),q;
...
}}}function o(u,t){var r=this,s=this.getMixedMarker(u),q=r.map.getProjection();
if(s){if(q){r.map.setCenter(this.getOffsetLatLng(s.getPosition(),t))
}else{r.ubber.once("idle",function(){r.map.setCenter(this.getOffsetLatLng(s.getPosition(),t))
})
}}}function k(u,t){var r=this,q=r.map.getProjection(),s=this.getMixedMarker(u);
if(s){if(q){r.map.panTo(r.getOffsetLatLng(s.getPosition(),t))
}else{r.ubber.once("idle",function(){r.map.panTo(r.getOffsetLatLng(s.getPosition(),t))
})
}}}function j(s,q){var r=this.getMixedMarker(s);
if(r){r.setOptions(q)
}}function b(s){var u,w=B.env,r=google.maps,v=r.event,q=this.ubber,y,t,x;
...
v.addListener(u,"click",function(z){if(z&&typeof z.preventDefault==="function"){z.preventDefault()
}q.trigger("marker-click",{evt:z,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
});
v.addListener(u,"mouseover",function(z){q.trigger("marker-hover",{evt:z,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
});
v.addListener(u,"mouseout",function(z){q.trigger("marker-out",{evt:z,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
});
this.markers.push(u)
}function a(r){var q=this.getMixedMarker(r);
