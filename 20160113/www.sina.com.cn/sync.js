...
¸ñÊ½: new RotatorAD(ÉÌÒµ¹ã¸æÊý×é, ·ÇÉÌÒµ¹ã¸æÊý×é, ²ãid)
ËµÃ÷: µÚÒ»´Î·ÃÎÊËæ»ú³öÏÖ£¬ÒÔºó·ÃÎÊË³ÐòÂÖ²¥£»×Ô¶¯¹ýÂË¹ýÆÚ¹ã¸æ£»cookieÊ±¼ä24Ð¡Ê±£»ÉÌÒµ¹ã¸æÊýÁ¿²»×ãÊ±£¬´Ó·ÇÉÌÒµ¹ã¸æÖÐ²¹³ä
      ÏÞÖÆ×îÉÙÂÖ²¥ÊýÁ¿£¬¹ã¸æÉÙÓÚÏÞÖÆÊýÊ±£¬²Å´Óµæµ×Àï²¹³ä£¬·ñÔò²»²¹µæµ×
*/
if (typeof (RotatorAD) != 'function') {
    var RotatorAD = function (rad, nad, div_id) {
        ...
        for (var i = 0; i < rad.length; i++) {
            ...
            //Ôö¼ÓÒÆ¶¯Éè±¸¹ýÂË coding by lingchen
		...
		//Ê¹ÓÃlocalStorageºÍuserData´æ´¢ÂÖ²¥Êý
		var localData = {
		...
		//document.title = 'Ëæ»úÊý£º'+globalRotatorId+' | ÂÖÊý£º'+id+' ';
        //ÈôÉÌ¹ãÎª¿Õ
        //nullRad = true;
        ...
        function showAD() {
            if (ary.length == 0) return; //Èç¹ûÃ»ÓÐ¹ã¸æÔò²»ÏÔÊ¾
            var n = id;
            try {
...
ÂÖ²¥±³Í¶Àà RotatorPB v3.1
Update by Dakular <shuhu@staff.sina.com.cn> 2008-8-25
¸ñÊ½£ºnew RotatorPB(¹ã¸æÊý×é)
ËµÃ÷£ºµÚÒ»´Î·ÃÎÊËæ»ú³öÏÖ£¬ÒÔºó·ÃÎÊË³ÐòÂÖ²¥£»×Ô¶¯¹ýÂË¹ýÆÚ¹ã¸æ£»cookieÊ±¼ä24Ð¡Ê±£»ÉÌÒµ¹ã¸æÊýÁ¿²»×ãÊ±²»ÏÔÊ¾
*/
if(typeof(RotatorPB)!='function'){
	var RotatorPB=function (rad){
		...
		//Show AD
		if(this.e==0 || this.ary.length==0) return; //Èç¹ûÃ»ÓÐ¹ã¸æÔò²»ÏÔÊ¾
		if(this.e==-1) return; //µ±Ö»ÓÐÒ»¸ö¹ã¸æÊ±£ºÊ¼ÖÕÏÔÊ¾µÚÒ»¸ö/ÆæÊý´ÎË¢ÐÂÏÔÊ¾
		var n = this.e-1;
...
if (local_index == '') {
    // location info
    var dictLoc = {"°²»Õ":0,"±±¾©":1,"ÖØÇì":2,"¸£½¨":3,"¸ÊËà":4,"¹ã¶«":5,"¹ãÎ÷":6,"¹óÖÝ" :7,"º£ÄÏ":8,"ºÓ±±":9,"ºÚÁú½­":10,"ºÓÄÏ":11,"ºþ±±":12,"ºþÄÏ":13,"ÄÚÃÉ¹Å":14,"½­ËÕ":15,"½­Î÷":16,"¼ªÁÖ":17,"ÁÉÄþ":18,"ÄþÏÄ":19,"Çàº£":20,"É½Î÷":21,"ÉÂÎ÷":22,"É½¶«":23,"ÉÏº£":24,"ËÄ´¨":25,"Ìì½ò":26,"Î÷²Ø":27,"ÐÂ½®":28,"ÔÆÄÏ":29,"Õã½­":30,"Ïã¸Û":31,"°ÄÃÅ":32,"Ì¨Íå":33};
	...
	if(typeof local_index == "undefined") {
		...
		location1 = "ÆäËü";
		location2 = "ÆäËü";
    	}else if(remote_ip_info.province == "±±¾©" || remote_ip_info.province == "Ìì½ò" || remote_ip_info.province == "ÉÏº£" || remote_ip_info.province == "ÖØÇì") {
    		location1 = remote_ip_info.province + 'ÊÐ';
    		location2 = remote_ip_info.province;
    	}else if(remote_ip_info.province != "Ïã¸Û" && remote_ip_info.province != "°ÄÃÅ" && remote_ip_info.province != "¹ãÎ÷" && remote_ip_info.province != "ÄþÏÄ" && remote_ip_info.province != "ÄÚÃÉ¹Å" && remote_ip_info.province != "Î÷²Ø" && remote_ip_info.province != "ÐÂ½®" && remote_ip_info.province != ""){
    		location1 = remote_ip_info.province + 'Ê¡';
...
function CoupletMediaOpenWin(){
  ...
  var d = CoupletMediaData;//¼ÓÔØÊý¾Ý
  var defaultW = 1000;
  ...
  this.ishide = false;
   ...
   this.initAdCM = function(ul,ur,ue,l,tk,tp){
	...
	try {
	 ...
	 o.addEvent(cliWrap,"mouseover",pthis.showCM);//×¢²á×ó´¥·¢ÊÂ¼þ
	 o.addEvent(criWrap,"mouseover",pthis.showCM);//×¢²áÓÒ´¥·¢ÊÂ¼þ
	 o.addEvent(clcBtn,"click",pthis.closeCM);//×¢²á×ó¹Ø±ÕÊÂ¼þ
	 o.addEvent(crcBtn,"click",pthis.closeCM);//×¢²áÓÒ¹Ø±ÕÊÂ¼þ
	 o.addEvent(cecBtn,"click",pthis.clcHideCM);////×¢²á´¥·¢¹Ø±ÕÊÂ¼þ

	 pthis.hideCM();//¼ÓÔØ¹ã¸æ
	 try{nextAD();}catch(e){}//¼ÓÔØºóÐø¹ã¸æ
