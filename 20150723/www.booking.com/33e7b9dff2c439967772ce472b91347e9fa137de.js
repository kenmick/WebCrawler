...
if(!B.atlas){return
}B.atlas.define("atlas-common-markers",a,function(t,z,u){var k,w=t.get("destinationId"),x=false,f=100,l=null,i=false,m=t.get("isMiniIWLandingPages"),e=t.get("isMiniIW");
...
A.getMarkers(function(D){var F=t.get("action"),G,C,E;
D=D||{};
C=[].concat(D.b_hotels||[],D.b_cities||[],D.b_airports||[]);
if(B&&B.env&&B.env.lp_map_background_running){C=[]
}E=C.length;
if(F==="searchresults"||F==="hotel"||B.atlas.getVariant("ebcMMECAFWTYcUNC")||t.get("isLP")&&F!=="city"){B.atlas.require(["map-markers-disperse"],function(H){H.initialize(C,A,w)
})
...
if(B.atlas.getVariant("ebcBUECAFJZVcNTBWe")||B.atlas.getVariant("ebcOLHMbdAFfQcUNC")){var F=E.get("isMarkerOpen");
if(F){E.closeIW();
E.set("isMarkerOpen",false);
return
}else{E.set("isMarkerOpen",true)
...
}if(e||m){E.on("iw-close",function(){if(l){E.setIconOut(l);
l=null
}});
if(l){E.setIconOut(l)
}l=A.b_id;
...
if(/city|airport/.test(A.b_type)){g(E,A);
E.openIW(A.b_id,{},B.jstmpl("atlas_iw_landmark").render(A.data));
E.trigger("iw-opened-click",{marker:A,data:A.data})
}else{E.getIW(A.b_id,function(G){G.b_map_nights=t.get("checkinCheckoutInterval");
G.map_price_per_x_nights_txt=t.get("transPricePerXNights");
G.b_url=A.data.b_url;
G.b_marker_type=A.data.b_marker_type;
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){G.transLengthOfStayExperiment=t.get("transLengthOfStayExperiment")
}if(/hotel.*/.test(A.data.b_type)){E.trigger("iw-hotel-selectable");
if(B.atlas.getVariant("ebcBUYSaBKHLLT",1)||B.atlas.getVariant("ebcOQPBRYCWfae",1)){g(E,A,{extraClasses:["map-hotel--selectable"]})
}else{g(E,A)
}}else{g(E,A)
}E.openIW(A.b_id,G);
E.trigger("iw-opened-click",{marker:A,data:G});
...
}}}function h(G,D){var F=this,A=F.getMarker(D),C,E=3000;
if(!D){return
}if(!A&&!x){x=true;
return F.once("markers-show",h,D)
}if(!t.get("tdot")&&t.get("action")=="searchresults"){if(!A){return
}}else{if(!A||(A&&A.b_type&&/city|airport/.test(A.b_type))){return
}}if(t.get("action")=="hotel"){C=function(){h.call(F,G,D,true);
...
}if(!t.get("tdot")&&t.get("action")=="searchresults"&&/city|airport/.test(A.b_type)){g(F,A);
...
}function o(E,D){var C,A;
C=E.filter(function(F){return F.b_marker_type==="city"
});
if(C&&C.length===1){A=C[0];
D.trigger("markers-emphasize-ufi",A);
if(B.atlas.getVariant("ebcBUeEFcJHeBJORe")){D.setMarkerType(A.b_id,A.b_marker_type+"_current")
