...
window.onload=function() {
    shownumber = Math.round(parseInt($(".midpic").width())/parseInt($(".midpic li").width()));
    var picnumber=$(".midpic ul li").length;//è¦æ¾ç¤ºçååæ°é
	var nowshowpic = 0,yidonglen = $(".midpic").width();
	...
	$(".rightbut").click(function(e){
		...
		if(!$(".midpic ul").is(":animated")){
				...
				if(nowshowpic < picamount){					
                    ...
                    slideOccur();
					$(".midpic ul").animate({"left":-picamount*yidonglen },300,function(){$(".midpic ul").css("left","0")});					nowshowpic=0					
				}
			}
		return false;
	})
	...
	$(".leftbut").click(function(e){
		...
		if(!$(".midpic ul").is(":animated")){
				...
				if(nowshowpic == -1){
                    slideOccur();
					$(".midpic ul").animate(
						{"left":-picamount*yidonglen },0,
						function(){
							$(".midpic ul").animate({"left":-(picamount-1)*yidonglen },300)})
	...
	function huantu(num){
        slideOccur();
		$(".midpic ul").animate({"left":-num*yidonglen },200)			
	}
    ...
    function slideOccur(isStarted,isLeft){
        ...
        if(shownumber>2){
            pindex.push($(".midpic ul li").eq(index*shownumber+2).attr("data-index"));
            pid.push($(".midpic ul li").eq(index*shownumber+2).attr("data-pid"));
        }
