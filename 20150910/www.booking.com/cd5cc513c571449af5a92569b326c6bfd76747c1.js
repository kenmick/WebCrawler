...
jQuery.extend(B.atlas,(function(e,h){var c=null;
function g(){var i=h.atlas.SUBSCRIPTIONS;
c={};
if(!i||!h.jst){return
}e.each(i,function(j,k){e.each(k,function(l,o){try{if(o.name&&o.cond&&h.jst.hasOwnProperty(o.name)){c[o.name]=parseInt(h.jst[o.name],10);
if(o.stage){h.track.exp(o.name)
}}}catch(m){var n="Atlas event "+j+": has invalid subscriptions. Maybe trailing comma?";
throw new Error(n)
}})
})
}function d(i){if(!c){g()
}return c.hasOwnProperty(i)?c[i]:false
}function b(j,k){var l=[],i=[],m=h.atlas.SUBSCRIPTIONS;
if(!m||!j||!m.hasOwnProperty(j)){return false
}m[j].forEach(function(n){if(d(n.name)!==false&&!n.touched&&k===n.instanceId){if(n.stage){i.push([n.name,n.stage])
}else{l.push(n.name)
}n.touched=true
}});
if(l.length){h.track.exp(l.join(","))
}i.forEach(function(n){h.track.stage.apply(null,n)
})
}function f(k,i){var l=h.atlas.SUBSCRIPTIONS,j=l&&l.templates?l.templates:[];
j.some(function(m){if(m.name===k&&d(m.name)!==false){h.track.stage(k,i||m.stage);
return true
}return false
});
return true
}function a(j){var k=h.atlas.SUBSCRIPTIONS,i=k&&k.templates?k.templates:[];
i.some(function(l){if(l.name===j&&d(l.name)!==false){if(!l.touched){h.track.exp(j);
l.touched=true
}return true
}return false
});
return d(j)
}h.run("Xfgrwdtafew").onLoad(function(){if(!c){g()
}});
return{getVariant:d,track:b,stageInTemplate:f,trackInTemplate:a}
}(jQuery,B)));
...
n=b.ajax({url:l+";BBOX="+m,data:this.get("markersOnMapURLData")||{},error:function(q,p){if(p!=="abort"){k.trigger("markers-get-fail",k)
...
B.atlas.define("transit-layer",function(){function a(){this.Interface.addTransitLayer()
}function b(){this.Interface.removeTransitLayer()
...
}function f(i,j){var g=this,h={map:j,data:{id:i.place_id,title:b(i.terms),address:a(i.terms)}};
e.extend(h,i);
...
function f(j,k){var i=k.map.getBounds(),h={input:j.query};
if(i){h.bounds=i
}else{h.location=new google.maps.LatLng(j.lat,j.lon);
...
a.getQueryPredictions(k,function(n,m){if(m&&m.indexOf("OK")>-1&&n){l(c(n,h))
...
c.addListener(this.map,"maptypeid_changed",function(){b.trigger("map-type-change")
})
}return{setEvents:a}
});
...
f.extend(c.prototype,q,h,t,d,e,g,s,m,k,r,v,o,u);
j=new i(a,c);
function n(x,w,y){if(!j.isLoading){j.append("GLOBAL_ATLAS_GOOGLE_MAPS_CALLBACK",x)
}j.queue(x,w,y,c)
}return{init:n}
});
