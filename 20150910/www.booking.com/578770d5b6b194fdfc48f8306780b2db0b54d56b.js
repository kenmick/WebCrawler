...
}e.autocomplete({source:a,extraParams:booking.env.search_autocomplete_params,minLength:c,delay:booking.env.search_autocomplete_delay||b}).bind("autocompleteselect",function(j,k){if(k.item){d=true;
g.cur_ui_item=k.item;
...
if(booking.track.getVariant("PVdIcdRAOAcEUC")=="1"){var h=k.item.dest_id,m="/get_recommended_facilities?lang="+B.env.b_lang_for_url+"&ufi="+h,n=$("#destinationSearch"),l=$("#facilityList");
l.empty();
$.ajax({url:m,context:document.body}).done(function(o){$.each(o,function(q,p){$facilityInput=$("<label class='facility-tag'><input type='checkbox' title='"+p.name+"' name='hotelfacility["+p.id+"]' value='"+p.id+"' /> "+p.name+"</label>");
l.append($facilityInput);
