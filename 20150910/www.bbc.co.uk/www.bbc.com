...
<![endif]-->
  ...
  <script type="text/javascript">/*<![CDATA[*/ (function(undefined){if(!window.bbc){window.bbc={}}var ROLLING_PERIOD_DAYS=30;window.bbc.Mandolin=function(id,segments,opts){var now=new Date().getTime(),storedItem,DEFAULT_START=now,DEFAULT_RATE=1,COOKIE_NAME="ckpf_mandolin";opts=opts||{};this._id=id;this._segmentSet=segments;this._store=new window.window.bbc.Mandolin.Storage(COOKIE_NAME);this._opts=opts;this._rate=(opts.rate!==undefined)?+opts.rate:DEFAULT_RATE;this._startTs=(opts.start!==undefined)?new Date(opts.start).getTime():new Date(DEFAULT_START).getTime();this._endTs=(opts.end!==undefined)?new Date(opts.end).getTime():daysFromNow(ROLLING_PERIOD_DAYS);this._signupEndTs=(opts.signupEnd!==undefined)?new Date(opts.signupEnd).getTime():this._endTs;this._segment=null;if(typeof id!=="string"){throw new Error("Invalid Argument: id must be defined and be a string")}if(Object.prototype.toString.call(segments)!=="[object Array]"){throw new Error("Invalid Argument: Segments are required.")}if(opts.rate!==undefined&&(opts.rate<0||opts.rate>1)){throw new Error("Invalid Argument: Rate must be between 0 and 1.")}if(this._startTs>this._endTs){throw new Error("Invalid Argument: end date must occur after start date.")}if(!(this._startTs<this._signupEndTs&&this._signupEndTs<=this._endTs)){throw new Error("Invalid Argument: SignupEnd must be between start and end date")}removeExpired.call(this,now);var overrides=window.bbccookies.get().match(/ckns_mandolin_setSegments=([^;]+)/);if(overrides!==null){eval("overrides = "+decodeURIComponent(RegExp.$1)+";");if(overrides[this._id]&&this._segmentSet.indexOf(overrides[this._id])==-1){throw new Error("Invalid Override: overridden segment should exist in segments array")}}if(overrides!==null&&overrides[this._id]){this._segment=overrides[this._id]}else{if((storedItem=this._store.getItem(this._id))){this._segment=storedItem.segment}else{if(this._startTs<=now&&now<this._signupEndTs&&now<=this._endTs&&this._store.isEnabled()===true){this._segment=pick(segments,this._rate);if(opts.end===undefined){this._store.setItem(this._id,{segment:this._segment})}else{this._store.setItem(this._id,{segment:this._segment,end:this._endTs})}log.call(this,"mandolin_segment")}}}log.call(this,"mandolin_view")};window.bbc.Mandolin.prototype.getSegment=function(){return this._segment};function log(actionType,params){var that=this;require(["istats-1"],function(istats){istats.log(actionType,that._id+":"+that._segment,params?params:{})})}function removeExpired(expires){var items=this._store.getItems(),expiresInt=+expires;for(var key in items){if(items[key].end!==undefined&&+items[key].end<expiresInt){this._store.removeItem(key)}}}function getLastExpirationDate(data){var winner=0,rollingExpire=daysFromNow(ROLLING_PERIOD_DAYS);for(var key in data){if(data[key].end===undefined&&rollingExpire>winner){winner=rollingExpire}else{if(+data[key].end>winner){winner=+data[key].end}}}return(winner)?new Date(winner):new Date(rollingExpire)}window.bbc.Mandolin.prototype.log=function(params){log.call(this,"mandolin_log",params)};window.bbc.Mandolin.prototype.convert=function(params){log.call(this,"mandolin_convert",params);this.convert=function(){}};function daysFromNow(n){var endDate;endDate=new Date().getTime()+(n*60*60*24)*1000;return endDate}function pick(segments,rate){var picked,min=0,max=segments.length-1;if(typeof rate==="number"&&Math.random()>rate){return null}do{picked=Math.floor(Math.random()*(max-min+1))+min}while(picked>max);return segments[picked]}window.bbc.Mandolin.Storage=function(name){validateCookieName(name);this._cookieName=name;this._isEnabled=(bbccookies.isAllowed(this._cookieName)===true&&bbccookies.cookiesEnabled()===true)};window.bbc.Mandolin.Storage.prototype.setItem=function(key,value){var storeData=this.getItems();storeData[key]=value;this.save(storeData);return value};window.bbc.Mandolin.Storage.prototype.isEnabled=function(){return this._isEnabled};window.bbc.Mandolin.Storage.prototype.getItem=function(key){var storeData=this.getItems();return storeData[key]};window.bbc.Mandolin.Storage.prototype.removeItem=function(key){var storeData=this.getItems();delete storeData[key];this.save(storeData)};window.bbc.Mandolin.Storage.prototype.getItems=function(){return deserialise(this.readCookie(this._cookieName)||"")};window.bbc.Mandolin.Storage.prototype.save=function(data){window.bbccookies.set(this._cookieName+"="+encodeURIComponent(serialise(data))+"; expires="+getLastExpirationDate(data).toUTCString()+";")};window.bbc.Mandolin.Storage.prototype.readCookie=function(name){var nameEq=name+"=",ca=window.bbccookies.get().split("; "),i,c;validateCookieName(name);for(i=0;i<ca.length;i++){c=ca[i];if(c.indexOf(nameEq)===0){return decodeURIComponent(c.substring(nameEq.length,c.length))}}return null};function serialise(o){var str="";for(var p in o){if(o.hasOwnProperty(p)){str+='"'+p+'"'+":"+(typeof o[p]==="object"?(o[p]===null?"null":"{"+serialise(o[p])+"}"):'"'+o[p].toString()+'"')+","}}return str.replace(/,\}/g,"}").replace(/,$/g,"")}function deserialise(str){var o;str="{"+str+"}";if(!validateSerialisation(str)){throw"Invalid input provided for deserialisation."}eval("o = "+str);return o}var validateSerialisation=(function(){var OBJECT_TOKEN="<Object>",ESCAPED_CHAR='"\\n\\r\\u2028\\u2029\\u000A\\u000D\\u005C',ALLOWED_CHAR="([^"+ESCAPED_CHAR+"]|\\\\["+ESCAPED_CHAR+"])",KEY='"'+ALLOWED_CHAR+'+"',VALUE='(null|"'+ALLOWED_CHAR+'*"|'+OBJECT_TOKEN+")",KEY_VALUE=KEY+":"+VALUE,KEY_VALUE_SEQUENCE="("+KEY_VALUE+",)*"+KEY_VALUE,OBJECT_LITERAL="({}|{"+KEY_VALUE_SEQUENCE+"})",objectPattern=new RegExp(OBJECT_LITERAL,"g");return function(str){if(str.indexOf(OBJECT_TOKEN)!==-1){return false}while(str.match(objectPattern)){str=str.replace(objectPattern,OBJECT_TOKEN)}return str===OBJECT_TOKEN}})();function validateCookieName(name){if(name.match(/ ,;/)){throw"Illegal name provided, must be valid in browser cookie."}}})(); /*]]>*/</script>  <script type="text/javascript">  document.documentElement.className += (document.documentElement.className? ' ' : '') + 'orb-js';  fig.manager.confirm(); </script> <script src="http://static.bbci.co.uk/frameworks/barlesque/2.87.7/orb/4/script/orb/api.min.js"></script> <script type="text/javascript"> var blq = { environment: function() { return 'live'; } } </script>   <script type="text/javascript"> /*<![CDATA[*/ function oqsSurveyManager(w, flag) { if (flag !== 'OFF') { w.document.write('<script type="text/javascript" src="http://static.bbci.co.uk/frameworks/barlesque/2.87.7/orb/4/script/vendor/edr.js"><'+'/script>'); } } oqsSurveyManager(window, 'ON'); /*]]>*/ </script>             <!-- BBCDOTCOM template: responsive webservice  -->
        <!-- BBCDOTCOM head --><script type="text/javascript"> /*<![CDATA[*/ var _sf_startpt = (new Date()).getTime(); /*]]>*/ </script><style type="text/css">.bbccom_display_none{display:none;}</style><script type="text/javascript"> /*<![CDATA[*/ var bbcdotcomConfig, googletag = googletag || {}; googletag.cmd = googletag.cmd || []; var bbcdotcom = false; (function(){ if(typeof require !== 'undefined') { require({ paths:{ "bbcdotcom":"http://static.bbci.co.uk/bbcdotcom/0.3.333/script" } }); } })(); /*]]>*/ </script><script type="text/javascript"> /*<![CDATA[*/ var bbcdotcom = { adverts: { keyValues: { set: function() {} } }, advert: { write: function () {}, show: function () {}, isActive: function () { return false; }, layout: function() { return { reset: function() {} } } }, config: { init: function() {}, isActive: function() {}, setSections: function() {}, isAdsEnabled: function() {}, setAdsEnabled: function() {}, isAnalyticsEnabled: function() {}, setAnalyticsEnabled: function() {}, setAssetPrefix: function() {}, setJsPrefix: function() {}, setSwfPrefix: function() {}, setCssPrefix: function() {}, setConfig: function() {}, getAssetPrefix: function() {}, getJsPrefix: function () {}, getSwfPrefix: function () {}, getCssPrefix: function () {} }, survey: { init: function(){ return false; } }, data: {}, init: function() {}, objects: function(str) { return false; }, locale: { set: function() {}, get: function() {} }, setAdKeyValue: function() {}, utils: { addEvent: function() {}, addHtmlTagClass: function() {}, log: function () {} }, addLoadEvent: function() {} }; /*]]>*/ </script><script type="text/javascript"> /*<![CDATA[*/ (function(){ if (typeof orb !== 'undefined' && typeof orb.fig === 'function') { if (orb.fig('ad') && orb.fig('uk') == 0) { bbcdotcom.data = { ads: (orb.fig('ad') ? 1 : 0), stats: (orb.fig('uk') == 0 ? 1 : 0), statsProvider: orb.fig('ap') }; } } else { document.write('<script type="text/javascript" src="'+('https:' == document.location.protocol ? 'https://ssl.bbc.com' : 'http://tps.bbc.com')+'/wwscripts/data">\x3C/script>'); } })(); /*]]>*/ </script><script type="text/javascript"> /*<![CDATA[*/ (function(){ if (typeof orb === 'undefined' || typeof orb.fig !== 'function') { bbcdotcom.data = { ads: bbcdotcom.data.a, stats: bbcdotcom.data.b, statsProvider: bbcdotcom.data.c }; } if (bbcdotcom.data.ads == 1) { document.write('<script type="text/javascript" src="'+('https:' == document.location.protocol ? 'https://ssl.bbc.co.uk' : 'http://www.bbc.co.uk')+'/wwscripts/flag">\x3C/script>'); } })(); /*]]>*/ </script><script type="text/javascript"> /*<![CDATA[*/ (function(){ if (window.bbcdotcom && (typeof bbcdotcom.flag == 'undefined' || (typeof bbcdotcom.data.ads !== 'undefined' && bbcdotcom.flag.a != 1))) { bbcdotcom.data.ads = 0; } if (/[?|&]ads/.test(window.location.href) || /(^|; )ads=on; /.test(document.cookie) || /; ads=on(; |$)/.test(document.cookie)) { bbcdotcom.data.ads = 1; bbcdotcom.data.stats = 1; } if (window.bbcdotcom && (bbcdotcom.data.ads == 1 || bbcdotcom.data.stats == 1)) { bbcdotcom.assetPrefix = "http://static.bbci.co.uk/bbcdotcom/0.3.333/"; document.write('<link rel="stylesheet" type="text/css" href="http://static.bbci.co.uk/bbcdotcom/0.3.333/style/orb/bbccom.css" />'); (function() { var useSSL = 'https:' == document.location.protocol; var src = (useSSL ? 'https:' : 'http:') + '//www.googletagservices.com/tag/js/gpt.js'; document.write('<scr' + 'ipt src="' + src + '">\x3C/script>'); })(); if (/(sandbox|int)(.dev)*.bbc.co*/.test(window.location.href) || /[?|&]ads-debug/.test(window.location.href) || document.cookie.indexOf('ads-debug=') !== -1) { document.write('<script type="text/javascript" src="http://static.bbci.co.uk/bbcdotcom/0.3.333/script/orb/individual.js">\x3C/script>'); } else { document.write('<script type="text/javascript" src="http://static.bbci.co.uk/bbcdotcom/0.3.333/script/orb/bbcdotcom.js">\x3C/script>'); } if(/[\\?&]ads=([^&#]*)/.test(window.location.href)) { document.write('<script type="text/javascript" src="http://static.bbci.co.uk/bbcdotcom/0.3.333/script/orb/adverts/adSuites.js">\x3C/script>'); } } })(); /*]]>*/ </script><script type="text/javascript"> /*<![CDATA[*/ (function(){ if (window.bbcdotcom && (bbcdotcom.data.ads == 1 || bbcdotcom.data.stats == 1)) { bbcdotcomConfig = {"adFormat":"standard","adKeyword":"","adMode":"smart","adsEnabled":true,"appAnalyticsSections":"","asyncEnabled":false,"disableInitialLoad":false,"advertInfoPageUrl":"http:\/\/www.bbc.co.uk\/faqs\/online\/adverts_general","advertisementText":"Advertisement","analyticsEnabled":true,"appName":"wwhp","assetPrefix":"http:\/\/static.bbci.co.uk\/bbcdotcom\/0.3.333\/","continuousPlayEnabled":false,"customAdParams":[],"customStatsParams":[],"headline":"","id":"","inAssociationWithText":"in association with","keywords":"","language":"","orbTransitional":false,"outbrainEnabled":true,"palEnv":"live","productName":"","sections":[],"siteCatalystEnabled":true,"comScoreEnabled":true,"slots":"","sponsoredByText":"is sponsored by","adsByGoogleText":"Ads by Google","summary":"","type":"","staticBase":"\/bbcdotcom","staticHost":"http:\/\/static.bbci.co.uk","staticVersion":"0.3.333","staticPrefix":"http:\/\/static.bbci.co.uk\/bbcdotcom\/0.3.333","dataHttp":"tps.bbc.com","dataHttps":"ssl.bbc.com","flagHttp":"www.bbc.co.uk","flagHttps":"ssl.bbc.co.uk","analyticsHttp":"sa.bbc.com","analyticsHttps":"ssa.bbc.com"}; bbcdotcom.config.init(bbcdotcomConfig, bbcdotcom.data, window.location, window.document); bbcdotcom.config.setAssetPrefix("http://static.bbci.co.uk/bbcdotcom/0.3.333/"); document.write('<!--[if IE 7]><script type="text/javascript">bbcdotcom.config.setIE7(true);\x3C/script><![endif]-->'); document.write('<!--[if IE 8]><script type="text/javascript">bbcdotcom.config.setIE8(true);\x3C/script><![endif]-->'); document.write('<!--[if IE 9]><script type="text/javascript">bbcdotcom.config.setIE9(true);\x3C/script><![endif]-->'); if (/[?|&]ex-dp/.test(window.location.href) || document.cookie.indexOf('ex-dp=') !== -1) { bbcdotcom.utils.addHtmlTagClass('bbcdotcom-ex-dp'); } } })(); /*]]>*/ </script>            <script type="text/javascript">/*<![CDATA[*/
    ...
    <h2 class="module__title">&nbsp;</h2>
        ...
        <li class="media-list__item media-list__item--1">
            <div class="media media--hero media--primary media--overlay block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/13AD3/production/_85459508_3qkdsnmp.jpg" data-alt="Small girl cries as she is removed by police from Danish train in Rodbyhavn (9 September)"><img src="http://ichef.bbci.co.uk/wwhp/144/cpsprodpb/13AD3/production/_85459508_3qkdsnmp.jpg" alt="Small girl cries as she is removed by police from Danish train in Rodbyhavn (9 September)" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/world-europe-34203366"
                              rev="promo|headline" >
                                                            Danes halt rail links over migrants                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Denmark suspends rail links with Germany after hundreds of asylum seekers are stopped at the border amid an EU-wide migrant crisis.                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="/news/world-europe-34203366"
        ...
        <li class="media-list__item media-list__item--2">
            <div class="media media--overlay block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/73A6/production/_85460692_wayne_rooney_getty.jpg" data-alt="Wayne Rooney"><img src="http://ichef.bbci.co.uk/wwhp/144/cpsprodpb/73A6/production/_85460692_wayne_rooney_getty.jpg" alt="Wayne Rooney" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/sport/football/34204969"
                              rev="promo|headline" >
                                                            Rooney 'can reach 70 England goals'                                                    </a>
                    </h3>
        ...
        <li class="media-list__item media-list__item--3">
            <div class="media media--overlay block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/ibroadcast/images/live/p0/31/z8/p031z8rw.jpg" data-alt="Range Rover"><img src="http://ichef.bbci.co.uk/wwhp/144/ibroadcast/images/live/p0/31/z8/p031z8rw.jpg" alt="Range Rover" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/autos/story/20150908-secrets-of-a-bulletproof-range-rover"
                              rev="promo|headline" >
                                                            Secrets of the bullet-proof Range Rover                                                    </a>
                    </h3>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/autos/story/20150908-secrets-of-a-bulletproof-range-rover"
                      rev="promo|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    Secrets of the bullet-proof Range Rover                </a>
            </div>
        ...
        <li class="media-list__item media-list__item--4">
            <div class="media media--overlay block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/A4D3/production/_85459124_gettyimages-455799304-1.jpg" data-alt="Syrian kurds fleeing across the Syria-Turkey border"><img src="http://ichef.bbci.co.uk/wwhp/144/cpsprodpb/A4D3/production/_85459124_gettyimages-455799304-1.jpg" alt="Syrian kurds fleeing across the Syria-Turkey border" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/news/world-us-canada-34192829"
                              rev="promo|headline" >
                                                            Should the US do more on migrants?                                                    </a>
                    </h3>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/news/world-us-canada-34192829"
                      rev="promo|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    Should the US do more on migrants?                </a>
            </div>
        ...
        <li class="media-list__item media-list__item--5">
            <div class="media media--overlay block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/news/660/cpsprodpb/EF32/production/_85443216_jordan-masters-604242.jpg" data-alt="Passengers flee from the plane 09 September 2015"><img src="http://ichef.bbci.co.uk/news/660/cpsprodpb/EF32/production/_85443216_jordan-masters-604242.jpg" alt="Passengers flee from the plane 09 September 2015" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/news/world-us-canada-34195463"
                              rev="promo|headline" >
                                                            'Everyone was pushing and shoving and screaming'                                                    </a>
                    </h3>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/news/world-us-canada-34195463"
                      rev="promo|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    'Everyone was pushing and shoving and screaming'                </a>
            </div>
        ...
        <li class="media-list__item media-list__item--6">
            <div class="media media--overlay block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/20EA/production/_85462480_gettyimages-486225974.jpg" data-alt="Dilma Rousseff"><img src="http://ichef.bbci.co.uk/wwhp/144/cpsprodpb/20EA/production/_85462480_gettyimages-486225974.jpg" alt="Dilma Rousseff" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/business-34205558"
                              rev="promo|headline" >
                                                            Brazil cut to 'junk' status by S&amp;P                                                    </a>
                    </h3>
     ...
     <div class="module__content"> <ul class="media-list media-list--fixed-height">         
        <li class="media-list__item media-list__item--1">
            <div class="media  block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/4F5C/production/_85461302_gettyimages-468944017.jpg" data-alt="A Russian Navy member stands guard on the deck of the Pyotr Velikiy (Peter the Great) Russian nuclear-powered missile cruiser docked in the Cypriot port of Limassol, on 12 February 2014"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="A Russian Navy member stands guard on the deck of the Pyotr Velikiy (Peter the Great) Russian nuclear-powered missile cruiser docked in the Cypriot port of Limassol, on 12 February 2014" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/world-europe-34205003"
                              rev="news|headline" >
                                                            Nato warns on Russian role in Syria                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Nato chief Jens Stoltenberg echoes the concerns of US officials over reports that Russia is increasing its military presence in Syria.                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="/news/world-europe-34205003"
        ...
        <li class="media-list__item media-list__item--2">
            <div class="media  block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/184A7/production/_85459499_028946020-1.jpg" data-alt="Tim Cook and iPad Pro"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Tim Cook and iPad Pro" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/technology-34200234"
                              rev="news|headline" >
                                                            Apple unveils iPad Pro and Apple TV                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Apple unveils a larger iPad, an app-centric TV set-top box and new iPhones at a launch event in San Francisco.                                                    </p>
        ...
        <li class="media-list__item media-list__item--3">
            <div class="media  block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/15469/production/_85454178_coa1uunusaaia4k.jpg" data-alt="A passenger runs as the smoke billows from the plane 9 September 2015"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="A passenger runs as the smoke billows from the plane 9 September 2015" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/world-us-canada-34200669"
                              rev="news|headline" >
                                                            'There's a fire' - terror on BA jet                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Passengers describe the terrifying moment a fire breaks out on board their British Airways plane at Las Vegas airport, as an investigation gets under way.                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="/news/world-us-canada-34200669"
                      rev="news|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    'There's a fire' - terror on BA jet                </a>
            </div>
     ...
     <div class="module__content"> <ul class="media-list media-list--fixed-height">         
        <li class="media-list__item media-list__item--1">
            <div class="media  block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/EFF2/production/_85462416_halep_reuters.jpg" data-alt="Simona Halep"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Simona Halep" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/sport/tennis/34204658"
                              rev="sport|headline" >
                                                            Halep &amp; Pennetta make US Open semis                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Simona Halep and Flavia Pennetta will meet in the US Open semi-finals after winning their respective semi-finals in three sets.                                                    </p>
        ...
        <li class="media-list__item media-list__item--2">
            <div class="media  block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/13529/production/_85454197_becksbusiness.jpg" data-alt="David Beckham"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="David Beckham" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/sport/football/34201047"
                              rev="sport|headline" >
                                                            Beckham finds site for Miami MLS team                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            MLS chief Don Garber says David Beckham has found a site for his proposed Major League Soccer team in Miami.                                                    </p>
        ...
        <li class="media-list__item media-list__item--3">
            <div class="media  block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/15FFA/production/_85460109_nfl_composite_getty.jpg" data-alt="NFL"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="NFL" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/sport/american-football/34179245"
                              rev="sport|headline" >
                                                            Beckham &amp; the Cheeseheads - NFL back                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            All you need to know for the new NFL season as the New England Patriots start the defence of their Super Bowl title.                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="/sport/american-football/34179245"
                      rev="sport|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    Beckham &amp; the Cheeseheads - NFL back                </a>
            </div>
     ...
     </ul> </div> </section>  </div> <div class="advert advert--mpu"><!-- BBCDOTCOM slot mpu --><div id="bbccom_mpu_1_2_3_4" class="bbccom_slot" aria-hidden="true"><div class="bbccom_advert bbccom_display_none"><script type="text/javascript"> /*<![CDATA[*/ (function() { if (window.bbcdotcom && bbcdotcom.slot) { bbcdotcom.slot('mpu', [1,2,3,4]); } })(); /*]]>*/ </script></div><script type="text/javascript"> /*<![CDATA[*/ (function() { if (window.bbcdotcom && bbcdotcom.show) { bbcdotcom.show() } })(); /*]]>*/ </script></div></div> </div> </div> </section>       <section class="module module--weather" data-wwhp-module="weather"> <div class="module__content"> <div class="weather"> <h2 class="weather__location"> London Weather <a class="weather__edit">Edit</a> </h2> <form class="weather__form" action="/wwhp"> <div class="weather__search"> <input class="weather__input" placeholder="Enter city, town or region" name="location_term" autocomplete="off"> <button class="weather__submit" type="submit" value=""> <span class="icon icon--search"></span> </button> </div> <p class="weather__error"></p> <ul class="weather__results"></ul> </form> <ul class="weather__forecasts">  <li class="forecast--1 forecast"> <a class="forecast__link" href="/weather/2643743?day=0"   rev="weather|link" > <img class="forecast__image" src="http://static.bbci.co.uk/weather/0.5.284/images/icons/individual_56_icons/en_on_light_bg/2.gif" alt="Partly Cloudy"> <h3 class="forecast__day">Wed</h3>  <p class="forecast__low">13&deg;C</p> </a> </li>  <li class="forecast--2 forecast"> <a class="forecast__link" href="/weather/2643743?day=1"   rev="weather|link" > <img class="forecast__image" src="http://static.bbci.co.uk/weather/0.5.284/images/icons/individual_56_icons/en_on_light_bg/1.gif" alt="Sunny"> <h3 class="forecast__day">Thu</h3>  <p class="forecast__high">21&deg;C</p>  <p class="forecast__low">12&deg;C</p> </a> </li>  <li class="forecast--3 forecast"> <a class="forecast__link" href="/weather/2643743?day=2"   rev="weather|link" > <img class="forecast__image" src="http://static.bbci.co.uk/weather/0.5.284/images/icons/individual_56_icons/en_on_light_bg/3.gif" alt="Sunny Intervals"> <h3 class="forecast__day">Fri</h3>  <p class="forecast__high">21&deg;C</p>  <p class="forecast__low">14&deg;C</p> </a> </li>  <li class="forecast--4 forecast"> <a class="forecast__link" href="/weather/2643743?day=3"   rev="weather|link" > <img class="forecast__image" src="http://static.bbci.co.uk/weather/0.5.284/images/icons/individual_56_icons/en_on_light_bg/7.gif" alt="Light Cloud"> <h3 class="forecast__day">Sat</h3>  <p class="forecast__high">20&deg;C</p>  <p class="forecast__low">11&deg;C</p> </a> </li>  </ul> </div> <div class="advert advert--module_weather advert--sponsor"><!-- BBCDOTCOM slot module_weather --><div id="bbccom_module_weather_1_2_3_4" class="bbccom_slot" aria-hidden="true"><div class="bbccom_advert bbccom_display_none"><script type="text/javascript"> /*<![CDATA[*/ (function() { if (window.bbcdotcom && bbcdotcom.slot) { bbcdotcom.slot('module_weather', [1,2,3,4]); } })(); /*]]>*/ </script></div><script type="text/javascript"> /*<![CDATA[*/ (function() { if (window.bbcdotcom && bbcdotcom.show) { bbcdotcom.show() } })(); /*]]>*/ </script></div></div> </div> </section>     <!-- Error: (null) Module not found or invalid -->
     ...
     <div class="module__content"> <div class="container"> <div class="editors-picks"> <ul class="media-list media-list--fixed-height layout--featured">         
        <li class="media-list__item media-list__item--1">
            <div class="media media--padded media--primary media--icon block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/11248/production/_85361207_queen.jpg" data-alt="Queen Elizabeth"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Queen Elizabeth" /></div></div>                </div>
                ...
                <div class="media__content">
                    ...
                    <h3 class="media__title">
                        <a class="media__link" href="/news/in-pictures-34143130"
                              rev="editors-picks|headline" >
                                                            Rarely seen pictures of Queen Elizabeth                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Rarely seen images of a young Queen Elizabeth by photographer Lisa Sheridan.                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="/news/in-pictures-34143130"
        ...
        <li class="media-list__item media-list__item--2">
            <div class="media media--padded block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/ibroadcast/images/live/p0/32/1x/p0321xwt.jpg" data-alt="In Siberia, a pair of ambitious five-year-old boys tunneled for an F-Type"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="In Siberia, a pair of ambitious five-year-old boys tunneled for an F-Type" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/autos/story/20150909-dig-tunnel-escape-kindergarten-buy-jaguar"
                              rev="editors-picks|headline" >
                                                            Escape kindergarten, buy Jaguar                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            In Siberia, a pair of ambitious five-year-old boys tunneled for an F-Type                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/autos/story/20150909-dig-tunnel-escape-kindergarten-buy-jaguar"
                      rev="editors-picks|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    Escape kindergarten, buy Jaguar                </a>
            </div>
        ...
        <li class="media-list__item media-list__item--3">
            <div class="media media--padded block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/ibroadcast/images/live/p0/31/v5/p031v5n6.jpg" data-alt="When even a âkâ can be pornographic"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="When even a âkâ can be pornographic" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/culture/story/20150907-the-most-controversial-logos-ever"
                              rev="editors-picks|headline" >
                                                            The most controversial logos ever?                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            When even a âkâ can be pornographic                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/culture/story/20150907-the-most-controversial-logos-ever"
                      rev="editors-picks|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    The most controversial logos ever?                </a>
            </div>
        ...
        <li class="media-list__item media-list__item--5">
            <div class="media media--padded block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/ibroadcast/images/live/p0/31/rm/p031rm9f.jpg" data-alt="The secret to revealing deception"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="The secret to revealing deception" /></div></div>                </div>
                ...
                <div class="media__content">
                    ...
                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/future/story/20150906-the-best-and-worst-ways-to-spot-a-liar"
                              rev="editors-picks|headline" >
                                                            How to spot a liar                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            The secret to revealing deception                                                    </p>
        ...
        <li class="media-list__item media-list__item--7">
     ...
     </ul> </div>   <div class="most-popular"> <div class="top-list"> <h2 class="top-list__heading">Most Popular in News</h2> <ul class="top-list__list">  <li class="top-list-item top-list-item__1 top-list-item--ranked top-list-item--odd"> <a class="top-list-item__link" href="http://www.bbc.co.uk/news/uk-34205654"   rev="most-popular|link" > <span class="top-list-item__bullet">1</span> <h3 class="top-list-item__headline">Jonathan King held in child abuse probe</h3> </a> </li>  <li class="top-list-item top-list-item__2 top-list-item--ranked top-list-item--even"> <a class="top-list-item__link" href="http://www.bbc.co.uk/news/world-africa-13036732"   rev="most-popular|link" > <span class="top-list-item__bullet">2</span> <h3 class="top-list-item__headline">Angola profile</h3> </a> </li>  <li class="top-list-item top-list-item__3 top-list-item--ranked top-list-item--odd"> <a class="top-list-item__link" href="http://www.bbc.co.uk/news/magazine-22772391"   rev="most-popular|link" > <span class="top-list-item__bullet">3</span> <h3 class="top-list-item__headline">10 things you might not know about India</h3> </a> </li>  <li class="top-list-item top-list-item__4 top-list-item--ranked top-list-item--even"> <a class="top-list-item__link" href="http://www.bbc.co.uk/news/technology-34185158"   rev="most-popular|link" > <span class="top-list-item__bullet">4</span> <h3 class="top-list-item__headline">Fat-shaming video causes YouTube row</h3> </a> </li>  </ul> </div> </div>  <div class="advert advert--native"><!-- BBCDOTCOM slot native --><div id="bbccom_native_1_2_3_4" class="bbccom_slot" aria-hidden="true"><div class="bbccom_advert bbccom_display_none"><script type="text/javascript"> /*<![CDATA[*/ (function() { if (window.bbcdotcom && bbcdotcom.slot) { bbcdotcom.slot('native', [1,2,3,4]); } })(); /*]]>*/ </script></div><script type="text/javascript"> /*<![CDATA[*/ (function() { if (window.bbcdotcom && bbcdotcom.show) { bbcdotcom.show() } })(); /*]]>*/ </script></div></div> </div> </div> </section>      <section class="module module--collapse-images module--special-features"> <div class="module__content"> <ul class="features">   <li class="feature feature--1"> <h2 class="feature__title"> <a class="feature__link" href="http://www.bbc.com/future/bespoke/specials/designing-innovation/index.html"   rev="special-features|header" > Designing Innovation </a> </h2> <div class="feature__content">     
        
            <div class="media media--overlay media--primary block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/ibroadcast/images/live/p0/30/sv/p030svt7.jpg" data-alt="Couch"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Couch" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/capital/story/20150901-luxury-for-the-pinterest-crowd"
                              rev="special-features|headline" >
                                                            Want the luxury look? Here's how to get it for less                                                    </a>
                    </h3>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/capital/story/20150901-luxury-for-the-pinterest-crowd"
            ...
            <div class="media media--overlay media--primary block-link">
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/travel/story/20150827-canadas-mysterious-fabled-sable-island"
                      rev="special-features|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    'I couldn't quite believe my eyes'                </a>
            </div>
     ...
     <div class="module__content"> <div class="video video--noslick" id="video" data-wwhp-module="video"> <div class="video__player"> <div id="player" class="video__smp"></div>     
            ...
            <div class="media media--primary media--overlay media--icon block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/ibroadcast/images/live/p0/32/0g/p0320gx9.jpg" data-alt="The plane on fire"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="The plane on fire" /></div></div>                </div>
                ...
                <div class="media__content">
                    ...
                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/news/world-us-canada-34194501"
                              rev="video|headline" >
                                                            Plane catches fire in Las Vegas                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            A British Airways flight caught fire while preparing for take-off on the runway at McCarran International Airport in Las Vegas.                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/news/world-us-canada-34194501"
     ...
     </div> <div class="video__content"> <ul class="video__tabs">  <li class="video__tab"> <a class="video__tab__link" data-category="recommended"> Recommended </a> </li>  <li class="video__tab"> <a class="video__tab__link" data-category="most-watched"> Most Watched </a> </li>  <li class="video__tab"> <a class="video__tab__link" data-category="latest"> Latest </a> </li>  </ul> <div class="video__viewport"> <ul class="video__items">  <li class="video__item video__item video__item--0 video__item--recommended" data-category="recommended" data-index="0" data-category-index="0" data-video="{&quot;id&quot;:&quot;201509091006-british-airways-plane-catches-fire-in-las-vegas&quot;,&quot;type&quot;:&quot;video&quot;,&quot;title&quot;:&quot;Plane catches fire in Las Vegas&quot;,&quot;url&quot;:&quot;http:\/\/www.bbc.com\/news\/world-us-canada-34194501&quot;,&quot;img&quot;:&quot;http:\/\/ichef.bbci.co.uk\/wwhp\/800\/ibroadcast\/images\/live\/p0\/32\/0g\/p0320gx9.jpg&quot;,&quot;imgalttext&quot;:&quot;The plane on fire&quot;,&quot;gelicon&quot;:&quot;video&quot;,&quot;summary&quot;:&quot;A British Airways flight caught fire while preparing for take-off on the runway at McCarran International Airport in Las Vegas.&quot;,&quot;allowadvertising&quot;:true,&quot;sourcename&quot;:&quot;US &amp;amp; Canada&quot;,&quot;sourceurl&quot;:&quot;\/news\/world\/us_and_canada&quot;,&quot;customsource&quot;:true,&quot;sourceid&quot;:&quot;us---canada&quot;,&quot;itemtype&quot;:&quot;curated&quot;,&quot;pid&quot;:&quot;p031zvf9&quot;}">     
     ...
     </li>  <li class="video__item video__item video__item--1 video__item--recommended" data-category="recommended" data-index="1" data-category-index="1" data-video="{&quot;id&quot;:&quot;201509081321-the-syrians-exploited-by-people-smugglers&quot;,&quot;type&quot;:&quot;video&quot;,&quot;title&quot;:&quot;The Syrians exploited by people-smugglers&quot;,&quot;url&quot;:&quot;http:\/\/www.bbc.com\/news\/world-europe-34183454&quot;,&quot;img&quot;:&quot;http:\/\/ichef.bbci.co.uk\/wwhp\/800\/cpsprodpb\/17AFE\/production\/_85422079_85421802.jpg&quot;,&quot;imgalttext&quot;:&quot;Mother and child&quot;,&quot;gelicon&quot;:&quot;video&quot;,&quot;summary&quot;:&quot;The BBC's Fergal Keane reports from the Turkish coast, where secret filming reveals the extent of the illegal people-smuggling trade.&quot;,&quot;allowadvertising&quot;:true,&quot;sourcename&quot;:&quot;Europe&quot;,&quot;sourceurl&quot;:&quot;\/news\/world\/europe&quot;,&quot;customsource&quot;:true,&quot;sourceid&quot;:&quot;europe&quot;,&quot;itemtype&quot;:&quot;curated&quot;,&quot;pid&quot;:&quot;p031x56f&quot;}">     
        
            <div class="media media--video media--horizontal media--icon block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/17AFE/production/_85422079_85421802.jpg" data-alt="Mother and child"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Mother and child" /></div></div>                </div>
                ...
                <div class="media__content">
                    ...
                    <h3 class="media__title">
                        <a class="media__link" href="http://www.bbc.com/news/world-europe-34183454"
                              rev="video|headline" >
                                                            The Syrians exploited by people-smugglers                                                    </a>
                    </h3>
                ...
                <a class="block-link__overlay-link"
                   href="http://www.bbc.com/news/world-europe-34183454"
                      rev="video|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    The Syrians exploited by people-smugglers                </a>
            </div>
     ...
     </li>  <li class="video__item video__item video__item--2 video__item--recommended" data-category="recommended" data-index="2" data-category-index="2" data-video="{&quot;contenttype&quot;:&quot;video&quot;,&quot;num&quot;:&quot;10&quot;,&quot;type&quot;:&quot;MAP&quot;,&quot;title&quot;:&quot;Dougie the donkey enjoys home life&quot;,&quot;url&quot;:&quot;\/news\/uk-england-york-north-yorkshire-34191637&quot;,&quot;summary&quot;:&quot;A three-year-old miniature donkey is living in the house of his owners in North Yorkshire.&quot;,&quot;img&quot;:&quot;http:\/\/ichef.bbci.co.uk\/wwhp\/800\/cpsprodpb\/62F6\/production\/_85443352_donkey.jpg&quot;,&quot;imgalttext&quot;:&quot;Dougie the donkey walking through the living room while his owners sit on the sofa&quot;,&quot;pid&quot;:&quot;p031z1q6&quot;,&quot;sourcename&quot;:&quot;York &amp;amp; North Yorkshire&quot;,&quot;sourceurl&quot;:&quot;\/news\/england\/york_and_north_yorkshire&quot;,&quot;created&quot;:&quot;2015-09-09T02:20:02+00:00&quot;,&quot;updated&quot;:&quot;2015-09-09T02:20:02+00:00&quot;,&quot;origincode&quot;:&quot;cpsprodpb&quot;,&quot;gelicon&quot;:&quot;video&quot;,&quot;sourceid&quot;:&quot;videos-must-see&quot;,&quot;itemtype&quot;:&quot;feed&quot;,&quot;allowadvertising&quot;:true,&quot;duration&quot;:&quot;PT1M51S&quot;}">     
        
            <div class="media media--video media--horizontal media--icon block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/62F6/production/_85443352_donkey.jpg" data-alt="Dougie the donkey walking through the living room while his owners sit on the sofa"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Dougie the donkey walking through the living room while his owners sit on the sofa" /></div></div>                </div>
                ...
                <div class="media__content">
                    ...
                    <h3 class="media__title">
                        <a class="media__link" href="/news/uk-england-york-north-yorkshire-34191637"
                              rev="video|headline" >
                                                            Dougie the donkey enjoys home life                                                    </a>
                    </h3>

                    
                                            <a class="media__tag tag tag--videos-must-see" href="/news/england/york_and_north_yorkshire"
                              rev="video|source" >York &amp; North Yorkshire</a>
                    
                </div>

                <a class="block-link__overlay-link"
                   href="/news/uk-england-york-north-yorkshire-34191637"
                      rev="video|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    Dougie the donkey enjoys home life                </a>
            </div>
     ...
     </li>  <li class="video__item video__item video__item--5 video__item--most-watched" data-category="most-watched" data-index="5" data-category-index="0" data-video="{&quot;contenttype&quot;:&quot;video&quot;,&quot;num&quot;:&quot;10&quot;,&quot;title&quot;:&quot;Faulks on 'terrible instability' of humans&quot;,&quot;url&quot;:&quot;\/news\/entertainment-arts-34205189&quot;,&quot;img&quot;:&quot;http:\/\/ichef.bbci.co.uk\/wwhp\/800\/cpsprodpb\/108E8\/production\/_85461876_85461874.jpg&quot;,&quot;allowadvertising&quot;:true,&quot;pid&quot;:&quot;p032259j&quot;,&quot;duration&quot;:&quot;PT2M37S&quot;,&quot;sourcename&quot;:&quot;Entertainment &amp;amp; Arts&quot;,&quot;sourceurl&quot;:&quot;\/news\/entertainment_and_arts&quot;,&quot;summary&quot;:&quot;Sebastian Faulks believes human beings are \&quot;incredibly unstable\&quot; due to the large evolutionary jump that they have experienced.&quot;,&quot;imgalttext&quot;:&quot;Sebastian Faulks&quot;,&quot;origincode&quot;:&quot;cpsprodpb&quot;}">     
        
            <div class="media media--video media--horizontal block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/108E8/production/_85461876_85461874.jpg" data-alt="Sebastian Faulks"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Sebastian Faulks" /></div></div>                </div>
                ...
                <div class="media__content">
                    ...
                    <h3 class="media__title">
                        <a class="media__link" href="/news/entertainment-arts-34205189"
                              rev="video|headline" >
                                                            Faulks on 'terrible instability' of humans                                                    </a>
                    </h3>
                ...
                <a class="block-link__overlay-link"
                   href="/news/entertainment-arts-34205189"
                      rev="video|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    Faulks on 'terrible instability' of humans                </a>
            </div>
                    ...
                    <h3 class="media__title">
                        <a class="media__link" href="/news/world-europe-34197413"
                              rev="bbc-world-news|headline" >
                                                            HARDtalk - 'No-one can expel Catalonia from EU' says Romeva                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Raul Romeva explains why he believes that an independent Catalonia could still be part of the EU                                                    </p>
     ...
     <div class="module__content"> <div class="container"> <div class="more-bbc"> <ul class="media-list media-list--fixed-height layout--featured">         
        <li class="media-list__item media-list__item--1">
            <div class="media media--padded media--primary block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/9B2C/production/_85442793_stephencolbert-ap.jpg" data-alt="Jeb Bush and Stephen Colbert"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Jeb Bush and Stephen Colbert" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/entertainment-arts-34195185"
                              rev="more-bbc|headline" >
                                                            Stephen Colbert debuts on Late Show                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Stephen Colbert makes his debut as the new host of the Late Show on US TV network CBS with guests...                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="/news/entertainment-arts-34195185"
        ...
        <li class="media-list__item media-list__item--2">
            <div class="media media--padded block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/17006/production/_85441249_85441213.jpg" data-alt="Wozniak"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Wozniak" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/technology-34188602"
                              rev="more-bbc|headline" >
                                                            Woz: Shocked and amazed by Jobs film                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            In an exclusive interview, Apple's co-founder Steve Wozniak gives his verdict.                                                    </p>
        ...
        <li class="media-list__item media-list__item--5">
            <div class="media media--padded block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/12345/production/_85456547_headless_promo_getty.jpg" data-alt="Mike, the headless chicken"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Mike, the headless chicken" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/magazine-34198390"
                              rev="more-bbc|headline" >
                                                            The chicken that lived for 18 months without a head                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            Seventy years ago, a farmer beheaded a chicken in Colorado, and it refused to die. How did Miracle...                                                    </p>
        ...
        <li class="media-list__item media-list__item--6">
            <div class="media media--padded block-link">
                <div class="media__image">
                    <div class="responsive-image"><div class="delayed-image-load" data-src="http://ichef.bbci.co.uk/wwhp/{width}/cpsprodpb/15275/production/_85454668_85454666.jpg" data-alt="Lunar far side"><img src="data:image/gif;base64,R0lGODlhEAAJAIAAAP///wAAACH5BAEAAAAALAAAAAAQAAkAAAIKhI+py+0Po5yUFQA7" alt="Lunar far side" /></div></div>                </div>

                
                <div class="media__content">

                    <h3 class="media__title">
                        <a class="media__link" href="/news/science-environment-34201946"
                              rev="more-bbc|headline" >
                                                            China aims for Moon's far side                                                    </a>
                    </h3>

                                            <p class="media__summary">
                                                            China is planning to attempt the first ever landing of a lunar probe on the far side of the Moon, an...                                                    </p>
                ...
                <a class="block-link__overlay-link"
                   href="/news/science-environment-34201946"
                      rev="more-bbc|overlay"                    tabindex="-1"
                   aria-hidden="true">
                    China aims for Moon's far side                </a>
            </div>
