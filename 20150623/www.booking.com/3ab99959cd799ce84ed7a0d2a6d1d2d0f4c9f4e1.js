...
}}}},_renderItem:function(r,G,K,C){var u=this;
var I=false;
var y=(G.cc1==booking.env.ip_country)?true:false;
G.position=K;
G.array_length=C;
var v="";
var n=", ";
...
}if(typeof(G.labels)!="undefined"&&(booking.env.b_lang=="zh"||booking.env.b_lang=="ja"||booking.env.b_lang=="ko")){var l=false;
booking.track.exp("MWAAQaNZFFEbdDbaVIFTdWe");
I=true;
b.each(G.labels,function(M,L){if(M===k-1||k==1){n=""
}if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){if(L.type=="region"&&M!=k-1&&M!=0){var N=L;
L=G.labels[M+1];
G.labels[M+1]=N
}if((M!=k-1&&G.labels[M+1].type=="region"&&!l)||(M==0&&k==2&&L.type=="region"&&!l)){n=" (";
l=true
}else{if(M==k-1&&l){n=")"
}else{if(M==k-1){n=""
}else{n=", "
}}}}if(L.hl){if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){v=v+"<span class='ac-hl-cjk'>"+L.text+"</span>"+n
}else{v=v+"<span class='ac-hl-cjk'>"+L.text+n+"</span>"
}}else{v=v+L.text+n
}})
}else{if(G.nr_hotels>=500){B.env.search_ac_is_popular_ufi=true;
if(booking.track.getVariant("PVdIZKZPHcaUC")){var q=G.labels[0];
v="";
if(q.hl===1){v+="<strong>"
}v+=q.text;
if(q.match){v+=" ("+q.match+")"
}if(q.hl===1){v+="</strong>"
}}else{v=G.label_highlighted||G.label
}}else{v=G.label_highlighted||G.label
}}var g=function(L){if(L=="undefined"||L==c||L==void 0){return 1
}else{return 0
}};
var p=booking.track.getVariant("MWBOYKbAJWe");
if(p===1||p===2){if(G.level){v='<em class="ac_item_tree_arrow"></em>'+v
}}var o=v,A="",d="",z="",E="",e="",h=G.photo||"";
if(G.header){A='<span class="autocomplete_header autocomplete_h_'+G.dest_type+'" >'+booking.env.autocomplete_categories[G.header]+"</span>";
d=(G.first)?"":' style="border-top: 1px solid #CCC;"';
z+=(G.first)?"":"first-in-group"
}if(G.nr_hotels_label&&G.nr_hotels_label.length){var J="text-align: left; display: inline;";
...
if(booking.env.rtl!=1&&G.rtl==1){e+='<span style="visibility:hidden;margin-left:-4px;">i</span>'
}var m=G.nr_hotels_label;
if(G.dest_type=="hotel"){if(G.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){e=""
}else{e+="("+m+")"
}}else{if(G.nr_hotels==0&&G.nr_hotels_25>0){e+='<span class="ac_prop_nearby">('+booking.env.autocomplete_counter_label+")</span>"
}else{e=""
}}if(G.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){e+="</span>"
}else{e+="</span></span>"
}}var t="";
if(G.dest_type=="theme"&&G.additional_label){t='<span class="label" style="position: static;color:#888;">&nbsp;<span style="direction: rtl; text-align: left; display: inline;">'+G.additional_label+"</span></span>"
}var i=B.tools.jsStaticUrl("/static/img/flags/16/"+G.cc1+".png");
var s="PVUELMBBdXUC";
if(G.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){var H="";
switch(G.dest_type){case"continent":H=booking.env.ac_tree_view_continent;
break;
case"country":H=booking.env.ac_tree_view_country;
break;
case"region":H=booking.env.ac_tree_view_region;
break;
case"city":H=booking.env.ac_tree_view_city;
break;
case"district":H=booking.env.ac_tree_view_district;
break;
case"airport":H=booking.env.ac_tree_view_airport;
break;
case"landmark":H=booking.env.ac_tree_view_landmark;
break;
case"hotel":H=booking.env.ac_tree_view_hotel;
break
}var D='&nbsp;&nbsp;<span class="autocomplete_icons_to_label">'+H+"</span></span>"
}else{var D='<i class="autocomplete_icons autocomplete_'+G.dest_type+'"></i>'
}if(G.dest_type!="theme"&&(g(G.cc1)!=1&&G.cc1.length==2)&&booking.track.getVariant("EKODAKSCNVAELIYRAfUZC")==1&&booking.track.getVariant("PVUELMEARLGUXO")==2){D+='<img class="search_ac_country_flags_right" src="'+i+'">'
}if(booking.env.b_is_loggedin_genius_user==1&&((G.genius_hotels/G.hotels)>0.01||G.is_genius===1)){booking.track.exp("PVUELMZNdGUKCMeMRfPWe");
if(G.dest_type!="theme"&&booking.track.getVariant("PVUELMZNdGUKCMeMRfPWe")===1){D+='<span class="ac_genius_right"><span class="ge-iconfont-plate"> <i class="bicon-dotgeniusbg"></i><i class="bicon-dotgeniusfold"></i><i class="bicon-dotgenius"></i></span></span>'
}}if(I){var f='<span class="ac-hl-cjk-wrapper">'
}else{var f="<span>"
}if((booking.track.getVariant("PVUELMNGEcdXUSPSELIae")!=0)){if(G.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){o=f+o+D+e+"</span></span>"
}else{o=f+o+e+t+"</span>"+D
}}else{if(G.dest_type!="theme"&&(g(G.cc1)!=1&&G.cc1.length==2)&&booking.track.getVariant("EKODAKSCNVAELIYRAfUZC")==1&&booking.track.getVariant("PVUELMEARLGUXO")==1){o=D+'<img class="search_ac_country_flags_left" src="'+i+'">'+f+o+e+t+"</span>"
}else{o=D+f+o+e+t+"</span>"
}}var x="";
if(booking.track.getVariant("bLYUELMaIIERcdNFQLNWe")===1&&G.dest_type!="theme"){x='<div class="si_menu"><div class="si_menu_item">Wifi</div><div class="si_menu_item">Non-smoking Rooms</div><div class="si_menu_item">Air conditioning</div><div class="si_menu_item">Flat-screen TV</div><div class="si_menu_item">Bathtub</div></div>';
z+=" si_parent_v1"
}else{if(booking.track.getVariant("bLYUELMaIIERcdNFQLNWe")===2&&G.dest_type!="theme"){z+=" si_parent_v2"
}}E='class="partition_item';
if(G.dest_type!="theme"){var p=booking.track.getVariant("MWBOYKbAJWe");
if(p){E+=" ac_item_label_wrapper"
}if(G.level&&(p===1||p===2)){E+=" ac_item_tree_wrapper"
}}else{E+=" ac_item_theme"
}E+='"';
result=b('<li class="'+z+'"'+d+"></li>").data("item.autocomplete",G).append(b("<a "+E+">"+o+"</a>")).append(x).appendTo(r);
if(booking.track.getVariant("bLYUELMaIIERcdNFQLNWe")===2&&G.dest_type!="theme"){var j='<div class="si_menu"><p>Filter by:</p><div class="si_menu_item">Wifi</div><div class="si_menu_item">Non-smoking Rooms</div><div class="si_menu_item">Air conditioning</div><div class="si_menu_item">Flat-screen TV</div><div class="si_menu_item">Bathtub</div></div>';
var F=b(j).appendTo(r);
var w=F.prev();
F.hover(function(){w.find("a").addClass("ui-state-hover")
},function(){w.find("a").removeClass("ui-state-hover")
})
}if(booking.track.getVariant("TMGCBOEQC")){if(G.position==0&&G.dest_type!="theme"){result.find("a").after(b("<span />",{"class":"sb-ac-menu-arrow-outer"}));
