...
}B.atlas.define("atlas-common-markers",a,function(t,z,v){var l,w=t.get("destinationId"),g=B.atlas.assert,x=false,f=100,m=null,j=false,n=t.get("isMiniIWLandingPages"),e=t.get("isMiniIW");
function d(C,L,A){var M,I,J=35,G=56,D=5,K=27,H=this.ubber.$domNode,F={w:H.width(),h:H.height()},E={w:C.outerWidth(),h:C.outerHeight()};
G=(G>F.w-A.x-D)?D:G;
if((E.w+D+G>F.w)){E.w=F.w-D-G;
C.find(".iw-container").css({"max-width":E.w})
}M=A.x-D-E.w/2;
M=(M+E.w+D>F.w)?F.w-E.w-D-G:M;
M=(M-G<3)?3:M;
I=(A.y+E.h>F.h-J)?A.y-E.h-K:A.y;
return{x:M,y:I}
}function k(A){A.setIW({disablePan:true,className:"iw-overlay-lp",engine:"html",getPosition:d,isFixed:true})
}function q(A){A.setIW({getPosition:getIWPositionMini})
}function o(A){A.setIW({getPosition:d})
}function h(C,A){if(A&&A.data&&A.data.b_id&&A.data.b_type){C.setIW({className:"iw-overlay-lp iw-overlay-"+A.data.b_type+((A.data.b_id===w)?"-current":"")})
}}function u(){var A=this;
A.getMarkers(function(D){D=D||{};
var F,C=[].concat(D.b_hotels||[],D.b_cities||[],D.b_airports||[]),E=C.length;
if(B.atlas.getVariant("eGECAFWTYcUNC")||B.atlas.getVariant("eGOLHMbdIePQNcO")||B.atlas.getVariant("eGMMECAFWTYcUNC")||B.atlas.getVariant("eGfEDXPdLbeMSSae")){B.atlas.require(["experiment-map-markers-disperse"],function(G){G.initialize(C,A,w)
})
}else{A.updateMarkers(C);
if(w){while(E--){F=C[E];
if(F&&w===F.b_id){if(!B.atlas.assert.tdot&&B.atlas.assert.hp){A.setMarkerOptions(F.b_id,{cursor:"default"})
}A.setMarkerType(F.b_id,F.b_marker_type+"_current");
break
}}}}});
A.closeIW()
}function b(A){var C=this;
if(l){clearTimeout(l)
}l=setTimeout(function(){C.setIconHover(A.id);
if(A.type==="geosearch_place"&&A.data){C.setIW({className:"iw-overlay-lp iw-overlay-geosearch-place"});
C.openIW(A.id,A.data.data)
}else{if(/city|airport/.test(A.type)){if(e||n){if(m!==A.id){if(m){C.setIconOut(m);
C.closeIW()
}h(C,A);
...
}}else{if(e||n){if(m!==A.id){if(m){C.setIconOut(m);
C.closeIW()
}h(C,A);
C.openIWLoading(A.id,B.jstmpl("atlas_iw_mini_loading").render({}));
if(B.atlas.getVariant("eGBUYSaBTbYFbVKIGHT")&&w!=A.id){v.getIWs([A.id,w],C,function(D){var E=D[0],F=D[1];
if(E.b_id!=A.id){E=D[1];
F=D[0]
}E.b_destination_reference=F;
E.b_marker_type=(A.data)?A.data.b_marker_type:"";
E.b_map_nights=t.get("checkinCheckoutInterval");
E.map_price_per_x_nights_txt=t.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){E.transLengthOfStayExperiment=t.get("transLengthOfStayExperiment")
}C.openIW(A.id,{},B.jstmpl("atlas_iw_mini").render(E))
})
}else{C.getIW(A.id,function(D){D.b_marker_type=(A.data)?A.data.b_marker_type:"";
D.b_map_nights=t.get("checkinCheckoutInterval");
D.map_price_per_x_nights_txt=t.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){D.transLengthOfStayExperiment=t.get("transLengthOfStayExperiment")
}C.openIW(A.id,{},B.jstmpl("atlas_iw_mini").render(D))
})
}}}else{C.openIWLoading(A.id);
C.getIW(A.id,function(D){D.b_marker_type=(A.data)?A.data.b_marker_type:"";
D.b_map_nights=t.get("checkinCheckoutInterval");
D.map_price_per_x_nights_txt=t.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){D.transLengthOfStayExperiment=t.get("transLengthOfStayExperiment")
}h(C,A);
...
}function y(A){var C=this;
if(l){clearTimeout(l)
}if(!(e||n)){C.setIconOut(A.id)
}if(e||n){if(m!==A.id){C.setIconOut(A.id);
C.setIW({className:"iw-overlay-lp"});
C.closeIW()
}}else{C.setIW({className:"iw-overlay-lp"});
C.closeIW()
}}function p(){var A=this;
view=A.getViewFromMap()
}function c(C){var A=$(this).offset();
mouse.x=C.pageX-A.left;
mouse.y=C.pageY-A.top
}function s(C){var E=this,A=E.getMarker(C.id),D=(A&&A.b_url)?A.b_url:"";
...
}if(B.atlas.getVariant("eGYSaBbTZSEISUaDZGTGC")||B.atlas.getVariant("eGfESAcCJZUNZXQeJVUcDcRe")){E.off("marker-hover",b);
E.off("marker-out",y);
E.once("iw-close",function(){E.on("marker-hover",b);
E.on("marker-out",y)
})
}if(e||n){E.on("iw-close",function(){if(m){E.setIconOut(m);
m=null
}});
if(m){E.setIconOut(m)
}m=A.b_id;
E.openIWLoading(A.b_id);
E.setIconHover(A.b_id);
if(/city|airport/.test(A.b_type)){h(E,A);
E.openIW(A.b_id,{},B.jstmpl("atlas_iw_landmark").render(A.data));
E.trigger("iw-opened-click",{marker:A,data:A.data})
}else{E.getIW(A.b_id,function(F){F.b_map_nights=t.get("checkinCheckoutInterval");
F.map_price_per_x_nights_txt=t.get("transPricePerXNights");
F.b_url=A.data.b_url;
F.b_marker_type=A.data.b_marker_type;
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){F.transLengthOfStayExperiment=t.get("transLengthOfStayExperiment")
}h(E,A);
E.openIW(A.b_id,F);
E.trigger("iw-opened-click",{marker:A,data:F});
if(B.atlas.getVariant("PcVBcOLVPIBeaT")){B.eventEmitter.trigger("wl:bind:dynamic");
E.on(E.EVENTS.IW_CLOSE,function(){$(".fly-dropdown.wl-dropdown").hide()
})
}})
}}else{if(D&&!(A.data.b_id===w&&(!B.atlas.assert.tdot&&B.atlas.assert.hp))){location.assign(D)
}}}function i(G,D){var F=this,A=F.getMarker(D),C,E=3000;
if(!D){return
}if(!A&&!x){x=true;
return F.once("markers-show",i,D)
}if(B.atlas.getVariant("eGHRXOSPCcTOSGC")){if(!A){return
}}else{if(!A||(A&&A.b_type&&/city|airport/.test(A.b_type))){return
}}if(t.get("action")=="hotel"){C=function(){i.call(F,G,D,true);
...
}if(typeof F.setIconHover==="function"){window.setTimeout(function(){F.setIconHover(D)
},0)
}if(B.atlas.getVariant("eGHRXOSPCcTOSGC")&&/city|airport/.test(A.b_type)){h(F,A);
F.openIW(A.b_id,{},B.jstmpl("atlas_iw_landmark").render(A.data));
F.trigger("iw-autoopen",F);
F.trigger("iw-opened-click",{marker:A,data:A.data});
return
}F.getIW(D,function(H){H.b_marker_type=(A.data)?A.data.b_marker_type:"";
H.b_map_nights=t.get("checkinCheckoutInterval");
H.map_price_per_x_nights_txt=t.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){H.transLengthOfStayExperiment=t.get("transLengthOfStayExperiment")
}h(F,A);
if(!B.atlas.getVariant("eGBUYSaBTHVDBHT")){F.openIW(D,H)
}else{F.trigger("iw-control-show",D,H)
}if(t.get("action")==="hotel"){E=4000
}l=setTimeout(function(){F.closeIW()
},E)
})
}function r(C){try{j=(window.localStorage&&window.sessionStorage?true:false);
if(j){window.localStorage.setItem("btest","1")
}}catch(A){j=false
}if(j){C.on("load",function(){C.trigger("load-dom-storage-supported")
})
}k(C);
C.on("bounds-change",u,C);
C.on("marker-hover",b);
C.on("marker-out",y);
C.on("marker-click",s);
C.on("close",C.closeIW);
C.on("open",i,w);
if(e||n){$("body").delegate("#marker_close","click",function(){C.closeIW()
})
}}return{init:r,setupIW:k,autoopenIW:i,getIWPosition:d,markerHover:b,markerOut:y,markerClick:s,boundsChange:u}
})
});
$(function(){if(!B.atlas){return
