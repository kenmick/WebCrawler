...
// Custom Double Whitelist Logic for Campaigns
...
if (!window.mmcore) {
  window.mmcore = {
    ...
    sample_rate1: 1,
    sample_rate2: 100,
    whitelist1: {is: [],  contains: ['//www.bbc.co.uk/sport', '//www.bbc.co.uk/iplayer', '//www.bbc.co.uk/news', '//m.bbc.co.uk/sport/live'], regex: []},
    whitelist2: {is: ['https://ssl.bbc.co.uk/iplayer/watching', 'http://pal.sandbox.dev.bbc.co.uk/news'], contains: ['https://ssl.bbc.co.uk/iplayer/watching?', 'www.test.bbc.co.uk/home/five/testing/', 'www.bbc.co.uk/guides/zxghycw'], regex: [/bbc\.co\.uk(\/?(\?.*)?(#.*)?)?$/]},
    ...
    isInWhitelist: function(wl) {var url = window.location.href;for (var i = 0; i < wl.is.length; i++) {if (wl.is[i] === url) return true;}for (var i = 0; i < wl.contains.length; i++) {if (url.indexOf(wl.contains[i]) > -1) return true;}for (var i = 0; i < wl.regex.length; i++) {if (wl.regex[i].test(url)) return true;}return false;},
    shouldCall: function() {if (!mmcore.isInWhitelist(mmcore.whitelist1) && !mmcore.isInWhitelist(mmcore.whitelist2)) {return false;} else if (mmcore.isInWhitelist(mmcore.whitelist1) && mmcore.isInWhitelist(mmcore.whitelist2) && mmcore.isInRate(this.sample_rate2)) {return true;} else if (mmcore.isInWhitelist(mmcore.whitelist1) && !mmcore.isInRate(this.sample_rate1)) {return false;} else if (mmcore.isInWhitelist(mmcore.whitelist2) && !mmcore.isInRate(this.sample_rate2)) {return false;} else {return true;}},
