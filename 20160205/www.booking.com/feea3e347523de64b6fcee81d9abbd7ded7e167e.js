...
}}function aa(ab){V=ab.split(",").reduce(function(ac,ad){ac[ad]=true;
return ac
},V)
}function Z(ab){V[ab]=true
}function W(ab){Z(ab);
...
var M=(function(){var V=["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"];
...
}function g(V,X,W){var e=c(V);
if(e===false){return false
}return !!(((X+W)>(e))&&V.is(":visible"))
}var v=(function(){var aa=false;
var ab;
var ad=[];
var af=[];
var ae=0;
var V=100;
function X(am){var ak=[];
var al=[];
var aj=[];
var an=[];
...
}function ac(ai){var ah=ai.url;
var am=ai.complete||function(){};
var ak=ai.headers||{};
if(!ah){return false
...
}}}ag.onreadystatechange=function aj(){if(ag.readyState===XMLHttpRequest.DONE){am(ag,ag.status)
}};
ag.send()
}function Y(){w.level&&w.report(w.events.BEACON_SENT,ad);
aa=false;
ae=0;
ab=null;
if(ad.length){e()
...
}}function e(){w.level&&w.report(w.events.SEND_BEACON,ad.slice(0));
aa=true;
af=ad;
var ah=h+"&"+X(af);
ad=[];
try{if(R){ac({url:ah,complete:function(ak,aj){if(aj===200){Y()
}else{Z()
}},headers:O})
}else{jQuery.ajax({url:ah,complete:function(ak,aj){if(aj==="success"){Y()
}else{Z()
}}})
...
ag.onerror=Z;
ag.src=h
}}function W(ah,ag,ai){w.level&&w.report(w.events.INIT_BEACON,ah,ag,ai);
...
var y=(function(){var e=false;
var ab=[];
var aa;
function V(){w.level&&w.report(w.events.BEACON_SENT,ab);
e=false;
if(ab.length){j()
}}function Y(){var ag=[];
var ah=[];
...
}function Z(ac){if(ac){ab.push(ac)
}}function X(){ab.length=0
}function W(ag,af,ai){w.level&&w.report(w.events.INIT_BEACON,ag,af,ai);
var ah;
var ad;
...
}function Q(W,V,X){var e;
if(W===f.experiment||W===f.goal){e=[W,V]
}else{e=[W,V,X]
}return e.join("-")
}function u(W){if(!T){return W
}if(l[W]){return l[W]
}var X=2166136261;
...
}function S(V,e,W){w.level&&w.report(w.events.TRACKING_ATTEMPT,{what:V,hash:e,id:W,variant:(V===f.experiment||V===f.stage)&&s(e)});
...
}}function K(V,e,W){N[Q(V,e,W)]=true
}function q(Y,X,Z){w.level&&w.report(w.events.SHOULD_TRACK,Y,X,Z);
...
}function I(e,W,Z,Y,aa){w.level&&w.report(w.events.ADD_EVENT_LISTENER,e,W,Z,Y,aa);
...
}}}function m(V,X,W,Y){w.level&&w.report(w.events.ADD_DEBOUNCED_VIEW_HANDLER,V,X,W,Y);
var e=Q(X,W,Y);
if(N[e]){return false
}H.push([V,X,W,Y]);
...
}window.setTimeout(function(){L=false
},x)
}function i(e,V,W){return function(X,Y){if(q(W,X,Y)){I(e,V,W,X,Y)
}else{w.level&&w.report(w.events.WONT_ATTACH_EVENT_TRACKING,e,V,W,X,Y)
}}
}function C(e,V){return{track:i(e,V,f.experiment),stage:i(e,V,f.stage),goal:i(e,V,f.goal),customGoal:i(e,V,f.customGoal)}
}function p(e,W){for(var V in W){if(W.hasOwnProperty(V)){e[V]=W[V]
}}}function z(e){d.f=e.f||{};
p(d.r,e.r||{});
p(d.t,e.t||{})
}function s(e){var V=u(e);
return d.r[V]||d.f[V]||0
}function t(e){if(arguments.length>1){throw"Track only accept one parameter"
}if(!e){return 0
}return S(f.experiment,e)
}function J(V,e){if(!V){return false
}if(e===undefined){return false
}if(e===0){return t(V)
}if(!/^[1-9]$/.test(e)){return false
}return S(f.stage,V,e)
}function D(e){if(!e){return false
}return S(f.goal,e)
}function E(V,e){if(!V||!e||!/^[1-5]$/.test(e)){return false
}return S(f.customGoal,V,e)
}function A(e){if(e.url){h=e.url
}if(e.jset){z(e.jset)
