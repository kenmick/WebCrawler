...
}if(B.atlas.getVariant("ebcBUaUSPaGXONCCCeSaT")||B.atlas.getVariant("ebcOLWNTJRLXTcMMOeBRe")){B.atlas.require(["ufi-toggle-polygon"],function(G){G.initialize({districtList:A,map:y})
})
}D=z.length;
...
A.once("fixed-iw-open",function(){var C="ebcNHTIYSaBVOHYRJTSDXZLO";
if(r.get("action")==="searchresults"){B.et.stage(C,2)
}else{if(r.get("action")==="hotel"){B.et.stage(C,3)
}}});
...
p.pubSub.on("close",function(){if(u){u.trigger("close")
}});
p.addEvents();
