...
if(booking.env.b_clientside_templates_new_api){b=function(d){return booking.et.track(d)
};
a=function(e,d){return booking.et.stage(e,d)
};
c=function(d){return booking.et.track(d)
}
...
}var g={experiment:"e",stage:"s",goal:"g",customGoal:"cg"};
var N=(function(){var ad={};
var ag;
var e="";
...
var T=(function(){var ad=["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"];
...
}function i(ad,af,ae){var e=c(ad);
if(e===false){return false
}return !!(((af+ae)>(e))&&ad.is(":visible"))
}var A=(function(){var ai=false;
var aj;
var al=[];
var an=[];
var am=0;
var ad=100;
function af(av){var at=[];
var au=[];
var ar=[];
var aw=[];
for(var aq,ap=0,ao=av.length;
ap<ao;
++ap){aq=av[ap];
switch(aq.what){case g.experiment:at.push(aq.hash);
break;
case g.stage:aw.push(aq.hash+"|"+aq.id);
break;
case g.goal:au.push(aq.hash);
break;
case g.customGoal:ar.push(aq.hash+"|"+aq.id);
break;
default:C.level&&C.report(C.events.UNABLE_TO_STRINGIFY,aq)
}}return"exps="+at.join(",")+"&what="+au.join(",")+"&custom="+ar.join(",")+"&stage="+aw.join(",")
}function ak(aq){var ap=aq.url;
var av=aq.complete||function(){};
var at=aq.headers||{};
if(!ap){return false
}var ao=new XMLHttpRequest();
ao.open("GET",ap,true);
var au;
if(at){for(au in at){if(at.hasOwnProperty(au)){ao.setRequestHeader(au,typeof at[au]==="function"?at[au].call():at[au])
}}}ao.onreadystatechange=function ar(){if(ao.readyState===XMLHttpRequest.DONE){av(ao,ao.status)
}};
ao.send()
}function ag(){C.level&&C.report(C.events.BEACON_SENT,al);
ai=false;
am=0;
aj=null;
if(al.length){e()
...
aj=null;
if(am>=10){an=[]
}else{al=al.concat(an);
an=[];
aj=window.setTimeout(e,am*ad)
}}function e(){C.level&&C.report(C.events.SEND_BEACON,al.slice(0));
ai=true;
an=al;
var ap=j+"&"+af(an);
al=[];
try{if(Z){ak({url:ap,complete:function(at,ar){if(ar===200){ag()
}else{ah()
}},headers:W})
}else{jQuery.ajax({url:ap,complete:function(at,ar){if(ar==="success"){ag()
}else{ah()
}}})
}}catch(aq){var ao=new Image();
ao.onload=ag;
ao.onerror=ah;
ao.src=j
}}function ae(ap,ao,aq){C.level&&C.report(C.events.INIT_BEACON,ap,ao,aq);
if(arguments.length===0){}al.push({what:ap,hash:ao,id:aq});
if(ai||aj){C.level&&C.report(C.events.DEFER_BEACON,al)
}else{aj=window.setTimeout(e,L)
}}return ae
}());
var E=(function(){var e=false;
var aj=[];
var ai;
function ad(){C.level&&C.report(C.events.BEACON_SENT,aj);
e=false;
if(aj.length){l()
}}function ag(){var ao=[];
var ap=[];
var an=[];
var aq=[];
for(var am,al=0,ak=aj.length;
al<ak;
++al){am=aj[al];
switch(am.what){case g.experiment:ao.push(am.hash);
break;
case g.stage:aq.push(am.hash+"|"+am.id);
break;
case g.goal:ap.push(am.hash);
break;
case g.customGoal:an.push(am.hash+"|"+am.id);
break;
default:C.level&&C.report(C.events.UNABLE_TO_STRINGIFY,am)
}}return"exps="+ao.join(",")+"&what="+ap.join(",")+"&custom="+an.join(",")+"&stage="+aq.join(",")
}function ah(ak){if(ak){aj.push(ak)
}}function af(){aj.length=0
}function ae(ao,an,aq){C.level&&C.report(C.events.INIT_BEACON,ao,an,aq);
var ap;
var al;
if(arguments.length>0){ah({what:ao,hash:an,id:aq})
}if(!e&&j){C.level&&C.report(C.events.SEND_BEACON,aj.slice(0));
ap=ag();
e=true;
al=j+"&"+ap;
try{jQuery.ajax({url:al,complete:ad})
}catch(am){var ak=new Image();
ak.onload=ak.onerror=ad;
ak.src=al
}af()
}else{C.level&&C.report(C.events.DEFER_BEACON,aj)
}}return ae
}());
var l=E;
var n={};
...
}function Y(ae,ad,af){var e;
if(ae===g.experiment||ae===g.goal){e=[ae,ad]
}else{e=[ae,ad,af]
}return e.join("-")
}function y(ae){if(!ab){return ae
}if(n[ae]){return n[ae]
}var af=2166136261;
...
af^=ae.charCodeAt(e)
}return(n[ae]=(af>>>0).toString(16))
}function aa(ad,e,ae){C.level&&C.report(C.events.TRACKING_ATTEMPT,{what:ad,hash:e,id:ae,variant:(ad===g.experiment||ad===g.stage)&&w(e)});
if(u(ad,e,ae)){switch(ad){case g.experiment:R(g.experiment,e);
N.populate(e);
l(g.experiment,e);
break;
case g.stage:R(g.stage,e,ae);
N.populate(e+"|"+ae);
l(g.stage,e,ae);
break;
case g.goal:R(g.goal,e);
l(g.goal,e);
break;
case g.customGoal:R(g.customGoal,e,ae);
l(g.customGoal,e,ae);
break;
default:C.level&&C.report(C.events.TRACK_UNKNOWN_ITEM,ad,e,ae)
}}if(ad===g.experiment){return w(e)
}else{return true
}}function R(ad,e,ae){U[Y(ad,e,ae)]=true
}function u(ag,af,ah){C.level&&C.report(C.events.SHOULD_TRACK,ag,af,ah);
var e={what:ag,hash:af,id:ah,variant:(ag===g.experiment||ag===g.stage)&&w(af)};
if(U[Y(ag,af,ah)]){C.level&&C.report(C.events.NOT_TRACKING_WAS_TRACKED,e);
return false
}else{if(ag===g.experiment||ag===g.stage){var ae=1<<(ah||0);
var ad=y(af);
...
return false
}else{if(d.t[ad]&ae){R(ag,af,ah);
C.level&&C.report(C.events.NOT_TRACKING_WAS_TRACKED,e);
return false
}}}}else{if(ag===g.customGoal){var ad=y(af);
if(d.f[ad]){C.level&&C.report(C.events.NOT_TRACKING_FULLON,e);
return false
}else{if(d.r[ad]===undefined){C.level&&C.report(C.events.NOT_TRACKING_NOT_RUNNING,e);
return false
}}}}}return true
}function P(e,ae,ah,ag,ai){C.level&&C.report(C.events.ADD_EVENT_LISTENER,e,ae,ah,ag,ai);
if(!X){X=jQuery(window)
}var af=jQuery(ae).eq(0);
if(m&&e!=="view"){af=jQuery(ae)
}if(!af.length){C.level&&C.report(C.events.NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER,ah,ag,ai);
return
}function ad(){aa(ah,ag,ai)
}if(e==="view"){p(af,ah,ag,ai)
}else{af.one(e,ad)
}}function p(ad,af,ae,ag){C.level&&C.report(C.events.ADD_DEBOUNCED_VIEW_HANDLER,ad,af,ae,ag);
var e=Y(af,ae,ag);
if(U[e]){return false
}O.push([ad,af,ae,ag]);
if(!q){C.level&&C.report(C.events.ATTACH_VIEW_LISTENER,O);
X.bind("scroll resize",v);
B&&B.eventEmitter&&B.eventEmitter.bind(T.join(" "),v);
q=true
...
var ah=X.scrollTop();
var ag=X.height();
C.level&&C.report(C.events.CHECK_IF_VISIBLE,O);
var ad;
for(var ae=0,e=O.length;
ae<e;
++ae){ad=O[ae];
if(ad&&i(ad[0],ah,ag)){C.level&&C.report(C.events.ONVIEW_TRACKING_TRIGGERED,ad[1],ad[2],ad[3]);
aa(ad[1],ad[2],ad[3])
}else{af.push(ad)
}}O=af;
C.level&&C.report(C.events.VISIBLE_CHECK_FINISHED,O);
if(O.length===0){q=false;
X.unbind("scroll resize",v);
B&&B.eventEmitter&&B.eventEmitter.unbind(T.join(" "),v);
C.level&&C.report(C.events.DETACH_VIEW_LISTENER)
}window.setTimeout(function(){S=false
},D)
}function h(ai,ae,ak,ag,ad){C.level&&C.report(C.events.ADD_EVENT_LISTENER,ai,ae,ak,ag,ad);
var e=H(ae);
if(e.length>0){if(ai==="view"){t(e[0],ak,ag,ad)
}else{function aj(){aa(ak,ag,ad);
...
}}function t(ad,af,ae,ag){C.level&&C.report(C.events.ADD_DEBOUNCED_VIEW_HANDLER,ad,af,ae,ag);
var e=Y(af,ae,ag);
if(U[e]){return false
}O.push([ad,af,ae,ag]);
if(!q){C.level&&C.report(C.events.ATTACH_VIEW_LISTENER,O);
...
ae<e;
++ae){ad=O[ae];
if(ad&&M(ad[0])){C.level&&C.report(C.events.ONVIEW_TRACKING_TRIGGERED,ad[1],ad[2],ad[3]);
aa(ad[1],ad[2],ad[3])
}else{af.push(ad)
}}O=af;
C.level&&C.report(C.events.VISIBLE_CHECK_FINISHED,O);
...
}function o(ae,ad,e){if(ae.attachEvent){ae["e"+ad+e]=e;
ae[ad+e]=function(){ae["e"+ad+e](window.event)
};
ae.attachEvent("on"+ad,ae[ad+e])
}else{ae.addEventListener(ad,e,false)
}}function f(ae,ad,e){if(ae.detachEvent){ae.detachEvent("on"+ad,ae[ad+e]);
ae[ad+e]=null
}else{ae.removeEventListener(ad,e,false)
}}function k(e,ad,ae){return function(af,ag){if(u(ae,af,ag)){if(window.jQuery&&!V){P(e,ad,ae,af,ag)
}else{h(e,ad,ae,af,ag)
}}else{C.level&&C.report(C.events.WONT_ATTACH_EVENT_TRACKING,e,ad,ae,af,ag)
}}
}function I(e,ad){return{track:k(e,ad,g.experiment),stage:k(e,ad,g.stage),goal:k(e,ad,g.goal),customGoal:k(e,ad,g.customGoal)}
}function s(e,ae){for(var ad in ae){if(ae.hasOwnProperty(ad)){e[ad]=ae[ad]
}}}function F(e){d.f=e.f||{};
s(d.r,e.r||{});
s(d.t,e.t||{})
}function w(e){var ad=y(e);
return d.r[ad]||d.f[ad]||0
}function x(e){if(arguments.length>1){throw"Track only accept one parameter"
}if(!e){return 0
}return aa(g.experiment,e)
}function Q(ad,e){if(!ad){return false
}if(e===undefined){return false
}if(e===0){return x(ad)
}if(!/^[1-9]$/.test(e)){return false
}return aa(g.stage,ad,e)
}function J(e){if(!e){return false
}return aa(g.goal,e)
}function K(ad,e){if(!ad||!e||!/^[1-5]$/.test(e)){return false
}return aa(g.customGoal,ad,e)
}function G(e){if(e.url){j=e.url
}if(e.jset){F(e.jset)
}if(typeof e.useFNV!=="undefined"){ab=e.useFNV
}m=!!e.onAllSelectorElements;
if(typeof e.noJqueryOn!=="undefined"){V=e.noJqueryOn
}if(typeof e.noJqueryAjax!=="undefined"){Z=e.noJqueryAjax
}if(typeof e.ajaxHeaders!=="undefined"){W=e.ajaxHeaders
}if(typeof e.delay!=="undefined"){if(e.delay>0){L=e.delay;
l=A
}else{L=0;
l=E
}}}function r(){}r.prototype.variant=x;
r.prototype.track=x;
r.prototype.stage=Q;
r.prototype.goal=J;
r.prototype.customGoal=K;
r.prototype.on=I;
r.prototype.set=F;
r.prototype.Trackables=g;
r.prototype.configure=G;
return new r()
})
}(B.define,B.require));