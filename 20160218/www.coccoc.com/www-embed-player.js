...
function vr(){var a=G("PLAYER_CONFIG",void 0),b=G("REVERSE_MOBIUS_PERCENT",void 0);if(hg&&100*Math.random()<b)try{var c;try{var d=r("yt.www.ads.biscotti.getId_"),e;if(d)e=d();else{if(!Hq){var f=Gq().then(Iq),h=Nm(Kq,Kq,void 0);h.j=!0;Um(f,h);Hq=f}e=Hq}c=e}catch(k){c=Om(k)}Pm(c,null,Lq,void 0)}catch(k){pc(k)}if(G("REQUEST_POST_MESSAGE_ORIGIN")){if(!qr){qr=new Mi;qr.f=vr;return}qr.origin&&"*"!=qr.origin&&(a.args.post_message_origin=qr.origin)}d=document.referrer;b=G("POST_MESSAGE_ORIGIN");c=!1;u(d)&&
u(b)&&-1<d.indexOf(b)&&Mg(b)&&Mg(d)&&(c=!0);window!=window.top&&d&&d!=document.URL&&(a.args.loaderUrl=d);G("LIGHTWEIGHT_AUTOPLAY")&&(a.args.autoplay="1");a.args.autoplay&&kj(a.args);rr=Hh("player",a);d=G("POST_MESSAGE_ID","player");G("ENABLE_JS_API")?tr=new Aq(rr):G("ENABLE_POST_API")&&u(d)&&u(b)&&(sr=new Cq(window.parent,d,b),tr=new wq(rr,sr.h));(ur=c&&!G("ENABLE_CAST_API"))?a.args.disableCast="1":(a=rr,tp(),cq=a,cq.addEventListener("onReady",eq),cq.addEventListener("onRemoteReceiverSelected",gq),
dq.push(xc("yt-remote-receiver-availability-change",fq)),dq.push(xc("yt-remote-auto-connect",hq)));G("BG_P")&&(G("BG_I")||G("BG_IU"))&&Mc();ke();iq=rr;iq.addEventListener("SUBSCRIBE",lq);iq.addEventListener("UNSUBSCRIBE",oq);jq.push(Zg(ei,pq),Zg(ji,qq))}
