...
if(!window.mmcore){
	...
	inline_state:true,
	sample_rate: 100,
	whitelist: {is: ['https://ssl.bbc.co.uk/iplayer/watching'],	contains: ['https://ssl.bbc.co.uk/iplayer/watching?'], regex: [/bbc\.co\.uk(\/?(\?.*)?(#.*)?)?$/]},
	getGeneratedRandom: function () {var number = this.GetCookie('mm_random', 1); return (number || Math.ceil(Math.random() * 100));},
	isInRate: function () {return !(this.getGeneratedRandom() > this.sample_rate);},
	saveRandom: function () {mmcore.SetCookie('mm_random', this.getGeneratedRandom(), 365, 1);},
	isInWhitelist: function(){var url = window.location.href;var wl= this.whitelist;for (var i=0;i<wl.is.length;i++){if(wl.is[i]===url)return true;}
		for(var i=0;i<wl.contains.length;i++){if(url.indexOf(wl.contains[i])>-1)return true;} for(var i=0;i<wl.regex.length;i++){if(wl.regex[i].test(url))return true;}return false;},
...
mmcore.saveRandom();
if (!mmcore.isInWhitelist() || !mmcore.isInRate() || navigator.userAgent.toLowerCase().indexOf("opera") != -1) {
	mmcore.CGRequest=function (args) {return false;};
