...
this.highlightFromNavigation(f)
},highlightFromNavigation:function(i,h){h=h||{};
...
},highlightFromAutocomplete:function(f){f=f||{};
...
function i(m){if(h.MD(a[0])){m+=b[1];
h.MN("index",0);
m+=b[1];
var p=(h.MC(a[0])||[]);
f.unshift(null);
...
}},modelInit:function(){if(this.model._ssce){var f={ss:this.input.value};
...
this.autocomplete()
}}},modelValidation:function(f){if(this.model._ssce&&f&&f.group=="destination"){this.$errors.filter(".-visible").removeClass("-visible");
...
},inputFocus:function(){if(this.model.destination.getOption("gpf")){B.et.stage("PVdIEUGcfdDJCHT",1);
if(!B.et.track("PVdIEUGcfdDJCHT")){this.model.destination.initPredictions()
}}a.prototype.inputFocus.apply(this,arguments)
},listItemClick:function(g){if(this.options.ssf0){var f=$(g.currentTarget);
...
},choose:function(f){if(this.options.ssf0&&f===0){B.et.customGoal("PVdINcOdAVSaLaO",1)
...
},initPredictions:function(){if(this.options.gpf){var g=B.require("search/destination/service-atlas");
var f={};
...
return q
}a.exports=p.extend({init:function(q,u){q=q||{};
...
this.init(r,function(){this.atlas.autocomplete(u,t,s)
}.bind(this));
...
},validate:function(){if(!this._ssce){return true
...
}},onlyCurrentCountryChange:function(){this.model.use("destination");
...
var k=[];
var i=false;
m.forEach(function(n){this.hiddenFields.push({name:n.name,value:n.value});
k.push(n.name);
...
},formSubmit:function(j){if(this.model._ssce){var i=this.model.validate();
