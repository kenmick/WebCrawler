...
}h&&h.call&&h.call(e,{type:d,data:f,stopPropagation:function(){_i_("158bb:919");c=true
;_r_()}})
});
return _r_( this);
};
...
;_r_()});
j.click(function(n){_i_("158bb:826");f(this).select()
...
B.eventEmitter.bind("SEARCH:date_changed",function(){_i_("158bb:802");f._updateDates()
;_r_()});
this.on("dates-changed",function(g){_i_("158bb:803");B.search.dates("checkin",g.checkin);
B.search.dates("checkout",g.checkout)
;_r_()});
this.globalEmitter.bind("dates-calendar-close",function(){_i_("158bb:804");f.checkoutSelector.getCalendarElement().trigger("hide");
f.checkinSelector.getCalendarElement().trigger("hide")
;_r_()});
this.checkinSelector.on("calendar-date-selected",function(g){_i_("158bb:805");setTimeout(function(){_i_("158bb:920");f.checkoutSelector.getCalendarElement().click()
;_r_()},0)
;_r_()})
...
;_r_()}})
;_r_()});
B.define("xsell/apis/restaurants",["et"],function(a){_i_("158bb:12");return _r_({searchByBoundingBox:function(d){_i_("158bb:319");var c=new B.promise();
var b=d.SW[1]+","+d.SW[0]+","+d.NE[1]+","+d.NE[0];
$.ajax({dataType:"json",url:"/ot_markers_on_map",data:{aid:B.env.b_aid,sid:B.env.b_sid,lang:B.env.b_lang,cc1:B.env.b_countrycode,BBOX:b},success:function(e){_i_("158bb:806");c.fulfill(e)
;_r_()},error:function(){_i_("158bb:807");c.reject()
;_r_()}});
return _r_( c);
}});
});
...
return _r_({init:function(c){_i_("158bb:692");this.experimentHash="BKbVPAJeGZQEeCRe";
this.map=c;
this.map.on(this.map.EVENTS.LOAD,(function(){_i_("158bb:921");this.loadRestaurants().then((function(){_i_("158bb:973");if(this.restaurants.length){B.et.stage(this.experimentHash,2);
...
;_r_()}).bind(this))
;_r_()},render:function(f){_i_("158bb:693");var d=document.createElement("div"),e=B.jstmpl("fe_xsell_opentable_map_control").render(),c=google.maps.ControlPosition.RIGHT_BOTTOM;
d.className="ot-control--position-bottom-right";
d.index=1;
d.innerHTML=e;
this.map.Interface.map.controls[c].push(d);
return _r_( a(d));
},loadRestaurants:function(){_i_("158bb:694");this.bbox=this.map.getBoundingBox();
return _r_( b.searchByBoundingBox(this.bbox).then((function(c){_i_("158bb:922");this.restaurants=c
;_r_()}).bind(this)));
},generateMarkers:function(){_i_("158bb:695");a.each(this.restaurants,(function(d,c){_i_("158bb:923");var e=c.b_restaurant_latitude+","+c.b_restaurant_latitude;
...
;_r_()},updateRestaurants:function(){_i_("158bb:696");var c=this.map.getBoundingBox();
B.et.customGoal(this.experimentHash,1);
if(JSON.stringify(c)===JSON.stringify(this.bbox)){this.generateMarkers();
this.toggleMarkers()
}else{this.loadRestaurants().then((function(){_i_("158bb:952");this.generateMarkers();
this.toggleMarkers()
;_r_()}).bind(this))
};_r_()},toggleMarkers:function(){_i_("158bb:697");if(this.$cb.attr("checked")){this.map.addMarkers(this.markers)
}else{var c=this.map.getActiveMarkers();
a.each(c,(function(d,e){_i_("158bb:953");if(e.b_type==="opentable_small"){this.map.clearMarker(e.b_id)
};_r_()}).bind(this))
};_r_()},_trackMarkerHover:function(c){_i_("158bb:698");if(c.data.b_type==="opentable_small"){B.et.customGoal(this.experimentHash,2)
};_r_()},_trackMarkerClick:function(c){_i_("158bb:699");if(c.data.b_type==="opentable_small"){B.et.customGoal(this.experimentHash,3)
};_r_()},_addEventListeners:function(){_i_("158bb:700");this.$el.find('input[type="checkbox"]').on("change",this.updateRestaurants.bind(this,this.map));
...
d.each(i,function(l,j){_i_("158bb:931");e.itemsPoplarHotelNames[l]=j.hotel_name
;_r_()});
e.queues[g]=i;
...
return _r_();
}if($(this).next(".hotels_in_city").hasClass("closed")){$(".hotels_in_city.opened").each(function(){_i_("158bb:827");booking.google.trackEvent(booking.google.clickTracker,"DestThemes","City Open HList - "+a);
...
if(B.env.b_action=="index"){B.when({events:"view .dcbi-countries"}).run(function(){_i_("158bb:614");$(".dcbi-countries .dcbi-country__container[data-img-url]").each(function(){_i_("158bb:828");$(this).removeClass("dcbi-country__desc_placeholder_load");
...
k=window.setTimeout(function(){_i_("158bb:829");h.find(".searchform-subscribe-box-warns-denied").fadeOut()
;_r_()},e);
c()
;_r_()};
...
f=window.setTimeout(function(){_i_("158bb:830");h.find(".searchform-subscribe-box-warns-success").fadeOut()
;_r_()},e);
c()
;_r_()};
...
r.delegate(".checkAvailabilitySwitch","click",function(t){_i_("158bb:808");t.stopPropagation();
...
b();
i();
j("#checkAvailabilityBtn").click(function(){_i_("158bb:809");var u=j(this).parent().parent().parent().parent().prev().attr("data-url");
...
}var s={type:"checkin",defaultDate:d.search.dates("checkin"),startDate:g(),endDate:f(),dayNames:r,monthNames:t,sundayFirst:w,title:l.transl_checkin_title,monthTitle:a,direction:l.rtl?"rtl":"ltr",onDateSelected:function(E,A){_i_("158bb:810");var G={year:A.getYear(),month:A.getMonth(),date:A.getDate()};
...
h()
};_r_()},onShow:function(C,A){_i_("158bb:811");if(C.options.type=="checkout"){return _r_();
...
d.on(d.UI_BLOCK_UPDATED,function(f){_i_("158bb:831");if(f.id==="search_history"){b()
};_r_()})
};_r_()}});
}(booking,jQuery));
...
g.bind("click",function(k){_i_("158bb:832");k.preventDefault();
...
h.bind("click",function(k){_i_("158bb:833");k.preventDefault();
var i=b(this),j=false;
...
z=z||{};
if(B.atlas.getVariant("ebcGcTGPbfRdcC")===2){z=x.enforceDataFormat(z,function(E){_i_("158bb:954");if(/^hotel/.test(E.b_type)&&!E.b_states.soldout&&E.b_u_total_price){E.b_states.overlay=true
};_r_()})
...
}if(B.atlas.getVariant("ebcBUaUSPaGXONCCCeSaT")||B.atlas.getVariant("ebcOLWNTJRLXTcMMOeBRe")){B.atlas.require(["ufi-toggle-polygon"],function(E){_i_("158bb:932");E.initialize({districtList:z,map:x})
;_r_()})
...
}if(B.atlas.getVariant("ebcBUaeDeeELSEO")===2||B.atlas.getVariant("ebcOLWeOeUPfBHT")===2||B.atlas.getVariant("ebcfEJOYeeZAAZKe")===2){B.atlas.require(["district-label"],function(E){_i_("158bb:966");E.add({map:x,districts:z.b_official_districts})
;_r_()})
}}}if(B.atlas.getVariant("ebcOLUTBQMQAbDPNOSdYKe")){w=y.filter(function(E){_i_("158bb:955");if(E.b_states.searchresults){return _r_( true);
}else{return _r_( false);
}});
if(w.length){B.et.customGoal("ebcOLUTBQMQAbDPNOSdYKe",1)
}else{B.et.customGoal("ebcOLUTBQMQAbDPNOSdYKe",2)
}}if(B.atlas.getVariant("ebcOLUTBQMQAbDPNOSdYKe")){y=y.map(function(E){_i_("158bb:956");if(E.b_states.searchresults){E.b_persistent=true
}return _r_( E);
})
...
}j.setTimeout=setTimeout(function(){_i_("158bb:812");y.setIconHover(w.id);
...
}y.getIW(w.id,function(A){_i_("158bb:977");if(x&&y.get("hoverMarker")!=A.b_id){return _r_();
...
}if(p.get("action")==="hotel"&&/^hotel.*/.test(w.data.b_type)&&!/current/.test(w.data.b_type)){setTimeout(function(){_i_("158bb:834");z.setIconVisited(x.id)
;_r_()},100)
...
if(p.get("action")=="hotel"){x=function(){_i_("158bb:835");g.call(A,C,y,true);
...
if(A.get("fixedIw")&&/hotel.*/.test(w.data.b_type)){window.setTimeout(function(){_i_("158bb:933");A.trigger("iw-control-show",{markerId:y,data:w.data});
...
}j.setTimeout=setTimeout(function(){_i_("158bb:924");A.closeIW()
;_r_()},z)
;_r_()})
...
}if(h){x.on("load",function(){_i_("158bb:836");x.trigger("load-dom-storage-supported")
;_r_()})
...
;_r_()}function o(F){_i_("158bb:398");h(document).on("click",s,function(){_i_("158bb:837");var G=h(this);
var H=G.data("source");
...
h(document).keyup(function(F){_i_("158bb:750");if(F.which===27){setTimeout(function(){_i_("158bb:934");p()
;_r_()},1)
};_r_()});
...
}if(!k){h(window).bind("hashchange",function(){_i_("158bb:838");return _r_((d.hash.indexOf(y)>-1)?n():q());
})
...
H.on("marker-out",function(J){_i_("158bb:752");I=setTimeout(function(){_i_("158bb:925");var K=H.getMarker(J.id);
...
if(!K){return _r_();
}if(F==K){I=setTimeout(function(){_i_("158bb:957");J=H.getMarker(K);
...
var j=new l();
var e=[{event:"map-open",source:function(m){_i_("158bb:813");return _r_( m);
},action:"show_map"},{event:"map-close",source:function(m){_i_("158bb:814");return _r_( m);
},action:"close_map"}];
var i=[{event:"marker-click",source:function(m){_i_("158bb:815");return _r_( m.data.b_type);
},action:"marker_click"},{event:"marker-hover",source:function(m){_i_("158bb:816");return _r_( m.data.b_type);
},action:"marker_hover"},{event:"iw-click-title",source:"title",action:"iw_click"},{event:"iw-click-button",source:"reserve_button",action:"iw_click"},{event:"iw-close.button",source:"close_button",action:"iw_close"},{event:"zoomin.button",source:"zoom_control",action:"zoomin"},{event:"zoomout.button",source:"zoom_control",action:"zoomout"},{event:"zoom",source:"all",action:function(n){_i_("158bb:817");var m;
if(n>k){m="zoomin"
}else{m="zoomout"
}k=n;
return _r_( m);
}},{event:"doubleclick",source:"doubleclick",action:"zoomin"},{event:"map-type-change",source:"maptype_control",action:"maptype_change",value:function(m){_i_("158bb:818");var n={hybrid:1,roadmap:2,satellite:3,terrain:4};
return _r_( n[m]);
...
Object.keys(p).forEach(function(r){_i_("158bb:958");if(typeof p[r]==="function"){p[r]=p[r](q)
};_r_()});
h(p)
;_r_()})
;_r_()})
...
var l=false;
b.each(p,function(u){_i_("158bb:839");var s=new e(u,k,n),v=true;
var t=false,q=false,r;
if(!i.length||u.b_persistent){i.push(u);
j.push(s)
}else{if(!m||i.length<=m){if(B.atlas.getVariant("ebcBUECAFWTYcUNSVGPQJJYJO")||B.atlas.getVariant("ebcOLHMbdIePQNZBaQDbAAPVT")||B.atlas.getVariant("ebcMMfEDXPdLbeMSSdPIFdELSVWe")||B.atlas.getVariant("ebcMbfWWJbVHMbdIePQNZBaQDbAAPVT")){b.each(j,function(x,w){_i_("158bb:974");if(s.intersects(x,o)){if(d(u,i[w])>0){r=w;
t=true;
l=true
}else{q=true
}};_r_()});
...
j.push(s)
}}}else{b.each(j,function(x,w){_i_("158bb:975");if(s.intersects(x,o)){v=false;
if(d(u,i[w])>0){l=true
}};_r_()});
if(v){i.push(u);
j.push(s)
...
this.$wrapper.on("click",".maps-iw-image-gal-nav",function(d){_i_("158bb:840");d.preventDefault();
var c=$(this);
...
;_r_()});
this.$wrapper.on("click",".maps-iw-image-gal-nav-bullets li",function(){_i_("158bb:841");b.galPos=$(this).index();
b.changeImage()
;_r_()})
;_r_()};
...
$(".maps-iw-image-gal-view .maps-iw-image-gal-images li").eq(this.galPos).addClass("current").hide().fadeIn(200,function(){_i_("158bb:842");$(".maps-iw-image-gal-view .maps-iw-image-gal-images li.previous").removeClass("previous")
;_r_()});
...
this.modulesList.forEach(function(i){_i_("158bb:843");var j=B.atlas.require(i);
...
e=this.animTime;
k.css({left:j}).animate({left:i+"px"},e,function(){_i_("158bb:844");h.isOpen=true;
...
}function a(n){_i_("158bb:445");if(!n instanceof b){return _r_();
...
function g(A,y){_i_("158bb:637");var j=new window.google.maps.LatLng(A.options.coordinates[0],A.options.coordinates[1]),w=h.fromLatLngToDivPixel(j),x=A.domNode.offsetWidth,z=A.domNode.offsetHeight,r={x:w.x-x/2,y:w.y},q={x:w.x+x/2,y:w.y+z};
...
this.map.on("bounds-change",function(){_i_("158bb:845");g._close()
;_r_()});
h.delegate(".iw-close","click",function(i){_i_("158bb:846");i.preventDefault();
g._close();
g.map.trigger("iw-close.button")
;_r_()});
h.delegate(".js-map-hotel__link","click",function(){_i_("158bb:907");g.map.trigger("iw-click-title")
;_r_()}).delegate(".js-map-hotel__button","click",function(){_i_("158bb:847");g.map.trigger("iw-click-button")
;_r_()})
;_r_()};
...
$(".iw-control").animate({opacity:0},200,function(){_i_("158bb:908");g&&k.html(h);
if(B.atlas.getVariant("ebcBUYSaBcLMeDPISRO")||B.atlas.getVariant("ebcOQPBRNQadeOJATXT")){b(k)
...
i.$wrapper.on("click",".maps-toggle-review-entry",function(){_i_("158bb:848");g.slideToggle(400);
...
;_r_()};
function b(g){_i_("158bb:447");setTimeout(function(){_i_("158bb:772");g.find(".maps-iw-just-booked").addClass("maps-iw-just-booked-fade-in")
;_r_()},10)
;_r_()}return _r_( c);
})
});
...
}function h(j){_i_("158bb:773");var k=j.data,l=k.b_id;
...
i.setIconActive(l)
}i.on("iw-control-show",function(l){_i_("158bb:849");var j=l.markerId,k=l.data;
g.getIWFromMap(j,k);
...
if(j){i.getIW(g,function(k){_i_("158bb:909");b.trigger("add",k)
;_r_()})
...
10904;
l.forEach(function(n){_i_("158bb:774");if(!d(n)){return _r_();
...
j.forEach(function(n){_i_("158bb:775");k.Interface.setMixedMarkerType(n.b_id,k[e(n)](n?n.iconType||n.b_type:""))
;_r_()});
i.forEach(function(n){_i_("158bb:776");k.Interface.setMixedMarkerType(n.b_id,k[e(n)](n?n.iconType||n.b_type:""))
;_r_()})
...
b.some(function(l){_i_("158bb:777");var k=false;
k=Object.keys(l.cond).map(function(o){_i_("158bb:926");var n=false,m=i.b_behaviors[o]||false;
if(m===l.cond[o]){n=true
}return _r_( n);
});
k=k.reduce(function(m,n){_i_("158bb:927");return _r_( m===n&&n===true);
});
if(k){j=l.applyIcon
}return _r_( k);
});
...
v.setZoom(w.zoom)
};_r_()}q.pubSub.on("open",function(w){_i_("158bb:778");if(!v){l(k.extend(j,w))
}else{v.done(function(){_i_("158bb:935");u(k.extend(j,w))
;_r_()})
}if(v){v.trigger("open")
};_r_()});
q.pubSub.on("close",function(){_i_("158bb:779");if(v){v.trigger("close")
};_r_()});
q.addEvents();
b.atlas.require(["google-analytics"],function(w){_i_("158bb:780");w.trackPage(q)
;_r_()});
function l(w){_i_("158bb:654");v=new o({modules:t,options:j});
v.set("fixedIw",w.fixedIw);
v.once("autoopen",function(){_i_("158bb:850");v.once("close",function(){_i_("158bb:936");v.once("open",function(){_i_("158bb:967");v.trigger("map-close-iw-open")
;_r_()})
;_r_()})
;_r_()});
v.done(function(){_i_("158bb:851");if(b.env.b_lang=="en"&&(b.env.b_countrycode=="gb"||b.env.b_countrycode=="us")){b.et.stage("BKbVPAJeGZQEeCRe",1);
if(b.et.variant("BKbVPAJeGZQEeCRe")>0){h.init(v)
}}function y(){_i_("158bb:910");k("#b_map_container").find(".iw-overlay").loadComponents();
k("#b_map_container").find(".iw-control").loadComponents()
;_r_()}v.on("iw-open",y);
v.on("iw-load",y);
k("#b_map_container").delegate(".iw-overlay-city a, .iw-overlay-airport a","click",function(){_i_("158bb:937");v.trigger("nonproperty-iw-link-click")
;_r_()});
if(b.atlas.getVariant("ebcfEFQUETOBXRZcO")){b.atlas.require(["best-areas-poly"],function(z){_i_("158bb:959");z.init({topOverlay:j.topOverlay,map:v})
;_r_()})
}if(booking.browserStorageHandler&&booking.browserStorageHandler.isLocalStorageSupported()){v.trigger("load-map-tip");
if(b.atlas.getVariant("ebcfESfRe")){b.atlas.require(["tip-block"],function(z){_i_("158bb:968");new z({map:v,topOverlay:w.topOverlay,modules:["tip-marker-click"]})
;_r_()})
}}function x(A,G,z){_i_("158bb:911");var K,I=35,F=56,C=5,J=27,H=v.$domNode,E={w:H.width(),h:H.height()},D={w:A.outerWidth(),h:A.outerHeight()};
F=(F>E.w-z.x-C)?C:F;
...
}if(r.get("isCustomZoom")){b.atlas.require(["zoom-control"],function(z){_i_("158bb:960");new z({map:v})
;_r_()})
}i.init(v);
b.atlas.require(["styler","styler-theme"],function(z,C){_i_("158bb:938");if(typeof C!=="undefined"){var A=new z({map:v,theme:C});
A.init()
};_r_()});
if(w.fixedIw){b.atlas.require(["iw-control"],function(z){_i_("158bb:961");new z({map:v,topOverlay:c})
;_r_()})
}v.trigger("ready-for-autoopen")
;_r_()});
b.atlas.require(["google-analytics"],function(x){_i_("158bb:852");x.trackMap(v)
;_r_()})
;_r_()};_r_()})
};_r_()}(window.booking));
...
var f=d.extend({},g,e);
return _r_( this.each(function(){_i_("158bb:781");var h=d(this);
d(f.$tabButton,this).bind("click",function(){_i_("158bb:912");var k=d(this);
...
var e=c.extend({},f,d);
return _r_( this.each(function(){_i_("158bb:655");c(this).bind({mouseover:function(){_i_("158bb:913");c(this).addClass("active").find("*").addClass("active")
;_r_()},mouseleave:function(){_i_("158bb:914");c(this).removeClass("active").find("*").removeClass("active")
;_r_()}})
;_r_()}));
}
;_r_()})(jQuery,window,document);
...
;_r_()}};
booking[sNSStartup].promotion_shortlist_interacts={init:function(){_i_("158bb:120");$("#home_featured_destinations").delegate("tr.dotter","click",function(c){_i_("158bb:783");var d=$(this).data("url");
...
}$.ajax({url:"/region_attractions",data:{dest_type:B.env.b_action,dest_id:B.env.b_dest_id,lang:B.env.b_lang_for_url,stype:booking.env.b_site_type_id,aid:B.env.b_aid},success:function(o){_i_("158bb:820");if(o&&o.b_items&&o.b_items.length){var m="";
var l,n;
...
;_r_()})
;_r_()};
var a=function(){_i_("158bb:345");if(d("#lp_animated_accomodation_tabs_track").length){d(".consolidated-tabs").find("a").bind("click",function(){_i_("158bb:821");b(1)
;_r_()});
c()
};_r_()};
...
d(".lp_animated_accomodations_switch_list").delegate("li.item","mouseenter",function(){_i_("158bb:928");var k=d(this).data("left")+"%";
h.css({left:k});
...
;_r_()}).delegate("li.item","mousedown",function(){_i_("158bb:822");if(!d(this).hasClass("active")){h.addClass("pressed")
...
z.find(".lp_bold_date_picker_group_container").click(function(E){_i_("158bb:784");var D=f(this).find(".main_guests_selector"),C=f(this).find(".lp_bold_date_picker_select");
if(!D.is(":visible")){E.stopPropagation();
D.show();
C.addClass("focus");
f.each(h(),function(){_i_("158bb:939");this.trigger("hide")
;_r_()});
f(document).one("click",function(){_i_("158bb:940");D.hide();
C.removeClass("focus")
;_r_()})
...
y.find(".lp_bold_date_picker_lightbox_secondary_cta, .lp_bold_date_picker_lightbox_close_button").click(function(){_i_("158bb:853");x.click()
...
};_r_()}function b(w){_i_("158bb:140");return _r_({rooms:w.find(".room_guests_selector input").val(),adults:w.find(".adults_guests_selector input").val(),children:w.find(".children_guests_selector input").val(),childrenAges:w.find(".age_guests_selector:visible input").map(function(){_i_("158bb:785");return _r_( parseInt(f(this).val()));
}).toArray()});
...
f.width(r+35).fadeIn(function(){_i_("158bb:941");f.trigger("focus")
;_r_()}).find(".dsf_social_proof").show()
}else{if(t&&q&&!e){i()
}}}function i(o){_i_("158bb:496");e=true;
f.stop(true,true).fadeOut(function(){_i_("158bb:786");e=false
;_r_()})
...
b=Math.ceil(g/k);
o=a();
f(".lp_horizontal_promotions_wrapper").delegate(".lp_horizontal_promotions_button","click",function(q){_i_("158bb:787");var r=f(this).hasClass("next")?1:-1;
h(r,true);
...
if(d.track.getVariant(e)==2){f(window).resize(function(){_i_("158bb:788");var q,r;
...
var n=Math.max.apply(Math,l.map(function(p,q){_i_("158bb:962");return _r_( b(q).height());
}).toArray());
l.find("> .lp_promotion_cards_list_holder").each(function(q,p){_i_("158bb:915");var r=parseInt(b(p).css("paddingBottom"),10);
b(p).height(n-r)
;_r_()})
};_r_()})
}else{var j=g.find(".lp_promotion_cards_list_holder").css("height","auto");
var h=Math.max.apply(Math,j.map(function(l,m){_i_("158bb:854");return _r_( b(m).height());
}).toArray());
j.height(h-5)
};_r_()}function d(){_i_("158bb:152");var g=this.name;
b(".lp_promotions_cards_list").delegate(".lp_promotion_cards_list_child_tooltipbtn","mouseenter mouseleave",function(m){_i_("158bb:789");m.preventDefault();
m.stopPropagation();
var l=b(this);
var i=l.data("rel");
var k=b(i).html();
...
n._createArrow=function(){_i_("158bb:942");var o=this.root();
return _r_( b('<i class="fly-dropdown__arrow"><i>').appendTo(o));
};
n._position=function(){_i_("158bb:943");var q=this._rect(this.root());
var w=this._rect(j);
var p=this._rect(l);
...
setTimeout(function(){_i_("158bb:855");h=true
;_r_()},f);
...
setTimeout(function(){_i_("158bb:856");if(k){j()
};_r_()})
;_r_()});
...
c(".lp-sh-manage").delegate(".lp-sh-action","click",function(h){_i_("158bb:792");e.call(this,c(this).data());
h.preventDefault()
...
aG(".fancybox-inline-tmp").unbind("fancybox-cancel").bind("fancybox-change",function(){_i_("158bb:858");aG(this).replaceWith(aA.children())
;_r_()});
...
if(!ap.is(":visible")){ab&&aG("select:not(#fancybox-tmp select)").filter(function(){_i_("158bb:964");return _r_( this.style.visibility!=="hidden");
}).css({visibility:"hidden"}).one("fancybox-cleanup",function(){_i_("158bb:929");this.style.visibility="inherit"
;_r_()});
ap.show()
}}else{ap.hide()
}aB=g();
...
aA.fadeTo(aF.changeFade,0.3,function(){_i_("158bb:859");var e=function(){_i_("158bb:944");aA.html(ay.contents()).fadeTo(aF.changeFade,1,H)
;_r_()};
...
}}};_r_()},c=function(){_i_("158bb:286");if(aF.enableEscapeButton||aF.enableKeyboardNav){aG(document).bind("keydown.fb",function(b){_i_("158bb:793");if(b.keyCode==27&&aF.enableEscapeButton){b.preventDefault();
aG.fancybox.close()
...
am.click(function(b){_i_("158bb:794");b.preventDefault();
aG.fancybox.prev()
;_r_()});
al.click(function(b){_i_("158bb:795");b.preventDefault();
aG.fancybox.next()
;_r_()});
aG.fn.mousewheel&&aD.bind("mousewheel.fb",function(b,d){_i_("158bb:823");if(aC){b.preventDefault()
...
$images.fancybox({centerOnScroll:"true",cyclic:"true",overlayColor:"#000",overlayOpacity:"0.5",titlePosition:"over",transitionIn:"fade",transitionOut:"fade",titleFormat:function(k,j,h,i){_i_("158bb:796");return _r_('<span id="fancybox-title-over">'+(h+1)+" / "+j.length+(k.length?" &nbsp; "+k:"")+"</span>");
}})
...
}B.atlas.define("lp-show-landmarks",function(a){_i_("158bb:719");function b(){_i_("158bb:824");var c=this,e="fEJMSeCXeLfHHHYbNKe",d=[],f=5;
c.done(function(){_i_("158bb:930");c.on("markers-get",function(j,g){_i_("158bb:965");if(B.track.getVariant(e)&&j&&j.b_landmarks&&j.b_landmarks.length){for(var h=0;
h<d.length;
h++){if(d[h].b_id){c.clearMarker(d[h].b_id)
}}d=j.b_landmarks.slice(0,f);
setTimeout(function(){_i_("158bb:978");c.addMarkers(d)
;_r_()},1)
};_r_()})
;_r_()})
...
h.showLeavingMessage_=function(){_i_("158bb:797");d()
;_r_()}
...
}function g(i){_i_("158bb:515");if(i){h.html(i).find(".similar_elements a").click(function(k){_i_("158bb:860");k.preventDefault();
...
setTimeout(function(){_i_("158bb:916");g=true
;_r_()},500);
f()
};_r_()})
...
j.map(function(){_i_("158bb:798");n=Math.max(n,a(this).find(".lp_cp_top_experiences_from_guides_list_item_content_wrapper").height())
;_r_()});
...
if(e.length){a(".lp_sh_show_expand_wrapper").find("#showAllHistory").click(function(){_i_("158bb:799");a(this).toggleClass("expanded");
e.slideToggle();
...
setTimeout(function(){_i_("158bb:861");g=true
;_r_()},e);
f(a(".lp_most_wishlisted_promotions_cards_list"))
};_r_()})
...
}function g(j){_i_("158bb:601");return _r_( $.map(j,function(k){_i_("158bb:825");return _r_( $.extend({},k,{name:d(k.name,18)}));
}));
}var f=$.fly.dropdown.extend({defaults:{gaAction:"Wishlist",extraClass:"wl-dropdown",content:function(j){_i_("158bb:917");var k=this;
h.fetch(this.params).then(function(l){_i_("158bb:963");j(k.tmpl(l.result))
;_r_()})
;_r_()}},timeout:null,init:function(){_i_("158bb:862");var k=this;
...
this.bindData();
k.handle().bind("mouseenter",function(){_i_("158bb:945");h.fetch(k.params)
;_r_()});
k.bind("show hide",function(l){_i_("158bb:946");setTimeout(function(){_i_("158bb:969");booking.eventEmitter.trigger("wl-dropdown-toggle",{node:k.root(),state:l.type==="show"})
;_r_()},0);
$("#tooltip_wrap").hide()
;_r_()})
;_r_()},bindRoot:function(){_i_("158bb:863");var j=this;
this.bind(this.events.rootready,function(){_i_("158bb:947");$.extend(j.options,{gaLabel:j.handle().attr("data-ga-label"),position:j.handle().attr("data-position")});
j.root().delegate(".js-wl-dropdown-item-text","keyup",$.proxy(j.ontext,j)).delegate(".js-wl-dropdown-item-checkbox","change",$.proxy(j.oncheckbox,j)).bind("mouseenter",function(){_i_("158bb:976");clearTimeout(j.timeout)
;_r_()}).bind("mouseleave",function(){_i_("158bb:970");j.timeout=setTimeout($.proxy(j.hide,j),2000)
;_r_()});
j.bind(j.events.show,function(){_i_("158bb:971");clearTimeout(j.timeout)
;_r_()})
;_r_()})
;_r_()},bindData:function(){_i_("158bb:864");var j=this;
h.bind([i.LOADINGSTART,i.LOADINGEND].join(" "),function(k,l){_i_("158bb:948");var m=l.params;
if(String(m.hotel_id)!==String(j.params.hotel_id)||!m.lists&&!m.name){return _r_();
}j.loading(k.type===i.LOADINGSTART,m.lists)
});
h.bind([i.CREATE,i.EDITHOTEL].join(" "),function(k,m){_i_("158bb:949");if(String(m.params.hotel_id)!==String(j.params.hotel_id)){return _r_();
}if(m.result.success){var l=!!m.params.new_state;
j.handle().toggleClass("saved_in_wl",l);
j.updateSprite(l)
}h.fetch(j.params).then(function(p){_i_("158bb:972");var o=$.grep(p.result.lists,function(r){_i_("158bb:979");return _r_( r.selected);
});
...
;_r_()},updateSprite:function(j){_i_("158bb:865");if(this.handle().hasClass("added_to_fav_lists")){if(B.env.b_is_villa_site){this.handle().find(".icon").toggleClass("icon-wishlist-selected",j).toggleClass("icon-wishlist",!j)
...
};_r_()},ontext:function(j){_i_("158bb:866");var l=$(j.currentTarget);
var k=l.parent();
k.find(".js-wl-dropdown-item-checkbox").attr("disabled",!l.val());
if(j.which===13){j.preventDefault();
this.createList(k)
};_r_()},oncheckbox:function(j){_i_("158bb:867");j.preventDefault();
var k=$(j.currentTarget).parent();
var l=k.find(".js-wl-dropdown-item-text");
this[l[0]?"createList":"toggleList"](k)
;_r_()},createList:function(j){_i_("158bb:868");var l=this;
var k=j.find(".js-wl-dropdown-item-text");
h.create({name:k.val(),hotel_id:this.params.hotel_id}).then(function(o){_i_("158bb:950");var n=o.result,m="";
n.selected=true;
m=$("<div/>").html(l.tmpl({lists:[n]}));
j.before(m.find(".js-wl-dropdown-item"));
j.replaceWith(m.find(".js-wl-dropdown-item-new"))
;_r_()})
;_r_()},checked:function(j,k){_i_("158bb:869");if(typeof k!=="undefined"){j.get(0).checked=k;
j.parent(".wl-dropdown-item").toggleClass("wl-dropdown-item--added",k)
}return _r_( j.get(0).checked);
},toggleListLink:function(j,k){_i_("158bb:870");j.find("span").toggleClass("g-hidden",!k);
this.root().css(this._position())
;_r_()},toggleList:function(k){_i_("158bb:871");var l=this;
var j=k.find(".js-wl-dropdown-item-checkbox");
h.editHotel({lists:j.attr("data-list-id"),hotel_id:l.params.hotel_id,new_state:Number(l.checked(j))}).then(function(){_i_("158bb:951");l.toggleListLink(k,l.checked(j))
;_r_()})
;_r_()},loading:function(k,l){_i_("158bb:872");var j=this.root().find("[data-list-id="+(l||'""')+"]").parent();
j.toggleClass("wl-dropdown-item_loading",k);
this.handle().toggleClass("wl-dropdown-handle_loading",k)
;_r_()},tmpl:function(j){_i_("158bb:873");return _r_( B.jstmpl("lists_hotel_dropdown").render($.extend(j,{lists:g(j.lists),b_user_auth_level_is_low_or_high:B.env.b_user_auth_level_is_low_or_high,b_action:B.env.b_action,b_is_tdot_traffic:B.env.b_is_tdot_traffic})));
}});
return _r_({wlDropdown:f,create:function(){_i_("158bb:800");$(".js-wl-dropdown-handle").each(function j(){_i_("158bb:918");f.create(this)
;_r_()})
;_r_()},init:function(){_i_("158bb:801");this.create()
;_r_()}});
})()
;_r_()});
...
e.bind(f,function(h,i){_i_("158bb:874");if(i.result&&i.result.success){g.updateState(i.params.hotel_id,Boolean(i.params.new_state))
};_r_()})
}}
;_r_()})
...
e=setTimeout(function(){_i_("158bb:875");b.addClass("book-challenge__stamp--current");
