window.onload = function () {
    ...
    var iteration = 0;
    var nb_iterations = 2;
    var bgExit = document.getElementById('link');
    ...
    tl.from(product1, 0.5, {top: 140, autoAlpha: 0}, "-=0.4");
    tl.from(women, 0.5, {bottom: -40, autoAlpha: 0}, "-=0.2");

    // screen 2
    tl.to(women, 0.4, {bottom: -100, autoAlpha: 0}, "+=1.5");
    tl.to(texts[0], 0.5, {top: 140, autoAlpha: 0}, "-=0.4");
    tl.to(product1, 0.5, {top: 140, autoAlpha: 0}, "-=0.5");
    ...
    var order = [6, 1, 5, 2, 4, 3, 7];
    var key;
    for (key in order) {
        var num = order[key];
        if (order.hasOwnProperty(key)) {
            var random_top = Math.floor((Math.random() * 150) + 20);
            tl.from(screen2_products[num-1], 0.4, {
                top: "-=" + random_top,
                autoAlpha: 0,
                ease: Circ.easeOut
            }, "-=0.32");
