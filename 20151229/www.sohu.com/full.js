...
*/
define("sjs/matrix/ad/form/full", ["sjs/matrix/ad/form/common", "sjs/data/cookie", "sjs/data/json","sjs/matrix/ad/special","sjs/flash/swfobject","sjs/matrix/ad/stat"], function(Common, cookie, JSON, special,swfobject,stat){
	var $ = jQuery;
	...
	var Full = Common.create({
		...
		init: function(){
			...
			if(!!adObj[adObj.special.dict.third].text){
				...
				stat.get("pv",thirdsource);//æçcube-pv
				adObj.trigger('afterload',this.isDue);
			...
			window['SOHU_AD_' + 'full' + '_cube'] = function(){
		...
		loadSelfFull:function(opt,adObj){
			...
			if (this.isDue){
				...
				//ç«æ¯éæ­
				container.bind("replay",function(){
					clearTimeout(opt.close_timer);
					self.play();
					stat.get("pv",adObj);//cube
					stat.get(adObj.imp);//ad_plus
				})
					return;
			}
			...
			this.play();
			this.adObject.trigger('afterload',this.isDue);
