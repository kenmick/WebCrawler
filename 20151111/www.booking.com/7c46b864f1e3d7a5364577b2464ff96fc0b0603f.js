...
b("et_trackables",[],function(){var c={experiment:"e",stage:"s",goal:"g",custom_goal:"cg"};
return c
});
b("et_view_tracking_events",[],function(){return["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"]
});
...
break;
case f.custom_goal:q.push(p.hash+"|"+p.id);
break;
default:d.level&&d.report(d.events.UNABLE_TO_STRINGIFY,p)
}}return"exps="+r.join(",")+";what="+s.join(",")+";ncustom="+q.join(",")+";stage="+t.join(",")
...
b("et",["send_beacon","is_visible","et_debug","request_info","et_trackables","et_view_tracking_events"],function(t,x,v,z,L,i){var l=booking.jset,s={},g=300,m={},p=[],k=false,D=false,K,E=booking.env||{};
if(!l){l={}
}if(!l.r){l.r={}
}if(!l.f){l.f={}
}if(!l.t){l.t={}
}function e(O,N,P){var M;
if(O===L.experiment||O===L.goal){M=[O,N]
}else{M=[O,N,P]
}return M.join("-")
}function d(O){if(!E.et||!E.et.new_js_hashing){return O
}if(s[O]){return s[O]
}var P=2166136261;
for(var M=0,N=O.length;
M<N;
++M){P+=(P<<1)+(P<<4)+(P<<7)+(P<<8)+(P<<24);
P^=O.charCodeAt(M)
}return(s[O]=(P>>>0).toString(16))
}function n(N,M,O){v.level&&v.report(v.events.TRACKING_ATTEMPT,{what:N,hash:M,id:O,variant:(N===L.experiment||N===L.stage)&&C(M)});
if(c(N,M,O)){switch(N){case L.experiment:f(L.experiment,M);
z.populate(M);
t(L.experiment,M);
break;
case L.stage:f(L.stage,M,O);
z.populate(M+"|"+O);
t(L.stage,M,O);
break;
case L.goal:f(L.goal,M);
t(L.goal,M);
break;
case L.custom_goal:f(L.custom_goal,M,O);
t(L.custom_goal,M,O);
break;
default:v.level&&v.report(v.events.TRACK_UNKNOWN_ITEM,N,M,O)
}}if(N===L.experiment){return C(M)
}else{return false
}}function f(N,M,O){m[e(N,M,O)]=true
}function c(P,O,Q){v.level&&v.report(v.events.SHOULD_TRACK,P,O,Q);
if(m[e(P,O,Q)]){v.level&&v.report(v.events.NOT_TRACKING_WAS_TRACKED,P,O,Q);
return false
}else{if(P===L.experiment||P===L.stage){var N=1<<(Q||0),M=d(O);
if(l.r[M]===undefined){v.level&&v.report(v.events.NOT_TRACKING_NOT_RUNNING,P,(Q||0),O,(Q||0));
return false
}else{if(l.f[M]){v.level&&v.report(v.events.NOT_TRACKING_FULLON,P,O,Q);
return false
}else{if(l.t[M]&N){f(P,O,Q);
v.level&&v.report(v.events.NOT_TRACKING_WAS_TRACKED,P,O,Q);
return false
}}}}else{if(P===L.custom_goal){var M=d(O);
if(l.r[M]===undefined){v.level&&v.report(v.events.NOT_TRACKING_NOT_RUNNING,P,Q,O,Q);
return false
}else{if(l.f[M]){v.level&&v.report(v.events.NOT_TRACKING_FULLON,P,O,Q);
return false
}}}}}return true
}function h(M,O,R,Q,S){v.level&&v.report(v.events.ADD_EVENT_LISTENER,M,O,R,Q,S);
if(window.jQuery){if(!K){K=jQuery(window)
}var P=jQuery(O).eq(0);
if(!P.length){v.level&&v.report(v.events.NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER,R,Q,S);
return
}if(M==="view"){A(P,R,Q,S)
}else{P.one(M,function N(){n(R,Q,S)
})
}}}function A(N,P,O,Q){v.level&&v.report(v.events.ADD_DEBOUNCED_VIEW_HANDLER,N,P,O,Q);
var M=e(P,O,Q);
if(m[M]){return false
}p.push([N,P,O,Q]);
if(!k){v.level&&v.report(v.events.ATTACH_VIEW_LISTENER,p);
K.bind("scroll resize",I);
K.bind(i.join(" "),I);
k=true
}}function I(){if(D){return
}D=true;
var P=[],R=K.scrollTop(),Q=K.height();
v.level&&v.report(v.events.CHECK_IF_VISIBLE,p);
var N;
for(var O=0,M=p.length;
O<M;
++O){N=p[O];
if(N&&x(N[0],R,Q)){v.level&&v.report(v.events.ONVIEW_TRACKING_TRIGGERED,N[1],N[2],N[3]);
n(N[1],N[2],N[3])
}else{P.push(N)
}}p=P;
v.level&&v.report(v.events.VISIBLE_CHECK_FINISHED,p);
if(p.length===0){k=false;
K.unbind("scroll resize",I);
K.unbind(i.join(" "),I);
v.level&&v.report(v.events.DETACH_VIEW_LISTENER)
}window.setTimeout(function(){D=false
},g)
}function y(M,N,O){return function(P,Q){if(c(O,P,Q)){h(M,N,O,P,Q)
}else{v.level&&v.report(v.events.WONT_ATTACH_EVENT_TRACKING,M,N,O,P,Q)
}}
}function o(M,N){return{track:y(M,N,L.experiment),stage:y(M,N,L.stage),goal:y(M,N,L.goal),custom_goal:y(M,N,L.custom_goal)}
}function F(M,O){for(var N in O){if(O.hasOwnProperty(N)){M[N]=O[N]
}}}function u(M){l.f=M.f||{};
F(l.r,M.r||{});
F(l.t,M.t||{})
}function j(M){return function(N){return{exp:function(O){return o(M,N).track(O)
},stage:function(O,P){return o(M,N).stage(O,P)
},goal:function(O){return o(M,N).goal(O)
},custom_goal:function(O,P){return o(M,N).custom_goal(O,P)
}}
}
}function C(M){var N=d(M);
return l.r[N]||l.f[N]||0
}function w(M){if(arguments.length>1){throw"Track only accept one parameter"
}if(!M){return 0
}return n(L.experiment,M)
}function J(N,M){if(!N){return false
}if(M===undefined){return false
}if(M===0){return w(N)
}if(!/^[1-9]$/.test(M)){return false
}return n(L.stage,N,M)
}function r(M){if(!M){return false
}return n(L.goal,M)
}function q(N,M){if(!N||!M||!/^[1-5]$/.test(M)){return false
}return n(L.custom_goal,N,M)
}function H(){}H.prototype.variant=w;
H.prototype.track=w;
H.prototype.stage=J;
H.prototype.goal=r;
H.prototype.custom_goal=q;
H.prototype.on=o;
H.prototype.set=u;
H.prototype.compat=j;
H.prototype.debug_level=C("GCbGTMEbZDVHT");
var G=new H();
if(C("GCbGTdeVSKe")){v.level&&v.report("using compatibility mode");
booking.jst&&Object.keys(booking.jst).forEach(function(M){l.r[d(M)]=booking.jst[M]
});
booking.track={queue:booking.track.queue,exp:G.track,getVariant:G.track,goal:G.goal,stage:G.stage,custom_goal:G.custom_goal,onView:G.compat("view"),onViewTouch:G.compat("view"),onClick:G.compat("click"),onMouseOver:G.compat("mouseenter")}
}return G
});
B.et=a("et")
}(B.define,B.require));