(function() {
  ...
  d3.random = {
    ...
    logNormal: function(Âµ, Ï) {
      var n = arguments.length;
      if (n < 2) Ï = 1;
      if (n < 1) Âµ = 0;
      var random = d3.random.normal();
      return function() {
        return Math.exp(Âµ + Ï * random());
      };
    },
  ...
  var d3_geo_area = {
    ...
    polygonEnd: function() {
      d3_geo_areaSum += d3_geo_areaRing < 0 ? 4 * Ï + d3_geo_areaRing : d3_geo_areaRing;
      d3_geo_area.lineStart = d3_geo_area.lineEnd = d3_geo_area.point = d3_noop;
    }
  };
  function d3_geo_areaRingStart() {
    var Î»00, Ï00, Î»1, Î»0, Ï0, cosÏ0, sinÏ0;
    d3_geo_area.point = function(Î», Ï) {
      d3_geo_area.point = nextPoint;
      Î»1 = Î»0 = (Î»00 = Î») * d3_radians, Ï0 = (Ï00 = Ï) * d3_radians, cosÏ0 = Math.cos(Ï0), 
      sinÏ0 = Math.sin(Ï0);
    };
    function nextPoint(Î», Ï) {
      Î» *= d3_radians, Ï *= d3_radians;
      if (Math.abs(Math.abs(Ï0) - Ï / 2) < Îµ && Math.abs(Math.abs(Ï) - Ï / 2) < Îµ) return;
      var cosÏ = Math.cos(Ï), sinÏ = Math.sin(Ï);
      if (Math.abs(Ï0 - Ï / 2) < Îµ) d3_geo_areaRing += (Î» - Î»1) * 2; else {
        var dÎ» = Î» - Î»0, cosdÎ» = Math.cos(dÎ»), d = Math.atan2(Math.sqrt((d = cosÏ * Math.sin(dÎ»)) * d + (d = cosÏ0 * sinÏ - sinÏ0 * cosÏ * cosdÎ») * d), sinÏ0 * sinÏ + cosÏ0 * cosÏ * cosdÎ»), s = (d + Ï + Ï0 + Ï) / 4;
        d3_geo_areaRing += (dÎ» < 0 && dÎ» > -Ï || dÎ» > Ï ? -4 : 4) * Math.atan(Math.sqrt(Math.abs(Math.tan(s) * Math.tan(s - d / 2) * Math.tan(s - Ï / 4 - Ï0 / 2) * Math.tan(s - Ï / 4 - Ï / 2))));
      }
      Î»1 = Î»0, Î»0 = Î», Ï0 = Ï, cosÏ0 = cosÏ, sinÏ0 = sinÏ;
    }
