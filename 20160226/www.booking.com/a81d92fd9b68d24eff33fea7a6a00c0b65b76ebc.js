...
B.define("component/date-selector",function(c,d,b){var i=c("component"),k=c("event-emitter"),j=c("formatting/date");
...
}B.atlas.define("opentable-control",["mixed-markers"],function(b){var a="-81.81,24.5457,-81.788,24.5643";
return{init:function(c){console.log(c);
this.restaurantMarkers=[];
this.$el=this.render(c);
this._addEventListeners(c);
this.retrieveRestaurants(a)
},render:function(f){var d=document.createElement("div"),e=B.jstmpl("fe_xsell_opentable_map_control").render(),c=google.maps.ControlPosition.LEFT_BOTTOM;
d.className="ot-control--position-bottom-right";
d.index=1;
d.innerHTML=e;
f.Interface.map.controls[c].push(d);
return $(d)
},toggleRestaurants:function(e,c){var d=c.currentTarget;
if(d.checked){this.showRestaurants(e)
}else{this.hideRestaurants(e)
}},retrieveRestaurants:function(d){var c=this;
$.getJSON("/ot_markers_on_map",{aid:B.env.b_aid,sid:B.env.b_sid,lang:B.env.b_lang,cc1:B.env.b_countrycode,BBOX:d}).done(function(e){var f=[];
$.each(e,function(h,g){f.push({b_type:"opentable_small",b_id:g.b_name,b_latitude:g.b_restaurant_latitude,b_longitude:g.b_restaurant_longitude,b_icon:"opentable_small"})
});
c.restaurantMarkers=f;
B.events.emit("xsell:opentable-lp-map:markers-loaded",c.restaurantMarkers)
}).fail(function(){console.log("error")
})
},showRestaurants:function(e){var d=b.getActiveMarkers.call(e),c=[].concat(d,this.restaurantMarkers);
b.updateMarkers.call(e,c);
console.log(b.getActiveMarkers.call(e))
},hideRestaurants:function(d){var c=b.getActiveMarkers.call(d);
console.log(c)
},_getMapBounds:function(d){var c=d.Interface?d.Interface.getBoundingBox():d.get("bounds"),e="";
if(c&&c.SW){e=[c.SW[1],c.SW[0],c.NE[1],c.NE[0]].join(",")
}else{if(c&&c.toMOMString){e=c.toMOMString()
}}console.log(e);
return e
},_addEventListeners:function(c){this.$el.find('input[type="checkbox"]').on("click",this.toggleRestaurants.bind(this,c))
}}
})
});
...
if(B.env.b_action=="index"){B.when({events:"view .dcbi-countries"}).run(function(){$(".dcbi-countries .dcbi-country__container[data-img-url]").each(function(){$(this).removeClass("dcbi-country__desc_placeholder_load");
...
}if(B.atlas.getVariant("ebcBUECBLKTRFO")===1){g="//r-ec.bstatic.com/static/img/maps-cp-prop-markers/d58ef3ef4bd43a7e3ea255f756291e3499624b94.png";
...
}if(B.atlas.getVariant("YdVNNeHHOIBeaT")===1){g="//r-ec.bstatic.com/static/img/map_marker_landmark_green/68f65997408731adbbd435867fdbbd1006743532.png";
...
}if(B.et.variant("BKbVPAJeGZQEeCRe")>0){g="//q-ec.bstatic.com/static/img/cross-sell/opentable/ot-map-pin/faa7228867da84a28dcc968d6c08a2cb12db0d55.png";
c.extend(o,{opentable_small:l({url:g,w:14,h:14,x:0,y:0,z:2000})})
}function q(){if(B.atlas.getVariant("HMDCceGECAFSdfCMYEZUVBC")===1){g="//q-ec.bstatic.com/static/img/map/hotel_sr/e69e343c7f884892e418bc96604c7e849cef07e0.png";
...
}}function k(r){return(r&&o.hasOwnProperty(r))?o[r]:o["default"]
}function e(r){return(r&&f.hasOwnProperty(r))?c.extend({},k(r),f[r]):k(r)
}function n(s){var r;
if((s&&h.hasOwnProperty(s))){r=c.extend({},k(s),h[s])
}else{r=k(s)
}return r
}function i(r){this.trigger("visited-marker-rendered");
...
}return{init:d,getInstance:function(){var e={};
d.call(e);
return e
}}
})
...
}B.atlas.define("atlas-common-markers",a,function(p,t,o,f){var j={setTimeout:null},r=p.get("destinationId"),s=false,d=100,k=null,h=false;
function c(z,L,w){var M,I,J=35,G=56,A=5,K=27,H=this.ubber.$domNode,F={w:H.width(),h:H.height()},C={w:z.outerWidth(),h:z.outerHeight()};
G=(G>F.w-w.x-A)?A:G;
...
}function e(z,w){if(w&&w.data&&w.data.b_id&&w.data.b_basic_type){var x={};
...
}}function q(){var w=this;
if(B.atlas.getVariant("HMDCceGECAFSdfCMYEZUVBC")&&w.isUpsortUpdateNeeded&&w.isUpsortUpdateNeeded()){w.clearMarkers()
}w.getMarkers(function(y){var C=p.get("action"),D,x,A;
y=y||{};
if(B.atlas.getVariant("ebcTGPbfRdcDBMdeObC")){y=w.enforceDataFormat(y,function(E){if(/^hotel/.test(E.b_type)){E.b_states.overlay=true
}})
...
}if(B.atlas.getVariant("ebcBUaUSPaGXONCCCeSaT")||B.atlas.getVariant("ebcOLWNTJRLXTcMMOeBRe")){B.atlas.require(["ufi-toggle-polygon"],function(E){E.initialize({districtList:y,map:w})
})
}A=x.length;
if(C==="searchresults"||C==="hotel"||B.atlas.getVariant("ebcMMECAFWTYcUNC")||p.get("isLP")&&C!=="city"){B.atlas.require(["map-markers-disperse"],function(E){if(B.atlas.getVariant("ebcTGPbfRdcDBMdeObC")){E.initialize(x,w,r,{top:0,left:-9,bottom:0,right:-9})
...
}j.setTimeout=setTimeout(function(){y.setIconHover(w.id);
...
}y.getIW(w.id,function(A){if(x&&y.get("hoverMarker")!=A.b_id){return
...
}function u(w){var x=this;
if(j.setTimeout){clearTimeout(j.setTimeout)
}if(k!==w.id){x.setIconOut(w.id);
x.setIW({className:"iw-overlay-lp"});
x.closeIW()
}}function m(x){var z=this,w=z.getMarker(x.id),y=(w&&w.b_url)?w.b_url:"";
...
}}if(!/current/.test(w.data.b_type)){setTimeout(function(){z.trigger("marker-visited",x)
},100)
...
}z.on("iw-close",function(){if(k){z.setIconOut(k);
k=null
}});
if(k){z.setIconOut(k)
}k=w.b_id;
...
}else{z.getIW(w.b_id,function(A){A.b_map_nights=p.get("checkinCheckoutInterval");
A.map_price_per_x_nights_txt=p.get("transPricePerXNights");
...
if(p.get("action")=="hotel"){x=function(){g.call(A,D,y,true);
...
w.data.b_is_current=(w.id||w.data.b_id)===r;
if(/^city.*/.test(w.data.b_type)&&B.atlas.getVariant("ebcYSaBaUSBCHC")==2){window.setTimeout(function(){A.trigger("iw-control-show",{markerId:y,data:w.data})
},0)
}else{A.openIW(w.b_id,{},B.jstmpl("atlas_iw_landmark").render(n(w.data)))
}A.trigger("iw-opened-click",{marker:w,data:w.data});
return
}A.getIW(y,function(F){F.b_basic_type=(w.data)?w.data.b_basic_type:"";
...
if((B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("ebcfEWWSBCHSAcCWCZLSKPCccSZWLKXe")||B.atlas.getVariant("eGOQPBRNZRUXe"))&&/hotel.*/.test(w.data.b_type)){window.setTimeout(function(){A.trigger("iw-control-show",{markerId:y,data:w.data});
...
}j.setTimeout=setTimeout(function(){A.closeIW()
},z)
})
}function l(y){try{h=(window.localStorage&&window.sessionStorage?true:false);
if(h){window.localStorage.setItem("btest","1")
}}catch(x){h=false
}if(h){y.on("load",function(){y.trigger("load-dom-storage-supported")
})
}i(y);
...
y.on("marker-out",u);
y.on("marker-click",m)
}y.on("marker-visited",function(z){y.setIconVisited(z.id)
});
...
y.on("marker-hover-triggered",function(z){if(r==z.id&&/^hotel/.test(z.type)){y.trigger("current-property-marker-hover")
}});
y.on("marker-click",function(z){if(r==z.id&&/^hotel/.test(z.type)){B.track.custom_goal("ebcBUHSCQWXXTTWEYcZbMKSGHRIcUZC",1)
}});
...
y.once("marker-hover",function(){var z="ebcTbYFYSaBGcTFeLT";
if(p.get("action")==="hotel"){B.track.stage(z,2)
}else{if(p.get("isLP")){B.track.stage(z,3)
}else{if(p.get("action")==="searchresults"){B.track.stage(z,4)
}}}});
y.once("load",function(){var z="ebcaUDXPdLEFDIPPQFLT";
if(p.get("action")==="searchresults"){B.et.stage(z,2)
}else{if(p.get("action")==="hotel"){B.et.stage(z,3)
}else{if(p.get("isLP")){B.et.stage(z,4)
}}}});
y.once("marker-hover",function(){var z="ebcTbYFYSaBZGTGHYO";
if(p.get("action")==="searchresults"){B.et.stage(z,2)
}else{if(p.get("action")==="hotel"){B.et.stage(z,3)
}else{if(p.get("isLP")){B.et.stage(z,4)
}}}});
y.once("fixed-iw-open",function(){var z="ebcNHTIYSaBVOHYRJTSDXZLO";
if(p.get("action")==="searchresults"){B.et.stage(z,2)
}else{if(p.get("action")==="hotel"){B.et.stage(z,3)
}}});
$("body").delegate("#marker_close","click",function(){y.closeIW()
})
}function n(x){if(B.atlas.getVariant("ebcBUaUDGDGae")){var w=$.extend({},x);
...
return
}setTimeout(function(){var P=M.getMarker(O.id);
if(P){M.trigger("marker-mini-iw-hover",O);
K=O.id
}},L)
});
M.on("marker-out",function(O){N=setTimeout(function(){var P=M.getMarker(O.id);
...
y.on("mouseleave",".iw-overlay-hotel .mini, .iw-overlay-hotel-current .mini",function(){var P=k(this).data("marker-id"),O;
if(!P){return
}if(K==P){N=setTimeout(function(){O=M.getMarker(P);
...
B.atlas.define("best-areas-poly",["polygons","geo.view","geometry.point","util-env"],function(q,h,r,p){var k={paths:[],clickable:false,strokeColor:"#FEBA02",strokeWeight:2,strokeOpacity:0.8,fillColor:"#FEBA02",fillOpacity:0.2},f=[],e=true,l=true;
...
c.prototype.renderTemplate=function(p){var q=p.tmplData,k=p.position,l=this.isVisible,m=this.$wrapper,n=this;
var j,h,i,g,o;
...
i=o.height();
h=o.width();
g.css({left:k.x,top:k.y}).animate({left:j.left,top:j.top},200,function(){if(B.atlas.getVariant("ebcBUYSaBcLMeDPISRO")||B.atlas.getVariant("ebcOQPBRNQadeOJATXT")){b(m)
}});
o.css({width:0,height:0}).animate({width:h,height:i},200,function(){o.removeAttr("style")
})
...
$(".iw-control").animate({opacity:0},200,function(){l&&m.html(q);
if(B.atlas.getVariant("ebcBUYSaBcLMeDPISRO")||B.atlas.getVariant("ebcOQPBRNQadeOJATXT")){b(m)
}if(B.atlas.getVariant("ebcBUYSaBYdXfdCTEIMeDXBHe")===2||B.atlas.getVariant("ebcOQPBRNHcXJcMUHVdeMXNWe")===2){n.initReview()
...
c.prototype.fixHotelUrl=function(h){if(B.atlas.getVariant("ebcBUcQIFcPXe")||B.atlas.getVariant("ebcOQQGAdQJKe")){return
...
c.prototype.initReview=function(){var i=this;
...
i.$wrapper.on("click",".maps-toggle-review-entry",function(){g.slideToggle(400);
...
$(function(){if(!B.atlas||!B.atlas.define){return
}B.atlas.define("marker-active-state",function(){var b=[{cond:{active:true,hover:true},applyIcon:"getActive"},{cond:{active:true,hover:false},applyIcon:"getActive"},{cond:{active:false,hover:true},applyIcon:"getHover"},{cond:{active:false,visited:true,hover:false},applyIcon:"getVisited"}],g="getIcon";
function a(j){var i=this.Interface.getMixedMarker(j);
if(!d(i)){return
...
}}function f(j){if(typeof j==="undefined"){this.setIconActive(undefined);
return
...
}}function c(m){var l=this.Interface.getActiveMixedMarkers();
if(!(l instanceof Array)){return
}var k=this,j=[],i=[];
10904;
l.forEach(function(n){if(!d(n)){return
...
j.forEach(function(n){k.Interface.setMixedMarkerType(n.b_id,k[e(n)](n?n.iconType||n.b_type:""))
});
i.forEach(function(n){k.Interface.setMixedMarkerType(n.b_id,k[e(n)](n?n.iconType||n.b_type:""))
})
}function h(j){var i=this.Interface.getMixedMarker(j);
if(!d(i)){return
...
}}function e(i){var j=g;
b.some(function(l){var k=false;
k=Object.keys(l.cond).map(function(o){var n=false,m=i.b_behaviors[o]||false;
if(m===l.cond[o]){n=true
}return n
});
k=k.reduce(function(m,n){return m===n&&n===true
});
if(k){j=l.applyIcon
}return k
});
return j
}function d(i){return typeof i=="object"&&i!=null
...
B.atlas.SUBSCRIPTIONS.load.forEach(function(o){if(o.name==="HMDCceGECAFSdfCMYEZUVBC"&&o.cond){n.eventLabel+=":upsort_v"+(B.atlas.getVariant("HMDCceGECAFSdfCMYEZUVBC")||"0")
}});
...
v.setZoom(w.zoom)
}}q.pubSub.on("open",function(w){if(!v){l(k.extend(j,w))
}else{v.done(function(){u(k.extend(j,w))
})
}if(v){v.trigger("open")
}});
q.pubSub.on("close",function(){if(v){v.trigger("close")
}});
q.addEvents();
if(b.atlas.getVariant("ebccZae")){b.atlas.require(["google-analytics"],function(w){w.trackPage(q)
})
}function l(){v=new o({modules:t,options:j});
v.once("autoopen",function(){v.once("close",function(){v.once("open",function(){v.trigger("map-close-iw-open")
})
})
});
v.done(function(){if(b.et.variant("BKbVPAJeGZQEeCRe")>0){h.init(v)
}function x(){k("#b_map_container").find(".iw-overlay").loadComponents();
k("#b_map_container").find(".iw-control").loadComponents()
}v.on("iw-open",x);
v.on("iw-load",x);
k("#b_map_container").delegate(".iw-overlay-city a, .iw-overlay-airport a","click",function(){v.trigger("nonproperty-iw-link-click")
});
if(b.atlas.getVariant("ebcfEFQUETOBXRZcO")){b.atlas.require(["best-areas-poly"],function(y){y.init({topOverlay:j.topOverlay,map:v})
})
...
}if(r.get("isCustomZoom")){b.atlas.require(["zoom-control"],function(y){new y({map:v})
})
}i.init(v);
b.atlas.require(["styler","styler-theme"],function(y,A){if(typeof A!=="undefined"){var z=new y({map:v,theme:A});
z.init()
}});
if(b.atlas.getVariant("ebcfEWWSBCHSAcCWCZLSKPCccSZWLKXe")>0){b.atlas.require(["iw-control"],function(y){new y({map:v,topOverlay:c})
})
}});
if(b.atlas.getVariant("ebccZae")){b.atlas.require(["google-analytics"],function(w){w.trackMap(v)
})
...
}v.on("map-type-change",function(){b.track.custom_goal("ebcNcDUKVZEQdMHeNcC",1)
});
k("#b_map_container").delegate("#close_map_lightbox","click",function(){b.track.custom_goal("ebcNcDUKVZEQdMHeNcC",2)
})
}})
}}(window.booking));
...
j.height(h-5)
}}function d(){var g=this.name;
b(".lp_promotions_cards_list").delegate(".lp_promotion_cards_list_child_tooltipbtn","mouseenter mouseleave",function(m){m.preventDefault();
m.stopPropagation();
var l=b(this);
var i=l.data("rel");
var k=b(i).html();
...
n._createArrow=function(){var o=this.root();
return b('<i class="fly-dropdown__arrow"><i>').appendTo(o)
};
n._position=function(){var q=this._rect(this.root());
var w=this._rect(j);
var p=this._rect(l);
...
}}).delegate(".lp_promotion_cards_list_holder","click",function(k){var i,j;
...
}).each(function(j,k){a(k)
});
var h=true;
var f=(b.browser.msie&&parseInt(b.browser.version,10)===7)?500:0;
b(window).resize(function(){if(h){h=false;
setTimeout(function(){h=true
},f);
...
$(document).on("click",".facility_item .removeThisFacility",function(){$(".facility_preference").remove();
...
(function(){$(".shorten_desc .more_less_link").on("click",function(a){a.preventDefault();
$(this).closest(".wrapper").addClass("opened")
});
$(".full_desc .more_less_link").on("click",function(a){a.preventDefault();
$(this).closest(".wrapper").removeClass("opened")
})
})();
(function(a,c){function b(){c.require(["require"],function(d){d("lists/lists-hotel-dropdown/lists-hotel-dropdown").init()
})
}c[sNSExperiments]["fEFHMPbGNNdURDfUFFdZYO"]={init:b}
})(jQuery,booking);
(function(a,d){var b="fEJdDBKSVMPYLVKGDJFKWe";
function c(){var f=d[sNSStartup]["fEFHMPdTUFQZHT"]?d[sNSStartup]["fEFHMPdTUFQZHT"].updateCardHeight:null;
if(typeof f!=="function"){return
}var g=true;
var e=(a.browser.msie&&parseInt(a.browser.version,10)===7)?500:0;
f(a(".lp_most_wishlisted_promotions_cards_list"));
a(window).resize(function(){if(g){g=false;
setTimeout(function(){g=true
},e);
f(a(".lp_most_wishlisted_promotions_cards_list"))
}})
...
i.bind([j.CREATE,j.EDITHOTEL].join(" "),function(n,p){if(String(p.params.hotel_id)!==String(l.params.hotel_id)){return
}if(B.et.variant("PcVBcNHRBZYUVVSGFGUfC")&&p.result.success){var o=!!p.params.new_state;
l.handle().toggleClass("saved_in_wl",o);
l.updateSprite(o)
}i.fetch(l.params).then(function(s){var r=$.grep(s.result.lists,function(w){return w.selected
});
...
if(B.track.getVariant("BOeaUFebYPQDYO")){B.define("lists/lists-searchresult-hotel-block/lists-searchresult-hotel-block",function(c,b,d){var e=booking[sNSStartup].wlData,f=B.track.getVariant("PcVBcdHKBdEKXO"),a=c("../lists-store/lists-store");
d.exports={updateState:function(g,h){$('.sr-lists-save-container[data-hotel-id="'+g+'"]').toggleClass("sr-lists-save--saved",h)
},init:function(){if(B.env.b_action!=="searchresults"){return
}var h=this,g=[e.EVENTS.EDITHOTEL,e.EVENTS.CREATE].join(" ");
if(f){a.on("list:changed",function(i,j){h.updateState(j.hotel_ids[0],Boolean(i.hotels.length))
});
a.on("list:created",function(i,j){h.updateState(j.hotel_ids[0],Boolean(i.hotels.length))
})
}else{e.bind(g,function(i,j){if(j.result&&j.result.success){h.updateState(j.params.hotel_id,Boolean(j.params.new_state))
}})
}}}
})
}B.require(["require"],function(a){var d=a("lists/lists-hotel-dropdown/lists-hotel-dropdown"),c;
d.init();
if(B.env.b_run_sr_ajax){var b=B.require("searchresults/events");
b.on(b.UI_RESULTS_UPDATED,function(){d.init()
})
