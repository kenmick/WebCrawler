...
this.highlightFromNavigation(f)
},highlightFromNavigation:function(i,h){_p=_o;_o="368:80";h=h||{};
if((this.highlightedIndex==-1||h.saveValueInBuffer)&&(h.beforeAutocomplete||!this.highlightPreserveBuffer)){this.valueBuffer=this.input.value;
if(h.beforeAutocomplete){this.highlightPreserveBuffer=true
}}if(!h.beforeAutocomplete){this.highlightPreserveBuffer=false
}if(i==-1){this.highlightReset({preserveFocus:true})
}else{if(this.supportsInputEvent){if(this.valueBuffer&&this.items[i].valueLowerCase.indexOf(this.valueBuffer.toLowerCase())==0){var k=this.valueBuffer.length,f=this.items[i].value.length,j=this.valueBuffer+this.items[i].value.substring(k);
...
}}}}else{this.setValue(this.items[i].value);
this.fixIEFocusLoss()
}}}this.highlight(i)
},highlightFromAutocomplete:function(f){_p=_o;_o="368:82";f=f||{};
var h=this.input.value,g=this.getListFirstIndex();
if(g!==-1&&this.options.highlightFirst&&!this.isDeletion&&h&&this.items[g].valueLowerCase.indexOf(h.toLowerCase())===0){this.highlightFromNavigation(g,f)
...
this.calculateListHeight();
this.highlightFromAutocomplete({saveValueInBuffer:true})
...
if(h.MJ(h.track_experiment("PVdIdJXIOfBOEQWKRaJOHHAYO"))){if(!(h.MD(a[4]))){m+=b[16];
h.MN(a[4],b[17])
}m+=b[18]
}}if((h.MJ(h.MB(a[1]))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET")))){m+=b[19]
}m+=[b[20],h.MC(a[5]),b[21],h.F.entities(h.MC(a[6])),b[22]].join("");
m+=b[23];
if((h.MJ(h.MB(a[8])+""==="combined_theme"))){m+=[b[24],h.MC(a[7]),b[23]].join("")
}else{if((h.MJ(h.MB(a[3])+""==="theme"))){m+=b[25];
m+=b[26];
if(h.MJ(h.track_experiment("PVdIdJXIOfBOEQWKRaJOHHAYO"))){m+=b[27]
}m+=[b[28],h.F.entities(h.MC(a[9])),b[29]].join("");
if(h.MJ(h.track_experiment("PVdIdJXIOfBOEQWKRaJOHHAYO"))){m+=[b[30],h.F.entities(h.MC(a[10])),b[31]].join("")
}else{m+=[b[30],h.F.entities(h.MC(a[10])),b[32],h.ME(b[33],h.MB,h.MN,null),b[34]].join("")
}m+=b[35]
}else{m+=b[23];
...
if((h.MJ(h.MB(a[3])+""==="hotel"))){m+=[b[23],h.ucfirst(h.MB(a[11])),b[23]].join("")
...
}m+=[b[23],h.F.entities(h.MB(a[12])),b[23]].join("");
if(h.MD(a[13])){m+=[b[42],h.F.entities(h.MB(a[13])),b[43]].join("")
}m+=b[23];
if((h.MJ(n==1))){m+=b[23];
if(h.MJ(h.MG((h.MC(a[14])||{})["popular"]))){m+=b[23];
...
m+=b[23];
if(h.MD(a[16])){m+=b[49]
}m+=[b[23],h.F.entities(h.MC(a[12])),b[23]].join("");
if(h.MD(a[16])){m+=b[47]
}m+=b[23]
}f.shift();
n=l;
...
o=k;
m+=b[23]
}else{if(h.MD(a[17])){m+=[b[23],h.MC(a[17]),b[23]].join("")
}else{if(h.MD(a[10])){m+=[b[23],h.F.entities(h.MC(a[10])),b[23]].join("")
}}}}m+=b[23];
if(h.MD(a[18])){m+=[b[50],h.MC(a[18]),b[47]].join("")
}m+=b[23];
if(!((h.MJ(h.MB(a[1]))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET"))))){m+=b[23];
if(h.MJ(h.MG((h.MC(a[14])||{})["popular"]))){m+=b[23];
if(((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))&&h.MJ(h.track_experiment_stage("PVdIGbGWZSXKbdRNYYdVTbLWZHOaO",1)))){}m+=[b[51],h.F.entities(h.ME(b[46],h.MB,h.MN,null)),b[47]].join("")
}m+=b[23]
}m+=b[23]
}}m+=b[23];
...
},listItemClick:function(g){_p=_o;_o="368:154";if(this.options.ssf0){var f=$(g.currentTarget);
...
}else{this.highlightFromAutocomplete({beforeAutocomplete:true})
...
if(this.options.ssf){this.options.highlightFirst=true
}if(this.options.ssf0){var g=this.input.value;
if(!this.isDeletion&&g&&this.items.length&&this.items[0].valueLowerCase.indexOf(g.toLowerCase())===0){B.et.stage("PVdINcOdAVSaLaO",1)
}}}else{if(!f){this.$list.html(this.templateResults.render(h))
}}this.postAutocomplete()
},choose:function(f){_p=_o;_o="368:172";if(this.options.ssf0&&f===0){B.et.customGoal("PVdINcOdAVSaLaO",1)
}if(this.options.ssf){clearTimeout(this.timer);
this.model.destination.abortPredictions()
...
},initPredictions:function(){_p=_o;_o="368:198";if(this.options.gpf){var f=B.require("search/destination/service-atlas");
f.init()
...
},setOnlyCurrentCountry:function(f){_p=_o;_o="368:210";this.options.onlyCurrentCountry=!!f;
this.emit("change",{onlyCurrentCountry:true})
},emit:function(f,g){_p=_o;_o="368:212";g=g||{};
this.model.emit(f,$.extend(g,{group:"destination"}))
}});
d.exports=e
});
B.define("component/search/destination/quick-link",function(c,a,d){_p=_o;_o="368:215";var b=c("component");
var f=c("search/searchbox/model");
var e=c("read-data-options");
d.exports=b.extend({init:function(){_p=_o;_o="368:216";this.options=e(this.$el,{modelId:{name:"sb-id",type:String,required:true},autoSubmit:{name:"auto-submit",type:Boolean}});
this.$el.on("click","[data-ufi]",this.click.bind(this));
this.model=f.getInstance(this.options.modelId);
this.model.use("destination")
},click:function(i){_p=_o;_o="368:218";i.preventDefault();
...
B.define("search/destination/service",function(b,a,d){_p=_o;_o="368:221";var f=b("search/destination/service-booking");
var e=b("search/destination/service-atlas");
function c(j,h,p){_p=_o;_o="368:222";var k;
var o=[];
var m=[];
var l=function(i,q){_p=_o;_o="368:223";if(o.length){if(i){m.push(i);
...
var n=function(){_p=_o;_o="368:225";for(k=0;
k<o.length;
k++){o[k].abort()
}};
for(k=0;
...
}function g(j,i,o){_p=_o;_o="368:228";var h;
var n={};
var l=function(p,q){_p=_o;_o="368:229";if(p&&j.length){k()
}else{o.call(null,p,q)
}};
var k=function(){_p=_o;_o="368:231";var p=j.shift();
h=p.search.call(p,$.extend({},i),l);
n.getResultDetails=h.getResultDetails
};
var m=function(){_p=_o;_o="368:233";if(h){h.abort()
}};
k();
n.abort=m;
return n
}d.exports={search:function(h,i){_p=_o;_o="368:236";if(!h||!h.ss||typeof i!="function"){throw new Error("Invalid arguments")
...
B.define("search/destination/service-atlas",function(c,f,a){_p=_o;_o="368:239";var p=c("event-emitter");
...
function i(){_p=_o;_o="368:240";return !!B.atlas
}function j(q,s,t){_p=_o;_o="368:242";var r={city:[]};
r.__upa__=s.filter(m).slice(0,5).map(function(w,v){_p=_o;_o="368:243";w=w.get("data");
...
t(null,r)
}function m(q){_p=_o;_o="368:246";var r=q.get("data");
return n.indexOf(r.id)===-1
}function h(q,r,s){_p=_o;_o="368:248";q=q||{};
...
s(null,q)
}function o(q,r){_p=_o;_o="368:250";this.atlas.getPlaceDetails({placeId:q.place_id},function(s){_p=_o;_o="368:251";if(!s||typeof s.get!=="function"){r(new Error("service-atlas-invalid-place"),null)
}else{var t=s.get("data");
t.upa=true;
h(q,t,r)
}})
}function g(t,w,s){_p=_o;_o="368:254";var q="";
s=s||{};
if(!w||w.length===0){return t
}var v=w.map(function(x){_p=_o;_o="368:255";return x.offset
});
var r=w.map(function(x){_p=_o;_o="368:257";return x.offset+x.length-1
});
for(var u=0;
u<t.length;
...
}function b(r,s){_p=_o;_o="368:260";var q=[];
if(!s||s.length===0){return[{text:r}]
}s.forEach(function(v,w){_p=_o;_o="368:261";var u=v.offset+v.length,t=((s[w+1]&&s[w+1].offset)||r.length)-u;
q.push({highlighted:true,text:r.substr(v.offset,v.length)});
if(t){q.push({text:r.substr(u,t)})
}});
return q
}a.exports=p.extend({init:function(s){_p=_o;_o="368:264";if(!i()){return
}if(this.initialized){if(s){this.atlas.done(s)
}return
}this.initialized=true;
var r=B.atlas.require("atlas-places");
var q=document.createElement("div");
this.atlas=new r({provider:"provider-places",modules:["autocomplete","places"],options:{domNode:q,channel:"booking-places"}});
if(s){this.atlas.done(s)
}},search:function(r,w){_p=_o;_o="368:266";if(!r||!r.ss||typeof w!="function"){throw new Error("service-atlas-invalid-arguments")
}if(!i()){setTimeout(function(){_p=_o;_o="368:267";w(new Error("service-atlas-no-atlas"),null)
},4);
...
u.lon=k;
u.radius=l
}var t=function(x){_p=_o;_o="368:269";if(!q){if(!x||!Array.isArray(x)){w(new Error("service-atlas-invalid-results"),null)
}else{j(r,x,w)
}}};
var s=function(x){_p=_o;_o="368:271";if(!q){w(new Error("service-atlas-error "+x),null)
}};
this.init(function(){_p=_o;_o="368:273";this.atlas.autocomplete(u,t,s)
}.bind(this));
var v=function(){_p=_o;_o="368:275";q=true
};
return{getResultDetails:o.bind(this),abort:v}
}})
});
B.define("search/destination/service-booking",function(c,e,a){_p=_o;_o="368:279";var j=c("event-emitter");
var i=c("et");
var g=B.env.autocomplete_use_auth?"/autocomplete_with_auth":"/autocomplete";
function d(k){_p=_o;_o="368:280";k.term=k.ss;
delete k.ss;
return k
}function h(l,k,m){_p=_o;_o="368:282";if(!l||!l.city){m(new Error("service-booking-data-invalid"),null)
...
}else{l.city=l.city.map(function(n,o){_p=_o;_o="368:283";if(!n.label&&n.labels){n.label=f(n.labels)
...
l.results=l.city;
if(l.bbtoollocations){l.results=l.bbtoollocations.map(function(n,o){_p=_o;_o="368:285";delete n.label_highlighted;
...
}if(l.theme){l.theme=l.theme.map(function(n,o){_p=_o;_o="368:287";n.ss=n.label;
n._ef=n._ef||[];
...
l._efd=l._efd||[];
m(null,l)
}}}function f(k){_p=_o;_o="368:290";return k.map(function(l){_p=_o;_o="368:291";return l.text
}).join(", ")
}function b(l){_p=_o;_o="368:294";var k=[];
l.forEach(function(m,n){_p=_o;_o="368:295";if(n>0){k.push({text:", "})
}k.push({highlighted:!!m.hl,text:m.text})
});
return k
}a.exports=j.extend({search:function(k,o){_p=_o;_o="368:298";if(!k||!k.ss||typeof o!="function"){throw new Error("Invalid arguments")
}if(g=="/autocomplete"&&B.et.track("PVdIVTBUYLACYO")){g="/autocomplete_2"
}var l=d(k);
this.emit("beforeajax",k,l);
var m=$.ajax({type:"get",url:g,data:l,success:function(q,p,r){_p=_o;_o="368:299";this.emit("afterajax",k,q);
h(q,k,o)
}.bind(this),error:function(r,p,q){_p=_o;_o="368:301";if(p!=="abort"){o(new Error("service-booking-error "+q),null)
}}.bind(this)});
var n=function(){_p=_o;_o="368:303";if(m&&m.abort){m.abort()
}};
return{abort:n}
}})
});
B.define("search/searchbox/model",function(c,b,d){_p=_o;_o="368:307";var e=c("event-emitter");
var f={};
function a(g){_p=_o;_o="368:308";e.extend(this);
this.id=g;
this.formAction;
this.fieldGroups={track:[]};
this.validators=[];
this._ssce=false
}$.extend(a.prototype,{use:function(j){_p=_o;_o="368:310";if(this[j]){return
...
}},submit:function(){_p=_o;_o="368:312";this.emit("submit")
},getFormAction:function(){_p=_o;_o="368:314";return this.formAction
},setFormAction:function(g){_p=_o;_o="368:316";this.formAction=g
},getFields:function(){_p=_o;_o="368:318";var g=[];
Object.keys(this.fieldGroups).forEach(function(h){_p=_o;_o="368:319";if(Array.isArray(this.fieldGroups[h])){g=g.concat(this.fieldGroups[h])
}}.bind(this));
return g
},setFields:function(h,g){_p=_o;_o="368:322";this.fieldGroups[h]=g
},validate:function(){_p=_o;_o="368:324";if(!this._ssce){return true
}var g=this.validators.filter(function(h){_p=_o;_o="368:325";return h.call()
});
return g.length==this.validators.length
}});
d.exports={getInstance:function(g){_p=_o;_o="368:328";if(!f[g]){f[g]=new a(g)
}return f[g]
}}
});
B.define("component/search/searchbox/searchbox",function(d,b,e){_p=_o;_o="368:331";var c=d("component");
...
e.exports=c.extend({init:function(){_p=_o;_o="368:332";this.hiddenFields=[];
...
g.on("beforeajax",function(i,j){_p=_o;_o="368:333";B.eventEmitter.trigger("AUTOCOMPLETE:beforeAjax",{params:j})
});
g.on("afterajax",function(i,j){_p=_o;_o="368:335";B.eventEmitter.trigger("AUTOCOMPLETE:afterAjax",{data:j})
})
},modelChange:function(){_p=_o;_o="368:338";this.updateHiddenFields()
},modelSubmit:function(){_p=_o;_o="368:340";this.$form.submit()
},initCurrentCountryField:function(){_p=_o;_o="368:342";var i=false;
...
}},onlyCurrentCountryChange:function(){_p=_o;_o="368:344";this.model.use("destination");
this.model.destination.setOnlyCurrentCountry(this.$onlyCurrentCountry.is(":checked"))
},updateHiddenFields:function(){_p=_o;_o="368:346";var j=this.model.getFormAction();
var l=this.$form.attr("action");
if(!j){j=this.originalAction
}if(l!=j){this.$form.attr("action",j)
}if(this.hiddenFields){this.hiddenFields.forEach(function(n){_p=_o;_o="368:347";n.$node.remove();
n.$node=null
})
...
var k=[];
var i=false;
m.forEach(function(n){_p=_o;_o="368:349";this.hiddenFields.push({name:n.name,value:n.value});
k.push(n.name);
if(a.indexOf(n.name)!=-1){i=true
}}.bind(this));
this.$form.find("input").filter(function(){_p=_o;_o="368:351";return(i&&a.indexOf(this.name)!=-1)||k.indexOf(this.name)!=-1
}).remove();
this.hiddenFields.forEach(function(n){_p=_o;_o="368:353";var o=$("<input>");
o.attr("type","hidden");
o.attr("name",n.name);
...
},formSubmit:function(j){_p=_o;_o="368:356";if(this.model._ssce){var i=this.model.validate();
if(!i){j.preventDefault();
return
}}}})
});
(function(a,b){_p=_o;_o="368:359";a(function(){_p=_o;_o="368:360";a(".ge-search-first-checkbox").change(function(){_p=_o;_o="368:361";var f="cQZJZadTQZGSRaPSWe";
