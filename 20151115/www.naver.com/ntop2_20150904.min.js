...
this._onHideRankPop(a);a.stop();$$.getSingle("#ad_top iframe").focus()}}};var ServiceMenu={aDefaultList:["dic","news","stock","land","map","movie","music","book","comic"],serviceTemplate:"<a class='mn_#GDID' title='#TITLE' href='http://#HREF'><span></span>#TITLE</a>",guideMsg:{view:"Myë©ë´ ì¤ì  ë²í¼ì ëë¥´ìë©´ ìì£¼ ì°ë ë©ë´ë¥¼ ì¤ì íì¤ ì ììµëë¤.",edit:"<strong>ìµë 5ê°</strong>ê¹ì§ ì¤ì íì¤ ì ìì¼ë©°, ì íí ììëë¡ ì ê³µë©ëë¤. (ë©ë´ìì­ìì ë°ë¡ íì¸íì¸ì)"},init:function(a){this.BaseURL=a;
...
}}if(navigator.userAgent.indexOf("SHW-M180S")!=-1){if($Element("svcmore").visible()){$("ad_top").getElementsByTagName("iframe")[0].style.display="none"}else{$("ad_top").getElementsByTagName("iframe")[0].style.display="block"
...
a.stop()}}},Action:{closeMyMenuSet:function(a){ServiceMenu._checkMoreButtonClass(a);CommonFn.layer.close("svcmore");if($("ad_top")){$("ad_top").getElementsByTagName("iframe")[0].style.display="block"}$Element("svcmore_h").toggleClass("close");
$("svcmore_h").focus();if(ServiceMenu.notiRoll){ServiceMenu.notiRoll.pause()}LoginCheck.nvLoginCheck("RealTimeRank",0)},openMyMenuEdit:function(){$Element("svc_defmenu").hide();$Element("svc_newmenu").hide();
$Element("svc_defdic").hide();$Element("svc_selmenu").show();$Element("svc_selmenu_new").show();$Element("svc_seldic").show();$Element($("svc_set_mymenu").parentNode).hide();$Element("svc_menu_btns").show();
this.disableHref("svc_story");this.disableHref("svc_tools_lst");$Element("svc_story").addClass("disabled");$Element("svc_tools_lst").addClass("disabled");this.changeImgDisable("svc_story",".gif","_off.gif");
...
},saveMyMenu:function(){var a=ServiceMenu;var c=365*10;var b=a.Action.areArraysEqual(a.aDefaultList,a.aCacheList);if(!b&&a.aCacheList.length>5){alert("ìì£¼ ì°ë ë©ë´ë\nìµë 5ê°ê¹ì§ë§ ì í ê°ë¥í©ëë¤.");a=null;return false
}if(a.aCacheList.length<1){alert("ì íë ë©ë´ê° ììµëë¤.ì´ê¸° ì¤ì ì¼ë¡ ëìê°ëë¤.");this.restoreDefalutMenu()}a.aCookieList=a.aCacheList.concat();if(a.isEqualsDefaultMenu(a.aCookieList)){$Cookie().remove("NVSERV","naver.com")
...
var e=[];e.push('<dt class="blind">ìì£¼ ì°ë ë©ë´</dt>');for(var g=0;g<d.length;++g){var b=d[g];var c=$("nsvc_"+b);if(!c){continue}var l=c.value.split("|");var a=l[0];var m=l[1];var j=l[2];e.push(f.replace(/#HREF/,a).replace(/#TITLE/g,j).replace(/#GDID/,m))
}$("fmenu").innerHTML=e.join("");k.aCacheList=d.concat();k=f=e=d=null},addItem:function(g){var j=ServiceMenu;var b=j.aCacheList.length;if(j.aCacheList.length>=5){alert("ìì£¼ ì°ë ë©ë´ë\nìµë 5ê°ê¹ì§ë§ ì í ê°ë¥í©ëë¤.");
...
JEagleEyeClient.sendError("ë¦¬ì¤í¸ê° ì¡´ì¬íì§ ììµëë¤.");return}if(this.sTemplate){var c="";var g="";for(var h=j.length,e=0;e<h;e++){var l=this.sTemplate.replace(/#MSG/g,"êµ¬ë í´ì§").replace("#BTNCLS","del_my");for(var d=0,f=j[e].length;
d<f;d++){l=l.split("@"+d).join(j[e][d])}c+=l;if(e+1>=a){c+='<li><a class="more" href="'+this.moreLink+'">ëë³´ê¸°</a></li>';break}}if(!c){c="<li></li>"}this.elMyList.innerHTML=c;for(var h=b.length,e=0;e<h;e++){var l=this.sTemplate.replace(/#MSG/g,"êµ¬ë").replace("#BTNCLS","add_my");
...
if(a.bMyNewsStand){a.My.initCategory()}if(global.isSupportedFlicking){a.initFlicking()}},onerror:function(){JEagleEyeClient.sendError("[NEWSSTAND] ì¸ë¡ ì¬ ì ë³´ ajax ì¤í¨ : "+b)},ontimeout:function(){JEagleEyeClient.sendError("[NEWSSTAND] ì¸ë¡ ì¬ ì ë³´ ë¡ë© timeout: "+b)
...
var a=NewsStand.getSortMode();if(c==NewsStand.Press.mode){b=(a=="thumb")?"nsd_all*p":"nsd_all*a"}else{if(c==NewsStand.My.mode){b=(a=="thumb")?"nsd_myn*p":"nsd_myn*a"}}return b},Press:{mode:0,noti:{text:"[ìë¦¼] ì¸ë¡ ì¬ë³ ì£¼ìë´ì¤ë¥¼ ë©ì¸ìì ë°ë¡ ë³¼ ì ìì´ì!",url:"http://news.naver.com/main/ombudsman/readView.nhn?notiId=320&notiDel=N"},init:function(){this.nPage=1;
this.nMaxPage=1;this.elPageSpan=$$.getSingle("#news_cast2 .nss_pg[data-mode=press] > span");this.aMoveBtn=$$("#news_cast2 .nss_pg[data-mode=press] a");this.template='<h3 class="blind" style="">ì ì²´ì¸ë¡ ì¬</h3><div class="newss_lst"><div class="newss"><ul>{for idx:press in pressList}{if press.pid != "add"}<li id="NS_{=press.pid}"{if press.amigo == "Y" || press.viewer == "N"} class="disabled""><span class="newssa">{else}><a href="http://'+newsStandUrl+'/?list={=press.mainCate}&pcode={=press.pid}" target="_blank" class="newssa">{/if}<img src="{=press.img}" width="81" height="73" alt="{=press.name}" {if press.amigo == "Y"} title="ìì±ì½ë ë°ìì¼ë¡ ê¸°ì¬ ë¸ì¶ì´\nì ì¸ë ìíìëë¤."{elseif press.viewer == "N"} title="í´ë¹ ì¸ë¡ ì¬ìì ìë¹ì¤ë¥¼\nì¤ë¹ì¤ìëë¤."{/if} />{if press.amigo == "Y" || press.viewer == "N"}</span>{else}</a>{/if}{if press.today == "Y"}<a href="http://'+newspaperUrl+'/viewer/index.nhn?pressCode=0{=press.pid}" class="today" title="ì¤ëì ì ë¬¸ ë³´ê¸°" target="_blank">ì¤ëì ì ë¬¸ ë³´ê¸°</a>{/if}</li>{else}<li class="add"><a href="http://'+newsStandUrl+'/config.html" class="newssa" target="_blank">ì¸ë¡ ì¬ ì¶ê°</a></li>{/if}\n{/for}</ul></div></div>';
...
}else{if(c.amigo=="Y"){this.amigoPressList.push(c)}else{if(c.viewer=="N"){this.noViewerPressList.push(c)}}}}return d}},My:{mode:1,noti:{text:"[ìë¦¼] ì¸ë¡ ì¬ë³ ì£¼ìë´ì¤ë¥¼ ë©ì¸ìì ë°ë¡ ë³¼ ì ìì´ì!",url:"http://news.naver.com/main/ombudsman/readView.nhn?notiId=320&notiDel=N"},init:function(a){this.baseHTML="<h3 class='blind'>MYë´ì¤</h3><div id='headline'></div>";
this.unsubscribeHTML="<h3 class='blind'>MYë´ì¤</h3><p class='mm_mes1'>ì¸ë¡ ì¬ êµ¬ë ì¤ì ìì <strong>MYì¸ë¡ ì¬</strong>ë¥¼ ì¶ê°íë©´<br>ì¤ì í ì¸ë¡ ì¬ì ê¸°ì¬ë¤ì ë¤ì´ë² íìì ë°ë¡ ë³´ì¤ ì ììµëë¤.</p><p class='mm_bt'><a href='http://"+window.newsStandUrl+"/config.html' class='add_media' target='_blank' onclick='clickcr(this,\"nsd_all.myadd\",\"\",\"\",event);'>ì¸ë¡ ì¬ ì¶ê°</a></p>",this.serviceReadyHTML="<h3 class='blind'>MYë´ì¤</h3><p class='mm_mes1'>MYë´ì¤ë¡ ì¤ì íì  ì¸ë¡ ì¬ë¤ì´ ëª¨ë <strong>ìë¹ì¤ ì¤ë¹ì¤</strong>ìëë¤.<br>ì¡°ê¸ë§ ê¸°ë¤ë ¤ ì£¼ì¸ì.</p><p class='mm_bt'><a href='http://"+window.newsStandUrl+"/config.html' class='add_media' target='_blank' onclick='clickcr(this,\"nsd_myn*a.add\",\"\",\"\",event);'>ì¸ë¡ ì¬ ì¶ê°</a></p>";
...
}}for(var d=0;d<b.length;d++){var g=b[d];var e=NewsStand.getPressInfo(g);if(e){var a="";var h="";var j="";if(this.isHeadlineAmigo(e.pid)){h="ìì±ì½ë ë°ìì¼ë¡ ê¸°ì¬ ë¸ì¶ì´ ì ì¸ë ìíìëë¤.";a="bad"}else{if(!this.isHeadlinePress(e.pid)){h="í´ë¹ ì¸ë¡ ì¬ìì ìë¹ì¤ë¥¼ ì¤ë¹ ì¤ìëë¤";
...
},ontimeout:function(g){a.Popup.Error.show(b);JEagleEyeClient.sendError("[NewsStand] Mynews delete failed(timeout)")}}).request()},Popup:{Sub:{template:"<div class='waiting add' style='display:block'>MYë´ì¤ì ì¶ê°ëììµëë¤.</div>",show:function(b){var a=this;
...
}},Unsub:{template:"<div id='ly_media_close' class='ly_media_close'><p><strong>{=name}</strong>ì(ë¥¼)<br>êµ¬ëí´ì§ íìê² ìµëê¹?</p><div class='lmc_act'><a href='#' onclick='NewsStand.My.unsubscribe(\"{=pid}\"); clickcr(this, \"nsd_myn*a.usdone\", \"\", \"\", event); return false;' class='lmc_check'>íì¸</a> <a href='#' onclick='NewsStand.My.Popup.Unsub.hide(); clickcr(this, \"nsd_myn*a.uscancel\", \"\", \"\", event); return false;' class='lmc_cancel'>>ì·¨ì</a></div><a href='#' onclick='NewsStand.My.Popup.Unsub.hide(); clickcr(this, \"nsd_myn*a.usclose\", \"\", \"\", event); return false;' class='lmc_close'>ë«ê¸°</a></div>",show:function(a){this.hide();
var c=$Element(a.pid);var b=$Template(this.template).process(a);c.append(b)},hide:function(){var a=$Element("ly_media_close");if(a){a.leave()}return false}},Error:{template:"<div class='waiting' style='display:block'>ì ì íì ë¤ì ìëí´ì£¼ì¸ì</div>",show:function(c){var b=this;
...
if(a){$Element(a).hide()}}}}},DataManager:{init:function(){this.oneSideEmptyHtml='<div class="cast2_media"><a href="http://'+newsStandUrl+'/config.html"target="_blank" class="plus_media" style="" onclick="clickcr(this,\'nsd_myn*a.add\', \'\',\'\',event);"><span></span>ì¸ë¡ ì¬ ì¶ê°</a></div>';
...
},onerror:function(){JEagleEyeClient.sendError("[NEWSSTAND] ì¸ë¡ ì¬ í¤ëë¼ì¸ ì ë³´ ë¡ë© ì¤í¨: "+b)},ontimeout:function(){JEagleEyeClient.sendError("[NEWSSTAND] ì¸ë¡ ì¬ í¤ëë¼ì¸ ì ë³´ ë¡ë© timeout: "+b)}});e.header("If-Modified-Since","Thu, 1 Jan 1970 00:00:00 GMT");
...
a>-1;--a){d[a].title=$Element(d[a]).text();d[a].title=d[a].title.replace(/#emg#/,"[ê¸´ê¸] ")}},_replaceCharacters:function(a){$(a).innerHTML=$(a).innerHTML.replace(/#emg#/g,"<strong>[ê¸´ê¸]</strong> ")}};