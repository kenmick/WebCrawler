...
c.prototype.renderTemplate=function(j){var h=j.view,i=j.tmplData,g=this.isVisible,l=this.$wrapper,k=this;
...
$(".iw-control").animate({opacity:0},200,function(){g&&l.html(i);
if(B.atlas.getVariant("ebcBUYSaBcLMeDPISRO")||B.atlas.getVariant("ebcOQPBRNQadeOJATXT")){b(l)
...
$(function(){if(!B.atlas||!B.atlas.define){return
}B.atlas.define("marker-active-state",function(){var b=[{cond:{active:true,hover:true},applyIcon:"getActive"},{cond:{active:true,hover:false},applyIcon:"getActive"},{cond:{active:false,hover:true},applyIcon:"getHover"},{cond:{active:false,visited:true,hover:false},applyIcon:"getVisited"}],f="getIcon";
function a(i){var h=this.Interface.getMixedMarker(i);
if(typeof h=="undefined"){return
}h.b_behaviors.hover=true;
if(h.b_states&&h.b_states.overlay){this.Interface.setMixedMarkerType(i,{zIndex:this.getMarkerZIndex(h)})
}else{this.Interface.setMixedMarkerType(i,this[d(h)](h?h.iconType||h.b_type:""))
}}function e(i){if(typeof i==="undefined"){this.setIconActive(undefined);
return
}var h=this.Interface.getMixedMarker(i);
if(typeof h=="undefined"){return
}h.b_behaviors.hover=false;
if(h.b_states&&h.b_states.overlay){this.Interface.setMixedMarkerType(i,{zIndex:this.getMarkerZIndex(h)})
}else{this.Interface.setMixedMarkerType(i,this[d(h)](h?h.iconType||h.b_type:""))
}}function c(l){var k=this.Interface.getActiveMixedMarkers();
if(!(k instanceof Array)){return
}var j=this,i=[],h=[];
10904;
k.forEach(function(m){if(typeof m=="undefined"){return
}m=j.Interface.getMixedMarker(m.b_id);
if(m.b_behaviors.active===true&&!(l===m.b_id)){m.b_behaviors.active=false;
i.push(m)
}if(l===m.b_id){m.b_behaviors.active=true;
h.push(m)
}});
i.forEach(function(m){j.Interface.setMixedMarkerType(m.b_id,j[d(m)](m?m.iconType||m.b_type:""))
});
h.forEach(function(m){j.Interface.setMixedMarkerType(m.b_id,j[d(m)](m?m.iconType||m.b_type:""))
})
}function g(i){var h=this.Interface.getMixedMarker(i);
if(typeof h=="undefined"){return
}h.b_behaviors.visited=true;
if(h.b_states&&h.b_states.overlay){this.Interface.setMixedMarkerType(i,{content:this.getMarkerContent(h),zIndex:this.getMarkerZIndex(h)})
}else{this.Interface.setMixedMarkerType(i,this[d(h)](h?h.iconType||h.b_type:""))
}}function d(h){var i=f;
b.some(function(k){var j=false;
j=Object.keys(k.cond).map(function(n){var m=false,l=h.b_behaviors[n]||false;
if(l===k.cond[n]){m=true
}return m
});
j=j.reduce(function(l,m){return l===m&&m===true
});
if(j){i=k.applyIcon
}return j
});
return i
}return{setIconHover:a,setIconOut:e,setIconActive:c,setIconVisited:g}
})
});
$(function(){if(!B.atlas){return
...
if(((((f.MJ(f.MB(a[104]))&&(f.MJ(parseInt(f.MB(a[104]))<=12)))&&f.MJ(f.MB(a[101])))&&f.MJ(f.MB(a[102])))&&((f.MJ(f.ATLAS_STAGE("ebcOQPBRNQadeOJATXT"))&&f.MJ(f.ATLAS_TRACK("ebcOQPBRNQadeOJATXT")))||(f.MJ(f.ATLAS_STAGE("ebcBUYSaBcLMeDPISRO"))&&f.MJ(f.ATLAS_TRACK("ebcBUYSaBcLMeDPISRO")))))){u+=b[167];
var r="";
r+=b[80];
...
if(j&&j.target){if(b(j.target).closest(".wl-text-block").length){j.preventDefault();
return
}if(!b(j.target).closest("h4").length||!b(j.target).is("a")){i="sr-url";
