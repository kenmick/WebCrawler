...
return
}}f&&b(this).parent().find(".lp_sticky_cta_date_picker_error").hide();
...
}B.atlas.define("atlas-common-markers",a,function(x,E,z){var o,A=x.get("destinationId"),h=B.atlas.assert,C=false,f=100,p=null,l=false,q=x.get("isMiniIWLandingPages"),e=x.get("isMiniIW");
...
}}}function j(G,P,F){var Q,M,N=35,K=56,H=5,O=27,L=this.ubber.$domNode,J={w:L.width(),h:L.height()},I={w:G.outerWidth(),h:G.outerHeight()};
K=(K>J.w-F.x-H)?H:K;
if((I.w+H+K>J.w)){I.w=J.w-H-K;
G.find(".iw-container").css({"max-width":I.w})
}Q=F.x;
Q=(Q+I.w+H>J.w)?Q-I.w:Q;
Q=(Q-K<3)?3:Q;
M=(F.y+I.h>J.h-N)?F.y-I.h-O:F.y;
return{x:Q,y:M}
}function d(G,P,F){if(booking.atlas.getVariant("eGYSaBbJHeLUbQWe")&&G.find(".iw-hotel, .iw-vertical").length>0){return g.apply(this,[G,P,F])
}var Q,M,N=35,K=56,H=5,O=27,L=this.ubber.$domNode,J={w:L.width(),h:L.height()},I={w:G.outerWidth(),h:G.outerHeight()};
K=(K>J.w-F.x-H)?H:K;
if((I.w+H+K>J.w)){I.w=J.w-H-K;
G.find(".iw-container").css({"max-width":I.w})
}Q=F.x-H-I.w/2;
Q=(Q+I.w+H>J.w)?J.w-I.w-H-K:Q;
Q=(Q-K<3)?3:Q;
M=(F.y+I.h>J.h-N)?F.y-I.h-O:F.y;
return{x:Q,y:M}
}function g(H,Y,F){var O,N=F.y,M=35,P=56,I=5,U=0,R=27,L=this.ubber.$domNode,J={w:L.width(),h:L.height()},G={w:H.outerWidth(),h:H.outerHeight()},S=n(this,H),W=0,V="right",T=H.find(".js-map_iw_arrow"),Q=T.length>0,K=2,X;
U=S.w;
R=S.h;
N=F.y-G.h/2-R/2;
W=N;
N=Math.max(N,0+I);
N=Math.min(N,J.h-G.h-I);
O=F.x+U/2-K;
if(O-G.w>0){O-=G.w+U-K*2;
V="left"
}X={x:O,y:N};
H.addClass("map_iw--"+V);
if(Q){O+=(V=="left"?-1:1)*T.width();
X={x:O,y:N};
T.css({top:u(H,W,G,O,N,T).arrowY}).addClass("map_iw_arrow--"+V)
}return X
}function n(H,G){var F=H.config.type,I=G[0].className;
if(I.indexOf("hotel-current")!=-1){F=F+"_current"
}return H.ubber.ICONS[F]
}function u(G,M,H,L,K,J){if(J.length<0){return{}
}var I=J.outerHeight(),F,N;
N=M-K+H.h/2-I/2;
N=Math.max(N,0);
N=Math.min(N,H.h-I);
return{arrowX:F,arrowY:N}
}function m(F){F.setIW({disablePan:true,className:"iw-overlay-lp",engine:"html",getPosition:d,isFixed:true})
}function t(F){F.setIW({getPosition:j})
}function r(F){F.setIW({getPosition:d})
}function i(G,F){if(F&&F.data&&F.data.b_id&&F.data.b_type){G.setIW({className:"iw-overlay-lp iw-overlay-"+F.data.b_type+((F.data.b_id===A)?"-current":"")})
}}function y(){var F=this;
F.getMarkers(function(H){H=H||{};
var J,G=[].concat(H.b_hotels||[],H.b_cities||[],H.b_airports||[]),I=G.length;
if(B.atlas.getVariant("eGECAFWTYcUNC")||B.atlas.getVariant("eGOLHMbdIePQNcO")||B.atlas.getVariant("eGMMECAFWTYcUNC")||B.atlas.getVariant("eGfEDXPdLbeMSSae")){B.atlas.require(["experiment-map-markers-disperse"],function(K){K.initialize(G,F,A)
})
}else{F.updateMarkers(G);
if(A){while(I--){J=G[I];
if(J&&A===J.b_id){if(!B.atlas.assert.tdot&&B.atlas.assert.hp){F.setMarkerOptions(J.b_id,{cursor:"default"})
}F.setMarkerType(J.b_id,J.b_marker_type+"_current");
break
}}}}});
F.closeIW()
}function b(F){var G=this;
if(o){clearTimeout(o)
}o=setTimeout(function(){if(B.atlas.getVariant("eGTbYFYSaBdbKOecO")||B.atlas.getVariant("eGTbYFYSaBdbKOeZVFKe")){t(G)
}G.setIconHover(F.id);
if(F.type==="geosearch_place"&&F.data){G.setIW({className:"iw-overlay-lp iw-overlay-geosearch-place"});
G.openIW(F.id,F.data.data)
}else{if(/city|airport/.test(F.type)){if(e||q){if(p!==F.id){if(p){G.setIconOut(p);
G.closeIW()
}i(G,F);
G.openIW(F.id,{},B.jstmpl("atlas_iw_mini_landmark").render(F.data))
}}else{G.openIWLoading(F.id);
i(G,F);
G.openIW(F.id,F.data)
}}else{if(e||q){if(p!==F.id){if(p){G.setIconOut(p);
G.closeIW()
}i(G,F);
G.openIWLoading(F.id,B.jstmpl("atlas_iw_mini_loading").render({}));
if(B.atlas.getVariant("eGBUYSaBTbYFbVKIGHT")&&A!=F.id){z.getIWs([F.id,A],G,function(H){var I=H[0],J=H[1];
if(I.b_id!=F.id){I=H[1];
J=H[0]
}I.b_destination_reference=J;
I.b_marker_type=(F.data)?F.data.b_marker_type:"";
I.b_map_nights=x.get("checkinCheckoutInterval");
I.map_price_per_x_nights_txt=x.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){I.transLengthOfStayExperiment=x.get("transLengthOfStayExperiment")
}G.openIW(F.id,{},B.jstmpl("atlas_iw_mini").render(I))
})
}else{G.getIW(F.id,function(H){H.b_marker_type=(F.data)?F.data.b_marker_type:"";
H.b_map_nights=x.get("checkinCheckoutInterval");
H.map_price_per_x_nights_txt=x.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){H.transLengthOfStayExperiment=x.get("transLengthOfStayExperiment")
}G.openIW(F.id,{},B.jstmpl("atlas_iw_mini").render(H))
})
}}}else{G.openIWLoading(F.id);
G.getIW(F.id,function(H){H.b_marker_type=(F.data)?F.data.b_marker_type:"";
H.b_map_nights=x.get("checkinCheckoutInterval");
H.map_price_per_x_nights_txt=x.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){H.transLengthOfStayExperiment=x.get("transLengthOfStayExperiment")
}i(G,F);
...
if(o){clearTimeout(o)
}if(!(e||q)){G.setIconOut(F.id)
}if(e||q){if(p!==F.id){G.setIconOut(F.id);
G.setIW({className:"iw-overlay-lp"});
G.closeIW()
}}else{G.setIW({className:"iw-overlay-lp"});
G.closeIW()
}}function s(){var F=this;
view=F.getViewFromMap()
}function c(G){var F=$(this).offset();
mouse.x=G.pageX-F.left;
mouse.y=G.pageY-F.top
}function w(G){var I=this,F=I.getMarker(G.id),H=(F&&F.b_url)?F.b_url:"";
if(F&&F.data&&/^hotel.*/.test(F.data.b_type)&&B.atlas.getVariant("eGBUYSaBTHVDBHT")){return
}if(F.b_type==="geosearch_place"){return false
}if(B.atlas.getVariant("eGTbYFYSaBdbKOecO")||B.atlas.getVariant("eGTbYFYSaBdbKOeZVFKe")){r(I)
}if(F&&F.data&&F.data.b_id===A&&F.data.b_type==="city"&&B.atlas.getVariant("eGYSaBHRXOSLTfdJdHPSYSbC")){return false
}if(B.atlas.getVariant("eGYSaBbTZSEISUaDZGTGC")||B.atlas.getVariant("eGfESAcCJZUNZXQeJVUcDcRe")){I.off("marker-hover",b);
I.off("marker-out",D);
I.once("iw-close",function(){I.on("marker-hover",b);
I.on("marker-out",D)
})
}if(e||q){if(B.atlas.getVariant("eGYSaBdBSSEfPRdcJZXe")||B.atlas.getVariant("eGYSaBdBSSEfPRdcJZVJHO")){if(p&&p===G.id){I.closeIW();
return false
}}}if(e||q){I.on("iw-close",function(){if(p){I.setIconOut(p);
p=null
}});
if(p){I.setIconOut(p)
}p=F.b_id;
I.openIWLoading(F.b_id);
I.setIconHover(F.b_id);
if(/city|airport/.test(F.b_type)){i(I,F);
I.openIW(F.b_id,{},B.jstmpl("atlas_iw_landmark").render(F.data));
I.trigger("iw-opened-click",{marker:F,data:F.data})
}else{I.getIW(F.b_id,function(J){J.b_map_nights=x.get("checkinCheckoutInterval");
J.map_price_per_x_nights_txt=x.get("transPricePerXNights");
J.b_url=F.data.b_url;
J.b_marker_type=F.data.b_marker_type;
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){J.transLengthOfStayExperiment=x.get("transLengthOfStayExperiment")
}i(I,F);
I.openIW(F.b_id,J);
I.trigger("iw-opened-click",{marker:F,data:J});
if(B.atlas.getVariant("PcVBcOLVPIBeaT")){B.eventEmitter.trigger("wl:bind:dynamic");
I.on(I.EVENTS.IW_CLOSE,function(){$(".fly-dropdown.wl-dropdown").hide()
})
}})
}}else{if(H&&!(F.data.b_id===A&&(!B.atlas.assert.tdot&&B.atlas.assert.hp))){location.assign(H)
}}}function k(K,H){var J=this,F=J.getMarker(H),G,I=3000;
if(!H){return
}if(!F&&!C){C=true;
return J.once("markers-show",k,H)
}if(B.atlas.getVariant("eGHRXOSPCcTOSGC")){if(!F){return
}}else{if(!F||(F&&F.b_type&&/city|airport/.test(F.b_type))){return
}}if(x.get("action")=="hotel"){G=function(){k.call(J,K,H,true);
J.isBoundChangeAutoopen=false
};
J.isBoundChangeAutoopen&&J.once("bounds-change",G);
J.isBoundChangeAutoopen=false
}C=false;
if(!B.atlas.getVariant("eGBUYSaBTHVDBHT")){J.openIWLoading(H)
}if(typeof J.setIconHover==="function"){window.setTimeout(function(){J.setIconHover(H)
},0)
}if(B.atlas.getVariant("eGHRXOSPCcTOSGC")&&/city|airport/.test(F.b_type)){i(J,F);
J.openIW(F.b_id,{},B.jstmpl("atlas_iw_landmark").render(F.data));
J.trigger("iw-autoopen",J);
J.trigger("iw-opened-click",{marker:F,data:F.data});
return
}J.getIW(H,function(L){L.b_marker_type=(F.data)?F.data.b_marker_type:"";
L.b_map_nights=x.get("checkinCheckoutInterval");
L.map_price_per_x_nights_txt=x.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){L.transLengthOfStayExperiment=x.get("transLengthOfStayExperiment")
}i(J,F);
if(!B.atlas.getVariant("eGBUYSaBTHVDBHT")){J.openIW(H,L)
}else{J.trigger("iw-control-show",H,L)
}if(x.get("action")==="hotel"){I=4000
}o=setTimeout(function(){J.closeIW()
},I)
})
}function v(G){try{l=(window.localStorage&&window.sessionStorage?true:false);
if(l){window.localStorage.setItem("btest","1")
}}catch(F){l=false
}if(l){G.on("load",function(){G.trigger("load-dom-storage-supported")
})
}m(G);
G.on("bounds-change",y,G);
G.on("marker-hover",b);
G.on("marker-out",D);
G.on("marker-click",w);
G.on("close",G.closeIW);
if(B.atlas.getVariant("eGECAFDNSNDRXe")==1||B.atlas.getVariant("eGfEDXPdLedeEWHT")==1){G.on("markers-before-get",function(){G.$domNode.parent().addClass("b_map_markers_loading_4_v1")
});
G.on("markers-show",function(){G.$domNode.parent().removeClass("b_map_markers_loading_4_v1")
})
}else{if(B.atlas.getVariant("eGECAFDNSNDRXe")==2||B.atlas.getVariant("eGfEDXPdLedeEWHT")==2){G.on("markers-before-get",function(){G.$domNode.parent().addClass("b_map_markers_loading_4_v2")
});
G.on("markers-show",function(){G.$domNode.parent().removeClass("b_map_markers_loading_4_v2")
})
}}G.on("open",k,A);
if(e||q){$("body").delegate("#marker_close","click",function(){G.closeIW()
})
}}return{init:v,setupIW:m,autoopenIW:k,getIWPosition:d,markerHover:b,markerOut:D,markerClick:w,boundsChange:y}
})
});
$(function(){if(!B.atlas){return
...
x.done(function(){x.on("iw-open",function(){j("#b_map_container").find(".iw-overlay").loadComponents()
});
j("#b_map_container").delegate(".iw-overlay-city a, .iw-overlay-airport a","click",function(){x.trigger("nonproperty-iw-link-click")
});
