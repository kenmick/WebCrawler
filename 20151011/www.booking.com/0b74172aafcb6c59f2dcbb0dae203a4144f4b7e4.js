...
}function g(E,A){if(A&&A.data&&A.data.b_id&&A.data.b_type){var C={};
if(B.atlas.getVariant("ebcYSaBfBFDae")){var D=A.data.b_type;
if(A.data.b_id===w){D+="-current"
}C.className="iw-overlay-lp iw-overlay-"+D;
C.iwType=D.replace(/-/g,"_");
E.setIW(C)
}else{C.className="iw-overlay-lp iw-overlay-"+A.data.b_type+((A.data.b_id===w)?"-current":"")
}E.setIW(C)
...
}if(!B.atlas.assert.tdot&&B.env.b_action=="hotel"){u.addClass("b_map_container_fixed").css({top:G,bottom:30})
