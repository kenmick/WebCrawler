...
}else{if(booking.env.lp_add_marker_to_landmark_lps&&A.type=="landmark"){if(e||m){if(l!==A.id){if(l){C.setIconOut(l);
C.closeIW()
}g(C,A);
...
C.getIW(A.id,function(D){if(B.atlas.getVariant("eGBUYSaBTbYFbVKIGHT")){C.trigger("iw-mini-better-price");
if(w==A.id){C._currentHotel=D
}else{D.b_destination_reference=C._currentHotel
...
}}if(booking.env.lp_add_marker_to_landmark_lps&&A.b_type=="landmark"){return
...
booking[sNSExperiments][a]=(function(d,g){var f,c,o,m,e={pickSuggestion:1,useKeyboard:2};
function h(){if(i()){n()
}else{j()
}}function n(){if(m){return
}m=true;
c.addClass("active_class");
booking.track.stage(a,1)
}function j(){if(!m){return
}m=false;
setTimeout(function(){if(!m){c.removeClass("active_class");
g(document).unbind("click",l)
}},200)
}function l(p){if(!g(p.target).closest(".loc_index_sb_search_history, .recent_search_in_sb").length){j()
}}function i(p){return(p||f).val().length<=1
}function b(){return g(".ui-autocomplete").is(":visible")
}function k(p){d.track.custom_goal(a,p)
}return{init:function(){f=g(".recent_search_in_sb--active");
c=g(".loc_index_sb_search_history");
m=false;
if(!f.length){return
}f.closest(".fake-placeholder-wrapper").css("position","relative");
f.focus(function(){h();
g(document).bind("click",l)
}).blur(j).bind("click keyup",h)
},initElse:function(){if(!g("#loc_index_sb_search_history_has_data").length){return
}var q=g(".recent_search_in_sb").bind("focus click keyup",p);
function p(){if(i(q)){booking.track.stage(a,1);
q.unbind("focus click keyup",p)
}}}}
})(booking,jQuery)
})();
