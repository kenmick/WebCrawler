...
(function(ADventori, window, undefined) {
    ...
    ADventori.exec = function(cmd, args) {
      if (conf) {
        try {
          ...
          }), /^https?:/.test(conf.origin) ? conf.origin : '*');
        } catch (e) {
          try {
  ...
  (function(ADventori) {
    ...
    function adaptText(minFontSizeInPx, innerHTML) {
      ...
      // we push span to top
      while (insertionPoint >= 0 && (document.getElementById(spanId).offsetTop + document.getElementById(spanId).offsetHeight - paddingHeight) > (this.offsetHeight - paddingHeight)) {
        insertionPoint--;
