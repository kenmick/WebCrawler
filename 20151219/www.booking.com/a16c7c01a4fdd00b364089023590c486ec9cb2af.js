...
d.exports=a.extend({init:function(){this.optionsSpec={delay:{name:"delay",type:Number,defaultValue:300},modelId:{name:"sb-id",type:Number,required:true},gpOnly:{name:"gp-only",type:Boolean,defaultValue:undefined},minLength:{name:"minlength",type:Number,defaultValue:2},_shat:{name:"shat",type:Boolean,defaultValue:0},ssf:{name:"ssf",type:Number,defaultValue:0}};
...
}if(this.options._shat){this.model.configureDestination({_shat:1})
...
if(!g.fromFocus){if(this.isDeletion){this.model.resetDestination()
...
if(this.options.gpOnly){f.gp_only=true
...
}else{if(h.gp_only){return e.search(h,i)
...
if(l._ssafa){if(o.dest_type=="country"&&o.cc1){o._fa=B.env.b_nonsecure_hostname+"/country/"+o.cc1+B.env.b_query_params_with_lang
}else{o._fa=B.env.b_nonsecure_hostname+"/searchresults"+B.env.b_query_params_with_lang;
o._fasr=true
...
this._gpf=!!i.gpf;
this._ssafa=!!i._ssafa;
this._extraFields.track.push({name:"ssafas",value:B.env.b_lang_for_url=="en-us"?2:1});
this._destinationId;
...
},setDestination:function(i){if(this._searchInstance&&this._searchInstance.getResultDetails&&!i.gotDetails){this._searchInstance.getResultDetails(i.place_id,function(k,j){j=j||{};
j.gotDetails=true;
...
return
}if(i.dest_id&&i.dest_type){this._destinationId=i.dest_id;
this._destinationType=i.dest_type;
...
}}this._formAction=i&&i._fa;
this._extraFields.destinationNotSelected=null;
this._extraFields.destinationSelected=i&&i._ef;
...
this._destinationLongitude=null;
this._extraFields.destinationNotSelected=null;
this._extraFields.destinationSelected=[{name:"dest_id",value:""},{name:"dest_type",value:""}];
if(this._ssafa){this._formAction=B.env.b_nonsecure_hostname+"/search"+B.env.b_query_params_with_lang
}this.emit("change",{group:"destination"})
...
}function j(m,n){this._extraFields.destination=n&&n._ef;
this._extraFields.destinationNotSelected=n&&n._efd;
l.apply(null,arguments)
}k.ss=i.query;
k.gpf=this._gpf;
k._ssafa=this._ssafa;
...
},getFormAction:function(){return this._formAction
...
this.extraFields=[];
this.data=f(this.$el,{modelId:{name:"sb-id",type:Number,required:true},gpf:{name:"sb-gpf",type:Boolean},_ssafa:{name:"sb-ssafa",type:Boolean}});
...
}if(this.data._ssafa){i._ssafa=true
...
},modelChange:function(i){},modelSubmit:function(i){this.$form.submit()
},onlyCurrentCountryChange:function(){this.model.setOnlyCurrentCountry(this.$onlyCurrentCountry.is(":checked"))
},formSubmit:function(l){var k=this.model.getFormAction();
if(k){this.$form.attr("action",k)
}if(this.extraFields){this.extraFields.forEach(function(m){m.$node.remove();
m.$node=null
})
}this.extraFields=this.model.getExtraFields();
var j=[];
var i=false;
this.extraFields.forEach(function(m){j.push(m.name);
if(a.indexOf(m.name)!=-1){i=true
}});
this.$form.find("input").filter(function(){return(i&&a.indexOf(this.name)!=-1)||j.indexOf(this.name)!=-1
}).remove();
this.extraFields.forEach(function(m){var n=$("<input>");
n.attr("type","hidden");
n.attr("name",m.name);
