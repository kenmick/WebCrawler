...
function setupContent() {
	...
	if (o.nodata == "yes") handleNoData(2); //nodata
		...
		if (o.montant_prochaine_tirelire) {
			setValue('#details .montant .inner', o.montant_prochaine_tirelire.split(".").join(" ") + "â¬");
...
function setupOffre() {
	try {
		setValue('#outro .offreMontant', getOffre() + "â¬");
	} catch (e) {
		sel('#outro .offre').style.display = 'none';
	}
}
...
function loadBackground(sMode, sInfo, sExt) {
	...
	sel('#intro .bg').appendChild(bgImg);
}
