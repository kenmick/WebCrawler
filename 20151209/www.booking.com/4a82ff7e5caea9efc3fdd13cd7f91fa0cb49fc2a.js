...
},choose:function(f){if(this.results[f].dest_type=="suggestion"){this.autocomplete({suggestionOptOut:1,suggestedTerm:this.results[f].suggestedTerm})
...
}}}}}}}}}if(((h.MJ(h.MB(a[3])+""==="theme"))&&(h.MJ(h.track_experiment("PVcLcDODAKWUMKKe")==2)))){m+=b[18]
}m+=[b[19],h.MC(a[4]),b[20],h.F.entities(h.MC(a[5])),b[21]].join("");
m+=b[22];
if(((h.MJ(h.MB(a[3])+""==="theme"))&&(h.MJ(h.track_experiment("PVcLcDODAKWUMKKe")==2)))){m+=[b[22],h.F.entities(h.MC(a[6])),b[22]].join("")
}else{m+=b[22];
if(((h.MJ(h.MB(a[3])+""==="suggestion"))&&h.MJ(h.MB(a[6])))){m+=[b[22],(f.unshift({user_searched_term:h.MB(a[6])}),(c=h.ME(b[23],h.MB,h.MN,null)),f.shift(),c),b[22]].join("")
}else{if(h.MD(a[7])){m+=b[22];
var l=n;
var j=p;
var k=o;
var p=(h.MC(a[7])||[]);
...
m+=b[22];
if(h.MD(a[8])){m+=b[24]
}m+=[b[22],h.F.entities(h.MC(a[9])),b[22]].join("");
if(h.MD(a[8])){m+=b[25]
}m+=b[22]
}f.shift();
n=l;
...
o=k;
m+=b[22]
}else{if(h.MD(a[10])){m+=[b[22],h.MC(a[10]),b[22]].join("")
}else{if(h.MD(a[6])){m+=[b[22],h.F.entities(h.MC(a[6])),b[22]].join("")
}}}}m+=b[22];
if(h.MD(a[11])){m+=[b[26],h.MC(a[11]),b[25]].join("")
}m+=b[22];
if(h.MJ(h.MG((h.MC(a[12])||{})["popular"]))){m+=[b[27],h.F.entities(h.ME(b[28],h.MB,h.MN,null)),b[25]].join("")
}m+=b[22]
}m+=b[22];
...
if(l.theme.length&&B.et.track("PVcLcDODAKWUMKKe")==2){l.theme=[l.theme[0]]
}l.results=l.results.concat(l.theme)
}if(l.__suggestions__&&l.__suggestions__.has_results){l.results.push({dest_id:"suggestion",dest_type:"suggestion",label:l.__suggestions__.initial_term,suggestedTerm:l.__suggestions__.updated_suggestion,suggestion:l.__suggestions__,ef:false});
