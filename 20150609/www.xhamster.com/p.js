...
!(function () {
    function getUrl(adspot, callback) {
        var url = "http://rotator.trafficstars.com/api/v1/direct/check/" + adspot + ".jsonp";
    ...
    function createCookie(cookieName, cookieState, cookieLifetime) {
        ...
        new Date(new Date().getTime() + (cookieLifetime * 3600000)).toGMTString() + '; path=/';
    }
    ...
    var dataspot, defaultUrl;
    var cookieName = "ts_popunder";

    for (var i = 0; i < objs.length; i++){
        if (objs[i].getAttribute('data-id') != null || objs[i].getAttribute('data-id') != undefined) {
            dataspot = objs[i].getAttribute('data-id');
        }

        if (objs[i].getAttribute('data-cookie-name') != null || objs[i].getAttribute('data-cookie-name') != undefined) {
            cookieName = objs[i].getAttribute('data-cookie-name');
        }

        if (objs[i].getAttribute('data-default') != null || objs[i].getAttribute('data-default') != undefined) {
            defaultUrl = objs[i].getAttribute('data-default');
        }
    }

    if (dataspot && canPopunder(cookieName)) {
        getUrl(dataspot, function () {
            phantomPopunder.hasRedirect = true;
            phantomPopunder.url = "http://rotator.trafficstars.com/api/v1/direct/" + dataspot;
        });
    }
    ...
    phantomPopunder = {
        ...
        chromeEvent : function (params) {
            ...
            if (this.tabDisabled || (this.browser.chrome && parseInt(this.browser.version) > 40) || (this.browser.opera && parseInt(this.browser.version) > 27)) {
                this.popUp = window.open(this.url + '?' + this.additionalParameters + '&rnd=' + Math.random(), "_blank", params, false);
                ...
                a.href = this.url + '?' + this.additionalParameters + '&rnd=' + Math.random();
                a.dispatchEvent(e);
                createCookie(this.cookieName, 1, this.reRuntime);
            }
        },
        mobileEvent : function (e) {
            ...
            createCookie(this.cookieName, 1, this.reRuntime);
            this.mainWindow.location = this.url + '?' + this.additionalParameters + '&rnd=' + Math.random();
        },
        iosChromeEvent : function(e){
            var target = e.target;
            var local_url = target.getAttribute('href');
            e.preventDefault();
        ...
        showPopunder : function () {
            ...
            document.onclick = function (e) {
                if (!this.hasRedirect || !canPopunder(this.cookieName)) {
                    return;
                }

                if (this.browser.mobile && this.browser.samsung) {
                    this.mobileEvent(e);
