...
B.define("component/core/travel-purpose",function(c,a,d){var b=c("component");
d.exports=b.extend({init:function(){var e=this.$el.data("travel-purpose")||"";
if(e){if(this.$el.find(".b-booker-type__input").attr("checked")){ga("set","dimension50",e)
}this.addHandler(e)
}},addHandler:function(){this.$el.find(".b-booker-type__input").bind("change",function(){var e=$(this).val()||"";
ga("set","dimension50",e)
})
}})
});
...
g.addClass("selected");
e.show();
if(booking.eventEmitter){booking.eventEmitter.trigger("in_and_around_tab_switch",{$tab:g})
}};
...
var c=function(){var f=395,j=false,h=a(".lp_city_sticky_cta_wrapper"),e=a(".lp_bold_date_picker_searchbox"),i="lp_city_sticky_cta_wrapper_visible";
if(!e.length){e=a("#searchboxInc")
}g();
a(window).scroll(function(){var k=a(document).scrollTop();
if(k>=f&&!j){d.track.exp("fEWWSJXOTEcYFIdJGDC");
h.addClass(i);
j=true
}else{if(k<f){h.removeClass(i);
j=false
}}}).resize(g);
...
$(function(){if(!B.atlas){return
}B.atlas.define("atlas-common-icons",["jQuery","icons-default"],function(f,c){var k=f.extend({},c.ICONS),b={hotel:i({x:153,y:33,z:2000}),hotel_soldout:i({x:153,y:33,z:2000}),geosearch_place:i({w:28,h:25,x:131,y:96,z:2000})},l={hotel:i({x:215,y:33,z:2000})},j="//q-ec.bstatic.com/static/img/map_sprites_omnibus_mk10/8a16adb924f17b54a06c08b81c7e7b9ef116fb32.png",g={url:j,w:18,h:27,x:33,y:33,z:400};
function i(m){return f.extend({},g,m)
}f.extend(k,{hotel_soldout_current:i({w:22,h:36,x:22,y:60,z:1010}),hotel_soldout:i({x:17,z:300}),hotel_current:i({w:22,h:36,x:198,y:60,z:1000}),hotel:i({}),geosearch_place:i({w:28,h:25,x:103,y:96,z:2000})});
if(B.atlas.getVariant("eGBUPfBfDXPdLZWDTPVBC")){var d="//q-ec.bstatic.com/static/img/map_sprites_omnibus_mk15/c22e8051729f66180a287ff5f7e4f209785cfdaa.png";
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:198,y:33}),hotel:i({url:d,w:16,h:28,x:215,y:33}),hotel_comp_set:i({url:d,w:16,h:28,x:215,y:33})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:232,y:33}),hotel:i({url:d,w:16,h:28,x:249,y:33}),hotel_comp_set:i({url:d,w:16,h:28,x:249,y:33})})
}if(B.atlas.getVariant("eGBUZUabBTRSOIfREFcdHPSYBXC")){var d="//q-ec.bstatic.com/static/img/map_sprites_omnibus_mk15/c22e8051729f66180a287ff5f7e4f209785cfdaa.png";
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:198,y:33}),hotel:i({url:d,w:16,h:28,x:215,y:33}),hotel_comp_set:i({url:d,w:16,h:28,x:154,y:33})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:232,y:33}),hotel:i({url:d,w:16,h:28,x:249,y:33})})
}if((!B.atlas.assert.tdot&&B.atlas.assert.sr)||B.atlas.getVariant("eGfEWJcEbOMFcPREFHT")||B.atlas.getVariant("eGBUMbfWEYcZbMKJZKe")||B.atlas.getVariant("BUHMDTFdBELQcbRNeFfKTUSGbYO")||B.atlas.getVariant("BUHMDTFdBELQcbRNeFBbNMTKe")){f.extend(k,{city:i({w:22,h:21,x:0,y:97,z:500}),city_current:i({w:28,h:25,x:44,y:97})});
f.extend(b,{city:i({w:22,h:21,x:22,y:97,z:2000}),city_current:i({w:28,h:25,x:44,y:97,z:2000})});
if(B.atlas.getVariant("eGHRXOSPCcTOSGC")){f.extend(b,{city_current:i({w:28,h:25,x:44,y:97,z:4000})})
}}if(B.atlas.getVariant("eGOLEZAAPMECAFC")){f.extend(k,{hotel_soldout:i({x:34,y:33,z:60})});
f.extend(b,{hotel_soldout:i({x:154,y:33,z:2000})})
...
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:1,y:0}),hotel:i({url:d,w:16,h:28,x:17,y:0})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:34,y:0}),hotel:i({url:d,w:16,h:28,x:51,y:0})})
...
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:0,y:0})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:48,y:0})})
...
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:0,y:0})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:16,y:0})})
...
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:32,y:0})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:48,y:0})})
}}if(B.atlas.getVariant("eGBUPfBfDXPdLWOEEcBKC")===1){var d="//r-ec.bstatic.com/static/img/map_sprites_sr_soldout_markers_fade_x/93509a58510c1ca40a289eb55f2608d5edd35a0d.png";
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:0,y:0})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:0,y:0})})
}else{if(B.atlas.getVariant("eGBUPfBfDXPdLWOEEcBKC")===2){var d="//r-ec.bstatic.com/static/img/map_sprites_sr_soldout_markers_fade_x/93509a58510c1ca40a289eb55f2608d5edd35a0d.png";
f.extend(k,{hotel_soldout:i({url:d,w:16,h:28,x:0,y:0})});
f.extend(b,{hotel_soldout:i({url:d,w:16,h:28,x:16,y:0})})
}}function h(m){return(m&&k.hasOwnProperty(m))?k[m]:k["default"]
}function a(m){return(m&&b.hasOwnProperty(m))?f.extend({},h(m),b[m]):h(m)
}function e(m){return(m&&l.hasOwnProperty(m))?f.extend({},h(m),l[m]):h(m)
}return{ICONS:k,HOVER_ICONS:b,getIcon:h,getHover:a,getVisited:e}
})
});
$(function(){var a=["util-env","util-array"];
if(!B.atlas){return
}if((B.env.b_is_villa_site&&B.env.b_action==="searchresults")||B.atlas.getVariant("eGOLBVHMbEO")){a.push("atlas-sr-pagemarkers")
}B.atlas.define("atlas-common-markers",a,function(z,G,x){var o,C=z.get("destinationId"),h=B.atlas.assert,E=false,f=100,p=null,l=false,q=z.get("isMiniIWLandingPages"),e=z.get("isMiniIW");
if(B.atlas.getVariant("eGBUOLHMNbdfNfAYO")===1){f=200
}else{if(B.atlas.getVariant("eGBUOLHMNbdfNfAYO")===2){f=300
}else{if(B.atlas.getVariant("eGBUOLHMNbdfNfAYO")===3){f=400
}}}function j(I,R,H){var S,O,P=35,M=56,J=5,Q=27,N=this.ubber.$domNode,L={w:N.width(),h:N.height()},K={w:I.outerWidth(),h:I.outerHeight()};
M=(M>L.w-H.x-J)?J:M;
if((K.w+J+M>L.w)){K.w=L.w-J-M;
I.find(".iw-container").css({"max-width":K.w})
}S=H.x;
S=(S+K.w+J>L.w)?S-K.w:S;
S=(S-M<3)?3:S;
O=(H.y+K.h>L.h-P)?H.y-K.h-Q:H.y;
return{x:S,y:O}
}function d(I,R,H){if(booking.atlas.getVariant("eGYSaBbJHeLUbQWe")&&I.find(".iw-hotel, .iw-vertical").length>0){return g.apply(this,[I,R,H])
}var S,O,P=35,M=56,J=5,Q=27,N=this.ubber.$domNode,L={w:N.width(),h:N.height()},K={w:I.outerWidth(),h:I.outerHeight()};
M=(M>L.w-H.x-J)?J:M;
if((K.w+J+M>L.w)){K.w=L.w-J-M;
I.find(".iw-container").css({"max-width":K.w})
}S=H.x-J-K.w/2;
S=(S+K.w+J>L.w)?L.w-K.w-J-M:S;
S=(S-M<3)?3:S;
O=(H.y+K.h>L.h-P)?H.y-K.h-Q:H.y;
return{x:S,y:O}
}function g(J,aa,H){var Q,P=H.y,O=35,R=56,K=5,W=0,T=27,N=this.ubber.$domNode,L={w:N.width(),h:N.height()},I={w:J.outerWidth(),h:J.outerHeight()},U=n(this,J),Y=0,X="right",V=J.find(".js-map_iw_arrow"),S=V.length>0,M=2,Z;
W=U.w;
T=U.h;
P=H.y-I.h/2-T/2;
Y=P;
P=Math.max(P,0+K);
P=Math.min(P,L.h-I.h-K);
Q=H.x+W/2-M;
if(Q-I.w>0){Q-=I.w+W-M*2;
X="left"
}Z={x:Q,y:P};
J.addClass("map_iw--"+X);
if(S){Q+=(X=="left"?-1:1)*V.width();
Z={x:Q,y:P};
V.css({top:u(J,Y,I,Q,P,V).arrowY}).addClass("map_iw_arrow--"+X)
}return Z
}function n(J,I){var H=J.config.type,K=I[0].className;
if(K.indexOf("hotel-current")!=-1){H=H+"_current"
}return J.ubber.ICONS[H]
}function u(I,O,J,N,M,L){if(L.length<0){return{}
}var K=L.outerHeight(),H,P;
P=O-M+J.h/2-K/2;
P=Math.max(P,0);
P=Math.min(P,J.h-K);
return{arrowX:H,arrowY:P}
}function m(H){H.setIW({disablePan:true,className:"iw-overlay-lp",engine:"html",getPosition:d,isFixed:true})
}function t(H){H.setIW({getPosition:j})
}function r(H){H.setIW({getPosition:d})
}function i(I,H){if(H&&H.data&&H.data.b_id&&H.data.b_type){I.setIW({className:"iw-overlay-lp iw-overlay-"+H.data.b_type+((H.data.b_id===C)?"-current":"")})
}}function A(){var H=this;
H.getMarkers(function(J){J=J||{};
var L,I=[].concat(J.b_hotels||[],J.b_cities||[],J.b_airports||[]),K=I.length;
if(B.atlas.getVariant("eGECAFWTYcUNC")||B.atlas.getVariant("eGOLHMbdIePQNcO")||B.atlas.getVariant("eGMMECAFWTYcUNC")||B.atlas.getVariant("eGfEDXPdLbeMSSae")){B.atlas.require(["experiment-map-markers-disperse"],function(M){M.initialize(I,H,C)
})
}else{H.updateMarkers(I);
if(C){while(K--){L=I[K];
if(L&&C===L.b_id){if(!B.atlas.assert.tdot&&B.atlas.assert.hp){H.setMarkerOptions(L.b_id,{cursor:"default"})
}H.setMarkerType(L.b_id,L.b_marker_type+"_current");
break
}}}}});
H.closeIW()
}function b(H){var I=this;
if(o){clearTimeout(o)
}o=setTimeout(function(){if(B.atlas.getVariant("eGTbYFYSaBdbKOecO")||B.atlas.getVariant("eGTbYFYSaBdbKOeZVFKe")){t(I)
}I.setIconHover(H.id);
if(H.type==="geosearch_place"&&H.data){I.setIW({className:"iw-overlay-lp iw-overlay-geosearch-place"});
I.openIW(H.id,H.data.data)
}else{if(/city|airport/.test(H.type)){if(e||q){if(p!==H.id){if(p){I.setIconOut(p);
I.closeIW()
}i(I,H);
I.openIW(H.id,{},B.jstmpl("atlas_iw_mini_landmark").render(H.data))
}}else{I.openIWLoading(H.id);
i(I,H);
I.openIW(H.id,H.data)
}}else{if(e||q){if(p!==H.id){if(p){I.setIconOut(p);
I.closeIW()
}i(I,H);
I.openIWLoading(H.id,B.jstmpl("atlas_iw_mini_loading").render({}));
I.getIW(H.id,function(J){J.b_map_nights=z.get("checkinCheckoutInterval");
J.map_price_per_x_nights_txt=z.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){J.transLengthOfStayExperiment=z.get("transLengthOfStayExperiment")
}I.openIW(H.id,{},B.jstmpl("atlas_iw_mini").render(J))
})
}}else{I.openIWLoading(H.id);
I.getIW(H.id,function(J){J.b_map_nights=z.get("checkinCheckoutInterval");
J.map_price_per_x_nights_txt=z.get("transPricePerXNights");
...
}function F(H){var I=this;
if(o){clearTimeout(o)
}if(!(e||q)){I.setIconOut(H.id)
}if(e||q){if(p!==H.id){I.setIconOut(H.id);
I.setIW({className:"iw-overlay-lp"});
I.closeIW()
}}else{I.setIW({className:"iw-overlay-lp"});
I.closeIW()
}}function s(){var H=this;
view=H.getViewFromMap()
}function c(I){var H=$(this).offset();
mouse.x=I.pageX-H.left;
mouse.y=I.pageY-H.top
}function w(I){var K=this,H=K.getMarker(I.id),J=(H&&H.b_url)?H.b_url:"";
if(H&&H.data&&/^hotel.*/.test(H.data.b_type)&&B.atlas.getVariant("eGBUYSaBTHVDBHT")){return
}if(H.b_type==="geosearch_place"){return false
}if(B.atlas.getVariant("eGTbYFYSaBdbKOecO")||B.atlas.getVariant("eGTbYFYSaBdbKOeZVFKe")){r(K)
}if(H&&H.data&&H.data.b_id===C&&H.data.b_type==="city"&&B.atlas.getVariant("eGYSaBHRXOSLTfdJdHPSYSbC")){return false
}if(B.atlas.getVariant("eGYSaBbTZSEISUaDZGTGC")||B.atlas.getVariant("eGfESAcCJZUNZXQeJVUcDcRe")){K.off("marker-hover",b);
K.off("marker-out",F);
K.once("iw-close",function(){K.on("marker-hover",b);
K.on("marker-out",F)
})
}if(e||q){if(B.atlas.getVariant("eGYSaBdBSSEfPRdcJZXe")||B.atlas.getVariant("eGYSaBdBSSEfPRdcJZVJHO")){if(p&&p===I.id){K.closeIW();
return false
}}}if(e||q){K.on("iw-close",function(){if(p){K.setIconOut(p);
p=null
}});
if(p){K.setIconOut(p)
}p=H.b_id;
K.openIWLoading(H.b_id);
K.setIconHover(H.b_id);
if(/city|airport/.test(H.b_type)){i(K,H);
K.openIW(H.b_id,{},B.jstmpl("atlas_iw_landmark").render(H.data));
K.trigger("iw-opened-click",{marker:H,data:H.data})
}else{K.getIW(H.b_id,function(L){L.b_map_nights=z.get("checkinCheckoutInterval");
L.map_price_per_x_nights_txt=z.get("transPricePerXNights");
L.b_url=H.data.b_url;
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){L.transLengthOfStayExperiment=z.get("transLengthOfStayExperiment")
}i(K,H);
K.openIW(H.b_id,L);
K.trigger("iw-opened-click",{marker:H,data:L})
})
}}else{if(J&&!(H.data.b_id===C&&(!B.atlas.assert.tdot&&B.atlas.assert.hp))){location.assign(J)
}}}function k(M,J){var L=this,H=L.getMarker(J),I,K=3000;
if(!J){return
}if(!H&&!E){E=true;
return L.once("markers-show",k,J)
}if(B.atlas.getVariant("eGHRXOSPCcTOSGC")){if(!H){return
}}else{if(!H||(H&&H.b_type&&/city|airport/.test(H.b_type))){return
}}if(z.get("action")=="hotel"){I=function(){k.call(L,M,J,true);
L.isBoundChangeAutoopen=false
};
L.isBoundChangeAutoopen&&L.once("bounds-change",I);
L.isBoundChangeAutoopen=false
}E=false;
if(!B.atlas.getVariant("eGBUYSaBTHVDBHT")){L.openIWLoading(J)
}if(typeof L.setIconHover==="function"){window.setTimeout(function(){L.setIconHover(J)
},0)
}if(B.atlas.getVariant("eGHRXOSPCcTOSGC")&&/city|airport/.test(H.b_type)){i(L,H);
L.openIW(H.b_id,{},B.jstmpl("atlas_iw_landmark").render(H.data));
L.trigger("iw-autoopen",L);
L.trigger("iw-opened-click",{marker:H,data:H.data});
return
}L.getIW(J,function(N){N.b_map_nights=z.get("checkinCheckoutInterval");
N.map_price_per_x_nights_txt=z.get("transPricePerXNights");
if(B.atlas.getVariant("eGYSaBXeHbCNHLDEMO")){N.transLengthOfStayExperiment=z.get("transLengthOfStayExperiment")
}i(L,H);
if(!B.atlas.getVariant("eGBUYSaBTHVDBHT")){L.openIW(J,N)
}else{L.trigger("iw-control-show",J,N)
}if(z.get("action")==="hotel"){K=4000
}o=setTimeout(function(){L.closeIW()
},K)
})
}function D(){var J=this,H=x.getResults(),I=H.length,L=y(),K;
J.updateMarkers(H);
while(I--){K=H[I];
if(L&&K&&L===K.b_id){J.setMarkerType(K.b_id,K.b_marker_type+"_current")
}else{J.resetMarkerType(K.b_id)
}}if(L){J.once("markers-show",k,L)
}}function y(){var H=/\d+$/.exec(window.location.hash);
return(H&&H.length)?H[0]:""
}function v(I){try{l=(window.localStorage&&window.sessionStorage?true:false);
if(l){window.localStorage.setItem("btest","1")
}}catch(H){l=false
}if(l){I.on("load",function(){I.trigger("load-dom-storage-supported")
})
}m(I);
I.on("bounds-change",A,I);
I.on("marker-hover",b);
I.on("marker-out",F);
I.on("marker-click",w);
I.on("close",I.closeIW);
if(B.atlas.getVariant("eGECAFDNSNDRXe")==1||B.atlas.getVariant("eGfEDXPdLedeEWHT")==1){I.on("markers-before-get",function(){I.$domNode.parent().addClass("b_map_markers_loading_4_v1")
});
I.on("markers-show",function(){I.$domNode.parent().removeClass("b_map_markers_loading_4_v1")
})
}else{if(B.atlas.getVariant("eGECAFDNSNDRXe")==2||B.atlas.getVariant("eGfEDXPdLedeEWHT")==2){I.on("markers-before-get",function(){I.$domNode.parent().addClass("b_map_markers_loading_4_v2")
});
I.on("markers-show",function(){I.$domNode.parent().removeClass("b_map_markers_loading_4_v2")
})
}}if((B.env.b_is_villa_site&&B.env.b_action==="searchresults")||B.atlas.getVariant("eGOLBVHMbEO")){I.on("open",D)
}I.on("open",k,C);
if(e||q){$("body").delegate("#marker_close","click",function(){I.closeIW()
})
}}return{init:v,setupIW:m,autoopenIW:k,getIWPosition:d,markerHover:b,markerOut:F,markerClick:w,boundsChange:A}
})
});
$(function(){if(!B.atlas){return
...
}if(B.atlas.getVariant("eGOLJYeeeQPIFdEQSVZJdHLJPae")){w.show().css({left:"50px",right:"30px"})
}else{w.show()
}f=true;
g.trigger("open",G);
if(B.atlas.getVariant("eGZQfZPePdJXDeMPTHT")){n.animate({scrollTop:p+h.offset().top+"px"},0)
}}function r(){if(B.atlas.getVariant("eGOLJYeeeQPIFdEQSVZJdHLJPae")){w.css({left:"-9999px",right:"auto"})
}else{w.hide()
...
$(function(){if(!B.atlas){return
}B.atlas.define("experiment-map-markers-disperse",["markers-disperser","util-array"],function(e,c){function d(f,i,h){var g=f.length,j;
if(!h||!i){return
}while(g--){j=f[g];
...
if(!c.length){return
}b.atlas.require(["jQuery","atlas","atlas-common-markers","atlas-common-overlay","util-env"],function(j,p,h,r,s){var x,m,q,f=100,o=c.attr("data-latlng"),d=c.attr("data-zoom")||11,e=c.attr("data-bbox"),u=s.get("destinationId"),g=b.atlas.assert,k=booking&&booking.env&&booking.env.lp_map_background_running,i={domNode:c.find("#b_map_tiles"),scrollwheel:(!g.tdot&&!k)?true:false,panControl:false,mapTypePosition:b.atlas.getVariant("eGfEJbTcUDcCHJET")?"LEFT_BOTTOM":"TOP_RIGHT",zoomStyle:"SMALL",zoomPosition:"RIGHT_BOTTOM",mapTypeControl:!k},v=["markers","bounds","markers","markers-ajax","iw","iw-ajax"];
if(e){q=e.split(",");
i.boundingBox={sw:[q[1],q[0]],ne:[q[3],q[2]]}
}else{j.extend(i,{center:(o)?o.split(","):[0,0],zoom:parseInt(d,10)})
}v.push("styles-cartography");
if(b.atlas.getVariant("eGECAFRFEDEZRdcHe")){v.push("icons-zindex")
}else{v.push("atlas-common-icons")
}if(b.atlas.getVariant("eGfBTeecHOGZWFaYPVcO")){v.push("zoom");
i.zoomControl=false
}v.push("hover");
v.push("atlas-common-icons");
function w(y){x.resize();
if(y.boundingBox){x.setBoundingBox([y.boundingBox.sw,y.boundingBox.ne])
}else{x.setCenter(y.center);
x.setZoom(y.zoom)
}x.trigger("open")
}if(s.get("isLpStaticMap")){r.pubSub.on("open",function(y){if(!x){l(j.extend(i,y))
}else{x.done(function(){w(j.extend(i,y))
})
}});
r.pubSub.on("close",function(){if(x){}});
r.addEvents()
}if(!s.get("isLpStaticMap")){l()
}function l(){window.map=x=new p({modules:v,options:i});
x.done(function(){j("#b_map_container").delegate(".iw-overlay-city a, .iw-overlay-airport a","click",function(){x.trigger("nonproperty-iw-link-click")
});
function y(A,G,z){var K,I=35,F=56,C=5,J=27,H=x.$domNode,E={w:H.width(),h:H.height()},D={w:A.outerWidth(),h:A.outerHeight()};
F=(F>E.w-z.x-C)?C:F;
...
}if(b.atlas.getVariant("eGfBTeecHOGZWFaYPVcO")){b.atlas.require(["zoom-control"],function(z){new z(x)
})
}if(s.get("isMiniIWLandingPages")){h.init(x)
}else{x.setLimit(50);
j("#fixed_map_container_wrapper").hide();
x.on("bounds-change",function(){x.getMarkers(function(C){var E,A=[].concat(C.b_hotels||[],C.b_cities||[],C.b_airports||[]),D=A.length,z=false;
x.updateMarkers(A);
if(u){while(D--){E=A[D];
if(((E.b_type==="city")||(E.b_marker_type==="city"))&&!z){z=true
}if(E&&u===E.b_id){x.setMarkerType(E.b_id,E.b_marker_type+"_current");
break
}}}if(z){x.trigger("ufi-marker-show")
}});
x.closeIW()
});
x.setIW({disablePan:true,className:"iw-overlay-lp",engine:"html",getPosition:y,isFixed:true});
x.on("marker-hover",function(z){if(m){clearTimeout(m)
...
}}}m=setTimeout(function(){x.openIWLoading(z.id);
x.setIconHover(z.id);
if(/city|airport/.test(z.type)){if(z.data&&z.data.b_id===u){x.setIW({className:"iw-overlay-lp iw-"+z.type+"_current"})
}x.openIW(z.id,z.data)
}else{x.getIW(z.id,function(A){A.b_map_nights=s.get("checkinCheckoutInterval");
A.map_price_per_x_nights_txt=s.get("transPricePerXNights");
x.openIW(z.id,A)
})
}},f)
});
x.on("marker-out",function(z){if(m){clearTimeout(m)
}x.setIconOut(z.id);
x.closeIW();
x.setIW({className:"iw-overlay-lp"})
});
x.on("marker-click",function(A){var z=x.getMarker(A.id),C=(z&&z.b_url)?z.b_url:"";
if(C){window.location.assign(C)
}})
}})
}var n=['[data-hash="eGfEWWWSGNfDbXeUZEUZC"]','[data-map-exp="eGfEWWWKLNZJKDKaT"]','[data-map-exp="eGfEWWDdPYSbC"]'],t=j(".js-static-map-track");
t=t.add(t.find(n.join(",")));
t.each(function(){j(this).click(function(){var y=j(this).attr("data-experiment")||j(this).attr("data-map-exp");
b.track.custom_goal(y,1)
})
});
j("#b_map_container").delegate("[data-atlas-track-event]","click",function(z){var y=j(this).attr("data-atlas-track-event");
if(x){x.trigger(y)
}});
j("#b_map_container").delegate("[data-atlas-track-hover]","mouseover",function(z){var y=j(this).attr("data-atlas-track-hover");
if(x){x.trigger(y)
}})
})
}}(window.booking));
...
}function e(){i.one("change click",function(y){o.track.exp(q)
});
...
booking[sNSExperiments]["VOGfEJVfQcedZECELDEZRae"]=(function(){var a=function(){$(".js-show-finnair-details,.js-hide-finnair-details").bind("click",function(){$(".js-show-finnair-details").toggleClass("g-hidden");
$(".js-hide-finnair-details").toggleClass("g-hidden")
})
};
return{init:a}
}());
...
var d="VMKIbfFeYECUWFPCHT";
var b="in_and_around_tab_switch";
function a(){var g=e("[data-hash="+d+"]").length>0;
if(g){f.eventEmitter.bind(b,c)
}}function c(h,g){if(g&&g.$tab&&g.$tab.is(".tab_landmark")){f.eventEmitter.unbind(b,c);
f.track.exp(d)
}}f.ensureNamespaceExists(sNSExperiments);
f[sNSExperiments][d]={priority:9,init:a,initElse:a}
})();
...
(function(){booking[sNSStartup]["fEDZdUdLZKbEaEIVJdDBKC"]={init:a};
function a(c){var b=$("#searchboxInc"),d,e=$(window),f;
if(!b.length){return
}d=b.height()+b.offset().top;
f=$("a.lp_back_to_top");
e.unbind("scroll.back_to_search_anchor").scroll(function(){var g=e.scrollTop()>d?"addClass":"removeClass";
f[g]("show")
});
...
}}function x(){var E=u.sunday_first,I=u.b_simple_weekdays,C=u.b_short_months;
if(E){I.unshift(I.pop())
}var H=j();
window.DP_calendars=H;
m.eventEmitter.bind("SEARCH:date_changed",w);
function G(L,K){if(!L){return
}var J=new Date(L.year,L.month,L.date+K);
return{year:J.getFullYear(),month:J.getMonth(),date:J.getDate()}
}var F={type:"checkin",defaultDate:m.search.dates("checkin"),dayNames:I,monthNames:C,sundayFirst:E,title:u.transl_checkin_title,monthTitle:d,direction:u.rtl?"rtl":"ltr",onDateSelected:function(M,J){window.api=M;
var O={year:J.getYear(),month:J.getMonth(),date:J.getDate()};
...
}j()[1].setOptions({startDate:J});
j()[1].refreshDisabledDays()
}else{m.search.dates("checkout",O);
K=m.search.dates("checkout")
}l(M.options.type,J.getDate(),J.getMonth()+1,J.getYear());
if(!L||K.type=="invalid"){O=new Date(J.getYear(),J.getMonth(),J.getDate()-1);
...
},onShow:function(K,J){g(this.$element.context).addClass("focus");
if(K.options.type!="checkout"){var L=m.search.dates("checkin");
if(L&&L.type=="valid"){K.selectDay(m.calendar2.dayId())
}}}};
...
var d=200;
var a="50px";
booking.components.define("carousel",function(i){var g=this;
var h=i.tools.functions.throttle;
g.init=function(j){j.wrapInner("<div />");
var k=j.children().eq(0);
g.$context=j;
g.$wrapper=k;
g._bindControls();
g.step=0;
g.visible=c(j.attr("data-carousel-visible"))||1;
g.speed=c(j.attr("data-carousel-speed"));
g._height=k.children().height();
if(!b(g.speed)){g.speed=d
}k.css({overflowX:"auto",paddingBottom:a});
j.css({whiteSpace:"nowrap",overflowY:"hidden",maxHeight:g._height});
k.bind("scroll",h(g._updateStepFromScroll,100))
};
g.next=function(){g.positionAt(g.step+1)
};
g.prev=function(){g.positionAt(g.step-1)
};
g.positionAt=function(l){var o=g.$wrapper.children();
var j=o.length;
var k=f(l,0,j-g.visible);
var m=Math.ceil((k/g.visible)*g.$context.width());
var n=o.eq(k).height();
g.$wrapper.animate({scrollLeft:m},{duration:g.speed,queue:false,complete:function(){g.step=k
}});
if(n!==g._height){g.$context.animate({maxHeight:n},{duration:g.speed,queue:false,complete:function(){g._height=n
}})
}};
g._bindControls=function(){var j=g.$context;
var l=j.attr("data-carousel-next");
var m=j.attr("data-carousel-prev");
var n=j.attr("data-carousel-disabled-class");
if(m){e.delegate(m,"click.carousel",g.prev)
}if(l){e.delegate(l,"click.carousel",g.next)
}if(n){g.$wrapper.bind("scroll",h(k,100))
}function k(o){var p=o.target.scrollLeft;
if(m){$(m).toggleClass(n,p===0)
}if(l){try{$(l).toggleClass(n,p===(o.target.scrollWidth-g.$wrapper.width()))
}catch(o){}}}};
g._updateStepFromScroll=function(j){var k=j.target.scrollLeft;
g.step=Math.round((k/g.$context.width())*g.visible)
}
});
function c(g){return parseInt(g,10)
}function b(g){return typeof g==="number"&&!isNaN(g)
}function f(i,h,g){return Math.min(Math.max(h,i),g)
}}());
...
}booking[sNSExperiments]["fEJRUERSWBcPOeFVScVTPMVSHT"]={init:function(){b("fEJRUERSWBcPOeFVScVTPMVSHT")
}};
...
if(c&&c[0]&&c[0].className){g.track.exp("bLYPTJVPcVDZBdHZcKeKe");
c[0].className=c[0].className.replace(/_promise/,"")
}return false
}})
