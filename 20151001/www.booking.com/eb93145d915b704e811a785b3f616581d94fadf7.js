...
var h;
if(booking.track.getVariant("MWCMcKNBaCFJVKe")&&booking.track.getVariant("bLYUELMfdDJCHT")){h=B.require("autocomplete/google_places")
...
},_renderMenu:function(u,x){var w=this,e=false,h=b("#frm");
w.isSynonymTracked=false;
var G,E,C,g,m,k=0,F=["city","airport","district","region","country","hotel","landmark","searchHistory"],z=F.length;
u.removeClass("ui-ac-powered-by-google");
if(x.__did_you_mean__||(x.city&&x.city.length==0)){if(x.city&&x.city.length==0&&x.__upa__&&x.__upa__.length){if(x.__js_upa__){}else{if(!booking.env.b_bbtool_address_search){booking.track.exp("bLYUELMNLNPORLO")
...
var H=A&&A.length;
var p=x.city&&x.city.length;
if(!p&&!H){u.addClass("ui-ac-empty")
}else{u.removeClass("ui-ac-empty")
}if(x.__did_you_mean__){var v=x.__did_you_mean__,D=v.suggestion,r={value:D,label:D};
w.menu.suggestedTerm=D;
booking.track.exp("PVdIVMGaOVaMCfFIMNDdWe");
var d=!booking.track.getVariant("PVdIVMGaOVaMCfFIMNDdWe");
if(d){var o=booking.jstmpl.translations("sb_autocomplete_suggest",null,{searchedDestination:D});
var n=booking.jstmpl.translations("sb_autocomplete_suggestions_for",null,{searchedDestination:D});
b("#ac-suggestion").live("click",function(){b("#destination").val(D);
b("#destination").trigger("keydown.autocomplete")
});
b('<li id="ac-suggestion" class="ui-menu-item" role="menuitem" data-suggestion="'+D+'"><a class="partition_item ui-corner-all did-you-mean-item">'+o+"</a></li>").data("item.autocomplete",r).appendTo("ul.ui-autocomplete")
}}else{h.find('input[name="suggestion_clicked"], input[name="suggested_term"]').remove();
w.menu.suggestedTerm=c
}(function(l){if(!l){return
}b("#destination").keydown(function(){b("ul.ui-autocomplete").undelegate(".partition_item","click.search_rewrite");
w.menu.suggestedTerm="";
h.find('input[name="suggestion_clicked"], input[name="suggested_term"]').remove()
});
if(x.__suggestions__&&x.__suggestions__.updated_suggestion){var i=x.__suggestions__.initial_term,t={value:i,label:i};
if(x.__suggestions__.has_results){B.track.stage("PAWdLYVJXIJdJQSVWe",1)
}if(l===2&&x.__suggestions__.updated_suggestion){w.menu.suggestedTerm=x.__suggestions__.updated_suggestion
}if(l===2&&x.__suggestions__.orig_searchstring_has_result){var I=booking.jstmpl.translations("search_box_opt_in_alternative_search",null,{search_term2:x.__suggestions__.initial_term});
var j=booking.jstmpl.translations("search_box_result_your_search",null,{user_searched_term:x.__suggestions__.searched_term});
b('<li id="ac-suggestion" class="ui-menu-item search-spellcheck ac-suggestion-opt-out" role="menuitem"><span class="spellcheck-notlink">'+j+'</span><br /><a class="partition_item ui-corner-all did-you-mean-item spellcheck-islink">'+I+"</a></li>").data("item.autocomplete",t).data("suggestion",i).appendTo("ul.ui-autocomplete");
b("#ac-suggestion.search-spellcheck a").bind("click",function(){w._search(i,{v_out:0});
b("ul.ui-autocomplete").delegate(".partition_item","click.search_rewrite",function(){w.menu.suggestedTerm=""
})
})
...
for(var s=0;
s<y;
s++){if(A[s]["match_start"]){q.push(A[s])
}}if(y!=q.length){x.theme=q
}if(B.env.b_action=="index"){var f=F.shift();
F.unshift("theme");
F.unshift(f);
z=F.length
}else{delete x.theme
...
G<z;
G++){m=F[G];
if(x[m]){g=w._normalize(x[m]);
for(E=0;
E<g.length;
E++){w._renderItem(u,g[E],E,g.length)
...
if(booking.track.getVariant("PVdIZKZPHcaUSfCSRBRT")){var r=booking.jstmpl.translations("search_top_50_badge");
...
if(D(H.dest_id,l)){result=b('<li class="'+z+'"'+d+"></li>").data("item.autocomplete",H).append(b("<a "+G+">"+o+"<strong class='ac_popular_ufi' style='display:inline-block;background-color:#55AF32;color:white;padding: 1px 5px;border-radius:3px;margin-left:5px;font-size:0.9em;'>"+r+"</strong></a>")).appendTo(s);
B.track.custom_goal("PVdIZKZPHcaUSfCSRBRT",1)
}else{result=b('<li class="'+z+'"'+d+"></li>").data("item.autocomplete",H).append(b("<a "+G+">"+o+"</a>")).appendTo(s)
}}else{result=b('<li class="'+z+'"'+d+"></li>").data("item.autocomplete",H).append(b("<a "+G+">"+o+"</a>")).appendTo(s)
...
},select:function(d){var f=a("#frm"),c=f.find('input[name="ss"]'),e=f.find('input[name="ss_raw"]');
if(c.length>0){if(e.length>0){e.val(c.val())
}else{f.append(a("<input>",{name:"ss_raw",type:"hidden",value:c.val()}))
}}if(this.suggestedTerm!==undefined){var b=f.find('input[name="suggested_term"]');
if(b.length){b.val(this.suggestedTerm)
}else{f.append(a("<input>",{name:"suggested_term",type:"hidden",value:this.suggestedTerm}))
}var g=f.find('input[name="suggestion_clicked"]');
if(g.length){g.val(this.suggestedTerm?"1":"0")
}else{f.append(a("<input>",{name:"suggestion_clicked",type:"hidden",value:this.suggestedTerm?"1":"0"}))
