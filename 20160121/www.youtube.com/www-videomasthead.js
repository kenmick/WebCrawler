...
function Pa(a){return a instanceof Na&&a.constructor===Na&&a.h===Oa?a.f:"type_error:SafeUrl"}
...
;function C(a){B.call(this);this.l=1;this.h=[];this.j=0;this.f=[];this.o={};this.m=Boolean(a)}
...
var mc=p.document,nc=mc&&G?ic()||("CSS1Compat"==mc.compatMode?parseInt(jc,10):5):void 0;!dc&&!G||G&&9<=Number(nc)||dc&&lc("1.9.1");var oc=G&&!lc("9");function pc(a){return a?new qc(rc(a)):na||(na=new qc)}
...
var gd=new dd(document);gd.h=3950;function hd(a){"?"==a.charAt(0)&&(a=a.substr(1));a=a.split("&");for(var b={},c=0,d=a.length;c<d;c++){var e=a[c].split("=");if(1==e.length&&e[0]||2==e.length){var f=decodeURIComponent((e[0]||"").replace(/\+/g," ")),e=decodeURIComponent((e[1]||"").replace(/\+/g," "));f in b?"array"==ca(b[f])?Aa(b[f],e):b[f]=[b[f],e]:b[f]=e}}return b}
...
m);m=k.responseStart-k.requestStart;0<m&&(l.req_=m);m=k.responseEnd-k.responseStart;0<m&&(l.rcv_=m);S.getEntriesByType&&Ie(d)}m=Ge();k=m.pbr;l=m.vc;m=m.pbs;k&&l&&m&&k<l&&l<m&&1==Le().info.yt_vis&&"youtube"==f&&(Le().info.yt_lt="hot_bg",f=d.vc,k=d.pbs,delete d.aft,c.aft=Math.round(k-f));(f=D("PREVIOUS_ACTION"))&&(e.pa=f);e.p=D("CLIENT_PROTOCOL")||"unknown";e.t=D("CLIENT_TRANSPORT")||"unknown";window.navigator&&window.navigator.sendBeacon&&(e.ba=1);for(var ya in e)"_"!=ya.charAt(0)&&(b[ya]=e[ya]);d.ps=
w();ya={};var e=[],za;for(za in d)"_"!=za.charAt(0)&&(l=Math.max(Math.round(d[za]-g),0),ya[za]=l,e.push(za+"."+l));b.rt=e.join(",");za=b;var d=[],O;for(O in c)"_"!=O.charAt(0)&&d.push(O+"."+c[O]);za.it=d.join(",");(O=t("ytdebug.logTiming"))&&O(b,ya,c);De();D("EXP_DEFER_CSI_PING")?(Ke(),r("yt.timing.deferredPingArgs_",b,void 0),O=E(Ke,0),r("yt.timing.deferredPingTimer_",O,void 0)):Je(b);Q(Ae,new ze(ya.aft+(h||0)))}}a.cancel();Oe(a);a.C=null;a.f&&(a.f.loaded=!1);O=Ye(a);"html5"==Ze(a)?a.P=O:O&&O.destroy&&
...
n.H=function(){$e(this);if(this.P&&this.f&&this.f.args.fflags&&-1!=this.f.args.fflags.indexOf("new_html5_dispose=true"))try{this.P.destroy()}catch(b){vb(b)}this.da=null;for(var a in this.l)p[this.l[a]]=null;this.h=null;delete this.m;delete this.B;this.f&&(this.ha=this.f=this.f.fallback=null);Ne.la.H.call(this)};var cf={},df="player_uid_"+(1E9*Math.random()>>>0);function ef(a,b){a=u(a)?sc(a):a;b=sd(b);var c=df+"_"+ga(a),d=cf[c];if(d)return d.ma(b),d.h;d=new Ne(a,c);cf[c]=d;Gb("player-added",d.h);ob(d,ma(ff,d));E(function(){d.ma(b)},0);
return d.h}
...
function Wf(a,b){b=b||{};for(var c in Xf){var d=D(Xf[c]),e;if(e=d){e=a;var f=void 0;f=window.location.href;var g=J(1,e),h=Wc(J(3,e));g&&h?(e=e.match(Vc),f=f.match(Vc),e=e[3]==f[3]&&e[1]==f[1]&&e[4]==f[4]):e=h?Wc(J(3,f))==h&&(Number(J(4,f))||null)==(Number(J(4,e))||null):!0;e||(e=c,f=D("CORS_HEADER_WHITELIST")||{},e=(g=Wc(J(3,a)))?(f=f[g])?va(f,e):!1:!0)}e&&(b[c]=d)}return b}
...
function kg(a,b,c,d,e,f){$b(c,U(a,"tip-reverse"),f);var g=0;f&&(g=1);a=yd(b);f=new F((a.width-10)/2,f?a.height:0);var h=rc(b),k=new F(0,0),l;l=h?rc(h):document;var m;(m=!G||9<=Number(nc))||(m=pc(l),m=wc(m.f));b!=(m?l.documentElement:l.body)&&(l=wd(b),h=pc(h),h=vc(h.f),k.x=l.left+h.x,k.y=l.top+h.y);f=new F(k.x+f.x,k.y+f.y);f=f.clone();k=(g&8&&"rtl"==vd(c,"direction")?g^4:g)&-9;g=yd(c);h=g.clone();l=f.clone();h=h.clone();0!=k&&(k&4?l.x-=h.width+0:k&2&&(l.x-=h.width/2),k&1&&(l.y-=h.height+0));f=new td(0,
...
(k=Bd(c,"paddingLeft"),h=Bd(c,"paddingRight"),l=Bd(c,"paddingTop"),m=Bd(c,"paddingBottom"),k=new L(l,h,m,k)):(k=ud(c,"paddingLeft"),h=ud(c,"paddingRight"),l=ud(c,"paddingTop"),m=ud(c,"paddingBottom"),k=new L(parseFloat(l),parseFloat(h),parseFloat(m),parseFloat(k)));if(!G||9<=Number(nc))h=ud(c,"borderLeftWidth"),l=ud(c,"borderRightWidth"),m=ud(c,"borderTopWidth"),K=ud(c,"borderBottomWidth"),h=new L(parseFloat(m),parseFloat(l),parseFloat(K),parseFloat(h));else{h=Dd(c,"borderLeft");l=Dd(c,"borderRight");
m=Dd(c,"borderTop");var K=Dd(c,"borderBottom"),h=new L(m,l,K,h)}f.pixelWidth=g.width-h.left-k.left-k.right-h.right;f.pixelHeight=g.height-h.top-k.top-k.bottom-h.bottom}else f.pixelWidth=g.width,f.pixelHeight=g.height;g=window.document;g=wc(g)?g.documentElement:g.body;f=new ac(g.clientWidth,g.clientHeight);1==c.nodeType?(c=wd(c),h=new F(c.left,c.top)):(c=c.changedTouches?c.changedTouches[0]:c,h=new F(c.clientX,c.clientY));c=yd(d);l=Math.floor(c.width/2);g=!!(f.height<h.y+a.height);a=!!(h.y<a.height);
