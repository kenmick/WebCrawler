...
/**
 * Checks a URLâs status using an HTTP head so the file doesnât load.
...
ac_Canvas.imageDataFromFile = function (src, callback) {
	...
	// If itâs a string, just make sure it isnât empty
...
// to the $super, which is the same method name on the parentâs prototype.
/** @ignore */
	...
	for (property in proto) {
		if (proto.hasOwnProperty(property)) {
			if (typeof proto[property] === 'function') {
				...
				// Get the parameter keys for this method. If the first is $super, letâs wrap it up!
...
var ac_DeferredQueue = ac_Class( /** @lends module:ac-base.DeferredQueue# */ {
	...
	__runNextAction: function () {
		...
		// Make sure that our queue has stuff in it and weâre not waiting for a didFinish
		if (this.queue().length && !this.isRunningAction()) {
			...
			// Donât allow us to run two at the same time
			if (this._options.asynchronous === true) {
				this.setIsRunningAction(true);

				// Donât run next action yet! Wait for didFinish to be called
				return;
			}
 ...
 * <dl>
 *     <dt>âfirstâ:</dt><dd>Inserts âelementâ as the first child of âtargetâ.</dd>
 *     <dt>âbeforeâ:</dt><dd>Inserts âelementâ immediately before âtargetâ.</dd>
 *     <dt>âafterâ:</dt><dd>Inserts âelementâ immediately after âtargetâ.</dd>
 *     <dt>Default behavior:</dt><dd>Appends âelementâ as the last child of âtargetâ.</dd>
 * </dl>
...
ac_Element.insert = function (element, target, placement) {
	...
	// Placement is optional; defaults to âlastâ
	switch (placement) {
	case 'before':
...
var eachAncestor = function (element, handler) {
	...
	while (ancestor && ac_Element.isElement(ancestor)) {
		...
		// if we arenât at document.body yet
		// keep moving up
 ...
 * If no <code>cssSelector</code> is specified, returns <code>parentNode</code>, as if <code>cssSelector</code> were â*â.
...
function onReady(event) {
	...
	// Stop caring. Itâs not going to happen again!
...
module.exports = function ac_onDOMReady(callback) {
	...
	// If the window has loaded, letâs just run the action immediately
	if (doc.readyState === 'complete') {
		callback.call(win, 'lazy');

	// Otherwise, queue âer up!
	} else {
...
/**
 * A method to run once the window has loaded, if it hasnât already
 * @ignore
 */
function onLoad() {
	// Run the queued actions!
...
/**
 * @returns {Array} Names of arguments as strings as described in functionâs implementation.
...
ac_Function.iterateFramesOverAnimationDuration = function (drawFrameFunction, duration, afterFinish) {
	...
	requestAnimationFrameIteratorFunction = function (t) {
		...
		// Determine the progress weâve made relative to how long we plan to animate in total
		...
		// Weâve still got some animating to do!
		if (percentProgress < 1) {
			animationFrameRequestID = window.requestAnimationFrame(requestAnimationFrameIteratorFunction);
		// Weâve ran for our entire duration
		} else {
			// Clean up requestAnimationFrame if itâs still outstanding
...
module.exports = /** @lends module:ac-base.NotificationCenter. */ {
	...
	unsubscribe: function (event, callback, target) {
		// Clone array so that we donât modify the original while we are iterating through it
	...
	hasSubscribers: function (event, target) {
		...
		while (i--) {
			...
			// If any subscribers have a target and weâre looking to match to a target
			if (subscriber.target && target) {
				// And if the target is what weâre trying to match
				if (subscriber.target === target) {
					return true;
				}
			}
 ...
 * with the source propertyâs value. If destination is not provided a blank object is created.
...
/**
	...
	@description The Component registry is used to create useful groups of instances of Components that are relevant to their intended use. (e.g. âgalleryâ vs. âslideshowâ Components)

	<br /><br />

	They are also used to match elements to a Component based on the html classname, qualifier function, and the Component hierarchy. The model contains a structural hierarchy of Components, but the actual link is maintained by the âparentâ variable in each Component.
...
ac_Registry.prototype = {
	...
	/**
		...
		@param {String} name A unique identifier for this Component. Also used for the html classname attribute to decide whether or not a gallery is of this Component (e.g. âimageâ would be the Component if the classname was âautogallery-imageâ).
		@param {Object} properties Optional; A dictionary of keys and values that this Componentâs intended use will understand. These are the properties that will be merged with the parentsâ and <code>_base</code> properties to form the properties Object passed to whatever Object uses this Component Registry. The default is empty object, which will inherit all parent properties.
	...
	*/
	addComponent: function (name, properties, qualifier, parentComponentName, context) {
		...
		// Make sure Component name isnât reserved
		if (!this.__isReserved(name)) {
			...
			if (typeof name === 'string') {
				...
				// Create the <code>_base</code> Component if one doesnât exist.
	...
	*/
	...
	/**
		@inner
		Test each Component for a match based on html classname attribute (e.g. âautogallery-imageâ);

		@params {Element} element
	*/
	__matchName: function (element) {
		
		// If we donât have an element, fail here
		if (!ac_Element.isElement(element)) {
			return null;
		}
	...
	*/
	...
	/**
		...
		@returns {module:ac-base.Registry.Component} The Component with that name, or <code>null</code> if there isnât one in this Registry.
	*/
 ...
 * @description A Component is collection of properties in an object that is attributed to a unique identifier. The intended use for this is for creating a set of âpropertiesâ to be used for another class or function, but it can really be used for anything similar. A Component is assumed to be associated with an element on the page, which is probably associated with an instance of a class.
...
ac_Registry_Component.prototype = /** @lends @module:ac-base.Registry.Component# */ {
	...
	/**
	 * @returns {Object} The properties for this Component, extended from the properties of its parentsâ, all the way back up to _base.
	 */
	properties: function () {
	...
	/**
	 * Getter for the context Dictionary. Use this method to take advantage of context inheritance. Context inheritance means that the Component will use itâs parentâs value for a context key if it does not have something defined on itâs own context for that key. There is no Object extending with Context inheritance.
	...
	context: function (key) {
		...
		// If there is no value for this key in this context, check the parentâs if that key is set to inherit
	...
	level: function () {
		...
		// Otherwise if there is a parent the level is always 1 more than the parentâs
		} else {
 ...
 * objectâs member data. There are any number of reasons why you might want to do this,
 * but just a couple examplesÂ are:

 * <ul>
 *     <li>To validate the value youâre trying to set will work for the intended use of your variable.</li>
 ...
 *     <li>It is not of the type âfunctionâ</li>
 * </ol>
 ...
 * why youâre creating getter/setter methods in the first place. External scripts are
...
/**
 * If thereâs a console, print a message.
 ...
 *
 * This bug occurs when an image is the grandchild of an <a> and the imageâs parent nodeâs
...
},{"./data":4}],3:[function(require,module,exports){
'use strict';
...
},{"./ac-feature/cssPropertyAvailable":6,"./ac-feature/localStorageAvailable":7}],6:[function(require,module,exports){
'use strict';
...
var FeatureDetect = require('./../shared/FeatureDetect');
...
},{"./../shared/FeatureDetect":9}],9:[function(require,module,exports){
'use strict';
...
var FeatureDetect = (function () {
	...
	var tests = {
		...
		ie: (ac_Browser.IE),
		ie9: (ac_Browser.IE && ac_Browser.IE.documentMode === 9),
