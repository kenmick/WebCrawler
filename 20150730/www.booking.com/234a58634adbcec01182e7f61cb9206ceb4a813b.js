...
C++;
T--
}}}}}},_renderItem:function(D,z,G,h){var l=this;
var q=false;
z.position=G;
z.array_length=h;
var n="";
var J=", ";
var v=0;
if(booking.track.getVariant("PVdIBBVXAKQVAeFEO")==1){var H="",d="ui-menu-item",x="partition_item partition_item_"+z.dest_type;
if(z.dest_type=="theme"){H+=z.highlighted_label;
if(z.additional_label){H+=' <span class="search_ac_hl_theme">'+z.additional_label+"</span>"
}}else{v=z.labels.length;
if(v){for(var K=0;
K<v;
K++){if(z.labels[K].is_required){if(z.labels[K].is_name_hl){H+='<span class="search_ac_hl_name">'
}if(z.labels[K].text_hl){var o=z.labels[K].text,p="",F="",w="";
if(z.labels[K].text_hl[0].start>0){p=o.substring(0,z.labels[K].text_hl[0].start)
}if(z.labels[K].text_hl[0].end<o.length){w=o.substring(z.labels[K].text_hl[0].end,o.length)
}F='<span class="search_ac_hl_token">'+o.substring(z.labels[K].text_hl[0].start,z.labels[K].text_hl[0].end)+"</span>";
H+=(p+F+w)
}else{H+=z.labels[K].text
}if(z.labels[K].match){if(z.labels[K].match_hl){var I=z.labels[K].match,u="",g="",C="";
if(z.labels[K].match_hl[0].start>0){u=I.substring(0,z.labels[K].match_hl[0].start)
}if(z.labels[K].match_hl[0].end<I.length){C=I.substring(z.labels[K].match_hl[0].end,I.length)
}g='<span class="search_ac_hl_token search_ac_hl_token_alt">'+I.substring(z.labels[K].match_hl[0].start,z.labels[K].match_hl[0].end)+"</span>";
H+=" ("+u+g+C+")"
}else{H+=" ("+z.labels[K].match+")"
}}if(z.labels[K].is_name_hl){H+="</span>"
}if(K<v-1){H+=J
}}}}}result=b('<li class="'+d+'"></li>').data("item.autocomplete",z).append(b("<a "+x+">"+H+"</a>")).appendTo(D);
return result
}else{if(z.labels!=null&&typeof(z.labels)!="undefined"){v=z.labels.length
}if(typeof(z.labels)!="undefined"&&(booking.env.b_lang==="zh"||booking.env.b_lang==="xt"||booking.env.b_lang==="ja"||booking.env.b_lang==="ko")){q=true
}if(typeof(z.labels)!="undefined"&&(z.lc==="zh"||z.lc==="xt"||z.lc==="ja"||z.lc==="ko")){var M=false;
booking.track.exp("MWAAQaNZFFEbdDbaVIFTdWe");
q=true;
b.each(z.labels,function(R,Q){if(R===v-1||v==1){J=""
}if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){if(Q.type=="region"&&R!=v-1&&R!=0){var S=Q;
Q=z.labels[R+1];
z.labels[R+1]=S
}if((R!=v-1&&z.labels[R+1].type=="region"&&!M)||(R==0&&v==2&&Q.type=="region"&&!M)){J=" (";
M=true
}else{if(R==v-1&&M){J=")"
}else{if(R==v-1){J=""
}else{J=", "
}}}}if(Q.hl){if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){n=n+"<strong>"+Q.text+"</strong>"+J
}else{n=n+"<strong>"+Q.text+J+"</strong>"
}}else{n=n+Q.text+J
}})
}else{if(z.nr_hotels>=500){B.env.search_ac_is_popular_ufi=true;
if(booking.track.getVariant("PVdIZKZPHcaUC")){var i=z.labels[0];
n="";
if(i.hl===1){n+="<strong>"
}n+=i.text;
if(i.match){n+=" ("+i.match+")"
}if(i.hl===1){n+="</strong>"
}}else{n=z.label_highlighted||z.label
}}else{n=z.label_highlighted||z.label
}}var r=function(k){if(k=="undefined"||k==c||k==void 0){return 1
}else{return 0
}};
var t=booking.track.getVariant("MWBOYKbAJWe");
if(t===1||t===2){if(z.level){n='<em class="ac_item_tree_arrow"></em>'+n
}}var H=n,f="",A="",d="",x="",N="",j=z.photo||"";
if(z.header){f='<span class="autocomplete_header autocomplete_h_'+z.dest_type+'" >'+booking.env.autocomplete_categories[z.header]+"</span>";
A=(z.first)?"":' style="border-top: 1px solid #CCC;"';
d+=(z.first)?"":"first-in-group"
}if(z.nr_hotels_label&&z.nr_hotels_label.length){var s="text-align: left; display: inline;";
if(booking.env.rtl==1){s+=" direction: rtl; unicode-bidi: embed;"
}N='<span class="label" style="position: static;"><span class="property-counter" style="'+s+'">';
if(booking.env.rtl!=1&&z.rtl==1){N+='<span style="visibility:hidden;margin-left:-4px;">i</span>'
}var e=z.nr_hotels_label;
if(z.dest_type=="hotel"){if(z.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){N=""
}else{N+="("+e+")"
}}else{if(z.nr_hotels==0&&z.nr_hotels_25>0){N+='<span class="ac_prop_nearby">('+booking.env.autocomplete_counter_label+")</span>"
}else{N=""
}}if(z.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){N+="</span>"
}else{N+="</span></span>"
}}var L="";
if(z.dest_type=="theme"&&z.additional_label){L='<span class="label" style="position: static;color:#888;">&nbsp;<span style="direction: rtl; text-align: left; display: inline;">'+z.additional_label+"</span></span>"
}var y=B.tools.jsStaticUrl("/static/img/flags/16/"+z.cc1+".png");
var E="PVUELMBBdXUC";
if(z.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){var O="";
switch(z.dest_type){case"continent":O=booking.env.ac_tree_view_continent;
break;
case"country":O=booking.env.ac_tree_view_country;
break;
case"region":O=booking.env.ac_tree_view_region;
break;
case"city":O=booking.env.ac_tree_view_city;
break;
case"district":O=booking.env.ac_tree_view_district;
break;
case"airport":O=booking.env.ac_tree_view_airport;
break;
case"landmark":O=booking.env.ac_tree_view_landmark;
break;
case"hotel":O=booking.env.ac_tree_view_hotel;
break
}var m='&nbsp;&nbsp;<span class="autocomplete_icons_to_label">'+O+"</span></span>"
}else{var m='<i class="autocomplete_icons autocomplete_'+z.dest_type+'"></i>'
}if(z.dest_type!="theme"&&(r(z.cc1)!=1&&z.cc1.length==2)&&booking.track.getVariant("EKODAKSCNVAELIYRAfUZC")==1&&booking.track.getVariant("PVUELMEARLGUXO")==2){m+='<img class="search_ac_country_flags_right" src="'+y+'">'
}if(booking.env.b_is_loggedin_genius_user==1&&((z.genius_hotels/z.hotels)>0.01||z.is_genius===1)){booking.track.exp("PVUELMZNdGUKCMeMRfPWe");
if(z.dest_type!="theme"&&booking.track.getVariant("PVUELMZNdGUKCMeMRfPWe")===1){m+='<span class="ac_genius_right"><span class="ge-iconfont-plate"> <i class="bicon-dotgeniusbg"></i><i class="bicon-dotgeniusfold"></i><i class="bicon-dotgenius"></i></span></span>'
}}if(q){var P='<span class="ac-hl-cjk-wrapper">'
}else{var P="<span>"
}if((booking.track.getVariant("PVUELMNGEcdXUSPSELIae")!=0)){if(z.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){H=P+H+m+N+"</span></span>"
}else{H=P+H+N+L+"</span>"+m
}}else{if(z.dest_type!="theme"&&(r(z.cc1)!=1&&z.cc1.length==2)&&booking.track.getVariant("EKODAKSCNVAELIYRAfUZC")==1&&booking.track.getVariant("PVUELMEARLGUXO")==1){H=m+'<img class="search_ac_country_flags_left" src="'+y+'">'+P+H+N+L+"</span>"
}else{H=m+P+H+N+L+"</span>"
}}x='class="partition_item partition_item_'+z.dest_type;
if(z.dest_type!="theme"){var t=booking.track.getVariant("MWBOYKbAJWe");
if(t){x+=" ac_item_label_wrapper"
}if(z.level&&(t===1||t===2)){x+=" ac_item_tree_wrapper"
}}else{x+=" ac_item_theme"
}x+='"';
result=b('<li class="'+d+'"'+A+"></li>").data("item.autocomplete",z).append(b("<a "+x+">"+H+"</a>")).appendTo(D);
if(booking.track.getVariant("TMGCBOEQC")){if(z.position==0&&z.dest_type!="theme"){result.find("a").after(b("<span />",{"class":"sb-ac-menu-arrow-outer"}));
