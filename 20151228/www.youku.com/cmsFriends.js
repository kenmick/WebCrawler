...
var cms_friends = {
	...
	follow: function(event) {
		...
		if (this.friendId == cms_friends.getUserId()) {
			cms_friends.showError('ä¸è½è®¢éèªå·±!');
			return false;
		}
	...
	follow_callback: function(res) {
		...
		try {
			...
			cms_friends.showError('æä½å¤±è´¥, è¯·ç¨ååè¯!');
			return false;
		}
...
var cms_follow_videoupdate = {
	...
	follow: function(event) {
		...
		if (this.friendId == cms_follow_videoupdate.getUserId()) {
			cms_follow_videoupdate.showError('ä¸è½è®¢éèªå·±!');
			return false;
		}
	...
	follow_batch_callback: function(res) {
		...
		if (res.error == -302) {
			cms_follow_videoupdate.showError('ä½ å·²ç»è®¢éäºææç¨æ·!');
			return true;
		} else if (res.error != 1) {
			cms_follow_videoupdate.showError(res.zh);
			return false;
