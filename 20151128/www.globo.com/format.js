...
var collapseTimeoutId, expandTimeoutId, currentState, fullscreenElement;
var fContainerWidth
var fContainerHeight
...
window.admReady = function() {
	...
	fullscreenElement = document.getElementById( config.fullscreenStateId || "ExpandedState" );
    ...
    if( config.useFullscreen  ) {
        ...
        Browser.fullscreen.addEvent(function ( evt ) {
            if ( !Browser.fullscreen.isActive() ) {
				$( "#InpageSection" ).css('display','none');
				doCollapse();
				setTimeout(function(){
					$( "#InpageSection" ).css('display','block');
				},150);
			}
        });
    }
...
function doExpand(isInteractive) {
	...
	if( config.useFullscreen &&  Browser.app.name != "ie" ) {
			expand.width 				= stripSizeString( config.collapsedWidth );
			expand.height 				= stripSizeString( config.collapsedHeight );
			expand.widthRule 			= "absolute";
			expand.heightRule 			= "absolute";
			expand.isFixedPositioning 	= false;
			expand.isWindowCoordinates 	= false;
	}

	HTMLCreative.expand( expand );
...
function activeFullscreen( isBool ){
    if( config.useFullscreen && Browser.app.name != "ie"  ){
		...
		Browser.fullscreen[ isBool ? "enter" : "exit"]( fullscreenElement );
    }
}
...
function doExpandWithCap(freqCap, delay){
	...
	if((freqCap == 0 || HTMLCreative.getCookieCount() < freqCap)){
		doExpand();
		if (delay > 0)
...
function doResize(){
	...
	var percent = ratio >= adRatio  ? (wHeight / fContainerHeight) : (wWidth / fContainerWidth);
        ...
        $('#ExpandedState_Container').css({
            ...
            '-ms-transform': 'scale(' + percent + ',' + percent + ')',
