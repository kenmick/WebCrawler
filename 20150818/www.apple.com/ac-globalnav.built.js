...
v.bag=v.bag||{};v.bag["items"]=a(v.bag["items"]);v.links=a(v.links);v.promoLinks=a(v.promoLinks);
v.buttons=a(v.buttons);if(v.bag["items"].length===0&&!v.message){v.message={type:"empty",text:v.bag["emptyBagMsg"]}
...
}if(v.promoLinks.length>0){v.explodedPromoLinks={promoLinks:v.promoLinks}}for(var W=0;
W<v.bag["items"].length;W+=1){var Y=v.bag["items"][W]||{};Y.qty=Y.qty>1?{text:Y.qty}:false
...
}}}};v._trapFocus=function(A){var C;var B;if(this.menu.isOpen()||this._bagVisible||this._searchVisible){C=x(A);
...
}else{this.segment.hide()}};v.showBadge=function(){f.add(this.bag.tab,r);f.add(this.bag.tabSmall,r);
this.bag.badge.setAttribute("aria-hidden",false);this.bag.badgeSmall.setAttribute("aria-hidden",false);
this.bag.link.setAttribute("aria-label",this.bag.labelBadge);this.bag.linkSmall.setAttribute("aria-label",this.bag.labelBadge)
};v.hideBadge=function(){f.remove(this.bag.tab,r);f.remove(this.bag.tabSmall,r);
this.bag.badge.setAttribute("aria-hidden",true);this.bag.badgeSmall.setAttribute("aria-hidden",true);
...
}};v.showSearch=function(){if(this._searchVisible){return}f.add(this.el,m);f.add(this.scrim,m);
c.lock();this._searchVisible=true;this.searchController.focusInput()};v.hideSearch=function(){if(!this._searchVisible){return
...
};v._isBreakpointWithMenu=function(){return !!(this._breakpoints.viewport==="small"||this._breakpoints.viewport==="xsmall")
...
this.selectionController.setSelected()};f.fetchData=function(){var p="globalnav";
if(this.searchSrc&&this.searchSrc.value){p=this.searchSrc.value}this.searchResultsModel.fetchData({id:this.searchID,src:p,query:this.searchInput.value,locale:this.locale})
...
}c.exports=f},{}],171:[function(f,b,h){f("ac-polyfills/Object/keys");var p=f("mustache");
...
var s;if(t in this.strings.segments){return this.strings.segments[t]}r=Object.keys(this.strings.segments);
for(s=0;s<r.length;s++){if(t.indexOf(r[s]+"-")===0){return this.strings.segments[r[s]]
