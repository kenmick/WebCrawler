...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
	...
	check.checkUsername = function(form,fn){
		...
		if (username != "" && username != "è¯·è¾å¥é®ç®±/ç¨æ·å/ææºå·") {
			if (util.isChinese(username)) { 
				str="ä¸è½ç¨æµç§°ç»å½ï¼è¯·ä½¿ç¨é®ç®±ãææºå·æç¨æ·å";
				checkResult.errorCode = "-11";
			}else if(username=="bjccpwj@126.com" || username=="2012052510115567280"){
				...
				if(flag==0){
					str="æ ¼å¼ä¸æ­£ç¡®ï¼è¯·ä½¿ç¨é®ç®±ãææºå·æç¨æ·å"; 
	...
	check.formSubmit = function(form,fn,type){
		...
		if(check.success==checkResult.msg){
			...
			if(check.success==checkResult.msg){
				...
				var arr = new Array();
				arr.push('https://reg.cntv.cn/login/login.action?callback=?&version=2.0',
	...
	check.logout = function(from,fn){
		var arr = new Array();
		arr.push('https://reg.cntv.cn/login/logout.action?callback=?&version=2.0',
				"&service=client_transaction",
				"&from=",encodeURIComponent(from));
		var url = arr.join("");
		c.getJSON(
	...
	check.jsonpRegister = function(form,fn){
		...
		var arr = new Array();
		arr.push('https://reg.cntv.cn/api/register.action?callback=?',
