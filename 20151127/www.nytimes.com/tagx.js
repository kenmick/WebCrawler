...
TAGX.ScrollManager = (function () {
    ...
    scrollWatcher.percentageGoals = function(oArg) {
        ...
        if (milestoneSubCount <= 0) {
            milestoneSubCount = milestoneSubs;
        }
        milestoneSubCount--;
        if (percentageScrolled >= scrollWatcher.scrollMilestones[0]) {
            numberAmount = scrollWatcher.scrollMilestones[0];
            if (milestoneSubCount <= 0) {
                scrollWatcher.scrollMilestones.shift();
            }
    ...
    scrollWatcher.addSubs = function (sub) {
        ...
        if (sub.functionToRun && sub.functionToRun === scrollWatcher.percentageGoals) {
            milestoneSubs++;
        }
    };
    ...
    return {
        ...
        trigger: function (eventName, eventData, eventType) {
            ...
            if( typeof eventTypeCopy === 'object' && eventTypeCopy instanceof Array ) {
                for (intLoop = 0; intLoop < eventTypeCopy.length; intLoop++) {
                 	...
                 	if(eventTypeItem.indexOf('tagx-') === 0) {
                        ...
                        var eventDataCopy = TAGX.Utils.copyObj({}, eventData),
                            ...
                            module = eventDataCopy.module ? eventDataCopy.module.toLowerCase() : null,
                      		eventType = eventTypeItem ? eventTypeItem.toLowerCase() : action; // temp
