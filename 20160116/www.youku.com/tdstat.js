...
TUDO[ 'clickStat' ] = {
	...
	getStatTarget : function(p){
		...
		if (!check[p.nodeName]) { // åªè®°å½é¾æ¥åæé®çç¹å»
			p = p.parentNode || {}; // é¾æ¥éå¯è½æå­åç´ ï¼æ¯å¦img
			if (!check[p.nodeName])
				return false;
		}
		return p;
	...
	send: function( event , target) {
		...
		// éè¿JSè§¦åäºä»¶æ¶ï¼ä¸åºè¯¥åéCKç»è®¡
...
window.clickStat = {
	getStatTarget: function (p) {//æågetStatTargetæ¹æ³
					   return TUDO.clickStat.getStatTarget(p);
				   },
	getPCode: function (p) {//æågetPositionCodeæ¹æ³
