...
}}function V(W){Q=W.split(",").reduce(function(X,Y){X[Y]=true;
return X
},Q)
}function U(W){Q[W]=true
}function R(W){U(W);
...
var J=(function(){var Q=["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"];
var e=["HP.MAP.OPEN","RT.room.expand","HP.block.expand","TABNAV.open"];
return[].concat(Q,e)
}());
var h;
var k;
function c(Q){var S=Q.offset();
...
}function g(Q,S,R){var e=c(Q);
if(e===false){return false
}return !!(((S+R)>(e))&&Q.is(":visible"))
}var j=(function(){var e=false;
var W=[];
var V;
function Q(){v.level&&v.report(v.events.BEACON_SENT,W);
e=false;
if(W.length){j()
}}function T(){var ab=[];
var ac=[];
...
}function U(X){if(X){W.push(X)
}}function S(){W.length=0
}function R(ab,aa,ad){v.level&&v.report(v.events.INIT_BEACON,ab,aa,ad);
var ac;
var Y;
...
}function M(R,Q,S){var e;
if(R===f.experiment||R===f.goal){e=[R,Q]
}else{e=[R,Q,S]
}return e.join("-")
}function u(R){if(!O){return R
}if(l[R]){return l[R]
}var S=2166136261;
...
}function N(Q,e,R){v.level&&v.report(v.events.TRACKING_ATTEMPT,{what:Q,hash:e,id:R,variant:(Q===f.experiment||Q===f.stage)&&s(e)});
...
}}function H(Q,e,R){K[M(Q,e,R)]=true
}function q(T,S,U){v.level&&v.report(v.events.SHOULD_TRACK,T,S,U);
...
}function F(e,R,U,T,V){v.level&&v.report(v.events.ADD_EVENT_LISTENER,e,R,U,T,V);
...
}}}function m(Q,S,R,T){v.level&&v.report(v.events.ADD_DEBOUNCED_VIEW_HANDLER,Q,S,R,T);
var e=M(S,R,T);
if(K[e]){return false
}E.push([Q,S,R,T]);
...
}function i(e,Q,R){return function(S,T){if(q(R,S,T)){F(e,Q,R,S,T)
}else{v.level&&v.report(v.events.WONT_ATTACH_EVENT_TRACKING,e,Q,R,S,T)
}}
}function z(e,Q){return{track:i(e,Q,f.experiment),stage:i(e,Q,f.stage),goal:i(e,Q,f.goal),customGoal:i(e,Q,f.customGoal)}
}function p(e,R){for(var Q in R){if(R.hasOwnProperty(Q)){e[Q]=R[Q]
}}}function x(e){d.f=e.f||{};
p(d.r,e.r||{});
p(d.t,e.t||{})
}function s(e){var Q=u(e);
return d.r[Q]||d.f[Q]||0
}function t(e){if(arguments.length>1){throw"Track only accept one parameter"
}if(!e){return 0
}return N(f.experiment,e)
}function G(Q,e){if(!Q){return false
...
}function C(Q,e){if(!Q||!e||!/^[1-5]$/.test(e)){return false
