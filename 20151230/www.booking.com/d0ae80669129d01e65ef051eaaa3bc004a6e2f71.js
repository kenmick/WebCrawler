...
m+=b[16];
if(((h.MJ(h.MB(a[2])+""==="theme"))&&(h.MJ(h.track_experiment("PVcLcDODAKWUMKKe")==2)))){m+=[b[17],h.F.entities(h.MC(a[5])),b[18],h.F.entities(h.MC(a[6])),b[19],h.ME(b[20],h.MB,h.MN,null),b[21]].join("")
}else{m+=b[16];
if(h.MD(a[7])){m+=b[16];
var l=n;
var j=p;
...
m+=b[16];
if(h.MD(a[8])){m+=b[22]
}m+=[b[16],h.F.entities(h.MC(a[9])),b[16]].join("");
if(h.MD(a[8])){m+=b[23]
}m+=b[16]
}f.shift();
n=l;
...
o=k;
m+=b[16]
}else{if(h.MD(a[10])){m+=[b[16],h.MC(a[10]),b[16]].join("")
}else{if(h.MD(a[6])){m+=[b[16],h.F.entities(h.MC(a[6])),b[16]].join("")
}}}m+=b[16];
if(h.MD(a[11])){m+=[b[24],h.MC(a[11]),b[23]].join("")
}m+=b[16];
if(h.MJ(h.MG((h.MC(a[12])||{})["popular"]))){m+=[b[25],h.F.entities(h.ME(b[26],h.MB,h.MN,null)),b[23]].join("")
}m+=b[16]
}m+=b[16];
...
if(this.options.ssafa){this.model.setFormAction(c("search/searchbox/searchbox-form-action").getSearchURL())
}this.emit("change")
...
if(q._ssafa){u._fa=c("search/searchbox/searchbox-form-action").getSearchResultsURL();
u._fasr=true
}return u
});
r.results=r.__upa__;
...
if(l._ssafa){o._fa=c("search/searchbox/searchbox-form-action").getSearchResultsURL();
o._fasr=true
...
break;
default:return
}if(typeof this[h].validate=="function"){this.validators.push(this[h].validate.bind(this[h]))
...
return
}}var k=this.model.getFormAction();
