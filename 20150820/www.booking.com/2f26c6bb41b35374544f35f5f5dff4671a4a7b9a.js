...
var b=200;
var g=booking.env.b_lang_for_url.match(/^(zh|ko|ja)/)?1:2;
...
if(booking.track.getVariant("bLYUELMAAJYET")){d.bind("autocompleteclose",function(i,j){if($.isEmptyObject(f.cur_ui_item)&&d.val().length>1){var h=d.data("autocomplete").menu.active;
if(h&&h.length&&h.data("item.autocomplete")){f.cur_ui_item=h.data("item.autocomplete");
d.val(f.cur_ui_item.label)
}}})
