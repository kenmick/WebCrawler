...
/**
 * è®¾ç½®é¡µé¢ä¸­çæè¡
 * 2015çï¼å æè¡æ°æ®ç»ææåå
 * @param {String||HTMLElement} elem   IDæèèç¹
 * @param {Number} count  æ¾ç¤ºæ°é
 * @param {String} rankname  æ¾ç¤ºæè¡æ°æ®åéå
 * @param {String} cmsNodeId  æ¾ç¤ºcmsåç±»çID
 * @param {String} url æ°æ®URL
 * @param {[String]} layout [å¸å±ï¼é»è®¤ä¸ºlistï¼åå«ä»¥ä¸æ¨¡å¼ï¼photo, photo2, photo3, video, list]
 * @param {Function} callback åè°å½æ°
 */
function setRank2015(elem, count, rankname, cmsNodeId, url, layout, callback) {
    ...
    if ( typeof(rankname) == "object" ) {// ä¸ºå è½½æ°æ®åå°±å­å¨è¿ä¸ªåéåç§»é¤
        window["rankname"] = "";
    };
    require([url], function() {
