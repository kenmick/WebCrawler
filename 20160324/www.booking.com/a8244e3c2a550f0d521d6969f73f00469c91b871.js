...
if(!a(k)){return _r_( k);
}if(k.forEach){k.forEach(function(l,i,m){_i_("96bbb:537");j.call(g,l,i,m)
;_r_()})
}else{for(f=0,h=k.length;
f<h;
++f){j.call(g,k[f],f,k)
}}return _r_( k);
}function c(h,g){_i_("96bbb:74");var f=b(h,function(i){_i_("96bbb:510");return _r_( i===g);
});
return _r_((f>-1)?true:false);
}return _r_({each:d,find:e,findIndex:b,isArray:a,contains:c});
});
B.atlas.define("util-getset",function(){_i_("96bbb:4");function a(b){_i_("96bbb:75");this.set=function(c,d){_i_("96bbb:372");if(c){b[c]=d;
return _r_( d);
}return _r_( undefined);
};
this.get=function(c){_i_("96bbb:373");return _r_( c?b[c]:undefined);
}
;_r_()}return _r_( a);
});
...
}var l=function(p,q,o){_i_("96bbb:374");var n="success";
o.success.call(o.context,p,n,q);
d(n,q,o)
;_r_()};
var j=function(n,p,q,o){_i_("96bbb:375");o.error.call(o.context,q,p,n);
d(p,q,o)
;_r_()};
var d=function(n,p,o){_i_("96bbb:376");o.complete.call(o.context,p,n)
;_r_()};
m.onload=function(){_i_("96bbb:377");var n,o;
...
m.onerror=function(){_i_("96bbb:378");j(null,"error",m,e);
return _r_();
};
m.open(e.type,e.url);
...
function h(){_i_("96bbb:316");if(a.SUBSCRIPTIONS&&b.jst){n={};
j.each(a.SUBSCRIPTIONS,function(o,p){_i_("96bbb:555");j.each(p,function(q,r){_i_("96bbb:590");if(r.name&&r.touched===true){n[r.name]=b.jst[r.name]
};_r_()})
;_r_()})
}return _r_( n);
}function f(){_i_("96bbb:317");j.post(m,{sid:l.b_sid,pid:l.pageview_id,context:"atlas",data:JSON.stringify({metrics:i.metrics,actions:i.actions,experiments:h()})});
i={metrics:[],actions:[]}
;_r_()}function g(q,p){_i_("96bbb:318");var r,o=window.performance&&window.performance.now?window.performance.now():new Date().getTime();
if(c){clearTimeout(c)
}p=p||"";
...
if(!f){return _r_();
}B.atlas.require(["polygons"],function(h){_i_("96bbb:379");h.createPolygon.call(e,{clickable:false,fillColor:"#ff0000",fillOpacity:0.1,strokeColor:"#ff0000",strokeOpacity:0.5,strokeWeight:1,paths:[f.ne,[f.sw[0],f.ne[1]],f.sw,[f.ne[0],f.sw[1]]]});
...
k.set=function(o,p){_i_("96bbb:380");if(o){i[o]=p;
return _r_( p);
}return _r_( f);
};
k.get=function(o){_i_("96bbb:381");return _r_( o?i[o]:f);
};
d.extend(k,new a(k));
k.trigger("pre-init");
k.$domNode=d(j.options.domNode);
function l(o){_i_("96bbb:325");setTimeout(function(){_i_("96bbb:541");n=true;
k.Interface=o;
k.trigger("ready",k)
;_r_()},4)
;_r_()}h.unshift(m,"center","options","analytics","goals");
h.push("debug");
B.atlas.require(h,function(s){_i_("96bbb:382");var p,o,r,q=[].slice.call(arguments,1);
for(p=0,o=q.length;
p<o;
++p){r=q[p];
...
s.init(k,i,l)
;_r_()});
k.done=function g(o){_i_("96bbb:383");if(n){o.call(k)
}else{k.on("ready",o)
};_r_()};
...
k.initialize(l,i,h)
}g.prototype.initialize=function(m,i,h){_i_("96bbb:326");var k=this,j=k.get("isInitialized"),l=this.get("domNode");
if(j){return _r_( false);
}b(l,k);
i.unshift(m);
B.atlas.require(i,function(n){_i_("96bbb:542");c.each([].slice.call(arguments,1),function(o){_i_("96bbb:586");if(o&&o.init){o.init.call(k)
}else{if(o){f.extend(k,o)
}};_r_()});
k.trigger("api-before-load");
n.init(k,h,function(o){_i_("96bbb:587");setTimeout(function(){_i_("96bbb:595");k.set("hasLoaded",true);
k.Interface=o;
k.trigger("ready",k)
;_r_()},4)
;_r_()})
...
g.prototype.done=function(h){_i_("96bbb:327");if(this.get("hasLoaded")){h.call(this)
...
function j(r){_i_("96bbb:328");if(o){console.debug("Atlas event: "+r)
};_r_()}function h(u,t){_i_("96bbb:329");var s=this,r=[];
t=t||null;
j(u);
if(m.hasOwnProperty(u)){b.each(m[u],function(v,w){_i_("96bbb:566");w.handler.call(s,t,w.args,u);
if(w.once){r.push([u,w.handler])
};_r_()});
b.each(r,function(v,w){_i_("96bbb:567");p(w[0],w[1])
;_r_()})
}if(d){B.atlas.track(u,d.get("id"))
}if(n.hasOwnProperty(u)){b.each(n[u],function(v,w){_i_("96bbb:568");w.handler.call(s,t,w.args,u)
;_r_()})
};_r_()}function i(s,u,r,t){_i_("96bbb:330");s=(s&&typeof s==="string")?[s]:s;
b.each(s,function(w,v){_i_("96bbb:543");if(t){n[v]=n[v]||[];
n[v].push({handler:u,args:r||null})
}else{m[v]=m[v]||[];
m[v].push({handler:u,args:r||null})
};_r_()})
;_r_()}function e(s,t,r){_i_("96bbb:331");s=(s&&typeof s==="string")?[s]:s;
b.each(s,function(v,u){_i_("96bbb:544");m[u]=m[u]||[];
m[u].push({handler:t,args:r||null,once:true})
;_r_()})
;_r_()}function p(r,s){_i_("96bbb:332");if(m.hasOwnProperty(r)&&(!s||s===true)){m[r]=[]
}else{if(m.hasOwnProperty(r)){m[r]=b.grep(m[r],function(t){_i_("96bbb:591");return _r_( t.handler!==s);
})
}};_r_()}function g(r){_i_("96bbb:333");return _r_( m[r]||[]);
}function l(){_i_("96bbb:334");var r=[];
b.each(m,function(s){_i_("96bbb:545");r.push(s)
;_r_()});
return _r_( r);
}function k(r){_i_("96bbb:335");o=r;
return _r_( o);
}i([c.MARKERS_SHOW,c.TILES_LOAD],function(t,s,r){_i_("96bbb:384");if(r===c.MARKERS_SHOW&&q&&!f){h(c.LOAD,d.get("id"))
...
B.atlas.require(l,function(){_i_("96bbb:385");var m=m=[].slice.call(arguments,0);
c.extend.apply(j,[j].concat(m));
h(j)
;_r_()},null,true)
;_r_()}d.prototype.done=function(j){_i_("96bbb:336");j.call(this)
;_r_()};
return _r_( d);
});
jQuery.extend(B.atlas,(function(i,d){_i_("96bbb:66");var b=null,g=null;
function c(){_i_("96bbb:319");b=null;
g=null
;_r_()}function j(){_i_("96bbb:320");var k=d.atlas.SUBSCRIPTIONS;
b={};
g={};
if(!k||!d.jst){return _r_();
}i.each(k,function(l,m){_i_("96bbb:512");i.each(m,function(n,q){_i_("96bbb:583");try{if(q.name&&q.cond&&d.jst.hasOwnProperty(q.name)&&q.old){b[q.name]=parseInt(d.jst[q.name],10);
...
;_r_()})
}function a(k){_i_("96bbb:321");if(!b){j()
}return _r_( b.hasOwnProperty(k)?b[k]:false);
}function e(l,m){_i_("96bbb:322");var n=[],k=[],o=d.atlas.SUBSCRIPTIONS;
if(!o||!l||!o.hasOwnProperty(l)){return _r_( false);
}o[l].forEach(function(p){_i_("96bbb:513");if(a(p.name)!==false&&!p.touched&&m===p.instanceId){if(p.stage){if(g[p.name]){d.et.stage(p.name,p.stage)
...
}k.forEach(function(p){_i_("96bbb:514");d.track.stage.apply(null,p)
;_r_()})
}function h(m,k){_i_("96bbb:323");var n=d.atlas.SUBSCRIPTIONS,l=n&&n.templates?n.templates:[];
l.some(function(o){_i_("96bbb:515");if(o.name===m&&a(o.name)!==false){if(g[o.name]){d.et.stage(o.name,k||o.stage)
}else{d.track.stage(m,k||o.stage)
}return _r_( true);
}return _r_( false);
});
return _r_( true);
}function f(l){_i_("96bbb:324");var m=d.atlas.SUBSCRIPTIONS,k=m&&m.templates?m.templates:[];
k.some(function(n){_i_("96bbb:516");if(n.name===l&&a(n.name)!==false){if(!n.touched){if(g[n.name]){d.et.track(n.name)
}else{d.track.exp(l)
}n.touched=true
}return _r_( true);
}return _r_( false);
...
return _r_( a(l));
}d.run("Xfgrwdtafew").onLoad(function(){_i_("96bbb:368");if(!b){j()
};_r_()});
...
this.extend(f)
};_r_()}a.prototype={extend:function(h){_i_("96bbb:386");var f=d(h),e=this.sw,g=this.ne;
...
},reset:function(){_i_("96bbb:387");this.ne=this.sw=null
;_r_()},getNorthEast:function(){_i_("96bbb:388");return _r_( this.ne||new c(0,0));
},getSouthWest:function(){_i_("96bbb:389");return _r_( this.sw||new c(0,0));
},setNorthEast:function(e){_i_("96bbb:390");this.ne=d(e)
;_r_()},setSouthWest:function(e){_i_("96bbb:391");this.sw=d(e)
;_r_()},getCenter:function(){_i_("96bbb:392");return _r_( new c((this.sw.lat+this.ne.lat)/2,(this.sw.lng+this.ne.lng)/2));
},isEmpty:function(){_i_("96bbb:393");return _r_((!this.sw&&!this.ne)||(this.sw.toString()===this.ne.toString()));
},equals:function(e){_i_("96bbb:394");return _r_( e instanceof a?this.toString()===e.toString():false);
},intersects:function(g){_i_("96bbb:395");var e=this.sw,i=this.ne,f=g.sw,h=g.ne;
return _r_( h.lat>=e.lat||f.lat<=i.lat||h.lng>=e.lng||f.lng<=i.lng);
},union:function(e){_i_("96bbb:396");this.extend(e.getSouthWest());
this.extend(e.getNorthEast());
return _r_( this);
},contains:function(g){_i_("96bbb:397");var i,f,e,h;
...
},toMOMString:function(){_i_("96bbb:398");b(this.sw);
b(this.ne);
return _r_([this.sw.lng,this.sw.lat,this.ne.lng,this.ne.lat].join(","));
},toString:function(){_i_("96bbb:399");b(this.sw);
b(this.ne);
...
this.lng=+b||0
}};_r_()}a.prototype={set:function(c,b){_i_("96bbb:400");this.lat=+c;
this.lng=+b;
return _r_( this);
},clone:function(){_i_("96bbb:401");return _r_( new a(this.lat,this.lng));
},toString:function(){_i_("96bbb:402");return _r_(["[",this.lat,",",this.lng,"]"].join(""));
}};
return _r_( a);
});
...
;_r_()}f.prototype={extend:function(h){_i_("96bbb:403");this.bounds.extend(h);
this.zoom=false;
return _r_( this);
},extrapolate:function(i,l){_i_("96bbb:404");var k=new d(i),m=b.latLngToPoint(k,l),n=new e(this.width/2,this.height/2),h=new e(m.x+n.x,m.y-n.y),j=new e(m.x-n.x,m.y+n.y);
this.setNorthEast(b.pointToLatLng(h,l));
this.setSouthWest(b.pointToLatLng(j,l));
return _r_( this);
},getCenter:function(k){_i_("96bbb:405");var i=this.bounds.getCenter(),j=this.getZoom(),h;
if(k){h=b.latLngToPoint(i,j).subtract(k);
i=b.pointToLatLng(h,j)
}return _r_( i);
},getNorthWest:function(){_i_("96bbb:406");var h=this.bounds.getCenter(),j=this.getZoom(),l=new e(this.width/2,this.height/2),k=b.latLngToPoint(h,j),i=k.subtract(l);
return _r_( b.pointToLatLng(i,j));
},getSouthEast:function(){_i_("96bbb:407");var i=this.bounds.getCenter(),j=this.getZoom(),l=new e(this.width/2,this.height/2),k=b.latLngToPoint(i,j),h=k.add(l);
return _r_( b.pointToLatLng(h,j));
},setNorthEast:function(h){_i_("96bbb:408");this.bounds.setNorthEast(h)
;_r_()},setSouthWest:function(h){_i_("96bbb:409");this.bounds.setSouthWest(h)
;_r_()},getZoom:function(h,m){_i_("96bbb:410");var q=h||g,j=true,i=this.bounds.getNorthEast(),n=this.bounds.getSouthWest(),k=new d(i.lat,n.lng),l=new d(n.lat,i.lng),o,p;
if(!this.zoom){m=m||a;
...
},latLngToPixel:function(j){_i_("96bbb:411");var i=this.getZoom(),h=b.latLngToPoint(this.getNorthWest(),i);
return _r_( b.latLngToPoint(j,i).subtract(h));
},pixelToLatLng:function(h){_i_("96bbb:412");var j=this.getZoom(),i=b.latLngToPoint(this.getNorthWest(),j);
return _r_( b.pointToLatLng(h.add(i),j));
},getDistance:function(i,h){_i_("96bbb:413");return _r_( b.getDistance(i,h));
},toString:function(){_i_("96bbb:414");return _r_( this.bounds.toString());
}};
return _r_( f);
});
B.atlas.define("geometry.point",function(){_i_("96bbb:16");function a(b,c){_i_("96bbb:103");this.x=+b;
this.y=+c
;_r_()}a.prototype={set:function(b,c){_i_("96bbb:415");this.x=+b;
this.y=+c;
return _r_( this);
},clone:function(){_i_("96bbb:416");return _r_( new a(this.x,this.y));
},add:function(b){_i_("96bbb:417");return _r_( new a(this.x+b.x,this.y+b.y));
},subtract:function(b){_i_("96bbb:418");return _r_( new a(this.x-b.x,this.y-b.y));
},toString:function(){_i_("96bbb:419");return _r_(["[",this.x,",",this.y,"]"].join(""));
}};
return _r_( a);
});
...
;_r_()}return _r_({init:function(){_i_("96bbb:369");var e=this;
if(!B.atlas||!c.get("analytics")){return _r_();
}d.each(a,function(g,h){_i_("96bbb:556");var f={topic:h.evt,value:h.value||""};
...
b=[{evt:"load",name:"atlas_hp_open",condition:a.hp,filter:function(e){_i_("96bbb:517");if(e==="tdotPOI"){return _r_( false);
}return _r_( true);
}},{evt:"load",name:"atlas_sr_open",condition:a.sr},{evt:"load",name:"atlas_lp_open",condition:a.lp},{evt:"close",name:"atlas_hp_close",condition:a.hp},{evt:"close",name:"atlas_sr_close",condition:a.sr},{evt:"close",name:"atlas_lp_close",condition:a.lp},{evt:"marker-click",name:"atlas_hp_click_property",condition:a.hp,filter:function(e){_i_("96bbb:518");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-click",name:"atlas_sr_click_property",condition:a.sr,filter:function(e){_i_("96bbb:519");return _r_( !/city|airport|vpm/.test(e.type));
}},{evt:"marker-click",name:"atlas_lp_click_property",condition:a.lp,filter:function(e){_i_("96bbb:520");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-click",name:"atlas_hp_click_ufi",condition:a.hp,filter:function(e){_i_("96bbb:521");return _r_(/city/.test(e.type));
}},{evt:"marker-click",name:"atlas_sr_click_ufi",condition:a.sr,filter:function(e){_i_("96bbb:522");return _r_(/city/.test(e.type)&&!/vpm/.test(e.type));
}},{evt:"marker-click",name:"atlas_sr_click_airport",condition:a.sr,filter:function(e){_i_("96bbb:523");return _r_(/airport/.test(e.type));
}},{evt:"marker-click",name:"atlas_lp_click_ufi",condition:a.lp,filter:function(e){_i_("96bbb:524");return _r_(/city/.test(e.type));
}},{evt:"marker-click",name:"atlas_lp_click_airport",condition:a.lp,filter:function(e){_i_("96bbb:525");return _r_(/airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_hp_hover_property",condition:a.hp,filter:function(e){_i_("96bbb:526");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_sr_hover_property",condition:a.sr,filter:function(e){_i_("96bbb:527");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_lp_hover_property",condition:a.lp,filter:function(e){_i_("96bbb:528");return _r_( !/city|airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_hp_hover_ufi",condition:a.hp,filter:function(e){_i_("96bbb:529");return _r_(/city/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_sr_hover_ufi",condition:a.sr,filter:function(e){_i_("96bbb:530");return _r_(/city/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_sr_hover_airport",condition:a.sr,filter:function(e){_i_("96bbb:531");return _r_(/airport/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_lp_hover_ufi",condition:a.lp,filter:function(e){_i_("96bbb:532");return _r_(/city/.test(e.type));
}},{evt:"marker-hover-triggered",name:"atlas_lp_hover_airport",condition:a.lp,filter:function(e){_i_("96bbb:533");return _r_(/airport/.test(e.type));
}}];
...
}return _r_({init:function(){_i_("96bbb:370");var e=this;
if(this.get("id")){return _r_();
}d.each(b,function(g,f){_i_("96bbb:557");if(f.condition){e.on(f.evt,c,f,true)
};_r_()})
}});
});
B.atlas.define("iw-ajax",["jQuery","util-env","util-array"],function(b,d,j){_i_("96bbb:24");var c=[],a=[];
function i(l){_i_("96bbb:122");var k=j.findIndex(c,function(m){_i_("96bbb:534");return _r_( m[0]===l);
});
if(k>-1){c[k][1].abort();
c.splice(k);
...
l=j.find(a,function(n){_i_("96bbb:535");return _r_( n[0]===k&&n[1]===m);
});
...
o=b.ajax({url:m,error:function(s,r){_i_("96bbb:558");if(r!=="abort"){l.trigger("iw-get-fail",l)
};_r_()},success:function(r){_i_("96bbb:559");l.trigger("iw-get-success",r,l);
...
a();
this.on("offset-change",function(p){_i_("96bbb:420");o.IW.OPTIONS.mapOffset=p
;_r_()})
}function a(){_i_("96bbb:129");if(!B.jstmpl){return _r_();
}B.jstmpl.fn.HELPER_FROM_FRICE=function(o){_i_("96bbb:421");return _r_((B.env.map_vars.translation_from.replace("price_placeholder",o)).replace("price_placeholdernbsp;","$ "));
};
B.jstmpl.fn.HELPER_IS_DEFINED=function(o){_i_("96bbb:422");return _r_( !!o||o===0||o===false);
};
B.jstmpl.fn.HELPER_ENV=function(o){_i_("96bbb:423");return _r_((B.env.hasOwnProperty(o))?B.env[o]:"");
};
B.jstmpl.fn.HELPER_IW_B_BLOCKS=function(q,o,p){_i_("96bbb:424");if(!q){return _r_( false);
}if(q[o]&&q[o][p]){return _r_( q[o][p]);
}if(q.b_room_group&&q.b_room_group[o]&&q.b_room_group[o][p]){return _r_( q.b_room_group[o][p]);
}return _r_( false);
};
B.jstmpl.fn.ATLAS_GET_VARIANT=function(o){_i_("96bbb:425");return _r_((B.atlas&&B.atlas.getVariant)?B.atlas.getVariant(o):false);
};
B.jstmpl.fn.ATLAS_TRACK=function(o){_i_("96bbb:426");return _r_((B.atlas&&B.atlas.trackInTemplate)?B.atlas.trackInTemplate(o):false);
};
B.jstmpl.fn.ATLAS_STAGE=function(p,o){_i_("96bbb:427");return _r_((B.atlas&&B.atlas.stageInTemplate)?B.atlas.stageInTemplate(p,o):true);
};
B.jstmpl.fn.ATLAS_ENV=function(o){_i_("96bbb:428");return _r_((o)?b.get(o):false);
};
B.jstmpl.fn.strings=function(q){_i_("96bbb:429");var o="";
try{o=B.strings(q)
...
B.atlas.define("hover",function(){_i_("96bbb:27");function c(){_i_("96bbb:139");this.on("marker-hover",function(d){_i_("96bbb:430");a.call(this,d.data.b_id)
;_r_()});
this.on("marker-out",function(d){_i_("96bbb:431");b.call(this,d.data.b_id)
;_r_()})
...
var e=this;
f.forEach(function(i){_i_("96bbb:432");var h;
...
}B.atlas.define("mixed-markers",["util-object","util-array"],function(i,g){_i_("96bbb:337");function h(v){_i_("96bbb:433");var u,t=[];
if(g.isArray(v)){return _r_( v);
}for(u in v){if(v.hasOwnProperty(u)){t.push(v[u])
}}return _r_( t);
}function j(v,u){_i_("96bbb:434");var t=h(v);
this.pruneMarkers(t);
this.addMarkers(t,u)
;_r_()}function r(){_i_("96bbb:435");return _r_( this.Interface.getActiveMixedMarkers());
}function n(){_i_("96bbb:436");this.Interface.clearMixedMarkers()
;_r_()}function p(t){_i_("96bbb:437");this.Interface.clearMixedMarker(t)
;_r_()}function l(u){_i_("96bbb:438");var t=h(u);
this.Interface.pruneOutboundedMixedMarkers();
this.addMarkers(t)
;_r_()}function o(x){_i_("96bbb:439");var w,v,u=this.Interface.getActiveMixedMarkers(),t=u.length;
while(t--){w=u[t].b_id;
v=x.length;
while(v--){if(x[v].b_id===w){break
}}if(v<0&&w){this.Interface.clearMixedMarker(w)
}};_r_()}function a(C,x){_i_("96bbb:440");var A,w,v=[],y=0,u=h(C),t=u.length;
this.trigger("markers-create",this);
while(t--){A=u[t];
...
;_r_()}function b(v){_i_("96bbb:441");var u,t=this.Interface.getMixedMarker(v);
...
}};_r_()}function c(t){_i_("96bbb:442");this.Interface.centerMixedMarker(t,this.get("offset"))
;_r_()}function k(t){_i_("96bbb:443");this.Interface.panMixedMarker(t,this.get("offset"))
;_r_()}function q(v,u){_i_("96bbb:444");var t=this.Interface.getMixedMarker(v);
if(u&&t){t.b_type=u;
...
}};_r_()}function f(t){_i_("96bbb:445");return _r_( this.Interface.getMixedMarker(t));
}function m(u,t){_i_("96bbb:446");return _r_( this.Interface.setMixedMarkerOptions(u,t));
}function e(t){_i_("96bbb:447");if(typeof this.Interface.animateMarker==="function"){this.Interface.animateMarker(t)
};_r_()}function s(){_i_("96bbb:448");return _r_( this.Interface.mixedMarkersExist());
}function d(t){_i_("96bbb:449");if(typeof this.Interface.getMixedMarkerPosition==="function"){return _r_( this.Interface.getMixedMarkerPosition(t));
...
m=b.ajax({url:k+";BBOX="+l,error:function(p,o){_i_("96bbb:560");if(o!=="abort"){j.trigger("markers-get-fail",j)
};_r_()},success:function(o){_i_("96bbb:561");j.trigger("markers-get-success",o,j);
...
;_r_()}f.prototype.getDetails=function(i){_i_("96bbb:338");var g=this.get("map"),h=this.get("place_id");
...
if(i.length>0){e.each(i,function(k){_i_("96bbb:547");if(k.place_id&&k.terms){h.push(new g(k,j))
};_r_()})
}return _r_( h);
...
a.push({context:this,evt:e,handler:h,listener:google.maps.event.addListener(this.core,e,function(){_i_("96bbb:592");f.ubber.ubber.trigger("circle-"+e,this);
h.call(f,g)
;_r_()})});
return _r_( this);
...
}d=setTimeout(function(){_i_("96bbb:584");b.trigger("bounds-change")
;_r_()},150)
;_r_()});
...
this.setMap(h)
;_r_()}b.prototype.remove=function(){_i_("96bbb:339");if(this.$domNode){this.$domNode.remove();
this.$domNode=null
}this.setMap(null)
;_r_()};
b.prototype.draw=function(){_i_("96bbb:340");var g=this,h=this.config,f=(h.isFixed)?this.ubber.$domNode:d(this.getPanes().floatPane);
...
;_r_()}).delegate(h.closeSelector,"click",function(i){_i_("96bbb:569");i.stopPropagation();
g.close()
;_r_()});
this.open()
}else{if(!h.isFixed&&this.isOpen){this.setPosition()
}};_r_()};
b.prototype.checkOverlay=function(j,h){_i_("96bbb:341");var i=this.ubber.get("topOverlay"),g=i?i:this.ubber.$domNode,f=this.getPanes().floatPane;
if(j&&h.get(0)!==g.get(0)){h.appendTo(g)
}else{if(!j&&h.get(0)!==f){h.appendTo(d(f))
}};_r_()};
b.prototype.open=function(h,g){_i_("96bbb:342");var k=d.extend(this.config,(h||{})),j=[e,k.className],i=this.$domNode,f=this.getProjection();
...
b.prototype.getPosition=function(g,f){_i_("96bbb:343");var h=this.config;
return _r_({x:f.x+h.offset.x-g.outerWidth()/2,y:f.y+h.offset.y});
};
b.prototype.setPosition=function(){_i_("96bbb:344");var g,m,l,k=this.config,h=this.$domNode,f=this.getProjection(),j=(k.getPosition)||this.getPosition,i=new google.maps.LatLng(k.latLng[0],k.latLng[1]);
if(!f||!this.$domNode){return _r_();
...
b.prototype.close=function(){_i_("96bbb:345");if(this.$domNode){this.isOpen=false;
this.$domNode.addClass(a);
this.$domNode.hide();
this.ubber.trigger("iw-close")
};_r_()};
b.prototype.panMap=function(){_i_("96bbb:346");var h=this.getMap(),g=this.config,n=d(h.getDiv()),i=this.$domNode,p=n.width(),l=n.height(),k=i.outerWidth()/2,o=i.outerHeight(),f=this.getProjection().fromLatLngToContainerPixel(new google.maps.LatLng(g.latLng[0],g.latLng[1])),m=0,j=0;
...
B.atlas.define("provider-google-markers",["jQuery","util-array"],function(f,l){_i_("96bbb:47");function g(o){_i_("96bbb:209");return _r_( l.findIndex(this.markers,function(p){_i_("96bbb:471");return _r_( p.b_id===String(o));
}));
...
}function n(o){_i_("96bbb:214");return _r_( l.find(this.markers,function(p){_i_("96bbb:472");return _r_( p.b_id===String(o));
}));
}function m(){_i_("96bbb:215");return _r_( f.map(this.markers,function(o){_i_("96bbb:473");return _r_({b_id:o.b_id,b_name:o.b_name,b_type:o.b_type,b_url:o.b_url,b_latitude:o.b_latitude,b_longitude:o.b_longitude,b_persistent:(o.data)?o.data.b_persistent:false,b_behaviors:(o.data)?o.data.b_behaviors:false,b_states:(o.data)?o.data.b_states:false});
}));
...
}else{q.ubber.once("idle",function(){_i_("96bbb:570");q.map.setCenter(this.getOffsetLatLng(p.getPosition(),r))
;_r_()})
...
}else{q.ubber.once("idle",function(){_i_("96bbb:571");q.map.panTo(q.getOffsetLatLng(p.getPosition(),r))
;_r_()})
...
;_r_()}a.prototype.onAdd=function(){_i_("96bbb:347");g(this)
;_r_()};
a.prototype.render=function(j){_i_("96bbb:348");var l=this.options,i=l.items,k=d(j||{});
...
a.prototype.position=function(k,m){_i_("96bbb:349");var j=this.getProjection(),i=google.maps,n,o,l;
...
a.prototype.add=function(j){_i_("96bbb:350");var k=this.options,i=this.options.queue;
...
a.prototype.remove=function(m){_i_("96bbb:351");var i=this.options.items,k=this.options.overlayNode,j,l;
j=f.findIndex(i,function(n){_i_("96bbb:562");return _r_( n.id===m);
});
if(j>-1){l=i.splice(j,1)[0]
}if(k&&l&&k.contains(l.domNode)){k.removeChild(l.domNode)
};_r_()};
a.prototype.removeAll=function(){_i_("96bbb:352");var i=this.options.items,j=this.options.overlayNode,k;
while(j&&i.length){k=i.pop().domNode;
if(j.contains(k)){j.removeChild(k)
}}this.options.items=[]
;_r_()};
a.prototype.draw=function(){_i_("96bbb:353");var j,i=this;
g(this);
j=this.options.items;
j.forEach(function(k){_i_("96bbb:548");i.position(k.options,k.domNode)
;_r_()})
;_r_()};
a.prototype.onRemove=function(){_i_("96bbb:354");var i=this.options.items,j=this.options.overlayNode;
...
d.each(j,function(o){_i_("96bbb:477");l=d.findIndex(m,function(p){_i_("96bbb:585");return _r_( o===p);
});
if(l>-1&&l<k){k=l;
n=m[k]
};_r_()});
...
i.atlas=f;
if(i.data){i.data.getImage=function(j){_i_("96bbb:549");return _r_( h.getImage.call(h,j));
}
...
;_r_()}b.prototype.extend=function(f){_i_("96bbb:355");this.set("data",d.extend(this.get("data")||{},f));
return _r_( this);
};
b.prototype.render=function(f){_i_("96bbb:356");if(this.get("atlas").Interface.map){this.renderMarker(f)
}else{this.renderHTML(f)
}this.set("isRendered",true)
;_r_()};
b.prototype.renderMarker=function(g){_i_("96bbb:357");var f=this.get("marker"),j=this.get("atlas").Interface.map,i=google.maps,h=e.normalizeMarker.call(this,g);
if(!j){return _r_();
...
b.prototype.renderHTML=function(f){_i_("96bbb:358");var i=this.get("$domNode"),g=c(this.get("atlas").get("resultsNode")),h=f.className,j=f.content||"";
if(!i){i=this.set("$domNode",c('<div class="'+h+'"/>'));
i.html(j);
g.append(i)
}else{i.html(j)
};_r_()};
b.prototype.remove=function(){_i_("96bbb:359");if(this.get("atlas")&&this.get("atlas").Interface.map){this.removeMarker()
}else{this.removeHTML()
}this.set("isRendered",false)
;_r_()};
b.prototype.removeMarker=function(){_i_("96bbb:360");var f=this.get("marker");
if(f){f.setMap(null);
f.set("marker",null)
};_r_()};
b.prototype.removeHTML=function(){_i_("96bbb:361");c(this.get("$domNode")).remove();
this.set("$domNode",null)
;_r_()};
b.prototype.getImage=function(g){_i_("96bbb:362");var j=this.get("data"),i=g?g.width:80,f=g?g.height:80,h;
...
b.prototype.getDetails=function(h){_i_("96bbb:363");var f=this.get("atlas"),g=this.get("id");
f.getPlaceDetails({placeId:g},h,this)
;_r_()};
b.prototype.getMarkerConfig=function(g){_i_("96bbb:364");var h=this.get("coordinates"),f;
f={b_id:this.get("id"),b_latitude:h.lat,b_longitude:h.lng,data:this.get("data")};
d.extend(f,g);
return _r_( f);
};
b.prototype.open=function(h){_i_("96bbb:365");var g=this.get("iw"),f=this.get("marker"),j=this.get("atlas").Interface.map,i=d.extend({},h||{});
if(!j){return _r_();
...
b.prototype.close=function(){_i_("96bbb:366");var f=this.get("iw");
...
c.each(m,function(s){_i_("96bbb:572");if(c.contains(q.types||[],s)){r=true
};_r_()});
...
function b(e){_i_("96bbb:249");return _r_( d.map(e,function(f){_i_("96bbb:488");return _r_( new google.maps.LatLng(f[0],f[1]));
}));
...
this.core.getPath().forEach(function(g){_i_("96bbb:573");f.extend(g)
;_r_()});
e=f.getCenter();
return _r_([e.lat(),e.lng()]);
},on:function(e,h,g){_i_("96bbb:491");var f=this;
c.push({context:this,evt:e,handler:h,listener:google.maps.event.addListener(this.core,e,function(){_i_("96bbb:593");f.ubber.ubber.trigger("polygon-"+e,this);
h.call(f,g)
;_r_()})});
return _r_( this);
...
B.atlas.define("provider-google-style",function(){_i_("96bbb:55");return _r_({setStyle:function(a){_i_("96bbb:371");this.map.setOptions({styles:a})
;_r_()}});
});
...
this._markerWrapper.onselectstart=function(){_i_("96bbb:550");return _r_( false);
};
...
this._draggableListner=google.maps.event.addDomListener(S,"mousedown",function(T){_i_("96bbb:536");R.startDrag(T)
;_r_()});
this._setCursor("draggable")
}function O(){_i_("96bbb:287");var R=this;
if(this._markerWrapper.setCapture){this._markerWrapper.setCapture(true);
this._draggableListner=[google.maps.event.addDomListener(this._markerWrapper,"mousemove",function(S){_i_("96bbb:574");R.drag(S)
;_r_()},true),google.maps.event.addDomListener(this._markerWrapper,"mouseup",function(){_i_("96bbb:575");R.stopDrag();
R._markerWrapper.releaseCapture()
;_r_()},true)]
}else{this._draggingListeners=[google.maps.event.addDomListener(window,"mousemove",function(S){_i_("96bbb:576");R.drag(S)
;_r_()},true),google.maps.event.addDomListener(window,"mouseup",function(){_i_("96bbb:577");R.stopDrag()
;_r_()},true)]
...
var S=this;
google.maps.event.addDomListener(this._markerContent,"click",function(T){_i_("96bbb:552");google.maps.event.trigger(S,"click",T)
;_r_()});
google.maps.event.addDomListener(this._markerContent,"mouseover",function(T){_i_("96bbb:553");google.maps.event.trigger(S,"mouseover",T)
;_r_()});
google.maps.event.addDomListener(this._markerContent,"mouseout",function(T){_i_("96bbb:554");google.maps.event.trigger(S,"mouseout",T)
;_r_()})
}this._ready=true;
...
B.atlas.define("provider-google-mixed-markers",["util-array","util-object","provider-google-overlay-marker"],function(o,d,h){_i_("96bbb:58");function m(r){_i_("96bbb:293");return _r_( o.findIndex(this.markers,function(s){_i_("96bbb:497");if(s){return _r_( s.b_id===String(r));
}}));
}function e(){_i_("96bbb:294");var v,u=this.markers,t=u.length,s=google.maps.event,r=this.map.getBounds();
while(t--){v=u[t];
if(r&&!r.contains(v.getPosition())&&!v.data.b_persistent){s.clearInstanceListeners(v);
v.setMap(null);
u.splice(t,1)
}};_r_()}function i(){_i_("96bbb:295");var s,r=this.markers.length;
while(r--){s=this.markers[r];
google.maps.event.clearInstanceListeners(s);
s.setMap(null)
}this.markers=[]
;_r_()}function j(t){_i_("96bbb:296");var r=m.call(this,t),s=(r>-1)?this.markers[r]:null;
if(s){google.maps.event.clearInstanceListeners(s);
s.setMap(null);
this.markers.splice(r,1)
};_r_()}function n(){_i_("96bbb:297");return _r_( this.markers?this.markers.length>0:false);
}function g(r){_i_("96bbb:298");return _r_( o.find(this.markers,function(s){_i_("96bbb:498");if(s){return _r_( s.b_id===String(r));
}}));
}function q(){_i_("96bbb:299");var s;
try{s=this.markers.map(function(w){_i_("96bbb:565");var v={};
Object.keys(w).forEach(function(x){_i_("96bbb:594");if(/^b_/.test(x)){v[x]=w[x]
};_r_()});
return _r_( v);
})
}catch(u){var t,r;
...
}function c(v,t,s){_i_("96bbb:300");var u=this.getMixedMarker(v),r;
...
}};_r_()}function p(v,u){_i_("96bbb:301");var s=this,t=this.getMixedMarker(v),r=s.map.getProjection();
if(t){if(r){s.map.setCenter(this.getOffsetLatLng(t.getPosition(),u))
}else{s.ubber.once("idle",function(){_i_("96bbb:579");s.map.setCenter(this.getOffsetLatLng(t.getPosition(),u))
;_r_()})
}};_r_()}function l(v,u){_i_("96bbb:302");var s=this,r=s.map.getProjection(),t=this.getMixedMarker(v);
if(t){if(r){s.map.panTo(s.getOffsetLatLng(t.getPosition(),u))
}else{s.ubber.once("idle",function(){_i_("96bbb:580");s.map.panTo(s.getOffsetLatLng(t.getPosition(),u))
;_r_()})
}};_r_()}function k(t,r){_i_("96bbb:303");var s=this.getMixedMarker(t);
if(s){s.setOptions(r)
};_r_()}function b(t){_i_("96bbb:304");var v,x=B.env,s=google.maps,w=s.event,r=this.ubber,z,u,y;
...
w.addListener(v,"click",function(A){_i_("96bbb:499");if(A&&typeof A.preventDefault==="function"){A.preventDefault()
}r.trigger("marker-click",{evt:A,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
;_r_()});
w.addListener(v,"mouseover",function(A){_i_("96bbb:500");r.trigger("marker-hover",{evt:A,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
;_r_()});
w.addListener(v,"mouseout",function(A){_i_("96bbb:501");r.trigger("marker-out",{evt:A,id:this.b_id,type:this.b_type,latLng:[this.b_latitude,this.b_longitude],data:this})
;_r_()});
this.markers.push(v)
}function a(s){_i_("96bbb:305");var r=this.getMixedMarker(s);
if(typeof r.setAnimation==="function"){r.setAnimation(google.maps.Animation.DROP)
};_r_()}function f(v){_i_("96bbb:306");var s=new google.maps.OverlayView(),t=this.getMixedMarker(v),r,u;
s.draw=function(){};
s.setMap(this.map);
r=s.getProjection();
...
function m(x){_i_("96bbb:307");return _r_((x instanceof Array)?x:false);
}function c(y,z){_i_("96bbb:308");this.markers=[];
...
k=new j(a,c);
function o(y,x,z){_i_("96bbb:309");if(!k.isLoading){k.append("GLOBAL_ATLAS_GOOGLE_MAPS_CALLBACK",y)
}k.queue(y,x,z,c)
...
a=new h(c,b);
function f(j,i,k){_i_("96bbb:311");if(!a.isLoading){a.get(j)
}a.queue(j,i,k,b)
;_r_()}return _r_({init:f});
});
B.atlas.define("provider-loader",["jQuery","util-env"],function(f,i){_i_("96bbb:61");var b=i.get("killSwitch"),h={};
function a(k,l){_i_("96bbb:312");if(h.hasOwnProperty(k)){return _r_( h[k]);
}this.url=k;
...
this.Interface=l;
h[k]=this
}a.prototype={queue:function g(){_i_("96bbb:505");var k=[].slice.call(arguments,0);
if(this.isLoaded){this.getInterface.apply(this,k)
}else{this.stack.push(k)
};_r_()},get:function d(k){_i_("96bbb:506");var l=this,m=this.url;
if(b){return _r_();
}if(k.get("channel")){m=e(m,"channel",k.get("channel"))
}f.getScript(m,function(){_i_("96bbb:581");k.trigger("api-load",k);
l.isLoaded=true;
while(l.stack.length>0){l.getInterface.apply(l,l.stack.pop())
};_r_()});
this.isLoading=true
},append:function c(n,k){_i_("96bbb:507");var l=document.createElement("script"),m=this,p=this.url;
if(b){return _r_();
}window[n]=function o(){_i_("96bbb:582");k.trigger("api-load",k);
m.isLoaded=true;
...
this.isLoading=true
},getInterface:function j(m,k,n,l){_i_("96bbb:508");l=l||this.Interface;
n.call(m,new l(k,m))
;_r_()}};
function e(k,m,n){_i_("96bbb:313");var l=new RegExp("\\b("+m+"=).*?(&|$)");
...
function b(i,k){_i_("96bbb:314");var j=this;
j.ubber=k;
B.atlas.require(["provider-google-places"],function(){_i_("96bbb:509");var l=[].slice.call(arguments,0);
while(l.length>0){g.extend(j,l.pop())
};_r_()})
;_r_()}$.extend(b.prototype,e);
a=new h(c,b);
function f(j,i,k){_i_("96bbb:315");if(!a.isLoading){a.append("GLOBAL_ATLAS_GOOGLE_MAPS_CALLBACK",j)
}a.queue(j,i,k,b)
;_r_()}return _r_({init:f});
});
