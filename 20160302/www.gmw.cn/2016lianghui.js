...
$.getJSON("http://bbs.gmw.cn/api/jp_times.php?jsoncallback=?", function(timeDateObj){
    ...
    if(timeDateObj["t_y"] == "2016" && timeDateObj["t_m"] == "03" && parseInt(timeDateObj["t_d"]) >= 2 && parseInt(timeDateObj["t_d"]) <= 13){
...
function show_ads(obj){
	...
	if(thisUrl.indexOf("m.gmw.cn") >= 0){
		sqdt += '<a style="display:block;" href="http://m.gmw.cn/node_66952.htm"><img src="'+mtopimg+'" width="100%" border="0" /></a>';
		// å°å·å¢æææ·»å è³BODYå¤´é¨
    	$("body").prepend(sqdt);
	}else{
		sqdt += '<div style="width:1000px;margin:0 auto;position:relative;text-align:center;z-index:999">';
		sqdt += '    <div style="position:absolute;top:60px;left:-142px;z-index:99999"><a href="'+linkurl+'"><img src="'+couplet_l+'" width="140" height="240" border="0"/></a></div>';
		sqdt += '    <div style="position:absolute;top:60px;left:1002px;z-index:99999"><a href="'+linkurl+'"><img src="'+couplet_r+'" width="140" height="240" border="0"/></a></div>';
		sqdt += '</div>';
