...
x.on("marker-hover",function(z){var y=/^soldout/.test(z.type);
B.track.stage("ebcEKOFaGBfDeOeXefLJDC",1);
if(!y){x.once("iw-get-success",A)
}function A(E){var D,C;
var F;
if(E&&E.b_hotels&&E.b_hotels[z.id]){F=E.b_hotels[z.id];
C=(F.can_accommodate_group==false);
D=F.soldout;
if(D||C){x.trigger("marker-iw-inconsistent");
if(D){B.track.custom_goal("ebcEKOFaGBfDeOeXefLJDC",1)
}if(C){B.track.custom_goal("ebcEKOFaGBfDeOeXefLJDC",2)
}}}}});
