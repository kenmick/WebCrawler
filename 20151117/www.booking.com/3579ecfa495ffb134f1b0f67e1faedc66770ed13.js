...
}function c(O,N,P){v.level&&v.report(v.events.SHOULD_TRACK,O,N,P);
if(n[e(O,N,P)]){v.level&&v.report(v.events.NOT_TRACKING_WAS_TRACKED,O,N,P);
return false
}else{if(O===K.experiment||O===K.stage){var M=1<<(P||0),L=d(N);
if(m.r[L]===undefined){v.level&&v.report(v.events.NOT_TRACKING_NOT_RUNNING,O,(P||0),N,(P||0));
return false
}else{if(m.f[L]){v.level&&v.report(v.events.NOT_TRACKING_FULLON,O,N,P);
return false
}else{if(m.t[L]&M){f(O,N,P);
v.level&&v.report(v.events.NOT_TRACKING_WAS_TRACKED,O,N,P);
return false
}}}}else{if(O===K.customGoal){var L=d(N);
if(m.r[L]===undefined){v.level&&v.report(v.events.NOT_TRACKING_NOT_RUNNING,O,P,N,P);
return false
}else{if(m.f[L]){v.level&&v.report(v.events.NOT_TRACKING_FULLON,O,N,P);
return false
}}}}}return true
