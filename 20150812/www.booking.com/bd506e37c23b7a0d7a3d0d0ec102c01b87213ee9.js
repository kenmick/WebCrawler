...
}if(v.updated_suggestion){var i=v.updated_suggestion,l={value:i,label:i};
if(k){var j=booking.jstmpl.translations("search_box_result_your_search",null,{user_searched_term:v.searched_term});
var t=booking.jstmpl.translations("search_box_opt_in_alternative_search",null,{search_term2:v.updated_suggestion})
}b('<li id="ac-suggestion" class="ui-menu-item search-spellcheck" role="menuitem" data-suggestion="'+i+'"><a class="partition_item ui-corner-all did-you-mean-item">'+j+"<br /><span>"+t+"</span></a></li>").data("item.autocomplete",l).appendTo("ul.ui-autocomplete");
b("#ac-suggestion.search-spellcheck").on("click",function(){ var _r_, _f_; try { _f_ = function() {b("#destination").val(i);
n._search(i,{v:0,fcparam:1})
}; _r_ = _f_.apply( this, [].slice.apply( arguments ) ); } catch( _e_ ) { onBookingError( _e_.message, _e_.sourceURL, 0, 0, _e_, _f_ ); onBookingError.skip = true; throw _e_; } return _r_; })
