//security ( Stanford Javascript Crypto Library / http://bitwiseshiftleft.github.io/sjcl/ )
 
  //"use strict";
  function q(a){"use strict";throw a;}var t=void 0,u=!1;var sjcl={cipher:{},hash:{},keyexchange:{},mode:{},misc:{},codec:{},exception:{corrupt:function(a){this.toString=function(){return"CORRUPT: "+this.message};this.message=a},invalid:function(a){this.toString=function(){return"INVALID: "+this.message};this.message=a},bug:function(a){this.toString=function(){return"BUG: "+this.message};this.message=a},notReady:function(a){this.toString=function(){return"NOT READY: "+this.message};this.message=a}}};
...
sjcl.misc.V={};sjcl.misc.cachedPbkdf2=function(a,b){var c=sjcl.misc.V,d;b=b||{};d=b.iter||1E3;c=c[a]=c[a]||{};d=c[d]=c[d]||{firstSalt:b.salt&&b.salt.length?b.salt.slice(0):sjcl.random.randomWords(2,0)};c=b.salt===t?d.firstSalt:b.salt;d[c]=d[c]||sjcl.misc.pbkdf2(a,c,b.iter);return{key:d[c].slice(0),salt:c.slice(0)}};
  ...
  Promotron = {}
  if(typeof(promotronOpts) === "undefined"){
    promotronOpts = {};
  }
  if(typeof(promotronOpts.minlimit) === "undefined"){
    promotronOpts.minlimit=4;
  }
  if(typeof(promotronOpts.maxlimit) === "undefined"){
    promotronOpts.maxlimit=6;
  }
  //promotronOpts.minlimit--; promotronOpts.maxlimit--;
  //console.log(promotronOpts);
  
  require(['foundation/main'], function () {
  require(['jquery/nyt', 'underscore/nyt','foundation/views/page-manager',], function($,_,PageManager) {
    // set up resize to fire after ads load
    var $mainHeight = $('.span-ab-layout.layout > .ab-column').height();
    var $colHeight = $('.c-column.column').height();
    var $diff = ($mainHeight)*1.0;
    
    PageManager.on('nyt:ads-rendered', function(adView) {
      // console.log("mainHeight:",$mainHeight, "colHeight:",$colHeight, "diff:",$diff);
              setTimeout(function(){
              Promotron.resizeCcol(adView);
              // console.log("nyt:ads-rendered event detected.")
              },2000)
            })
    ...
    if (typeof(window.localStorage) === 'undefined') {
      ...
      try {
        window.localStorage.setItem("ls:beacon","a");
        window.localStorage.removeItem("ls:beacon");
    ...
    Promotron.fetchReadingHistory = function() {
      return $.ajax("https://www.nytimes.com/svc/recommendations/v3/userhist.jsonp?access_key=776ACB1E-6C7F-4702-9E18-CF329376F5A3", {
        dataType: 'jsonp'
      }).done(function( data ) {
        // store reading history urls as read
          // localStorage.fetchReadingHistory=JSON.stringify(data.articles);
          /// this function will loop through local store for this url is it there? if not adds it. if so just update the bestof value
        // store featured promos
          /// this function will loop through local store for this url is it there? if not adds it. if so just update the featured value
        // store pinned promos
          /// this function will loop through local store for this url is it there? if not adds it. if so just update the pinned value
        // store geo_targeted promos
          /// this function will loop through local store for this url is it there? if not adds it. if so just update the geo_targeted value
      });
    }

    Promotron.fetchRankings = function() {
      // TO DO: SUBSTITUTE NEW WEEKEND READS PROMOTRON ID
      return $.ajax("http://int.nyt.com/data/promotron/items/568c343162266c000100001e.js", { dataType: 'json' });
    }

    Promotron.recordServe = function(slug, urlsServed) {

      var currentServes;
      if(localStorage.promotronServes) {
        currentServes = JSON.parse(localStorage.promotronServes);
      } else {
        currentServes = {};
      }

      _(urlsServed).each(function(url) {
        if(currentServes[url]) {
          currentServes[url] += 1;
        } else {
          currentServes[url] = 1;
        }
      ...
      // TODO prune if object is too big.
      if(hasLocalStorage) {
        localStorage.promotronServes=JSON.stringify(currentServes);
      }
    }

    Promotron.getServeExclusions = function() {
      var exclusions = [];
      var currentServes;
      if(hasLocalStorage){
        if(localStorage.promotronServes) {
      ...
      _(currentServes).each(function(count, url) {
        if(count>=5){
          exclusions.push(url);
        }
      });
    ...
    Promotron.buildPromos = function(rankings, readingHistory, opts) {
      var promos, exclusions;
      // Exclude anything you've already read.
      exclusions = _.pluck(readingHistory.articles, 'url');

      if(opts.exclusions) {
        exclusions = exclusions.concat(opts.exclusions);
      }

      // TODO: exclude any items marked as clicked in localstorage

      // check current view counts in localstorage and exclude if > 5
      exclusions = exclusions.concat(Promotron.getServeExclusions());

      // scrub querystrings from exclusions.
      exclusions = _(exclusions).map(function(url){
        return url.split('?')[0];
      });

      // Start with pinned.
      promos = rankings.pinned;

      // Then add Best Of.
      if(promos.length < opts.limit) {
        var bestOfs = _(rankings.best_of).difference(exclusions);
        promos = promos.concat(_.shuffle(bestOfs));
      }

      // If we still need items, add featured.
      if(promos.length < opts.limit) {
        var featureds = _(rankings.featured).difference(exclusions);
        promos = promos.concat(_.shuffle(featureds));
      }

      // Second chance: if we're still under the limit (all bestOf or featured items read),
      // append a random selection.
      if(promos.length < opts.limit) {
        var remainders = _.flatten([rankings.best_of, rankings.featured]);
        
        // still consider opts.exclsuions a hard exclusion.
        if(opts.exclusions) {
          remainders = _.difference(remainders, opts.exclusions);
        }
        promos = promos.concat(_.shuffle(remainders));
      }

      promos = _.uniq(promos);
      promos = promos.slice(0, opts.limit);

      console.log('and we got...', _(promos).map(function(url) { return rankings.items[url]; }))

      return _(promos).map(function(url) { return rankings.items[url]; });
    }

    Promotron.renderPromos = function(promos, template, minlimit, maxlimit) {
      // var mainHeight = $('.span-ab-layout.layout > .ab-column').height();
      // var colHeight = $('.c-column.column').height();
      // var diff = (mainHeight-colHeight)*1.0;
          // console.log("render=> mainHeight:",$mainHeight, "colHeight:",$colHeight, "diff:",$diff);
          if($colHeight > $diff){
            //$('#watching-beta-opt-in').css("height",$diff);
          }
      $('#watching-beta-opt-in').empty();
      $("#watching-beta-opt-in-container").css("overflow","hidden");


      template ='<article class="story theme-feature promo" data-rank="0" data-collection-renderstyle="HpHedSumMediumMediaFeature">'+
          '<figure class="media photo medium-thumb">'+
            '<div class="image">'+
              '<a href="<%= promos.url %>?contentCollection=weekendreads">'+
                '<img alt="" src="<%= promoImage.url %>">' +
              '</a>'+
            '</div>'+
          '</figure>'+
         ...
         ' <h2 class="story-heading">'+
            '<a href="<%= promos.url %>?contentCollection=weekendreads">'+
             ' <%= promos.headline_override || promos.headline %>'+
           ' </a>'+
           ' <span class="product-label theme-nyt-now hidden">'+
             '</span>'+
          '</h2>'+
         // '<p class="byline">'+
         //  'By <%= promos.authors[0] %> </p>'+ //this needs to be changed to render ALL authors not just the first. 
          '<p class="summary">'+
           '<%= promos.summary_override || promos.summary  %>'+
          '</p>'+
          '</article>';
           ...
           // for (var i=0; i < promos.length ; i++ ){
            for (var i=0; i < promos.length ; i++ ){
              var output = _.template(template);
                  promo = promos[i],
                  promoImage = null;
              // console.log("resize=> mainHeight:",$mainHeight, "colHeight:",$colHeight, "diff:",$diff);

              // decide which image to use from the promotron v2 data structure
              if (promo.promo_image_override && promo.promo_image_override.thumbWide) {
                promoImage = promo.promo_image_override.thumbWide
              } else if (promo.promo_image && promo.promo_image.thumbWide) {
                promoImage = promo.promo_image.thumbWide;
              }

              $("#watching-beta-opt-in").append(output({
                promos:promo,
                promoImage: promoImage
              }));
                // hide the promos that are over the test.
                if(i>minlimit){
                  // console.log("i"+i,"minlimit:"+minlimit,"maxlimit:"+maxlimit)
                   $("#watching-beta-opt-in article").eq(i).css("display","none")
                } 
                if( $("#watching-beta-opt-in-container").height() > $diff){
                  $('#watching-beta-opt-in-container article:last').remove()
                  // console.log("append break cause #watching-beta-opt-in-container > $diff:"+$diff)
                  break;
                  // TODO: add part that parses keys for un appended promos and drops their count by one. 
              // this will prevent them from hitting 5 and dropping out from being shown.
                }
            }
          });
      
      //$('#watching-beta-opt-in-container').css("height",diff)
      
    }

    // this is where the magic happens
    $.when(Promotron.fetchRankings(),Promotron.fetchReadingHistory()).then(
      function(rankingsRes, readingHistoryRes) {

        // Transform from Promotron v2 data structure to expected structure:
        var newRankingsRes = {
          pinned: [],
          best_of: [],
          featured: _.map(rankingsRes[0], function(v, k) {
            return k;
          }),
          items: rankingsRes[0]
        };

        rankingsRes[0] = newRankingsRes;


        var promos;
        var existingPromos;
        
        // TODO: move this out of when() block so that we don't make ajax requests if we're just reading promos from        
          if(hasLocalStorage && localStorage.promotronPromos) {
            existingPromos = JSON.parse(localStorage.promotronPromos);
          } 
          
          if(typeof(existingPromos) === "undefined" || typeof(existingPromos.promos) === "undefined") {
            existingPromos = { promos: [], timestamp: 0 };
          }
          
          var timedOut = Date.now() - existingPromos.timestamp > 360000;
          var hpJog = window.location.search.match('WT.z_jog');
          var forceRefresh;
          if(timedOut && hpJog && Date.now() - existingPromos.timestamp > (360000 * 20)) {
            forceRefresh = true;
          } else {
            forceRefresh = false;
          }

          if(forceRefresh || existingPromos.promos.length == 0 || (timedOut && !hpJog)) { 
              Promotron.isRefreshed=true;
            promos = Promotron.buildPromos( rankingsRes[0], readingHistoryRes[0], {
            limit: promotronOpts.maxlimit,
            exclusions: _.map($('.top-news a,.pocket-region a,.inside-nyt a'), function(a) { return $(a).attr("href") })
          });       
            
            if(hasLocalStorage){
              localStorage.promotronPromos = JSON.stringify({promos: promos, timestamp: Date.now()})
            }
          } else {
            promos = existingPromos.promos;
          }


        // store best of promos
        // store featured promos
        // store pinned promos
        // store geo_targeted promos
        var template={};

        Promotron.renderPromos(promos,template,promotronOpts.minlimit, promotronOpts.maxlimit)
        $("#watching-beta-opt-in-container .portal-header h4").css("display","block");
      ...
      Promotron.resizeCcol = function(adView,promos,template) {
        
      //resize column with based on ab column
             // console.log('resizeCcol triggered to resize and show hidden promos', adView.el);
            // console.log(  $("div.region.c-column-middle-span-region") )
            //wait until articles appear 
            $(document).ready(function(){
              // console.log("page ready");
              $("#watching-beta-opt-in article").ready(function() {
                // console.log("watching article ready");
                // console.log( $("#watching-beta-opt-in article") );
                // loop through all the weekend read articles and display them as long as the column height isn't too high
                var serves=[];
                            

                $($("#watching-beta-opt-in article").get().reverse()).each( function(i,val){
                            

                // console.log( $(this) )
            // console.log(i,"i",val,"val")
            // console.log("displayed the articles.")
            var mainHeight = $('.span-ab-layout.layout > .ab-column').height();
            var colHeight = $('.c-column.column').height();
            var diff = (mainHeight)*1.0;
            ...
            // console.log("resize=> mainHeight:",mainHeight, "colHeight:",colHeight, "diff:",diff);
            if(colHeight > diff){
               // console.log("c column is too long breaking.")
              // TODO: add part that parses keys for hidden promos and drops their count by one. 
              // this will prevent them from hitting 5 and dropping out from being shown.
              $(this).hide()
                  }else{
                    $(this).css("display","block")
                    serves.push( $(this).find("a").attr("href") );
               // console.log(serves)
                }
            
          

            // console.log("displayed promo:",val)
          });
                if(Promotron.isRefreshed){
            Promotron.recordServe('hp-watching', serves);
            // console.log(serves)
          }
        });
            });
  ...
  // Things we'd have to figure out:
  // 1. click track would have to record read in localstorage as well as sending to server
  // 2. some way to avoid localstorage from growing infinitely.  Maybe cap reads at a certain number of URLs?  Set some kind of expiry?
  // 3. only fire serve track when component is visible
  // 4. do not fire serve track on meta refresh

  //playing with security
  //if random number 1 exist pull from localstorage, if doesnt generate, put in localstorage then pull.
  //if random number 2 exist pull from localstorage, if doesnt generate, put in localstorage then pull.
  //var magicmessage = sjcl.encrypt(navigator.userAgent.slice(rnd1,rnd2),  window.localStorage.promotronPromos);
  //var magicmessage2 = sjcl.decrypt(navigator.userAgent.slice(7,16), window.magicmessage);

  //localStorage.encrypted(magicmessage)
  //localStorage.decrypted(magicmessage2)
