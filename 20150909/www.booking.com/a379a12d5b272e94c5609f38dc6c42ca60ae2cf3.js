...
(function(){var b="bLYQCLBMWRVDEISBXe";
function e(){B.eventEmitter.bind("CALENDAR:shown",d);
c()
}function d(f,g){if(!g.instance.fixedForChrome){g.instance.fixedForChrome=1;
g.instance.$element.addClass("c2-wrapper--inline-block")
}}function c(){B.eventEmitter.bind("CALENDAR:shown",function(f,g){B.track.exp(b);
g.instance.$calendarElement.delegate(".c2-button-earlier","click touchstart",a);
g.instance.$calendarElement.delegate(".c2-button-further","click touchstart",a);
if(B.env.b_browser=="chrome"&&B.env.b_browser_version>=45){B.track.stage(b,1)
}})
}function a(){B.track.custom_goal(b,1)
}B[sNSExperiments][b]={init:e,initElse:c}
})();
...
c.eventEmitter.bind("CALENDAR:opened",function(f,g){var e=g.instance.$footer.find(".c2-calendar-footer--bpg");
var d='<div class="c2-calendar-footer--bpg">'+booking.jstmpl("bpg_module_clientside").render()+"</div>";
if(!e.length){g.instance.$footer.html(g.instance.$footer.html()+d)
}})
},initElse:a}
})(booking);