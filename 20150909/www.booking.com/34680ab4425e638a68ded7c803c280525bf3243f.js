...
e.one("focus",function(){booking.track.exp("OQJbQMASHDSeEGGO");
...
}f.bind("submit",function(j,l){booking.env.ac_close_on_enter_hide=false;
...
}f.find("input").filter(function(){if(k.hasOwnProperty(this.name)){return true
}}).remove();
if(k.place_id){k.dest_id=null
}for(var h in k){if(k[h]===null||k[h]===undefined){continue
}if(!k.hasOwnProperty(h)){continue
}f.append($("<input>",{type:"hidden",name:h,value:k[h]}))
