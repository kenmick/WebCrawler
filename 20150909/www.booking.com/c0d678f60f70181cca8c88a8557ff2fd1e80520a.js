...
booking.define("et",["send_beacon","is_visible","et_debug"],function(q,u,s){var i=booking.jset,r={},p={},e=300,j={},m=[],h=false,y=false,F,z=booking.env||{};
if(!i){i={}
}if(!i.r){i.r={}
}if(!i.f){i.f={}
}if(!i.t){i.t={}
}function c(I,H,J){var G;
if(I===ItemsToTrack.experiment||I===ItemsToTrack.goal){G=[I,H]
}else{G=[I,H,J]
}return G.join("-")
}function b(I){if(!z.et||!z.et.new_js_hashing){return I
}if(p[I]){return p[I]
}var J=2166136261;
for(var G=0,H=I.length;
G<H;
++G){J+=(J<<1)+(J<<4)+(J<<7)+(J<<8)+(J<<24);
J^=I.charCodeAt(G)
}return(p[I]=(J>>>0).toString(16))
}function k(H,G,I){if(a(H,G,I)){switch(H){case ItemsToTrack.experiment:t(G);
break;
case ItemsToTrack.stage:E(G,I);
break;
case ItemsToTrack.goal:o(G);
break;
case ItemsToTrack.custom_goal:n(G,I);
break;
default:s.shouldReport&&s.log("attempt to track unknown item",H,G,I)
}}}function d(H,G,I){j[c(H,G,I)]=true
}function a(J,I,K){s.shouldReport&&s.log("should we track",J,I,K,"total tracked experiments:",Object.keys(j).length);
if(j[c(J,I,K)]){s.shouldReport&&s.log("not tracking",J,I,K,"because it was already tracked");
return false
}else{if(J===ItemsToTrack.experiment||J===ItemsToTrack.stage){var H=(K||0)+1,G=b(I);
if(i.r[G]===undefined){return false
}else{if(i.f[G]){s.shouldReport&&s.log("not tracking",J,I,K,"because it's a fullon, variant:",i.f[G]);
return false
}else{if(i.t[G]&H){d(J,I,K);
s.shouldReport&&s.log("not tracking",J,I,K,"because it was already tracked");
return false
}}}}}return true
}function f(G,I,L,K,M){s.shouldReport&&s.log("adding event handler for",G,I,L,K,M);
if(window.jQuery){if(!F){F=jQuery(window)
}var J=jQuery(I).eq(0);
if(!J.length){s.shouldReport&&s.log("element",I,"is not on the page, won't track ",L,K,M);
return
}if(G==="view"){w(J,L,K,M)
}else{J.one(G,function H(){k(L,K,M)
})
}}}function w(H,J,I,K){s.shouldReport&&s.log("enabling debounced view for",H,J,I,K);
var G=c(J,I,K);
if(j[G]){return false
}m.push([H,J,I,K]);
...
}function D(){if(y){return
}y=true;
var J=[],L=F.scrollTop(),K=F.height();
var H;
for(var I=0,G=m.length;
I<G;
++I){H=m[I];
if(H&&u(H[0],L,K)){s.shouldReport&&s.log("tracking experiment on view",H[1],H[2],H[3]);
k(H[1],H[2],H[3])
}else{J.push(H)
}}m=J;
...
}window.setTimeout(function(){y=false
},e)
}function v(G,H,I){return function(J,K){if(a(I,J,K)){f(G,H,I,J,K)
}else{s.shouldReport&&s.log(["not going to track",I,J,K].join(" "))
}}
}function l(G,H){return{track:v(G,H,ItemsToTrack.experiment),stage:v(G,H,ItemsToTrack.stage),goal:v(G,H,ItemsToTrack.goal),custom_goal:v(G,H,ItemsToTrack.custom_goal)}
}function g(G){return function(H){return{exp:function(I){return l(G,H).track(I)
},stage:function(I,J){return l(G,H).stage(I,J)
},goal:function(I){return l(G,H).goal(I)
},custom_goal:function(I,J){return l(G,H).custom_goal(I,J)
}}
}
}function x(G){var H=b(G);
return i.r[H]||i.f[H]||0
}function t(G){if(arguments.length>1){throw"Track only accept one parameter"
}if(!G){return 0
}if(a(ItemsToTrack.experiment,G)){d(ItemsToTrack.experiment,G);
q(ItemsToTrack.experiment,G)
}return x(G)
}function E(H,G){if(!H){return false
}if(G===undefined){return false
}if(G===0){return t(H)
}if(!/^[1-9]$/.test(G)){return false
}if(!a(ItemsToTrack.experiment,H,G)){d(ItemsToTrack.stage,H,G);
q(ItemsToTrack.stage,H,G);
return true
}return false
}function o(G){if(!G){return false
}if(a(ItemsToTrack.goal,G)){d(ItemsToTrack.goal,G);
q(ItemsToTrack.goal,G);
return true
}}function n(H,G){if(!H||!G||!/^[1-5]$/.test(G)){return false
}if(a(ItemsToTrack.custom_goal,H,G)){d(ItemsToTrack.custom_goal,H,G);
q(ItemsToTrack.custom_goal,H,G)
}}function C(){}C.prototype.variant=t;
C.prototype.track=t;
C.prototype.stage=E;
C.prototype.goal=o;
C.prototype.custom_goal=n;
C.prototype.on=l;
C.prototype.compat=g;
C.prototype.debug_level=x("GCbGTMEbZDVHT");
var A=new C();
if(x("GCbGTdeVSKe")){s.shouldReport&&s.warn("using compatibility mode");
B.jst&&Object.keys(B.jst).forEach(function(G){i.r[b(G)]=B.jst[G]
});
booking.track={queue:booking.track.queue,exp:A.track,getVariant:A.track,goal:A.goal,stage:A.stage,custom_goal:A.custom_goal,onView:A.compat("view"),onViewTouch:A.compat("view"),onClick:A.compat("click"),onMouseOver:A.compat("mouseenter")}
}return A
});
