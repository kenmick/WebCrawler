...
h}var h=D("TIMING_INFO",{}),k;for(k in h)e[k]=h[k];k=e.srt;delete e.srt;var m;k||0===k||(m=Q.timing||{},k=Math.max(0,m.responseStart-m.navigationStart),isNaN(k)&&e.pt&&(k=e.pt));if(k||0===k)e.srt=Math.round(k);e.h5jse&&(h=window.location.protocol+t("ytplayer.config.assets.js"),(h=Q.getEntriesByName?Q.getEntriesByName(h)[0]:null)?e.h5jse=Math.round(e.h5jse-h.responseEnd):delete e.h5jse);d.aft=We();h=d._start;if("cold"==e.yt_lt){m||(m=Q.timing||{});var p;a:if(p=m,p.msFirstPaint)p=Math.max(0,p.msFirstPaint);
else{var u=window.chrome;if(u&&(u=u.loadTimes,ea(u))){var u=u(),Qb=1E3*Math.min(u.requestTime||Infinity,u.startLoadTime||Infinity),Qb=Infinity===Qb?0:p.navigationStart-Qb;p=Math.max(0,Math.round(1E3*u.firstPaintTime+Qb)||0);break a}p=0}0<p&&p>h&&(d.fpt=p);p=c||bf().span;u=m.redirectEnd-m.redirectStart;0<u&&(p.rtime_=u);u=m.domainLookupEnd-m.domainLookupStart;0<u&&(p.dns_=u);u=m.connectEnd-m.connectStart;0<u&&(p.tcp_=u);u=m.connectEnd-m.secureConnectionStart;m.secureConnectionStart>=m.navigationStart&&
0<u&&(p.stcp_=u);u=m.responseStart-m.requestStart;0<u&&(p.req_=u);u=m.responseEnd-m.responseStart;0<u&&(p.rcv_=u);Q.getEntriesByType&&Ze(d);(m=D("SPEEDINDEX_FOR_ACTIONS",void 0))&&-1<m.indexOf(g)&&(m=yb(),g=Le(),m=yb()-m,0<g.ga&&(e.si=g.ga,d.vsc=Ye(g.Na),c.sid=m))}p=Xe();m=p.pbr;g=p.vc;p=p.pbs;m&&g&&p&&m<g&&g<p&&1==bf().info.yt_vis&&"youtube"==f&&(bf().info.yt_lt="hot_bg",f=d.vc,m=d.pbs,delete d.aft,c.aft=Math.round(m-f));(f=D("PREVIOUS_ACTION"))&&(e.pa=f);e.p=D("CLIENT_PROTOCOL")||"unknown";e.t=
