...
n++){f[0]=p[n-1];
m+=b[2];
if((((h.MJ(h.MB(a[1]))&&(h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2])))))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET")))&&h.MJ(h.track_experiment_stage("$b_experiment_hash_loc_sb_autocomplete_newstyle",1)))){}m+=b[3];
if((((h.MJ(h.MB(a[1]))&&(h.MJ(!/zh|xt|ja|ko/.test(h.MB(a[2])))))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET")))&&h.MJ(h.track_experiment_stage("$b_experiment_hash_loc_sb_autocomplete_newstyle",2)))){}m+=b[4];
m+=b[5];
...
}}}}}}}}if(((h.MJ(h.MB(a[3])+""==="theme"))&&(h.MJ(h.track_experiment("PVcLcDODAKWUMKKe")==2)))){m+=b[15]
}if((h.MJ(h.MB(a[1]))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET")))){m+=b[16]
}m+=[b[17],h.MC(a[4]),b[18],h.F.entities(h.MC(a[5])),b[19]].join("");
m+=b[20];
if(((h.MJ(h.MB(a[3])+""==="theme"))&&(h.MJ(h.track_experiment("PVcLcDODAKWUMKKe")==2)))){m+=[b[21],h.F.entities(h.MC(a[6])),b[22],h.F.entities(h.MC(a[7])),b[23],h.ME(b[24],h.MB,h.MN,null),b[25]].join("")
}else{m+=b[20];
...
if((h.MJ(n==1))){m+=b[27];
if((h.MJ(h.MB(a[3])+""==="hotel"))){m+=[b[20],h.ucfirst(h.MB(a[8])),b[20]].join("")
}else{m+=[b[20],h.F.entities(h.ME(b[28],h.MB,h.MN,null)),b[20]].join("")
}m+=b[29]
}m+=[b[20],h.F.entities(h.MB(a[9])),b[20]].join("");
if(h.MD(a[10])){m+=[b[30],h.F.entities(h.MB(a[10])),b[31]].join("")
}m+=b[20];
if((h.MJ(n==1))){m+=b[20];
if(h.MJ(h.MG((h.MC(a[11])||{})["popular"]))){m+=b[20];
if(((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))&&h.MJ(h.track_experiment_stage("PVdIGbGWZSXKbdRNYYdVTbLWZHOaO",1)))){}m+=[b[32],h.F.entities(h.ME(b[33],h.MB,h.MN,null)),b[34]].join("")
}m+=b[35]
}m+=b[20]
}f.shift();
n=l;
...
m+=b[20];
if(h.MD(a[13])){m+=b[36]
}m+=[b[20],h.F.entities(h.MC(a[9])),b[20]].join("");
if(h.MD(a[13])){m+=b[34]
}m+=b[20]
}f.shift();
n=l;
...
o=k;
m+=b[20]
}else{if(h.MD(a[14])){m+=[b[20],h.MC(a[14]),b[20]].join("")
}else{if(h.MD(a[7])){m+=[b[20],h.F.entities(h.MC(a[7])),b[20]].join("")
}}}}m+=b[20];
if(h.MD(a[15])){m+=[b[37],h.MC(a[15]),b[34]].join("")
}m+=b[20];
if(!((h.MJ(h.MB(a[1]))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET"))))){m+=b[20];
if(h.MJ(h.MG((h.MC(a[11])||{})["popular"]))){m+=b[20];
if(((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))&&h.MJ(h.track_experiment_stage("PVdIGbGWZSXKbdRNYYdVTbLWZHOaO",1)))){}m+=[b[32],h.F.entities(h.ME(b[33],h.MB,h.MN,null)),b[34]].join("")
...
d.exports=a.extend({init:function(){this.optionsSpec={delay:{name:"delay",type:Number,defaultValue:300},modelId:{name:"sb-id",type:String,required:true},gpOnly:{name:"gp-only",type:Boolean},minLength:{name:"minlength",type:Number,defaultValue:2},shat:{name:"shat",type:Boolean},ssf:{name:"ssf",type:Number,defaultValue:0}};
...
if(f.ss!=this.input.value){this.input.value=f.ss
...
if(this.results[f].dest_type=="theme"&&this.options.shat){this.model.submit()
...
}else{if(f._fasr){this.model.setFormAction("searchresults")
}else{if(f._fas){this.model.setFormAction("search")
}}}f._ef=f._ef||[];
f._ef.push({name:"dest_id",value:this.data.dest_id});
f._ef.push({name:"dest_type",value:this.data.dest_type});
f._ef.push({name:"place_id",value:this.data.place_id});
f._ef.push({name:"place_id_lat",value:this.data.latitude});
f._ef.push({name:"place_id_lon",value:this.data.longitude});
f._ef.push({name:"place_types",value:this.data.place_types});
...
if(this.options.ssafa){this.model.setFormAction("search")
}this.emit("change")
...
var e=c("read-data-options");
d.exports=b.extend({init:function(){this.options=e(this.$el,{modelId:{name:"sb-id",type:String,required:true}});
...
if(this.model.destination.getOption("ssafa")){j._fasr=true
...
if(l.theme.length&&k._shat){l.theme=[l.theme[0]]
}l.results=l.results.concat(l.theme)
}l._ef=l._ef||[];
l._ef.push({name:"ac_pageview_id",value:B.env.pageview_id});
l._ef.push({name:"ac_suggestion_list_length",value:l.results.length});
l._ef.push({name:"ac_suggestion_list_length_without_theme",value:l.city.length});
l._efd=l._efd||[];
m(null,l)
...
B.define("search/searchbox/model",function(c,b,e){var f=c("event-emitter");
var d=c("search/searchbox/searchbox-form-action");
var g={};
function a(h){f.extend(this);
this.id=h;
this.formAction;
...
}switch(k){case"destination":var i=c("search/destination/model");
this[k]=new i(this);
break;
case"dates":var j=c("search/dates/model");
this[k]=new j(this);
break;
case"group":var h=c("search/group/model");
this[k]=new h(this);
break;
default:return
...
},setFormAction:function(h){if(h==="search"){h=d.getSearchURL()
}else{if(h==="searchresults"){h=d.getSearchResultsURL()
}}this.formAction=h
},getFields:function(){var h=[];
Object.keys(this.fieldGroups).forEach(function(i){if(Array.isArray(this.fieldGroups[i])){h=h.concat(this.fieldGroups[i])
}}.bind(this));
return h
},setFields:function(i,h){this.fieldGroups[i]=h
},validate:function(){if(!this.e_v){return true
}var h=this.validators.filter(function(i){return i.call()
});
return h.length==this.validators.length
}});
e.exports={getInstance:function(h){if(!g[h]){g[h]=new a(h)
}return g[h]
}}
});
...
this.options=f(this.$el,{modelId:{name:"sb-id",type:String,required:true},gpf:{name:"sb-gpf",type:Boolean},e_v:{name:"e-v",type:Boolean},ssafa:{name:"sb-ssafa",type:Boolean}});
...
},formSubmit:function(m){if(this.model.e_v){var l=this.model.validate();
if(!l){m.preventDefault();
return
}}if(this.options.ssafa){var k=this.model.getFormAction();
if(k){this.$form.attr("action",k)
}}if(this.hiddenFields){this.hiddenFields.forEach(function(n){n.$node.remove();
n.$node=null
})
}this.hiddenFields=this.model.getFields();
var j=[];
var i=false;
this.hiddenFields.forEach(function(n){j.push(n.name);
if(a.indexOf(n.name)!=-1){i=true
}});
this.$form.find("input").filter(function(){return(i&&a.indexOf(this.name)!=-1)||j.indexOf(this.name)!=-1
}).remove();
this.hiddenFields.forEach(function(n){var o=$("<input>");
o.attr("type","hidden");
o.attr("name",n.name);
...
B.define("search/searchbox/searchbox-form-action",function(e,d,f){var c=false;
var b=false;
function a(h){var g=B.env.b_nonsecure_hostname;
var i=B.env.b_query_params_with_lang;
if(g===undefined){g="";
if(!c){B.squeak("PVSfHcMSPUUMWCMbSUBHET");
c=true
}}if(i===undefined){i=".html";
if(!b){B.squeak("PVSfHcMSPUUMWCMeCTRe");
b=true
}}return g+"/"+h+i
}d.getSearchURL=function(){return a("search")
};
d.getSearchResultsURL=function(){return a("searchresults")
}
});
