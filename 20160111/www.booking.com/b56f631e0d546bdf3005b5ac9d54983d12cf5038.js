...
var I=(function(){var P=["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"];
...
var V=[];
var U;
function P(){u.level&&u.report(u.events.BEACON_SENT,V);
e=false;
if(V.length){j()
}}function S(){var aa=[];
var ab=[];
...
break;
default:u.level&&u.report(u.events.UNABLE_TO_STRINGIFY,Y)
}}return"exps="+aa.join(",")+"&what="+ab.join(",")+"&custom="+Z.join(",")+"&stage="+ac.join(",")
}function T(W){if(W){V.push(W)
}}function R(){V.length=0
}function Q(aa,Z,ac){u.level&&u.report(u.events.INIT_BEACON,aa,Z,ac);
var ab;
var X;
if(arguments.length>0){T({what:aa,hash:Z,id:ac})
}if(!e&&h){u.level&&u.report(u.events.SEND_BEACON,V.slice(0));
ab=S();
e=true;
X=h+"&"+ab;
...
}else{u.level&&u.report(u.events.DEFER_BEACON,V)
}}return Q
}());
var k={};
var v=300;
...
}function L(Q,P,R){var e;
if(Q===f.experiment||Q===f.goal){e=[Q,P]
}else{e=[Q,P,R]
}return e.join("-")
}function t(Q){if(!N){return Q
}if(k[Q]){return k[Q]
}var R=2166136261;
...
R^=Q.charCodeAt(e)
}return(k[Q]=(R>>>0).toString(16))
}function M(P,e,Q){u.level&&u.report(u.events.TRACKING_ATTEMPT,{what:P,hash:e,id:Q,variant:(P===f.experiment||P===f.stage)&&r(e)});
if(p(P,e,Q)){switch(P){case f.experiment:G(f.experiment,e);
C.populate(e);
j(f.experiment,e);
break;
case f.stage:G(f.stage,e,Q);
C.populate(e+"|"+Q);
j(f.stage,e,Q);
break;
case f.goal:G(f.goal,e);
j(f.goal,e);
break;
case f.customGoal:G(f.customGoal,e,Q);
j(f.customGoal,e,Q);
break;
default:u.level&&u.report(u.events.TRACK_UNKNOWN_ITEM,P,e,Q)
}}if(P===f.experiment){return r(e)
}else{return true
}}function G(P,e,Q){J[L(P,e,Q)]=true
}function p(S,R,T){u.level&&u.report(u.events.SHOULD_TRACK,S,R,T);
var e={what:S,hash:R,id:T,variant:(S===f.experiment||S===f.stage)&&r(R)};
if(J[L(S,R,T)]){u.level&&u.report(u.events.NOT_TRACKING_WAS_TRACKED,e);
return false
}else{if(S===f.experiment||S===f.stage){var Q=1<<(T||0);
var P=t(R);
if(d.f[P]){u.level&&u.report(u.events.NOT_TRACKING_FULLON,e);
return false
}else{if(d.r[P]===undefined){u.level&&u.report(u.events.NOT_TRACKING_NOT_RUNNING,e);
return false
}else{if(d.t[P]&Q){G(S,R,T);
u.level&&u.report(u.events.NOT_TRACKING_WAS_TRACKED,e);
return false
}}}}else{if(S===f.customGoal){var P=t(R);
if(d.f[P]){u.level&&u.report(u.events.NOT_TRACKING_FULLON,e);
return false
}else{if(d.r[P]===undefined){u.level&&u.report(u.events.NOT_TRACKING_NOT_RUNNING,e);
return false
}}}}}return true
}function E(e,Q,T,S,U){u.level&&u.report(u.events.ADD_EVENT_LISTENER,e,Q,T,S,U);
if(window.jQuery){if(!K){K=jQuery(window)
}var R=jQuery(Q).eq(0);
if(!R.length){u.level&&u.report(u.events.NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER,T,S,U);
return
}function P(){M(T,S,U)
}if(e==="view"){l(R,T,S,U)
}else{R.one(e,P)
}}}function l(P,R,Q,S){u.level&&u.report(u.events.ADD_DEBOUNCED_VIEW_HANDLER,P,R,Q,S);
var e=L(R,Q,S);
if(J[e]){return false
}D.push([P,R,Q,S]);
if(!m){u.level&&u.report(u.events.ATTACH_VIEW_LISTENER,D);
K.bind("scroll resize",q);
B&&B.eventEmitter&&B.eventEmitter.bind(I.join(" "),q);
m=true
}}function q(){if(H){return
}H=true;
var R=[];
var T=K.scrollTop();
var S=K.height();
u.level&&u.report(u.events.CHECK_IF_VISIBLE,D);
var P;
for(var Q=0,e=D.length;
Q<e;
++Q){P=D[Q];
if(P&&g(P[0],T,S)){u.level&&u.report(u.events.ONVIEW_TRACKING_TRIGGERED,P[1],P[2],P[3]);
M(P[1],P[2],P[3])
}else{R.push(P)
}}D=R;
u.level&&u.report(u.events.VISIBLE_CHECK_FINISHED,D);
if(D.length===0){m=false;
K.unbind("scroll resize",q);
B.eventEmitter.unbind(I.join(" "),q);
u.level&&u.report(u.events.DETACH_VIEW_LISTENER)
}window.setTimeout(function(){H=false
},v)
}function i(e,P,Q){return function(R,S){if(p(Q,R,S)){E(e,P,Q,R,S)
}else{u.level&&u.report(u.events.WONT_ATTACH_EVENT_TRACKING,e,P,Q,R,S)
}}
}function y(e,P){return{track:i(e,P,f.experiment),stage:i(e,P,f.stage),goal:i(e,P,f.goal),customGoal:i(e,P,f.customGoal)}
}function o(e,Q){for(var P in Q){if(Q.hasOwnProperty(P)){e[P]=Q[P]
}}}function w(e){d.f=e.f||{};
o(d.r,e.r||{});
o(d.t,e.t||{})
}function r(e){var P=t(e);
return d.r[P]||d.f[P]||0
}function s(e){if(arguments.length>1){throw"Track only accept one parameter"
}if(!e){return 0
}return M(f.experiment,e)
}function F(P,e){if(!P){return false
}if(e===undefined){return false
}if(e===0){return s(P)
}if(!/^[1-9]$/.test(e)){return false
}return M(f.stage,P,e)
}function z(e){if(!e){return false
}return M(f.goal,e)
}function A(P,e){if(!P||!e||!/^[1-5]$/.test(e)){return false
}return M(f.customGoal,P,e)
}function x(e){if(e.url){h=e.url
}if(e.jset){w(e.jset)
}if(typeof e.useFNV!=="undefined"){N=e.useFNV
}}function n(){}n.prototype.variant=s;
n.prototype.track=s;
n.prototype.stage=F;
n.prototype.goal=z;
n.prototype.customGoal=A;
n.prototype.on=y;
n.prototype.set=w;
n.prototype.Trackables=f;
n.prototype.configure=x;
return new n()
})
}(B.define,B.require));