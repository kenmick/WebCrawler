...
},selected:function(r,s){if(typeof(s.item)!=="undefined"&&s.item!==null){var q=s.item.data("item.autocomplete");
var p=m.previous;
if(m.element[0]!==l.activeElement){m.element.focus();
m.previous=p;
setTimeout(function(){m.previous=p
},1)
}if(false!==m._trigger("select",r,{item:q})){if(typeof(q)!=="undefined"){m.element.val(q.value)
...
if(booking.track.getVariant("bLYUELMAAJYET")||booking.track.getVariant("MWMJYMdHPSYJfOSWe")){this.menu.element.bind("mouseleave",function(){setTimeout(function(){var n=b("#destination").data("autocomplete").menu;
...
var h;
if(booking.track.getVariant("bLYUELMfdDJCHT")){h=B.require("autocomplete/google_places")
...
},_renderMenu:function(s,y){var x=this,d=false;
x.isSynonymTracked=false;
var I,G,E,f,m,h=0,H=["city","airport","district","region","country","hotel","landmark","searchHistory"],C=H.length;
...
if(y.city&&y.city.length==0&&y.__upa__&&y.__upa__.length){if(!booking.env.b_bbtool_address_search){booking.track.exp("bLYUELMNLNPORLO")
...
var K=D&&D.length;
var o=y.city&&y.city.length;
if(!o&&!K){s.addClass("ui-ac-empty")
}else{s.removeClass("ui-ac-empty")
}if(booking.track.getVariant("IZESfPYPaEPKSfMJO")&&y.reservation&&y.reservation.length){s.removeClass("ui-ac-empty")
}var z="PVUELMERPdRT";
var J=booking.track.getVariant(z);
if(J){var g;
if(J==1){g=booking.env.autocomplete_suggest_click1
}else{if(J==2){g=booking.env.autocomplete_suggest_click2
}else{if(J==3){g=booking.env.autocomplete_suggest_click3
}}}b('<li class="ui-suggest-click" role="menuitem">'+g+"</li>").appendTo("ul.ui-autocomplete")
}if(y.__did_you_mean__){var u=y.__did_you_mean__,F=u.suggestion,q={value:F,label:F};
booking.env.trackExperiment("TMGCEQdPMfaNKe");
if(booking.track.getVariant("TMGCEQdPMfaNKe")){var n=B.env.autocomplete_opt_out_header_copy,L="Match instead <a id='ac-suggestion-out' data-suggestion='"+b("#destination").val()+"'>"+b("#destination").val()+"</a>";
...
}else{var n=booking.jstmpl.translations("sb_autocomplete_suggest",null,{searchedDestination:F});
b('<li id="ac-suggestion" class="ui-menu-item" role="menuitem" data-suggestion="'+F+'"><a class="partition_item ui-corner-all did-you-mean-item">'+n+"</a></li>").data("item.autocomplete",q).appendTo("ul.ui-autocomplete")
}if(booking.track.getVariant("bLYUELMAAJYET")||booking.track.getVariant("MWMJYMdHPSYJfOSWe")){b("#ac-suggestion a").live("mouseover",function(){b("#destination").data("autocomplete").menu.deactivate()
});
...
}}if(booking.track.getVariant("MWMJYMdHPSYJfOSWe")!=0){var M='" '+b("#destination").val()+' "';
var w=booking.jstmpl.translations("autosearch_i_want_search_for",null,{typed_search:M});
b('<li class="ui-menu-item ac_next_step_hint" role="menuitem"><a class="partition_item ui-corner-all" tabindex="-1">'+w+"</a></li>").appendTo("ul.ui-autocomplete")
}if(D&&D.length){var p=[];
var A=D.length;
for(var r=0;
r<A;
r++){if(D[r]["match_start"]){p.push(D[r])
}}if(A!=p.length){y.theme=p
...
I<C;
I++){m=H[I];
if(y[m]){if(y[m].length>0){h++
}f=x._normalize(y[m]);
E=f.length;
var v=0;
for(G=0;
G<E;
G++){if(G===0){f[0].header=m;
if(h===1){f[0].first=1
}}if(typeof f[G].level==="undefined"){v=G
}else{f[G].parent_index=f[v]
}x._renderItem(s,f[G],G,f.length)
...
var h,l,o;
var q={};
f.fullfill=function(s,t,u){if(!s||s&&s.cities&&s.cities.length){return u(s)
}B.track.stage(e,1);
g(t,function(v){if(v&&v.length){B.track.stage(e,3);
if(B.track.getVariant(e)){s.__upa__=$.map(v.slice(0,5),function(w){var x=q[w.place_id]||{};
...
booking.track.exp("MWMJYMdHPSYJfOSWe");
booking.track.exp("MWBOYKbAJWe");
booking.track.exp("PVdIeOSTIPQfQVT");
$(".ac_next_step_hint").click(function(){booking.track.custom_goal("MWMJYMdHPSYJfOSWe",1)
});
...
}if(booking.track.getVariant("bLYUELMAAJYET")||(booking.track.getVariant("MWdLAcMHLaSSaHcVXcBdHC")==2&&$("#sb_enter_key_ac_request").val()==1)||booking.track.getVariant("MWMJYMdHPSYJfOSWe")){$("#sb_enter_key_ac_request").val(2);
var i=f.data("autocomplete");
if(i){var m=i.menu;
if(m&&m.activate&&m.element&&m.element.children&&m.element.children().not("#ac-suggestion").length){if(booking.track.getVariant("MWdLAcMHLaSSaHcVXcBdHC")==2&&$("#sb_enter_key_ac_request").val()==1&&booking.track.getVariant("MWMJYMdHPSYJfOSWe")!=0){m.activate($.Event({type:"mouseenter"}),m.element.children().not("#ac-suggestion").not(".ac_next_step_hint").first())
