...
dwBox.height = (typeof(adInfoTempDw.height)=="undefined")?700:adInfoTempDw.height;
dwBox.height2 = (typeof(adInfoTempDw.height2)=="undefined")?500:adInfoTempDw.height2;
...
dwBox.topNum = adInfoTempDw.index?43:0;
dwBox.createElement = function(){
	...
	this.leftDiv = adBox.createDiv((adBox.getClientInfo("width") - 1000)*0.5,this.height2);
	this.leftDiv.style.position = "absolute";
	this.rightDiv = adBox.createDiv((adBox.getClientInfo("width") - 1000)*0.5,this.height2);
	this.rightDiv.style.position = "absolute";
	this.leftDiv.innerHTML = "<div style=\"height:"+this.height2+"px; width:"+(adBox.getClientInfo("width") - 1000)*0.5+"px; background:#00F; cursor:pointer; filter:alpha(opacity=0); opacity:0; \" onclick=\"dwBox.getURL()\"></div>"
	this.leftDiv.style.overflow = "hidden";
	this.rightDiv.innerHTML = "<div style=\"height:"+this.height2+"px; width:"+(adBox.getClientInfo("width") - 1000)*0.5+"px; background:#00F; cursor:pointer; filter:alpha(opacity=0); opacity:0; \" onclick=\"dwBox.getURL()\"></div>"
	this.rightDiv.style.overflow = "hidden";

	this.topDiv.innerHTML = "<div style=\"height:45px; width:960px; overflow:hidden; \"><div style=\"height:45px; width:960px; background:#F00; cursor:pointer; filter:alpha(opacity=0); opacity:0;\" onclick=\"dwBox.getURL()\"></div><div id=\"dwCloseBtn\" style=\"background:url("+this.closeBtnImg1+"); height:17px; width:49px; left:50%; margin-left:448px; position:absolute; top:0px; cursor:pointer;\" onclick=\"dwBox.action('close')\" onmouseover=\"dwBox.action('over')\" onmouseout=\"dwBox.action('out')\"></div></div>";
	...
	var IndexFlashBg = adBox.createSwf("IndexFlashBg20140417","100%","700px",dwBox.src);
...
//éåå¸éé£ç²æµ£é©è¿éîç§µéï¿ æé¯çîæ¥ çªå½éå¶å´«éîåè¤°æçº§é¿çæ·
dwBox.go = function(){
