var SohuTopad = function(beans){
    ...
    b.style.cssText ="position:absolute;z-index:101;top:"+beans.top+"px;width:100%;cursor: pointer;cursor:hand;margin:auto;left:0px;height:45px;";
    	...
    	var replay_btn = this.replay_btn = jQuery('<div/>').attr('id', 'banner_replay').css({
			...
			this.replay_btn.bind('click', function(e){
                    ...
                    if(tag){
                        tag = true;
                        var full = jQuery("#NEWFULL");
                        if(!!full[0]){
                            full.children().css("display","block")
                            full.empty();
                            full.append("<div id='fullflash'></div>")
                             // full.show();//sohu_flash new
                                 var sohuFlash2 = new sohuFlash("http://images.sohu.com/bill/s2015/yanlu/taobao/1212/7604002009.swf","_NEWFULL",760,400,"7");
                                    sohuFlash2.addParam("quality", "high");
                                    sohuFlash2.addParam("wmode", "transparent");
                                    sohuFlash2.addParam("allowScriptAccess", "always");
                                    sohuFlash2.addVariable("clickthru",escape("http://clk.optaim.com/event.ng/Type=click&FlightID=201512&TargetID=sohu&Values=9f36dc06,4ba06a57,b14c2bfc,189390eb&AdID=9722102"));
                        ...
                        img.src = "http://imp.optaim.com/201512/9f36dc064ba06a57b14c2bfc189390eb.php?a=99";//é²çæ¹è¿
                    }
                });
    ...
    jQuery('#AdAction').attr('onclick','').click(eval(function(){
		window.open('http://clk.optaim.com/event.ng/Type=click&FlightID=201512&TargetID=sohu&Values=9f36dc06,4ba06a57,b14c2bfc,189390eb&AdID=9722102');
	}));
...
SohuTopad.topreplay =function(id){
    SohuTopad.tag2 = 0 ;
    _SoAD_replay(id);
...
//æ§å¶å¨å±
    ...
    setTimeout(function(){
        jQuery("#banner_replay").bind("click",function(){
            SohuTopad.topreplay("NEWFULL");
        })
    },8000)
