...
}}},blur:function(o,n){if(booking.track.getVariant("bLYUELMAAJYET")||booking.track.getVariant("PVdIdAHEZWDTPVJTTC")){return false
...
},_renderMenu:function(s,x){var w=this,d=false;
w.isSynonymTracked=false;
var H,F,D,g,m,k=0,G=["city","airport","district","region","country","hotel","landmark","searchHistory"],A=G.length;
if(booking.track.getVariant("IZESfPYPaEPKSfMJO")){G.push("reservation");
A+=1
}s.removeClass("ui-ac-powered-by-google");
if(x.__did_you_mean__||(x.city&&x.city.length==0)){booking.env.trackExperiment("bLYUELMERZLFZDfeWffCRe");
if(x.city&&x.city.length==0&&x.__upa__&&x.__upa__.length){if(x.__js_upa__){}else{if(!booking.env.b_bbtool_address_search){booking.track.exp("bLYUELMNLNPORLO")
}}s.addClass("ui-ac-powered-by-google");
x.city=x.__upa__
}}booking.env.trackExperiment("bLYUELMYWbMDfeWfKe");
if(booking.env.autocomplete_bbtoollocations&&x.bbtoollocations&&x.bbtoollocations.length){booking.tools.object.each(x.bbtoollocations,b.proxy(function(i){if(i.name&&i.address){i.label_highlighted="<b>"+this._escapeHTML(i.name)+"</b>, "+this._escapeHTML(i.address)
}},this));
x.city=x.bbtoollocations.concat(x.city)
}var C=x.theme;
var L=C&&C.length;
var o=x.city&&x.city.length;
if(!o&&!L){s.addClass("ui-ac-empty")
}else{s.removeClass("ui-ac-empty")
}if(booking.track.getVariant("IZESfPYPaEPKSfMJO")&&x.reservation&&x.reservation.length){s.removeClass("ui-ac-empty")
}var y="PVUELMERPdRT";
var K=booking.track.getVariant(y);
if(K){var h;
if(K==1){h=booking.env.autocomplete_suggest_click1
}else{if(K==2){h=booking.env.autocomplete_suggest_click2
}else{if(K==3){h=booking.env.autocomplete_suggest_click3
}}}b('<li class="ui-suggest-click" role="menuitem">'+h+"</li>").appendTo("ul.ui-autocomplete")
}var I="PVdIdAHEZWDTPVJTTC";
var J=booking.track.getVariant(I);
if(J){var f=b("#destination").val();
...
b("#destination").trigger("keydown.autocomplete")
}if(x.__did_you_mean__){var u=x.__did_you_mean__,E=u.suggestion,q={value:E,label:E};
booking.env.trackExperiment("TMGCEQdPMfaNKe");
if(booking.track.getVariant("TMGCEQdPMfaNKe")){var n=B.env.autocomplete_opt_out_header_copy,M="Match instead <a id='ac-suggestion-out' data-suggestion='"+b("#destination").val()+"'>"+b("#destination").val()+"</a>";
b('<li class="ui-menu-item" role="menuitem"><div class="partition_item ac_optout_header">'+n+"</div></li>").appendTo("ul.ui-autocomplete");
b("#ac-suggestion").html(E)
}else{var n=booking.jstmpl.translations("sb_autocomplete_suggest",null,{searchedDestination:E});
b('<li id="ac-suggestion" class="ui-menu-item" role="menuitem" data-suggestion="'+E+'"><a class="partition_item ui-corner-all did-you-mean-item">'+n+"</a></li>").data("item.autocomplete",q).appendTo("ul.ui-autocomplete")
...
b("#ac-suggestion a").live("click",function(){b("#destination").val(E);
b("#destination").trigger("keydown.autocomplete")
})
}else{b("#ac-suggestion").live("click",function(){b("#destination").val(E);
b("#destination").trigger("keydown.autocomplete")
})
}}if(C&&C.length){var p=[];
var z=C.length;
for(var r=0;
r<z;
r++){if(C[r]["match_start"]){p.push(C[r])
}}if(z!=p.length){x.theme=p
}if(B.env.b_action=="index"){var e=G.shift();
G.unshift("theme");
G.unshift(e);
A=G.length
}else{delete x.theme
...
H<A;
H++){m=G[H];
if(x[m]){if(x[m].length>0){k++
}g=w._normalize(x[m]);
D=g.length;
var v=0;
for(F=0;
F<D;
F++){if(F===0){g[0].header=m;
if(k===1){g[0].first=1
}}if(typeof g[F].level==="undefined"){v=F
}else{g[F].parent_index=g[v]
}w._renderItem(s,g[F],F,g.length)
}}}},_renderItem:function(r,F,J,A){var u=this;
var H=false;
F.position=J;
F.array_length=A;
var v="";
var n=", ";
var k=0;
if(F.labels!=null&&typeof(F.labels)!="undefined"){k=F.labels.length
}if(typeof(F.labels)!="undefined"&&(booking.env.b_lang==="zh"||booking.env.b_lang==="xt"||booking.env.b_lang==="ja"||booking.env.b_lang==="ko")){H=true
}if(typeof(F.labels)!="undefined"&&(F.lc==="zh"||F.lc==="xt"||F.lc==="ja"||F.lc==="ko")){var l=false;
booking.track.exp("MWAAQaNZFFEbdDbaVIFTdWe");
H=true;
b.each(F.labels,function(L,K){if(L===k-1||k==1){n=""
}if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){if(K.type=="region"&&L!=k-1&&L!=0){var M=K;
K=F.labels[L+1];
F.labels[L+1]=M
}if((L!=k-1&&F.labels[L+1].type=="region"&&!l)||(L==0&&k==2&&K.type=="region"&&!l)){n=" (";
l=true
}else{if(L==k-1&&l){n=")"
}else{if(L==k-1){n=""
}else{n=", "
}}}}if(K.hl){if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){v=v+"<strong>"+K.text+"</strong>"+n
}else{v=v+"<strong>"+K.text+n+"</strong>"
}}else{v=v+K.text+n
}})
}else{if(F.nr_hotels>=500){B.env.search_ac_is_popular_ufi=true;
if(booking.track.getVariant("PVdIZKZPHcaUC")){var q=F.labels[0];
v="";
if(q.hl===1){v+="<strong>"
}v+=q.text;
if(q.match){v+=" ("+q.match+")"
}if(q.hl===1){v+="</strong>"
}}else{v=F.label_highlighted||F.label
}}else{v=F.label_highlighted||F.label
}}var g=function(K){if(K=="undefined"||K==c||K==void 0){return 1
}else{return 0
}};
var p=booking.track.getVariant("MWBOYKbAJWe");
if(p===1||p===2){if(F.level){v='<em class="ac_item_tree_arrow"></em>'+v
}}var o=v,y="",d="",z="",D="",e="",h=F.photo||"";
if(F.header){y='<span class="autocomplete_header autocomplete_h_'+F.dest_type+'" >'+booking.env.autocomplete_categories[F.header]+"</span>";
d=(F.first)?"":' style="border-top: 1px solid #CCC;"';
z+=(F.first)?"":"first-in-group"
}if(F.nr_hotels_label&&F.nr_hotels_label.length){var I="text-align: left; display: inline;";
if(booking.env.rtl==1){I+=" direction: rtl; unicode-bidi: embed;"
}e='<span class="label" style="position: static;"><span class="property-counter" style="'+I+'">';
if(booking.env.rtl!=1&&F.rtl==1){e+='<span style="visibility:hidden;margin-left:-4px;">i</span>'
}var m=F.nr_hotels_label;
if(F.dest_type=="hotel"){if(F.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){e=""
}else{e+="("+m+")"
}}else{if(F.nr_hotels==0&&F.nr_hotels_25>0){e+='<span class="ac_prop_nearby">('+booking.env.autocomplete_counter_label+")</span>"
}else{e=""
}}if(F.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){e+="</span>"
}else{e+="</span></span>"
}}var t="";
if(F.dest_type=="theme"&&F.additional_label){t='<span class="label" style="position: static;color:#888;">&nbsp;<span style="direction: rtl; text-align: left; display: inline;">'+F.additional_label+"</span></span>"
}var i=B.tools.jsStaticUrl("/static/img/flags/16/"+F.cc1+".png");
var s="PVUELMBBdXUC";
if(F.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){var G="";
switch(F.dest_type){case"continent":G=booking.env.ac_tree_view_continent;
break;
case"country":G=booking.env.ac_tree_view_country;
break;
case"region":G=booking.env.ac_tree_view_region;
break;
case"city":G=booking.env.ac_tree_view_city;
break;
case"district":G=booking.env.ac_tree_view_district;
break;
case"airport":G=booking.env.ac_tree_view_airport;
break;
case"landmark":G=booking.env.ac_tree_view_landmark;
break;
case"hotel":G=booking.env.ac_tree_view_hotel;
break
}var C='&nbsp;&nbsp;<span class="autocomplete_icons_to_label">'+G+"</span></span>"
}else{var C='<i class="autocomplete_icons autocomplete_'+F.dest_type+'"></i>'
}if(F.dest_type!="theme"&&(g(F.cc1)!=1&&F.cc1.length==2)&&booking.track.getVariant("EKODAKSCNVAELIYRAfUZC")==1&&booking.track.getVariant("PVUELMEARLGUXO")==2){C+='<img class="search_ac_country_flags_right" src="'+i+'">'
}if(booking.env.b_is_loggedin_genius_user==1&&((F.genius_hotels/F.hotels)>0.01||F.is_genius===1)){booking.track.exp("PVUELMZNdGUKCMeMRfPWe");
if(F.dest_type!="theme"&&booking.track.getVariant("PVUELMZNdGUKCMeMRfPWe")===1){C+='<span class="ac_genius_right"><span class="ge-iconfont-plate"> <i class="bicon-dotgeniusbg"></i><i class="bicon-dotgeniusfold"></i><i class="bicon-dotgenius"></i></span></span>'
}}if(H){var f='<span class="ac-hl-cjk-wrapper">'
}else{var f="<span>"
}if((booking.track.getVariant("PVUELMNGEcdXUSPSELIae")!=0)){if(F.dest_type!="theme"&&booking.track.getVariant("MWBOYKbAJWe")){o=f+o+C+e+"</span></span>"
}else{o=f+o+e+t+"</span>"+C
}}else{if(F.dest_type!="theme"&&(g(F.cc1)!=1&&F.cc1.length==2)&&booking.track.getVariant("EKODAKSCNVAELIYRAfUZC")==1&&booking.track.getVariant("PVUELMEARLGUXO")==1){o=C+'<img class="search_ac_country_flags_left" src="'+i+'">'+f+o+e+t+"</span>"
}else{o=C+f+o+e+t+"</span>"
}}var x="";
if(booking.track.getVariant("bLYUELMaIIERcdNFQLNWe")===1&&F.dest_type!="theme"){x='<div class="si_menu"><div class="si_menu_item">Wifi</div><div class="si_menu_item">Non-smoking Rooms</div><div class="si_menu_item">Air conditioning</div><div class="si_menu_item">Flat-screen TV</div><div class="si_menu_item">Bathtub</div></div>';
z+=" si_parent_v1"
}else{if(booking.track.getVariant("bLYUELMaIIERcdNFQLNWe")===2&&F.dest_type!="theme"){z+=" si_parent_v2"
}}D='class="partition_item';
if(F.dest_type!="theme"){var p=booking.track.getVariant("MWBOYKbAJWe");
if(p){D+=" ac_item_label_wrapper"
}if(F.level&&(p===1||p===2)){D+=" ac_item_tree_wrapper"
}}else{D+=" ac_item_theme"
}D+='"';
result=b('<li class="'+z+'"'+d+"></li>").data("item.autocomplete",F).append(b("<a "+D+">"+o+"</a>")).append(x).appendTo(r);
if(booking.track.getVariant("bLYUELMaIIERcdNFQLNWe")===2&&F.dest_type!="theme"){var j='<div class="si_menu"><p>Filter by:</p><div class="si_menu_item">Wifi</div><div class="si_menu_item">Non-smoking Rooms</div><div class="si_menu_item">Air conditioning</div><div class="si_menu_item">Flat-screen TV</div><div class="si_menu_item">Bathtub</div></div>';
var E=b(j).appendTo(r);
var w=E.prev();
E.hover(function(){w.find("a").addClass("ui-state-hover")
},function(){w.find("a").removeClass("ui-state-hover")
})
}if(booking.track.getVariant("TMGCBOEQC")){if(F.position==0&&F.dest_type!="theme"){result.find("a").after(b("<span />",{"class":"sb-ac-menu-arrow-outer"}));
...
}}}}}).bind("autocompleteopen",function(j,k){$("#sb_ac_pending_tracking").addClass("opened");
...
if(j.dest_type){var l={dest_type:j.dest_type,dest_id:j.dest_id,ac_pageview_id:booking.env.pageview_id,ac_position:j.position,ac_langcode:j.lc,ac_suggestion_list_length:j.array_length,ss_short:j.ss_short,place_id:j.place_id,place_id_lat:j.place_id_lat,place_id_lon:j.place_id_lon};
