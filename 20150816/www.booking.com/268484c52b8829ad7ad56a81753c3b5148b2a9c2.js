...
booking.define("et",function(){var F,D,b=booking.jset,G={},w={},r=false,y,s,x,N=[],e=[],k={},q=300,A,j,H,g=booking.env||{},O=g.b_dev_server?(g.et&&g.et.debug_level?g.et.debug_level+1:0):0,n=20,J=booking.require("tmpl_data");
if(!b){b={}
}if(!b.r){b.r={}
}if(!b.f){b.f={}
}if(!b.t){b.t={}
}function M(R){if(!g.et||!g.et.new_hashing){return R
}if(w[R]){return w[R]
}var S=2166136261;
for(var P=0,Q=R.length;
P<Q;
++P){S+=(S<<1)+(S<<4)+(S<<7)+(S<<8)+(S<<24);
S^=R.charCodeAt(P)
}return(w[R]=(S>>>0).toString(16))
}function a(Q){var S=Q.offset(),P=(typeof S.constructor==="function")?S.constructor:{name:"OtherConstructor"},R;
if(P.name==="ClientRect"){R=Q.offset();
if(R){return R.top
}return false
}return S.top
}function d(Q,S,R){var P=a(Q);
if(P===false){return false
}return !!(((S+R)>(P+n))&&Q.is(":visible"))
}function m(){if(A){if(O>20){console.warn("skipping")
}return
}A=true;
if(O>20){console.time("onviewcheck")
}var R=[],T=H.scrollTop(),S=H.height();
for(var Q=0,P=e.length;
Q<P;
++Q){if(e[Q]&&d(e[Q][1],T,S)){if(e[Q][3]){L(e[Q][2],e[Q][3])
}else{o(e[Q][2])
}e[Q]=null
}else{R.push(e[Q])
}}e=R;
...
H.unbind("scroll",m);
H.unbind("resize",m);
if(O>20){console.log("de-attaching onView Handler")
}}if(O>20){console.timeEnd("onviewcheck")
}window.setTimeout(function(){if(O>20){console.timeEnd("onviewwait")
}A=false
},q);
if(O>20){console.time("onviewwait")
}}function h(P,R,S,Q){if(k[S]&&k[S]["el"+(Q||0)]){return false
}e.push([P,R,S,Q]);
k[S]={};
k["el"+(Q||0)]=1;
if(!j){if(O>20){console.log("attaching onView Handler")
}H.bind("scroll",m);
H.bind("resize",m);
j=true
}}function z(P,R,U,Q){if(window.jQuery){if(!H){H=jQuery(window)
}var T=jQuery(R).eq(0);
if(!T.length){if(O){console.log("Element not present on the page for exp: "+U)
}return
}if(P==="view"){h(P,T,U,Q);
return
}var S=function S(){Q?L(U,Q):o(U);
T.unbind(P,S)
};
if(O){console.log("Set event handler [ "+P+", "+U+"]")
}T.bind(P,S)
}else{}}function I(P,Q,R){return function(U,T){var S=R?T:undefined;
if(l(U,S)){z(P,Q,U,S)
}else{if(O){console.log((b.f[M(U)]?"Full-on":"Not running")+" experiment: "+U)
}}}
}function t(Q,S){var R=I(Q,S),P=I(Q,S,true);
return{track:R,stage:P}
}function u(P){return function(Q){return t(P,Q)
}
}function p(){r=false;
if(N.length){f()
}}function i(){var T=[],U=[],S=[],V=[];
for(var R,Q=0,P=N.length;
Q<P;
++Q){R=N[Q];
if(R.hash&&R.num){if(O){console.log("Stage track: "+R.hash+" - "+R.num)
}V.push(R.hash+"|"+R.num)
}else{if(R.hash){T.push(R.hash);
if(O){console.log("EXP track: "+R.hash)
}}else{if(R.custom){if(O){console.log("Custom goal track: "+R.custom+" - "+R.num)
}S.push(R.custom+"|"+R.num)
}else{if(R.goal){if(O){console.log("Goal track: "+R.goal)
}S.push(R.goal)
}else{if(O){console.warn("We have a property we don't recognize",R)
}}}}}}return"exps="+T.join("")+";what="+U.join(",")+";ncustom="+S.join(",")+";stage="+V.join(",")
}function c(P){if(P){if(P.flush){N.length=0;
return""
}else{N.push(P)
}}}function f(R){if(!x){x=J(["b_action","b_sid","b_aid","b_lang","pageview_id","b_site_type"])
}var T,Q;
if(R){c(R)
}if(r){if(O){console.log("Defering js_tracking")
}return
}else{if(T=i()){r=true;
Q="/js_tracking?ver=2;"+T+";ref="+x.b_action+";sid="+x.b_sid+";aid="+x.b_aid+";lang="+x.b_lang+";pid="+x.pageview_id+";stype="+x.b_site_type_id;
try{$.ajax({url:Q,complete:p})
}catch(S){var P=new Image();
P.onload=P.onerror=p;
P.src=Q
}c({flush:1})
}}}function l(R,Q){var P;
if(Q===undefined){P=!!(b.r[M(R)]!==undefined&&!G["e"+R]&&!(b.t[M(R)]&1))
}else{P=!!(b.r[M(R)]!==undefined&&!G["s"+R+"-"+Q]&&!(b.t[M(R)]&(Q+1)))
}return P
}function K(P){return b.r[M(P)]||b.f[M(P)]||0
}function o(P){if(arguments.length>1){throw"Track only accept one parameter"
}if(l(P)){G["e"+P]=true;
f({hash:P})
}return K(P)
}function v(P){if(!P||G["g"+P]){return
}G["g"+P]=true;
f({goal:P})
}function C(Q,P){if(!Q||!P||!/^[1-5]$/.test(P)||G["cg"+Q+"-"+P]){return
}G["cg"+Q+"-"+P]=true;
f({custom:Q,num:P})
}function L(Q,P){if(!Q||!P||!/^[1-9]$/.test(P)||!l(Q,P)){return true
}G["s"+Q+"-"+P]=true;
f({hash:Q,num:P});
return true
}function E(){}E.prototype.variant=K;
E.prototype.track=o;
E.prototype.stage=L;
E.prototype.goal=v;
E.prototype.custom_goal=C;
E.prototype.on=t;
E.prototype.compat=u;
return new E()
});
booking.et=booking.require("et");
if(booking.et.variant("GCbGTdeVSKe")){if(booking.env.b_dev_server&&booking.et.debug_level>9){console.warn("Using ET compat mode")
}booking.track={queue:booking.track.queue,exp:booking.et.track,getVariant:booking.et.variant,goal:booking.et.goal,stage:booking.et.track_stage,custom_goal:booking.et.custom_goal,onView:booking.et.compat("view"),onViewTouch:booking.et.compat("view"),onClick:booking.et.compat("click"),onMouseOver:booking.et.compat("mouseenter")}
};