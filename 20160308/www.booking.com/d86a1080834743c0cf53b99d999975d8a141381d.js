...
booking.jstmpl("search_dates_dates_los",(function(){_p=_o;_o="8febb:203";var b=["\n\t",'\n\t\u003cdiv class="sb-dates__los"\u003e\n\t\t',"sbox_dates_num_nights_1","\n\t\u003c/div\u003e\n","\n"],a=["b_checkin_checkout_interval"],e,d,c;
return function(f){_p=_o;_o="8febb:204";var h="",j=this.fn;
function g(k){_p=_o;_o="8febb:205";if(j.MJ(j.MC(a[0]))){k+=[b[1],(f.unshift({num_nights:j.MC(a[0])}),(c=j.VP(b[2],"b_checkin_checkout_interval")),f.shift(),c),b[3]].join("")
}return k
}h+=b[0];
h=g(h);
h+=b[4];
...
n+=b[28];
n+=[b[29],h.F.entities(h.MC(a[10])),b[30],h.F.entities(h.MC(a[11])),b[31]].join("")
}else{n+=b[23];
...
if((h.MJ(h.MB(a[3])+""==="hotel"))){n+=[b[23],h.ME(b[34],h.MB,h.MN,null),b[23]].join("")
}else{n+=[b[23],h.F.entities(h.ME(b[35],h.MB,h.MN,null)),b[23]].join("")
}n+=b[36]
}n+=b[27];
n+=b[37];
if((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))){n+=b[38]
}n+=b[22]
}n+=[b[23],h.F.entities(h.MB(a[12])),b[23]].join("");
if(h.MD(a[13])){n+=[b[39],h.F.entities(h.MB(a[13])),b[40]].join("")
}n+=b[23];
if((h.MJ(o==1))){n+=b[23];
if(h.MJ(h.MG((h.MC(a[14])||{})["popular"]))){n+=b[23];
if(((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))&&h.MJ(h.track_experiment_stage("PVdIGbGWZSXKbdRNYYdVTbLWZHOaO",1)))){}n+=b[41];
n+=b[42];
if((h.MJ(/zh|xt|ja|ko/.test(h.MB(a[2]))))){n+=b[38]
}n+=[b[22],h.F.entities(h.ME(b[43],h.MB,h.MN,null)),b[44]].join("")
}n+=b[45]
}n+=b[23]
}f.shift();
o=m;
...
n+=b[23];
if(h.MD(a[16])){n+=b[46]
}n+=[b[23],h.F.entities(h.MC(a[12])),b[23]].join("");
if(h.MD(a[16])){n+=b[44]
}n+=b[23]
}f.shift();
o=m;
...
p=l;
n+=b[23]
}else{if(h.MD(a[17])){n+=[b[23],h.MC(a[17]),b[23]].join("")
}else{if(h.MD(a[11])){n+=[b[23],h.F.entities(h.MC(a[11])),b[23]].join("")
}}}}n+=b[23];
if(h.MD(a[18])){n+=[b[47],h.MC(a[18]),b[44]].join("")
}n+=b[23];
if(!((h.MJ(h.MB(a[1]))&&h.MJ(h.track_experiment("YdVSfPYPaEYZBMURET"))))){n+=b[23];
if(h.MJ(h.MG((h.MC(a[14])||{})["popular"]))){n+=b[23];
...
}},modelInit:function(){_p=_o;_o="8febb:378";if(this.model._ssce){var f={ss:this.input.value};
...
this.autocomplete()
}}},modelValidation:function(f){_p=_o;_o="8febb:382";if(this.model._ssce&&f&&f.group=="destination"){this.$errors.filter(".-visible").removeClass("-visible");
...
t(null,r)
}function p(r,s){_p=_o;_o="8febb:486";if(B.et.track("PVdIHMDeJISUWe")){this.atlas.getGeocodeDetails({placeId:r.place_id},function(t){_p=_o;_o="8febb:487";h(r,t,s)
})
}else{this.atlas.getPlaceDetails({placeId:r.place_id},function(t){_p=_o;_o="8febb:489";if(!t||typeof t.get!=="function"){s(new Error("service-atlas-invalid-place"),null)
}else{var u=t.get("data");
u.upa=true;
h(r,u,s)
}})
}}function g(u,x,t){_p=_o;_o="8febb:492";var r="";
t=t||{};
if(!x||x.length===0){return u
}var w=x.map(function(y){_p=_o;_o="8febb:493";return y.offset
});
var s=x.map(function(y){_p=_o;_o="8febb:495";return y.offset+y.length-1
});
for(var v=0;
v<u.length;
...
}function b(s,t){_p=_o;_o="8febb:498";var r=[];
if(!t||t.length===0){return[{text:s}]
}t.forEach(function(w,x){_p=_o;_o="8febb:499";var v=w.offset+w.length,u=((t[x+1]&&t[x+1].offset)||s.length)-v;
r.push({highlighted:true,text:s.substr(w.offset,w.length)});
if(u){r.push({text:s.substr(v,u)})
}});
return r
}a.exports=q.extend({init:function(r,v){_p=_o;_o="8febb:502";r=r||{};
...
}},search:function(s,x){_p=_o;_o="8febb:504";if(!s||!s.ss||typeof x!="function"){throw new Error("service-atlas-invalid-arguments")
}if(!j()){setTimeout(function(){_p=_o;_o="8febb:505";x(new Error("service-atlas-no-atlas"),null)
},4);
...
v.lon=l;
v.radius=m
}var u=function(y){_p=_o;_o="8febb:507";if(!r){if(!y||!Array.isArray(y)){x(new Error("service-atlas-invalid-results"),null)
}else{k(s,y,x)
}}};
var t=function(y){_p=_o;_o="8febb:509";if(!r){x(new Error("service-atlas-error "+y),null)
}};
this.init(s,function(){_p=_o;_o="8febb:511";this.atlas.autocomplete(v,u,t)
}.bind(this));
var w=function(){_p=_o;_o="8febb:513";r=true
};
return{getResultDetails:p.bind(this),abort:w}
}})
});
B.define("search/destination/service-booking",function(c,e,a){_p=_o;_o="8febb:517";var k=c("event-emitter");
var j=c("et");
var g=B.env.autocomplete_use_auth?"/autocomplete_with_auth":"/autocomplete_2";
function d(l){_p=_o;_o="8febb:518";l.term=l.ss;
delete l.ss;
return l
}function h(m,l,n){_p=_o;_o="8febb:520";if(!m||!m.city){n(new Error("service-booking-data-invalid"),null)
...
}else{m.city=m.city.map(function(o,p){_p=_o;_o="8febb:521";if(!o.label&&o.labels){o.label=f(o.labels)
...
m.results=m.city;
if(m.bbtoollocations){m.results=m.bbtoollocations.map(function(o,p){_p=_o;_o="8febb:523";delete o.label_highlighted;
...
}if(m.theme){m.theme=m.theme.map(function(o,p){_p=_o;_o="8febb:525";o.ss=o.label;
o._ef=o._ef||[];
...
m._efd=m._efd||[];
n(null,m)
}}}function f(l){_p=_o;_o="8febb:528";return l.map(function(m){_p=_o;_o="8febb:529";return m.text
}).join(", ")
}function b(m){_p=_o;_o="8febb:532";var l=[];
m.forEach(function(n,o){_p=_o;_o="8febb:533";if(o>0){l.push({text:", "})
}l.push({highlighted:!!n.hl,text:n.text})
});
return l
}a.exports=k.extend({search:function(l,p){_p=_o;_o="8febb:536";if(!l||!l.ss||typeof p!="function"){throw new Error("Invalid arguments")
}var m=d(l);
this.emit("beforeajax",l,m);
var n=$.ajax({type:"get",url:g,data:m,success:function(r,q,s){_p=_o;_o="8febb:537";this.emit("afterajax",l,r);
h(r,l,p)
}.bind(this),error:function(s,q,r){_p=_o;_o="8febb:539";if(q!=="abort"){p(new Error("service-booking-error "+r),null)
}}.bind(this)});
var o=function(){_p=_o;_o="8febb:541";if(n&&n.abort){n.abort()
}};
return{abort:o}
}})
});
B.define("search/searchbox/model",function(c,b,d){_p=_o;_o="8febb:545";var e=c("event-emitter");
var f={};
function a(g){_p=_o;_o="8febb:546";e.extend(this);
this.id=g;
this.formAction;
this.fieldGroups={track:[]};
this.validators=[];
this._ssce=false
}$.extend(a.prototype,{use:function(k){_p=_o;_o="8febb:548";if(this[k]){return
...
}},submit:function(){_p=_o;_o="8febb:550";this.emit("submit")
},getFormAction:function(){_p=_o;_o="8febb:552";return this.formAction
},setFormAction:function(g){_p=_o;_o="8febb:554";this.formAction=g
},getFields:function(){_p=_o;_o="8febb:556";var g=[];
Object.keys(this.fieldGroups).forEach(function(h){_p=_o;_o="8febb:557";if(Array.isArray(this.fieldGroups[h])){g=g.concat(this.fieldGroups[h])
}}.bind(this));
return g
},setFields:function(h,g){_p=_o;_o="8febb:560";this.fieldGroups[h]=g
},validate:function(){_p=_o;_o="8febb:562";if(!this._ssce){return true
}var g=this.validators.filter(function(h){_p=_o;_o="8febb:563";return h.call()
});
return g.length==this.validators.length
}});
d.exports={getInstance:function(g){_p=_o;_o="8febb:566";if(!f[g]){f[g]=new a(g)
}return f[g]
}}
});
B.define("component/search/searchbox/searchbox",function(d,b,e){_p=_o;_o="8febb:569";var c=d("component");
...
e.exports=c.extend({init:function(){_p=_o;_o="8febb:570";this.hiddenFields=[];
...
g.on("beforeajax",function(j,k){_p=_o;_o="8febb:571";B.eventEmitter.trigger("AUTOCOMPLETE:beforeAjax",{params:k})
});
g.on("afterajax",function(j,k){_p=_o;_o="8febb:573";B.eventEmitter.trigger("AUTOCOMPLETE:afterAjax",{data:k})
})
},modelChange:function(){_p=_o;_o="8febb:576";this.updateHiddenFields()
},modelSubmit:function(){_p=_o;_o="8febb:578";this.$form.submit()
},initCurrentCountryField:function(){_p=_o;_o="8febb:580";var j=false;
...
}},onlyCurrentCountryChange:function(){_p=_o;_o="8febb:582";this.model.use("destination");
this.model.destination.setOnlyCurrentCountry(this.$onlyCurrentCountry.is(":checked"))
},updateHiddenFields:function(){_p=_o;_o="8febb:584";var k=this.model.getFormAction();
var m=this.$form.attr("action");
if(!k){k=this.originalAction
}if(m!=k){this.$form.attr("action",k)
}if(this.hiddenFields){this.hiddenFields.forEach(function(o){_p=_o;_o="8febb:585";o.$node.remove();
o.$node=null
})
...
var l=[];
var j=false;
n.forEach(function(o){_p=_o;_o="8febb:587";this.hiddenFields.push({name:o.name,value:o.value});
l.push(o.name);
if(a.indexOf(o.name)!=-1){j=true
}}.bind(this));
this.$form.find("input").filter(function(){_p=_o;_o="8febb:589";return(j&&a.indexOf(this.name)!=-1)||l.indexOf(this.name)!=-1
}).remove();
this.hiddenFields.forEach(function(o){_p=_o;_o="8febb:591";var p=$("<input>");
p.attr("type","hidden");
p.attr("name",o.name);
...
},formSubmit:function(k){_p=_o;_o="8febb:594";if(this.model._ssce){var j=this.model.validate();
if(!j){k.preventDefault();
return
}}}})
});
(function(a,b){_p=_o;_o="8febb:597";a(function(){_p=_o;_o="8febb:598";a(".ge-search-first-checkbox").change(function(){_p=_o;_o="8febb:599";var f="cQZJZadTQZGSRaPSWe";
