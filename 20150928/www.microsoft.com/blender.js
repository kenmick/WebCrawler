    ...
    */
    var openBlend = function (postBody, parentElementId, environment, flight, clientType, hash, iframeOptions) {
        if (!htmlFetched) {
            ...
            var emberCliFlightSwitch = 'embercli:1';
            var isEmberCliFlight = (document && document.cookie && document.cookie.indexOf(emberCliFlightSwitch) > -1);
            if (!isEmberCliFlight) {
                var flights = flight && flight.split(',') || [];
                for (var i = 0; !isEmberCliFlight && i < flights.length; i++) {
                    if (flights[i] === emberCliFlightSwitch) {
                        isEmberCliFlight = true;
                    }
                }
            }
