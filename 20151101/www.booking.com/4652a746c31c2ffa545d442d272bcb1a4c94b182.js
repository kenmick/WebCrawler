if(window.B&&B.env){B.env.b_run_se_unity=1;
B.env.b_run_se_unity_hash="PVdIESZPOSBZTNCMC"
}B.define("component/search/destination/autocomplete",function(b,a,c){var d=b("component/autocomplete");
c.exports=d.extend({init:function(){d.prototype.init.apply(this,arguments);
this.$el
}})
});
...
B.define("search/destination/service-atlas",function(b,e,a){var o=b("event-emitter");
var d=B.env.b_map_center_latitude||B.env.b_latitude||undefined;
var j=B.env.b_map_center_longitude||B.env.b_longitude||undefined;
var k=30000;
var m=["ChIJ2xJC2SwmsUcRBqiHnUEubtY"];
var c={administrative_area_level_1:"region",administrative_area_level_2:"region",administrative_area_level_3:"region",administrative_area_level_4:"region",administrative_area_level_5:"region",airport:"airport",country:"country",locality:"city",point_of_interest:"landmark",route:"district",postal_code:"district",sublocality:"district"};
function h(){return !!B.atlas
}function i(p,r,s){var q={city:[]};
q.__upa__=r.filter(l).slice(0,5).map(function(t){var t=t.get("data");
return{upa:true,nr_hotels:0,nr_hotels_25:0,ss_raw:p.ss,dest_id:t.id,place_id:t.id,dest_type:c[t.types&&t.types[0]]||"landmark",label:t.description,label_highlighted:f(t.description,t.matched_substrings),cjk:(B.env.b_lang==="zh"||B.env.b_lang==="xt"||B.env.b_lang==="ja"||B.env.b_lang==="ko")}
});
q.__js_upa__=true;
s(null,q)
}function l(p){var q=p.get("data");
return m.indexOf(q.id)===-1
}function g(q,r){var p={};
p.name=q.name;
p.latitude=q.geometry.location.lat();
p.longitude=q.geometry.location.lng();
p.place_types=q.types;
p.dest_type=c[q.types&&q.types[0]]||"landmark",r(null,p)
}function n(q,p){this.atlas.getPlaceDetails({placeId:q},function(r){if(!r||typeof r.get!=="function"){p(new Error("service-atlas-invalid-place"),null)
}else{var s=r.get("data");
g(s,p)
}})
}function f(r,u){var p="";
if(!u||u.length===0){return r
}var t=u.map(function(v){return v.offset
});
var q=u.map(function(v){return v.offset+v.length-1
});
for(var s=0;
s<r.length;
s++){if(t.indexOf(s)!=-1){p+="<b>"
}p+=r.charAt(s);
if(q.indexOf(s)!=-1){p+="</b>"
}}return p
}a.exports=o.extend({init:function(r){if(!h()){return
}if(this.initialized){if(r){this.atlas.done(r)
}return
}this.initialized=true;
var q=B.atlas.require("atlas-places");
var p=document.createElement("div");
this.atlas=new q({provider:"provider-places",modules:["autocomplete","places"],options:{domNode:p}});
if(r){this.atlas.done(r)
}},search:function(q,v){if(!q||!q.ss||typeof v!="function"){throw new Error("service-atlas-invalid-arguments")
}if(!h()){setTimeout(function(){v(new Error("service-atlas-no-atlas"),null)
},4);
...
t.lon=j;
t.radius=k
}var s=function(w){if(!p){if(!w||!Array.isArray(w)){v(new Error("service-atlas-invalid-results"),null)
}else{i(q,w,v)
}}};
var r=function(w){if(!p){v(new Error("service-atlas-error "+w),null)
}};
this.init(function(){this.atlas.autocomplete(t,s,r)
}.bind(this));
var u=function(){p=true
};
return{getResultDetails:n.bind(this),abort:u}
}})
});
B.define("search/destination/service-booking",function(b,a,c){var g=b("event-emitter");
var e=B.env.autocomplete_use_auth?"/autocomplete_with_auth":"/autocomplete";
function d(h){h.term=h.ss;
delete h.ss;
return h
}function f(h,i){if(typeof h!=="object"||typeof h.city!=="object"){i(new Error("service-booking-data-invalid"),null)
}else{if(!h.__upa__&&h.city.length===0){i(new Error("service-booking-no-results"),h)
}else{i(null,h)
}}}c.exports=g.extend({search:function(h,l){if(!h||!h.ss||typeof l!="function"){throw new Error("Invalid arguments")
}var i=d(h);
this.emit("beforeajax",h,i);
var j=$.ajax({type:"get",url:e,data:i,success:function(n,m,o){this.emit("afterajax",h,n);
f(n,l)
}.bind(this),error:function(o,m,n){if(m!=="abort"){l(new Error("service-booking-error "+n),null)
}}.bind(this)});
var k=function(){if(j&&j.abort){j.abort()
}};
return{abort:k}
}})
});
...
if(booking.env.autocomplete_bbtool_enhancements){s._renderPartitionHeading(q,k)
}for(A=0;
A<f.length;
...
}if(booking.env.autocomplete_bbtool_enhancements&&E.dest_type==="bbtoollocation"){x+=" ui-menu-item--bbtoollocation"
