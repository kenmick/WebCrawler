...
x.on("marker-hover",function(z){var C=/^hotel/.test(z.type),y=/soldout/.test(z.type);
if(!C){return
}B.track.stage("ebcEKOFaGBfDeOeXefLJDC",1);
if(!y){x.once("iw-get-success",A)
}function A(F){var E,D;
var G;
if(F&&F.b_hotels&&F.b_hotels[z.id]){G=F.b_hotels[z.id];
D=(G.can_accommodate_group==false);
E=G.soldout;
if(E||D){x.trigger("marker-iw-inconsistent");
if(E){B.track.custom_goal("ebcEKOFaGBfDeOeXefLJDC",1)
}if(D){B.track.custom_goal("ebcEKOFaGBfDeOeXefLJDC",2)
}}}}});
