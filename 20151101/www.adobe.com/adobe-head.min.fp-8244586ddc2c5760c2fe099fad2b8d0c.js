...
}},setCookie:function(c,g,e){var f=new Date(),b=g?g:"",h=e?e:0,a;
f.setTime(f.getTime()+(h*24*60*60*1000));
a=f>0?"expires="+f.toUTCString()+";":"";
...
if(geoOb.options.urlItems.hostname.indexOf("adobe")===-1){cookies.resetCookieHost(geoOb.options.urlItems.hostname)
...
return this.col
}},modalcol2:{col:eld("div",{"class":"grid-span-3of5 modalcol2"}),scroller:eld("div",{"class":"paddedwr"}),build:function(d){var c,h,b,g,i,a,f,e,j;
for(c=0,h=Object.keys(d),b=h.length;
c<b;
c++){g=d[h[c]];
i=eld("div",{"class":"geowr"});
a=eld("p");
f=eld("a",{href:"","data-alang":h[c]});
e=eld("div",{"class":"grid-span-3of5 ui-button"});
j=eld("p");
a.innerHTML=g.blurb;
...
i.appendChild(e);
i.appendChild(j);
i.setAttribute("lang",g.adobe_language_path);
i.setAttribute("writing_direction",g.writing_direction);
...
function Loader(){var j=geoOb.options,d=j.akamai,i=j.geoMap,b=j.urlItems,g=undefined,e=i.intlCookie,a=e===j.defaultRegCookie,c=j.geoArray.indexOf(b.path.split("/")[1])>=0?b.path.split("/")[1]:j.defaultRegCookie;
...
var b=new Loader(),a=b.redirect(),c=document.createEvent("Event");
if(a===false){geoOb.ajpf(geoOb.options.geoAdobecom,geoOb.options.regparams,function(d){geoOb.options.akamai={};
geoOb.mapMaps("akamai",d);
b.load();
c.initEvent("aceui:geoLoaded",true,true);
