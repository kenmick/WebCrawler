...
},_renderMenu:function(G,v){var n=this,u=false;
...
if(!booking.track.getVariant("PVdIVMGaOVaMCfFIMNDdWe")==1){b("#ac-suggestion").live("click",function(){b("#destination").val(K);
b("#destination").trigger("keydown.autocomplete")
})
...
A++;
S--
}}}}}},_renderItem:function(s,H,K,D){var w=this;
var I=false;
H.position=K;
H.array_length=D;
var x="";
var o=", ";
var j=0;
if(booking.track.getVariant("PVdIBBVXAKQVAeFEO")==1){var p="",A="ui-menu-item",G="partition_item partition_item_"+H.dest_type;
if(H.dest_type=="theme"){p+=H.highlighted_label;
if(H.additional_label){p+=' <span class="search_ac_hl_theme">'+H.additional_label+"</span>"
}}else{j=H.labels.length;
if(j){for(var y=0;
y<j;
y++){if(H.labels[y].is_required){if(H.labels[y].is_name_hl){p+='<span class="search_ac_hl_name">'
}if(H.labels[y].text_hl){var l=H.labels[y].text,L="",t="",q="";
if(H.labels[y].text_hl[0].start>0){L=l.substring(0,H.labels[y].text_hl[0].start)
}if(H.labels[y].text_hl[0].end<l.length){q=l.substring(H.labels[y].text_hl[0].end,l.length)
}t='<span class="search_ac_hl_token">'+l.substring(H.labels[y].text_hl[0].start,H.labels[y].text_hl[0].end)+"</span>";
p+=(L+t+q)
}else{p+=H.labels[y].text
}if(H.labels[y].match){if(H.labels[y].match_hl){var E=H.labels[y].match,f="",C="",r="";
if(H.labels[y].match_hl[0].start>0){f=E.substring(0,H.labels[y].match_hl[0].start)
}if(H.labels[y].match_hl[0].end<E.length){r=E.substring(H.labels[y].match_hl[0].end,E.length)
}C='<span class="search_ac_hl_token search_ac_hl_token_alt">'+E.substring(H.labels[y].match_hl[0].start,H.labels[y].match_hl[0].end)+"</span>";
p+=" ("+f+C+r+")"
}else{p+=" ("+H.labels[y].match+")"
}}if(H.labels[y].is_name_hl){p+="</span>"
}if(y<j-1){p+=o
}}}}}result=b('<li class="'+A+'"></li>').data("item.autocomplete",H).append(b("<a "+G+">"+p+"</a>")).appendTo(s);
return result
}else{if(H.labels!=null&&typeof(H.labels)!="undefined"){j=H.labels.length
...
}if(typeof(H.labels)!="undefined"&&(H.lc==="zh"||H.lc==="xt"||H.lc==="ja"||H.lc==="ko")){var m=false;
booking.track.exp("MWAAQaNZFFEbdDbaVIFTdWe");
I=true;
b.each(H.labels,function(N,M){if(N===j-1||j==1){o=""
}if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){if(M.type=="region"&&N!=j-1&&N!=0){var O=M;
M=H.labels[N+1];
H.labels[N+1]=O
}if((N!=j-1&&H.labels[N+1].type=="region"&&!m)||(N==0&&j==2&&M.type=="region"&&!m)){o=" (";
m=true
}else{if(N==j-1&&m){o=")"
}else{if(N==j-1){o=""
}else{o=", "
}}}}if(M.hl){if(booking.track.getVariant("MWAAQaNZFFEbdDbaVIFTdWe")==1){x=x+"<strong>"+M.text+"</strong>"+o
}else{x=x+"<strong>"+M.text+o+"</strong>"
}}else{x=x+M.text+o
}})
}else{x=H.label_highlighted||H.label
}var h=function(k){if(k=="undefined"||k==c||k==void 0){return 1
}else{return 0
}};
var p=x,z="",d="",A="",G="",e="",i=H.photo||"";
if(H.header){z='<span class="autocomplete_header autocomplete_h_'+H.dest_type+'" >'+booking.env.autocomplete_categories[H.header]+"</span>";
d=(H.first)?"":' style="border-top: 1px solid #CCC;"';
A+=(H.first)?"":"first-in-group"
...
}var n=H.nr_hotels_label;
if(H.dest_type=="hotel"){e+="("+n+")"
}else{if(H.nr_hotels==0&&H.nr_hotels_25>0){e+='<span class="ac_prop_nearby">('+booking.env.autocomplete_counter_label+")</span>"
}else{e=""
}}e+="</span></span>"
}var v="";
if(H.dest_type=="theme"&&H.additional_label){v='<span class="label" style="position: static;color:#888;">&nbsp;<span style="direction: rtl; text-align: left; display: inline;">'+H.additional_label+"</span></span>"
}var u="PVUELMBBdXUC";
var F='<i class="autocomplete_icons autocomplete_'+H.dest_type+'"></i>';
if(I){var g='<span class="ac-hl-cjk-wrapper">'
}else{var g="<span>"
}p=g+p+e+v+"</span>"+F;
G='class="partition_item partition_item_'+H.dest_type+' ac_item_theme"';
result=b('<li class="'+A+'"'+d+"></li>").data("item.autocomplete",H).append(b("<a "+G+">"+p+"</a>")).appendTo(s);
