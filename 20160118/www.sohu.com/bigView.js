...
define("sjs/matrix/ad/form/bigView", 
	...
	var BigView = Common.create({
		...
		init : function(){
			...
			if(isNewAuto){
				jQuery('#' + adObject.cont_id+'_left').css("background",'url(' + adObject.resource.file + ') no-repeat');
				jQuery('#' + adObject.cont_id+'_right').css("background",'url(' + adObject.resource.file + ') no-repeat');
			}
