...
g._details=true;
this.set(g);
if(window.ga){window.ga("send","event","ac_google_places",[g.place_types||"unknown",g.name].join("|"),this.data.ss,f.position)
}}.bind(this));
return
...
if(q._ssafa){u._fa=B.env.b_nonsecure_hostname+"/searchresults"+B.env.b_query_params_with_lang;
u._fasr=true
}return u
});
r.results=r.__upa__;
...
this.options=f(this.$el,{modelId:{name:"sb-id",type:Number,required:true},gpf:{name:"sb-gpf",type:Boolean},e_v:{name:"e-v",type:Boolean},ssafa:{name:"sb-ssafa",type:Boolean}});
