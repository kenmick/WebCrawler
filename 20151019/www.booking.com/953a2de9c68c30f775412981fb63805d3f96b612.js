...
}B.atlas.define("atlas-common-markers",a,function(r,w,q,g){var k,t=r.get("destinationId"),u=false,e=100,l=null,i=false,m=r.get("isMiniIWLandingPages"),d=r.get("isMiniIW");
function c(A,L,z){var M,I,J=35,G=56,C=5,K=27,H=this.ubber.$domNode,F={w:H.width(),h:H.height()},D={w:A.outerWidth(),h:A.outerHeight()};
G=(G>F.w-z.x-C)?C:G;
...
if(x.data.b_id===t){z+="-current"
}y.className="iw-overlay-lp iw-overlay-"+z;
y.iwType=z.replace(/-/g,"_");
A.setIW(y)
}else{y.className="iw-overlay-lp iw-overlay-"+x.data.b_type+((x.data.b_id===t)?"-current":"")
}A.setIW(y)
}}function s(){var x=this;
x.getMarkers(function(z){var C=r.get("action"),D,y,A;
z=z||{};
y=[].concat(z.b_hotels||[],z.b_cities||[],z.b_airports||[]);
if(B&&B.env&&B.env.lp_map_background_running){y=[]
}A=y.length;
if(C==="searchresults"||C==="hotel"||B.atlas.getVariant("ebcMMECAFWTYcUNC")||r.get("isLP")&&C!=="city"){B.atlas.require(["map-markers-disperse"],function(E){E.initialize(y,x,t)
});
...
if(t){while(A--){D=y[A];
if(D&&t===D.b_id){if(!B.atlas.assert.tdot&&B.atlas.assert.hp){x.setMarkerOptions(D.b_id,{cursor:"default"})
...
}else{if(/^(city|airport)/.test(x.type)||(B.atlas.getVariant("fEJMSeCXeLfHHHYbNKe")&&x.type=="landmark")){if(d||m){if(l!==x.id){if(l){y.setIconOut(l);
y.closeIW()
}f(y,x);
x.data.b_is_current=(x.id||x.data.b_id)===t;
...
y.getIW(x.id,function(z){if(t==x.id){y.set("currentHotel",z)
}else{z.b_destination_reference=y.get("currentHotel")
}z.b_marker_type=(x.data)?x.data.b_marker_type:"";
z.b_map_nights=r.get("checkinCheckoutInterval");
z.map_price_per_x_nights_txt=r.get("transPricePerXNights");
...
}}if(x&&x.data&&/^hotel.*/.test(x.data.b_type)&&(B.atlas.getVariant("eGBUYSaBNHTXT")||B.atlas.getVariant("eGOQPBRNZRUXe"))){return
}if(x.b_type==="geosearch_place"){return false
}if(x&&x.data&&x.data.b_id===t&&x.data.b_type==="city"&&B.atlas.getVariant("eGYSaBHRXOSLTfdJdHPSYSbC")){return false
...
if(/^(city|airport)/.test(x.b_type)){f(C,x);
x.data.b_is_current=(x.id||x.data.b_id)===t;
C.openIW(x.b_id,{},B.jstmpl("atlas_iw_landmark").render(p(x.data)));
C.trigger("iw-opened-click",{marker:x,data:x.data})
}else{C.getIW(x.b_id,function(E){E.b_map_nights=r.get("checkinCheckoutInterval");
E.map_price_per_x_nights_txt=r.get("transPricePerXNights");
...
}}else{if(A&&!(x.data.b_id===t&&(!B.atlas.assert.tdot&&B.atlas.assert.hp))){location.assign(A)
}}}function h(E,z){var C=this,x=C.getMarker(z),y,A=3000;
if(B.atlas.getVariant("eGTNVIdEaUDPNcQLCHT")){var D=z;
z=q.getCurrentHotelId();
if(z){z=z
}else{z=D
}}if(!z){return
}if(!x&&!u){u=true;
...
}if(!r.get("tdot")&&r.get("action")=="searchresults"){if(!x){return
}}else{if(!x||(x&&x.b_type&&/^(city|airport)/.test(x.b_type))){return
}}if(r.get("action")=="hotel"){y=function(){h.call(C,E,z,true);
...
}if(!r.get("tdot")&&r.get("action")=="searchresults"&&/^(city|airport)/.test(x.b_type)){f(C,x);
x.data.b_is_current=(x.id||x.data.b_id)===t;
C.openIW(x.b_id,{},B.jstmpl("atlas_iw_landmark").render(p(x.data)));
C.trigger("iw-opened-click",{marker:x,data:x.data});
return
}C.getIW(z,function(F){F.b_marker_type=(x.data)?x.data.b_marker_type:"";
F.b_map_nights=r.get("checkinCheckoutInterval");
F.map_price_per_x_nights_txt=r.get("transPricePerXNights");
if(t==z){C.set("currentHotel",F)
}f(C,x);
...
}}return{init:n,setupIW:j,autoopenIW:h,getIWPosition:c,markerHover:b,markerOut:v,markerClick:o,boundsChange:s}
})
});
$(function(){if(!B.atlas){return
