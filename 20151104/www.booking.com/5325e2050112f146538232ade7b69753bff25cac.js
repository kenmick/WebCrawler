...
B.define("et_debug_level",function(b,a,c){var d=booking.env;
c.exports=(d.b_dev_server&&d.et&&d.et.debug_level)?parseInt(d.et.debug_level,10):0
});
B.define("et_debug_events",function(b,a,c){c.exports={UNABLE_TO_STRINGIFY:"unable_to_stringify",INIT_BEACON:"init_beacon",DEFER_BEACON:"defer_beacon",SEND_BEACON:"send_beacon",BEACON_SENT:"beacon_sent",TRACK_UNKNOWN_ITEM:"track_unknown_item",SHOULD_TRACK:"should_track",TRACKING_ATTEMPT:"tracking_attempt",NOT_TRACKING_WAS_TRACKED:"not_tracking_was_tracked",NOT_TRACKING_FULLON:"not_tracking_fullon",NOT_TRACKING_NOT_RUNNING:"not_tracking_not_running",ADD_EVENT_LISTENER:"add_event_listener",NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER:"not_existing_element_wont_add_listener",ADD_DEBOUNCED_VIEW_HANDLER:"add_view_handler",ATTACH_VIEW_LISTENER:"attach_view_listener",DETACH_VIEW_LISTENER:"detach_view_listener",CHECK_IF_VISIBLE:"check_if_visible",ONVIEW_TRACKING_TRIGGERED:"onview_tracking_triggered",VISIBLE_CHECK_FINISHED:"visible_check_finished",WONT_ATTACH_EVENT_TRACKING:"wont_attach_event_tracking"}
});
booking.define("et_trackables",function(b,a,c){var d={experiment:"e",stage:"s",goal:"g",custom_goal:"cg"};
c.exports=d
});
booking.define("et_view_tracking_events",function(b,a,c){c.exports=["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"]
});
booking.define("et_debug",function(c,b,e){var g=c("et_debug_level"),d=c("et_debug_events");
var f;
function a(h,j){try{if(!f){f=c("et_event_emitter")
}f.trigger.apply(f,[].slice.call(arguments))
}catch(i){if(g){console.log("Error when trying to report et events",i)
}}}e.exports={level:g,events:d,report:a}
});
booking.define("is_visible",function(c,b,d){function e(g){var i=g.offset(),f=(typeof i.constructor==="function")?i.constructor:{name:"OtherConstructor"},h;
if(f.name==="ClientRect"){h=g.offset();
if(h){return h.top
}return false
}return i.top
}function a(g,i,h){var f=e(g);
if(f===false){return false
}return !!(((i+h)>(f))&&g.is(":visible"))
}d.exports=a
});
booking.define("send_beacon",function(d,h,c){var k=d("tmpl_data"),b=d("et_debug"),f=d("et_trackables");
var l=false,n=[],e;
function j(){b.level&&b.report(b.events.BEACON_SENT,n);
l=false;
if(n.length){i()
}}function g(){var s=[],t=[],r=[],u=[];
for(var q,p=0,o=n.length;
p<o;
++p){q=n[p];
switch(q.what){case f.experiment:s.push(q.hash);
break;
case f.stage:u.push(q.hash+"|"+q.id);
break;
case f.goal:t.push(q.hash);
break;
case f.custom_goal:r.push(q.hash+"|"+q.id);
break;
default:b.level&&b.report(b.events.UNABLE_TO_STRINGIFY,q)
}}return"exps="+s.join(",")+";what="+t.join(",")+";ncustom="+r.join(",")+";stage="+u.join(",")
}function a(o){if(o){n.push(o)
}}function m(){n.length=0
}function i(s,r,u){b.level&&b.report(b.events.INIT_BEACON,s,r,u);
var t,p;
if(arguments.length>0){a({what:s,hash:r,id:u})
}if(!l){b.level&&b.report(b.events.SEND_BEACON,n.slice(0));
t=g();
l=true;
if(!e){e=k(["b_action","b_sid","b_aid","b_lang_for_url","pageview_id","b_site_type_id"])
}p="/js_tracking?ver=2;"+t+";ref="+e.b_action+";sid="+e.b_sid+";aid="+e.b_aid+";lang="+e.b_lang_for_url+";pid="+e.pageview_id+";stype="+e.b_site_type_id;
try{jQuery.ajax({url:p,complete:j})
}catch(q){var o=new Image();
o.onload=o.onerror=j;
o.src=p
}m()
}else{b.level&&b.report(b.events.DEFER_BEACON,n)
}}c.exports=i
});
booking.define("et",function(d,H,b){var g=d("send_beacon"),e=d("is_visible"),q=d("et_debug"),w=d("request_info"),c=d("et_trackables"),p=d("et_view_tracking_events"),a=booking.jset,v={},r=300,D={},x=[],j=false,A=false,G,h=booking.env||{};
if(!a){a={}
}if(!a.r){a.r={}
}if(!a.f){a.f={}
}if(!a.t){a.t={}
}function I(N,M,O){var L;
if(N===c.experiment||N===c.goal){L=[N,M]
}else{L=[N,M,O]
}return L.join("-")
}function o(N){if(!h.et||!h.et.new_js_hashing){return N
}if(v[N]){return v[N]
}var O=2166136261;
...
O^=N.charCodeAt(L)
}return(v[N]=(O>>>0).toString(16))
}function J(M,L,N){q.level&&q.report(q.events.TRACKING_ATTEMPT,{what:M,hash:L,id:N,variant:(M===c.experiment||M===c.stage)&&m(L)});
if(k(M,L,N)){switch(M){case c.experiment:z(c.experiment,L);
w.populate(L);
g(c.experiment,L);
break;
case c.stage:z(c.stage,L,N);
w.populate(L+"|"+N);
g(c.stage,L,N);
break;
case c.goal:z(c.goal,L);
g(c.goal,L);
break;
case c.custom_goal:z(c.custom_goal,L,N);
g(c.custom_goal,L,N);
break;
default:q.level&&q.report(q.events.TRACK_UNKNOWN_ITEM,M,L,N)
}}if(M===c.experiment){return m(L)
}else{return false
}}function z(M,L,N){D[I(M,L,N)]=true
}function k(O,N,P){q.level&&q.report(q.events.SHOULD_TRACK,O,N,P);
if(D[I(O,N,P)]){q.level&&q.report(q.events.NOT_TRACKING_WAS_TRACKED,O,N,P);
return false
}else{if(O===c.experiment||O===c.stage){var M=1<<(P||0),L=o(N);
if(a.r[L]===undefined){q.level&&q.report(q.events.NOT_TRACKING_NOT_RUNNING,O,(P||0),N,(P||0));
return false
}else{if(a.f[L]){q.level&&q.report(q.events.NOT_TRACKING_FULLON,O,N,P);
return false
}else{if(a.t[L]&M){z(O,N,P);
q.level&&q.report(q.events.NOT_TRACKING_WAS_TRACKED,O,N,P);
return false
}}}}}return true
}function y(L,N,Q,P,R){q.level&&q.report(q.events.ADD_EVENT_LISTENER,L,N,Q,P,R);
if(window.jQuery){if(!G){G=jQuery(window)
}var O=jQuery(N).eq(0);
if(!O.length){q.level&&q.report(q.events.NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER,Q,P,R);
return
}if(L==="view"){i(O,Q,P,R)
}else{O.one(L,function M(){J(Q,P,R)
})
}}}function i(M,O,N,P){q.level&&q.report(q.events.ADD_DEBOUNCED_VIEW_HANDLER,M,O,N,P);
var L=I(O,N,P);
if(D[L]){return false
}x.push([M,O,N,P]);
if(!j){q.level&&q.report(q.events.ATTACH_VIEW_LISTENER,x);
G.bind("scroll resize",l);
G.bind(p.join(" "),l);
j=true
}}function l(){if(A){return
}A=true;
var O=[],Q=G.scrollTop(),P=G.height();
q.level&&q.report(q.events.CHECK_IF_VISIBLE,x);
var M;
for(var N=0,L=x.length;
N<L;
++N){M=x[N];
if(M&&e(M[0],Q,P)){q.level&&q.report(q.events.ONVIEW_TRACKING_TRIGGERED,M[1],M[2],M[3]);
J(M[1],M[2],M[3])
}else{O.push(M)
}}x=O;
q.level&&q.report(q.events.VISIBLE_CHECK_FINISHED,x);
if(x.length===0){j=false;
G.unbind("scroll resize",l);
G.unbind(p.join(" "),l);
q.level&&q.report(q.events.DETACH_VIEW_LISTENER)
}window.setTimeout(function(){A=false
},r)
}function f(L,M,N){return function(O,P){if(k(N,O,P)){y(L,M,N,O,P)
}else{q.level&&q.report(q.events.WONT_ATTACH_EVENT_TRACKING,L,M,N,O,P)
}}
}function s(L,M){return{track:f(L,M,c.experiment),stage:f(L,M,c.stage),goal:f(L,M,c.goal),custom_goal:f(L,M,c.custom_goal)}
}function t(L){return function(M){return{exp:function(N){return s(L,M).track(N)
},stage:function(N,O){return s(L,M).stage(N,O)
},goal:function(N){return s(L,M).goal(N)
},custom_goal:function(N,O){return s(L,M).custom_goal(N,O)
}}
}
}function m(L){var M=o(L);
return a.r[M]||a.f[M]||0
}function n(L){if(arguments.length>1){throw"Track only accept one parameter"
}if(!L){return 0
}return J(c.experiment,L)
}function K(M,L){if(!M){return false
}if(L===undefined){return false
}if(L===0){return n(M)
}if(!/^[1-9]$/.test(L)){return false
}return J(c.stage,M,L)
}function u(L){if(!L){return false
}return J(c.goal,L)
}function C(M,L){if(!M||!L||!/^[1-5]$/.test(L)){return false
}return J(c.custom_goal,M,L)
}function F(){}F.prototype.variant=n;
F.prototype.track=n;
F.prototype.stage=K;
F.prototype.goal=u;
F.prototype.custom_goal=C;
F.prototype.on=s;
F.prototype.compat=t;
F.prototype.debug_level=m("GCbGTMEbZDVHT");
var E=new F();
if(m("GCbGTdeVSKe")){q.level&&q.report("using compatibility mode");
booking.jst&&Object.keys(booking.jst).forEach(function(L){a.r[o(L)]=booking.jst[L]
});
booking.track={queue:booking.track.queue,exp:E.track,getVariant:E.track,goal:E.goal,stage:E.stage,custom_goal:E.custom_goal,onView:E.compat("view"),onViewTouch:E.compat("view"),onClick:E.compat("click"),onMouseOver:E.compat("mouseenter")}
}b.exports=E
});
