...
B.define("component/date-selector",function(d,e,b){var j=d("component"),l=d("event-emitter"),k=d("date-formatter");
...
var a=B.env.b_lang;
b.exports=j.extend({init:function(){l.extend(this);
var m=this.$el.find("[data-dateselector-calendar='']");
...
}this.calendar=m.length?this._initCalendar(m):null;
this._attachEvents();
if(!this.$el.data("no-init-change-trigger")){this.monthYearSelect.trigger("change")
}},_onMonthYearSelectChange:function(o){var m=this.getDay(),p=this.getMonth(),n=this.getYear();
m=this._updateDaySelect(n,p);
if(this.calendar){this._setCalendarDate(n,p,m)
}if(this.dateLabel.length>0){this._updateDateLabel(n,p,m)
}this.trigger("date-change",this._buildDateObject(n,p,m))
},_onDaySelectChange:function(o){var m=this.getDay(),p=this.getMonth(),n=this.getYear();
if(this.calendar){this._setCalendarDate(n,p,m)
}if(this.dateLabel.length>0){this._updateDateLabel(n,p,m)
}this.trigger("date-change",this._buildDateObject(n,p,m))
},_onCalendarDateSelected:function(p,m,n){var o=m.getYear(),r=m.getMonth(),q=m.getDate();
this._setSelectsDate(o,r,q);
if(this.dateLabel.length>0){this._updateDateLabel(o,r,q)
}this.trigger("calendar-date-selected");
this.trigger("date-change",this._buildDateObject(o,r,q))
},_onCalendarOpened:function(n,m){var o=this.calendar.data("calendar2");
if(o&&m.id===o.id()){this.trigger("calendar-open",m)
...
},_initCalendar:function(o){if(!$.fn.calendar2){return null
}var n=B.env.b_is_tablet===1?400:261;
B.eventEmitter.bind("CALENDAR:opened",$.proxy(this._onCalendarOpened,this));
var m=booking.calendar2.defaults.startDate;
var p=new Date(m.getFullYear()+1,m.getMonth()+1,0);
return $(o).calendar2({dayNames:i?g:h,sundayFirst:i,monthNames:f,endDate:p,defaultDate:this.getFullDate(),onDateSelected:$.proxy(this._onCalendarDateSelected,this),direction:B.env.rtl?"rtl":"ltr",closeButton:true,monthWidth:n,arrow:!!this._showArrow})
},_setCalendarDate:function(n,o,m){this.calendar.trigger("dateSelected",{value:this._buildDateObject(n,o,m)})
},_setSelectsDate:function(n,o,m){this.monthYearSelect.val(n+"-"+(o+1));
this._updateDaySelect(n,o);
this.daySelect.val(m)
},_updateDateLabel:function(o,p,m){var n=k.format({year:o,month:p,day:m},this.dateFormat);
this.dateLabel.html(n)
},_buildDateObject:function(n,o,m){return{date:m,day:m,month:o,year:n,string:n+"-"+(o+1)+"-"+m,raw:new Date(n,o,m)}
},_updateDaySelect:function(o,p){var m=this.getDay(),n,q;
this.daySelect.html(this._buildDayOptionsForMonthYear(o,p));
n=this.daySelect.find("[value='"+m+"']");
q=n.length?m:(B.et.track("HBNHJLBWHcBPTIRFTWRe")?this.getDaysInMonth(o,p):c[p]);
this.daySelect.val(q);
return q
},_buildDayOptionsForMonthYear:function(q,t){var n=new Date(q,t,1),s=n.getDay(),p=(B.et.track("HBNHJLBWHcBPTIRFTWRe")?this.getDaysInMonth(q,t):c[t]),m=[],r;
for(var o=1;
o<=p;
o++){if(a==="ja"||a==="zh"||a==="ko"||a==="hu"){r=o+" "+g[s]
}else{r=g[s]+" "+o
}m.push("<option value='"+o+"'>"+r+"</option>");
s=s===6?0:s+1
}return m.join("")
},getDaysInMonth:function(m,n){m=parseInt(m,10);
switch(n){case 1:return(m%4===0)&&(m%100!==0)||(m%400===0)?29:28;
...
},getFullDate:function(){var m=this.getDay(),o=this.getMonth(),n=this.getYear();
return this._buildDateObject(n,o,m)
...
},setDate:function(n,o,m){this._setSelectsDate(n,o,m);
if(this.calendar){this._setCalendarDate(n,o,m)
}if(this.dateLabel.length>0){this._updateDateLabel(n,o,m)
}},broadcastDateChange:function(){var m=this.getDay(),o=this.getMonth(),n=this.getYear();
this.trigger("date-change",this._buildDateObject(n,o,m))
}})
});
...
}function m(p){if((g.MJ(g.MC(a[51]))&&(g.MJ(/^(searchresults|hotel)$/.test(g.ATLAS_ENV("action")))))){p+=b[3];
g.MN(a[50],g.MC(a[51]));
p+=b[6]
}p+=b[2];
if(g.MJ(g.ATLAS_STAGE("ebcEKOTBbLOdQOQbTFNeKe"))){}p+=b[6];
if(g.MJ(g.ATLAS_STAGE("ebcEKCcYSaBZGTGFedZUC"))){}p+=b[119];
if((g.MJ(g.ATLAS_ENV("action")+""==="hotel"))){p+=[b[120],"ebcEKOTBbLOdQOQbTFNeKe",b[121]].join("")
}else{if((g.MJ(g.ATLAS_ENV("action")+""==="searchresults"))){p+=[b[120],"ebcEKCcYSaBZGTGFedZUC",b[121]].join("")
}}p+=b[122];
if((((g.MJ(g.MC(a[55]))&&(g.MJ(g.MC(a[52]))||g.MJ(g.MC(a[50]))))&&g.MJ(g.MC(a[53])))&&(g.MJ(g.ATLAS_ENV("action")+""==="searchresults")))){p+=b[18];
g.MN("b_map_hotel_location",(g.MC(a[50])||g.MC(a[52])));
p+=[b[18],g.MC(a[53]),b[123],g.MC(a[54]),b[3]].join("")
}else{if((g.MJ(g.MC(a[55]))&&(g.MJ(g.MC(a[52]))||g.MJ(g.MC(a[50]))))){p+=b[18];
g.MN("b_map_hotel_location",(g.MC(a[50])||g.MC(a[52])));
p+=[b[124],g.MC(a[55]),g.MC(a[56]),b[123],g.MC(a[54]),b[3]].join("")
}else{if(g.MD(a[55])){p+=[b[18],g.MC(a[55]),g.MC(a[56]),b[3]].join("")
}else{if(g.MD(a[50])){p+=[b[18],g.MC(a[50]),b[3]].join("")
}else{if(g.MD(a[52])){p+=[b[18],g.MC(a[52]),b[3]].join("")
}}}}}p+=b[52];
if((g.MJ(g.ATLAS_STAGE("ebcBUYSaBEEFXbaCdTUC"))&&g.MJ(g.ATLAS_TRACK("ebcBUYSaBEEFXbaCdTUC")))){p+=[b[125],"ebcBUYSaBEEFXbaCdTUC",b[126],g.MC(a[57]),b[127],g.MC(a[58]),b[128],g.MB(a[59]),b[129]].join("")
}p+=b[130];
if((g.MJ(g.MC(a[63]))&&((g.MJ(g.ATLAS_STAGE("ebcOQPBRNQTDCPZC"))&&(g.MJ(g.ATLAS_TRACK("ebcOQPBRNQTDCPZC")==2)))||(g.MJ(g.ATLAS_STAGE("ebcBUYSaBcSPWYNWe"))&&(g.MJ(g.ATLAS_TRACK("ebcBUYSaBcSPWYNWe")==2)))))){p+=b[131];
if((g.MJ(g.ATLAS_ENV("action")+""==="hotel"))){p+=[b[120],"ebcBUYSaBcSPWYNWe",b[121]].join("")
}else{if((g.MJ(g.ATLAS_ENV("action")+""==="searchresults"))){p+=[b[120],"ebcOQPBRNQTDCPZC",b[121]].join("")
}}p+=[b[132],g.MB(a[60]),b[133]].join("");
if(g.MD(a[50])){p+=b[134];
g.MN(a[61],g.MC(a[50]));
p+=b[80]
}else{if(g.MD(a[52])){p+=b[135];
g.MN(a[61],g.MC(a[52]));
p+=b[80]
}}p+=[b[136],g.MB(a[62]),b[137]].join("")
}p+=b[138];
return p
}function l(p){if((g.MJ(g.MC(a[51]))&&(g.MJ(/^(searchresults|hotel)$/.test(g.ATLAS_ENV("action")))))){p+=b[3];
g.MN(a[50],g.MC(a[51]));
p+=b[6]
}p+=b[2];
if(g.MJ(g.ATLAS_STAGE("ebcEKOTBbLOdQOQbTFNeKe"))){}p+=b[6];
...
if((g.MJ(g.ATLAS_ENV("lpFullWidthMap"))&&g.MJ(g.MC(a[32])))){q+=b[5];
g.MN("b_url",g.MC(a[32]));
q+=b[4]
}q+=b[9];
g.MN(a[33],undefined);
q+=b[4];
if(((g.MJ(/current/.test(g.MC(a[34]))))&&g.MJ(g.ATLAS_TRACK("ebcBUYSaBHRXOSPQfQVT")))){q+=b[5];
g.MN(a[33],b[60]);
q+=b[4]
}else{q+=b[5];
g.MN(a[33],undefined);
...
if(g.MJ(g.ATLAS_ENV("isRTL"))){q+=b[61];
g.MN(a[35],b[62]);
q+=b[6]
}else{q+=b[3];
g.MN(a[35],b[63]);
...
if((((g.MJ(g.MC(a[38]))&&(g.MJ(g.ATLAS_STAGE("ebcBUVKQPbBbLOdaHOBQHe"))&&g.MJ(g.ATLAS_TRACK("ebcBUVKQPbBbLOdaHOBQHe"))))||(g.MJ(g.ATLAS_STAGE("ebcOLaCbJFYSaBcWRBNbWe"))&&g.MJ(g.ATLAS_TRACK("ebcOLaCbJFYSaBcWRBNbWe"))))||(g.MJ(g.ATLAS_STAGE("ebcfEWXdBGZJNKZGMKZQBO"))&&g.MJ(g.ATLAS_TRACK("ebcfEWXdBGZJNKZGMKZQBO"))))){q+=b[5];
g.MN(a[37],b[66]);
q+=b[4]
}q+=b[4];
...
var p="";
p+=b[67];
if((g.MJ(g.ATLAS_ENV("action")+""==="hotel"))){p+=[b[68],"ebcBUVKQPbBbLOdaHOBQHe",b[69]].join("")
}else{if((g.MJ(g.ATLAS_ENV("action")+""==="searchresults"))){p+=[b[68],"ebcOLaCbJFYSaBcWRBNbWe",b[70]].join("")
}else{if(g.MJ(g.ATLAS_ENV("isLP"))){p+=[b[68],"ebcfEWXdBGZJNKZGMKZQBO",b[69]].join("")
}}}p+=b[3];
if(g.MD(a[37])){p+=[b[71],g.F.html(g.MC(a[38])),b[72]].join("")
}p+=b[73];
g.MN(a[39],p);
q+=b[4]
}q+=[b[74],g.MC(a[17]),b[75],g.MB(a[19]),b[75],g.MB(a[0]),b[75],g.MC(a[20]),b[75],g.MC(a[34]),b[75],g.MC(a[33]),b[75],g.MC(a[25]),b[76]].join("");
if(((g.MJ(g.ATLAS_TRACK("eGBUYSaBNHTXT"))||g.MJ(g.ATLAS_TRACK("ebcfESAcCSBCHC")))||g.MJ(g.ATLAS_TRACK("eGOQPBRNZRUXe")))){q+=b[77]
}q+=b[78];
if(((g.MJ(g.MC(a[41]))&&(g.MJ(g.ATLAS_TRACK("ebcBUYSaBbRPUMVBFUWe")==2)))||(g.MJ(g.ATLAS_TRACK("ebcOQPBRYSKDYEaNdNC")==2)))){q+=b[79];
if((g.MJ(g.ATLAS_ENV("action")+""==="hotel"))){q+=b[80];
g.MN(a[40],[b[81],"ebcBUYSaBbRPUMVBFUWe",b[82]].join(""));
q+=b[18]
}else{if((g.MJ(g.ATLAS_ENV("action")+""==="searchresults"))){q+=b[80];
g.MN(a[40],[b[81],"ebcOQPBRYSKDYEaNdNC",b[82]].join(""));
q+=b[18]
}}q+=[b[83],g.MB(a[40]),b[84],g.MB(a[40]),b[85]].join("");
var w=(g.MC(a[41])||[]);
...
q+=b[90]
}else{if(((g.MJ(g.MC(a[28]))&&(g.MJ(g.ATLAS_STAGE("ebcOLaCbJFYSaBbJPBIO",1))&&g.MJ(g.ATLAS_TRACK("ebcOLaCbJFYSaBbJPBIO"))))||(g.MJ(g.ATLAS_STAGE("ebcBUVKQPbBbLOAPESKC",1))&&g.MJ(g.ATLAS_TRACK("ebcBUVKQPbBbLOAPESKC"))))){q+=[b[91],g.MC(a[21]),b[92],g.MC(a[28]),b[93]].join("");
if((g.MJ(g.MC(a[22]))&&(g.MJ(g.ATLAS_STAGE("ebcOQPBRNPIHAEAUeAC"))&&(g.MJ(g.ATLAS_TRACK("ebcOQPBRNPIHAEAUeAC")==2))))){q+=[b[94],g.MB(a[42]),b[95]].join("")
}q+=b[96]
}else{q+=b[6];
if(g.MD(a[44])){q+=b[97];
if((g.MJ(g.MC(a[43])>1))){q+=b[98]
}q+=b[99];
if((g.MK(g.ATLAS_ENV("tdot"))&&((g.MJ(g.ATLAS_ENV("action")+""==="hotel"))||(g.MJ(g.ATLAS_ENV("action")+""==="searchresults"))))){q+=b[100]
}q+=[b[101],g.MC(a[44]),b[102]].join("")
}q+=b[6]
}}q+=[b[103],g.MC(a[24]),b[104],g.MC(a[9]),g.MC(a[29]),g.MC(a[23]),b[105],g.MC(a[37]),b[13],g.MC(a[39]),b[75]].join("");
if(g.MK(g.ATLAS_ENV("isPartner413084"))){q+=b[30]
}q+=b[18];
if(((g.MJ(g.ATLAS_STAGE("ebcOQPBRNQWOGKPRHe"))&&g.MJ(g.ATLAS_TRACK("ebcOQPBRNQWOGKPRHe")))||(g.MJ(g.ATLAS_STAGE("ebcBUYSaBcHDQXYHET"))&&g.MJ(g.ATLAS_TRACK("ebcBUYSaBcHDQXYHET"))))){q+=b[106]
}q+=[b[107],g.MC(a[45]),b[108]].join("");
if(g.MD(a[48])){q+=b[109];
if(g.MD(a[46])){q+=[b[110],g.HELPER_ENV("b_icons_url"),b[111],g.MC(a[46]),g.MC(a[47]),b[112]].join("")
}q+=b[113]
}else{q+=[b[114],g.MC(a[46]),g.MC(a[47]),b[115]].join("")
}q+=b[3];
if(g.MD(a[49])){q+=b[116]
}q+=b[117];
if(((g.MJ(g.ATLAS_TRACK("eGBUYSaBNHTXT"))||g.MJ(g.ATLAS_TRACK("ebcfESAcCSBCHC")))||g.MJ(g.ATLAS_TRACK("eGOQPBRNZRUXe")))){q+=b[52];
if((g.MJ(g.ATLAS_ENV("action")+""==="searchresults"))){q+=b[118];
q=m(q);
q+=b[139]
}q+=b[140];
if(g.MJ(g.MC(a[65]))){q+=b[141];
if(g.MD(a[64])){q+=[b[142],g.MC(a[64]),b[143]].join("");
if(g.MJ(g.ATLAS_TRACK("eGBfQHKbdCMAWe"))){q+=[b[144],g.format_number_decimal(g.MC(a[65])),b[143]].join("")
}else{q+=[b[144],g.MC(a[65]),b[143]].join("")
}q+=b[145]
}q+=b[146];
g.MN(a[66],g.MC(a[67]));
q+=b[146];
if(g.MJ(g.MC(a[67]))){q+=[b[147],g.MB(a[68]),b[148]].join("");
if((g.MJ(g.MC(a[22]))&&(g.MJ(g.ATLAS_STAGE("ebcBUYSaBYWEIFIeTJWe"))&&(g.MJ(g.ATLAS_TRACK("ebcBUYSaBYWEIFIeTJWe")==2))))){q+=[b[149],g.MB(a[42]),b[148]].join("")
}q+=b[80]
}q+=b[139]
}q+=b[150];
if((g.MJ(g.MC(a[69]))&&(g.MJ(g.ATLAS_ENV("action")+""==="hotel")))){q+=b[151];
if(!((g.MJ(g.ATLAS_STAGE("ebcBUYSaBHaZFBKWXbbQMdOeZZSeae"))&&g.MJ(g.ATLAS_TRACK("ebcBUYSaBHaZFBKWXbbQMdOeZZSeae"))))){q+=[b[152],g.MC(a[69]),b[153]].join("")
}q+=b[3]
}q+=b[154]
}q+=b[155];
if(!(((g.MJ(g.ATLAS_TRACK("eGBUYSaBNHTXT"))||g.MJ(g.ATLAS_TRACK("ebcfESAcCSBCHC")))||g.MJ(g.ATLAS_TRACK("eGOQPBRNZRUXe"))))){q+=b[6];
if((g.MJ(g.MC(a[51]))&&g.MJ(g.ATLAS_TRACK("eGYSaBaUJBZNSTHT")))){q+=b[3];
g.MN(a[50],g.MC(a[51]));
q+=b[6]
}q+=b[156];
q=l(q);
...
if(!(((g.MJ(g.ATLAS_TRACK("eGBUYSaBNHTXT"))||g.MJ(g.ATLAS_TRACK("ebcfESAcCSBCHC")))||g.MJ(g.ATLAS_TRACK("eGOQPBRNZRUXe"))))){q+=b[158];
if((g.MJ(g.MC(a[69]))&&(g.MJ(g.ATLAS_ENV("action")+""==="hotel")))){q+=[b[159],g.MC(a[69]),b[160]].join("")
}q+=b[161];
if(!(g.MJ(g.ATLAS_TRACK("ebcfESAcCFGZJdQFePBYKe")))){q+=b[162]
}q+=b[163]
}q+=b[164];
if(g.MJ(g.MC(a[18])["b_room_group"])){q+=b[18];
g.MN(a[70],g.MC(a[18])["b_u_total_price"]);
q+=b[18];
g.MN(a[71],g.MC(a[18])["b_total_adults_and_children"]);
q+=b[3]
}else{q+=b[18];
var w=(g.MC(a[18])||[]);
j.unshift(null);
...
g.MN(a[74],(g.MI(g.MC(a[75]))-g.MI(1)));
q+=b[164];
if(g.MD(a[77])){q+=b[18];
g.MN(a[76],b[165]);
q+=b[3]
...
}else{if((g.MJ(g.MB(a[78])+""==="false"))){q+=b[18];
g.MN(a[76],b[169]);
q+=b[3]
}}}}q+=b[3];
g.MN(a[79],g.MG(((g.MC(a[18])||[])[0]||{})["rackrate"]));
q+=b[3];
if(((g.MJ(g.MB(a[79]))&&(g.MJ(g.MB(a[79])+""!==""+g.MB(a[70]))))&&g.MJ(g.ATLAS_TRACK("eGYSaBZEGJGEO")))){q+=b[18];
g.MN(a[76],[g.MB(a[76]),b[170]].join(""));
q+=b[3]
}q+=b[3];
if(g.MD(a[17])){q+=b[18];
g.MN(a[76],[g.MB(a[76]),b[171]].join(""));
q+=b[3]
}q+=b[52];
if((g.MJ(g.MC(a[65]))&&g.MK(((g.ATLAS_TRACK("eGBUYSaBNHTXT")||g.ATLAS_TRACK("ebcfESAcCSBCHC"))||g.ATLAS_TRACK("eGOQPBRNZRUXe"))))){q+=b[172];
if(g.MD(a[64])){q+=[b[173],g.MC(a[64]),b[144]].join("");
if(g.MJ(g.ATLAS_TRACK("eGBfQHKbdCMAWe"))){q+=[b[174],g.format_number_decimal(g.MC(a[65])),b[144]].join("")
}else{q+=[b[174],g.MC(a[65]),b[144]].join("")
}q+=b[175]
}q+=b[135];
g.MN(a[66],g.MC(a[67]));
q+=b[176];
if(g.MJ(g.MC(a[67]))){q+=[b[177],g.MB(a[68]),b[148]].join("")
}q+=b[178]
}q+=b[52];
if((g.MJ(g.ATLAS_STAGE("ebcBUYSaBYdXfTYMNJFC"))&&g.MJ(g.ATLAS_TRACK("ebcBUYSaBYdXfTYMNJFC")))){q+=b[179];
q=l(q);
q+=b[139]
}q+=[b[180],g.MB(a[76]),b[181]].join("");
g.MN(a[80],g.ATLAS_ENV("checkinDay"));
q+=b[80];
g.MN(a[81],g.ATLAS_ENV("checkoutDay"));
q+=b[146];
g.MN(a[82],g.ATLAS_ENV("checkinMonthShort"));
q+=b[146];
g.MN(a[83],g.ATLAS_ENV("checkoutMonthShort"));
q+=b[80];
g.MN(a[84],g.ATLAS_ENV("checkoutMonthShort"));
q+=b[182];
if(g.MD(a[77])){q+=b[183];
if((g.MJ(g.ATLAS_TRACK("ebcOQPBRNZRUAJJSVHDFNIVSHT"))||g.MJ(g.ATLAS_TRACK("ebcBUYSaBNHTIPfbJEfZBWIbYO")))){q+=b[135];
g.MN(a[85],[b[135],g.HELPER_ENV("ajax_hotel_details_soldout"),b[135]].join(""));
q+=b[135];
g.MN(a[86],[b[135],g.HELPER_ENV("ajax_hotel_details_soldout"),b[135]].join(""));
q+=b[184];
if(((((((((((g.MJ(g.MB(a[80])+""===""))||(g.MJ(g.MB(a[81])+""==="")))||(g.MJ(g.MB(a[82])+""==="")))||(g.MJ(g.MB(a[83])+""==="")))||(g.MJ(g.MB(a[84])+""==="")))||g.MK(g.MB(a[80])))||g.MK(g.MB(a[81])))||g.MK(g.MB(a[82])))||g.MK(g.MB(a[83])))||g.MK(g.MB(a[84])))){q+=[b[143],g.HELPER_ENV("ajax_hotel_details_soldout"),b[135]].join("")
}else{q+=b[143];
if((g.MJ(g.MB(a[82])+""===""+g.MB(a[84])))){q+=[b[144],g.MB(a[87]),b[143]].join("")
}else{q+=[b[144],g.MB(a[88]),b[143]].join("")
}q+=b[135]
}q+=b[185]
}else{q+=[b[135],g.HELPER_ENV("ajax_hotel_details_soldout"),b[80]].join("")
}q+=b[186]
}else{if(((g.MJ((g.MC(a[18])).length)&&g.MJ(g.MC(a[18])[0]["checkin"]))&&g.MJ(g.MC(a[18])[0]["checkout"]))){q+=b[187];
if(g.MJ(g.ATLAS_TRACK("eGOQRWSEeTOZPbaPWAUC"))){q+=[b[188],g.MG(((g.MC(a[18])||[])[0]||{})["price"]),b[189]].join("")
}else{q+=b[143];
if(g.MJ(g.ATLAS_TRACK("eGBUYSaBGcPDeBEJYcMEfTRe"))){q+=b[190];
if((g.MJ(g.ATLAS_ENV("currency")+""==="EUR"))){q+=b[191];
g.MN(a[89],b[192]);
q+=b[174]
}else{if((g.MJ(g.ATLAS_ENV("currency")+""==="GBP"))){q+=b[191];
g.MN(a[89],b[193]);
q+=b[174]
}else{if((g.MJ(g.ATLAS_ENV("currency")+""==="JPY"))){q+=b[191];
g.MN(a[89],b[194]);
q+=b[174]
}else{if((g.MJ(g.ATLAS_ENV("currency")+""==="CNY"))){q+=b[191];
g.MN(a[89],b[195]);
q+=b[174]
}else{q+=b[196];
g.MN(a[89],b[197]);
q+=b[174]
}}}}q+=[b[198],g.MC(a[89]),b[199],g.MC(a[90]),b[200]].join("");
if((g.MJ(g.ATLAS_ENV("currency")+""==="EUR"))){q+=b[201]
}else{if((g.MJ(g.ATLAS_ENV("currency")+""==="GBP"))){q+=b[202]
}else{if((g.MJ(g.ATLAS_ENV("currency")+""==="JPY"))){q+=b[203]
}else{if((g.MJ(g.ATLAS_ENV("currency")+""==="CNY"))){q+=b[204]
}else{q+=b[205]
}}}}q+=b[206]
}else{q+=[b[174],g.HELPER_FROM_FRICE(g.MC(a[18])[0]["price"]),b[143]].join("")
}q+=b[143]
}q+=b[207]
}else{if(g.MD(a[18])){q+=b[208];
g.MN(a[79],g.MG(((g.MC(a[18])||[])[0]||{})["rackrate"]));
q+=b[135];
if(((g.MJ(g.MB(a[79]))&&(g.MJ(g.MB(a[79])+""!==""+g.MB(a[70]))))&&g.MJ(g.ATLAS_TRACK("eGYSaBZEGJGEO")))){q+=b[143];
g.MN(a[91],g.MG(((g.MC(a[18])||[])[0]||{})["price_discount"]));
q+=b[143];
g.MN(a[92],g.MB(a[91]));
q+=b[143];
var p="";
p+=b[144];
if(g.MJ(g.MB(a[79]))){p+=b[209];
if((g.MJ(g.ATLAS_GET_VARIANT("eGYSaBZEGJGEO")==2))){p+=b[210]
}p+=[b[211],g.MB(a[94]),b[212],g.MB(a[79]),b[213]].join("")
}p+=b[143];
g.MN(a[93],p);
q+=b[135]
}else{q+=b[143];
...
g.MN(a[95],g.MC(a[96]));
q+=b[135];
if((g.MJ(g.ATLAS_STAGE("eGBUYSaBdbLMaCffHPSaTfRe"))&&g.MJ(g.ATLAS_TRACK("eGBUYSaBdbLMaCffHPSaTfRe")))){q+=b[143];
g.MN(a[97],undefined);
q+=b[135]
}else{q+=b[143];
g.MN(a[97],b[66]);
...
q+=b[184];
var p="";
p+=[b[216],g.MB(a[71]),b[75],g.MB(a[97]),b[217],"eGBUYSaBdbLMaCffHPSaTfRe",b[218],g.MB(a[93]),b[219],g.MB(a[70]),b[220]].join("");
if(((((g.MJ(g.MC(a[95])>0))&&(g.MJ(g.MC(a[95])==1)))&&g.MJ(g.ATLAS_STAGE("ebcYSaBGceZdAVIXe")))&&g.MJ(g.ATLAS_TRACK("ebcYSaBGceZdAVIXe")))){p+=[b[221],g.VP(b[222],"b_checkin_checkout_interval"),b[223]].join("")
}else{if((g.MJ(g.MC(a[95])>1))){p+=[b[221],g.VP(b[222],"b_checkin_checkout_interval"),b[224]].join("")
}}p+=b[135];
g.MN(a[100],p);
q+=b[208];
if((g.MJ(g.HELPER_IW_B_BLOCKS(g.MC(a[18]),0,"num_rooms_available_translated"))&&((g.MJ(g.ATLAS_ENV("action")+""==="hotel"))||g.MJ(g.ATLAS_TRACK("eGOLaBQPBRNSaBGKe"))))){q+=[b[225],g.MC(a[100]),b[226],g.HELPER_IW_B_BLOCKS(g.MC(a[18]),0,"num_rooms_available_translated"),b[227]].join("")
}else{q+=[b[143],g.MC(a[100]),b[135]].join("")
}q+=b[146]
}else{if((g.MJ(g.MB(a[78])+""==="false"))){q+=[b[228],g.strings("map_occupancy_disclaimer_1"),b[229]].join("")
}}}}q+=b[230];
var p="";
p+=b[80];
if(g.MD(a[106])){p+=b[135];
if(g.MD(a[77])){p+=b[143];
if(g.MK(g.MC(a[103]))){p+=[b[144],g.MB(a[102]),b[143]].join("")
}else{p+=[b[144],g.MB(a[104]),b[143]].join("")
}p+=b[135]
}else{p+=b[143];
if((((g.MJ(g.ATLAS_STAGE("ebcOQPBRYcYFMbcMZFNTeae"))&&g.MJ(g.ATLAS_TRACK("ebcOQPBRYcYFMbcMZFNTeae")))||(g.MJ(g.ATLAS_STAGE("ebcBUYSaBdBbCANCdZBSeLT"))&&g.MJ(g.ATLAS_TRACK("ebcBUYSaBdBbCANCdZBSeLT"))))||(g.MJ(g.ATLAS_STAGE("VOGBUYSaBdBbCANCdZBSeLT"))&&g.MJ(g.ATLAS_TRACK("VOGBUYSaBdBbCANCdZBSeLT"))))){p+=[b[144],g.MB(a[105]),b[143]].join("")
}else{p+=[b[144],g.MB(a[105]),b[143]].join("")
}p+=b[135]
}p+=b[80]
}else{p+=b[135];
if(g.MK(g.MC(a[103]))){p+=[b[143],g.MB(a[102]),b[135]].join("")
}else{p+=[b[143],g.MB(a[104]),b[135]].join("")
}p+=b[80]
}p+=b[18];
g.MN(a[101],p);
q+=b[18];
...
if(g.MK((g.MB(a[77])&&(g.ATLAS_GET_VARIANT("eGYSaBAeCQJJYDBeWe")||g.ATLAS_GET_VARIANT("eGYSaBAeCQJJYDBePART"))))){q+=[b[231],g.MC(a[26]),b[232],g.MC(a[9]),g.MC(a[30]),g.MC(a[23]),g.MC(a[31]),b[233],g.MC(a[36]),b[234]].join("");
if(g.MJ(g.ATLAS_TRACK("ebcBUYSaBNHFVXKNUADDbddSdceUPTDPFHe"))){q+=b[235]
}q+=b[82];
if(g.MK(g.ATLAS_ENV("isPartner413084"))){q+=b[30]
}q+=b[236];
if(g.MJ(g.MB(a[77]))){q+=[b[144],g.MC(a[101]),b[143]].join("")
}else{if(((g.MJ(g.MC(a[106]))&&g.MK(g.MB(a[77])))&&((g.MJ(g.ATLAS_STAGE("ebcOLaCbJFYSaBdBaHIKKDfXYZBdRHe"))&&g.MJ(g.ATLAS_TRACK("ebcOLaCbJFYSaBdBaHIKKDfXYZBdRHe")))||(g.MJ(g.ATLAS_STAGE("ebcBUVKQPbBbLOMSVEKXRPLRBdSMHET"))&&g.MJ(g.ATLAS_TRACK("ebcBUVKQPbBbLOMSVEKXRPLRBdSMHET")))))){q+=b[237];
if((g.MJ(/213|220|228|230|232/.test(g.MC(a[108]))))){q+=[b[174],g.MB(a[107]),b[75],g.MC(a[35]),b[144]].join("")
}else{if((g.MJ(/201|219|230|229/.test(g.MC(a[108]))))){q+=[b[174],g.MB(a[109]),b[75],g.MC(a[35]),b[144]].join("")
}else{q+=[b[174],g.MB(a[110]),b[75],g.MC(a[35]),b[144]].join("")
}}q+=b[143]
}else{if(((g.MJ(g.MC(a[106]))&&g.MK(g.MB(a[77])))&&(g.MJ(g.ATLAS_STAGE("VOGBUYSaBdBbCANCdZBSeLT"))&&g.MJ(g.ATLAS_TRACK("VOGBUYSaBdBbCANCdZBSeLT"))))){q+=[b[144],g.MC(a[101]),b[75],g.MC(a[35]),b[143]].join("")
}else{q+=[b[144],g.MC(a[101]),b[143]].join("")
}}}q+=b[238]
}q+=b[18]
}q+=b[239];
if(!(((g.MJ(g.ATLAS_TRACK("eGBUYSaBNHTXT"))||g.MJ(g.ATLAS_TRACK("ebcfESAcCSBCHC")))||g.MJ(g.ATLAS_TRACK("eGOQPBRNZRUXe"))))){q+=b[240]
}q+=b[155];
if(((g.MJ(g.ATLAS_TRACK("eGBUYSaBNHTXT"))||g.MJ(g.ATLAS_TRACK("ebcfESAcCSBCHC")))||g.MJ(g.ATLAS_TRACK("eGOQPBRNZRUXe")))){q+=b[2];
if(!(((g.MJ(g.ATLAS_ENV("action")+""==="searchresults"))||g.MJ(g.ATLAS_TRACK("ebcBUYSaBYdXfTYMNJFC"))))){q+=b[241];
q=m(q);
q+=b[96]
}q+=b[155]
}q+=b[19];
if(((g.MJ(g.ATLAS_TRACK("eGBUYSaBNHTXT"))||g.MJ(g.ATLAS_TRACK("ebcfESAcCSBCHC")))||g.MJ(g.ATLAS_TRACK("eGOQPBRNZRUXe")))){q+=b[242];
if(((g.MJ(g.ATLAS_STAGE("ebcOLaCbJFYSaBbJZSYT",1))&&g.MJ(g.ATLAS_TRACK("ebcOLaCbJFYSaBbJZSYT")))||(g.MJ(g.ATLAS_STAGE("ebcBUVKQPbBbLOAPdZUO",1))&&g.MJ(g.ATLAS_TRACK("ebcBUVKQPbBbLOAPdZUO"))))){q+=[b[243],g.MC(a[9]),g.MC(a[30]),g.MC(a[23]),g.MC(a[31]),b[244],g.MC(a[36]),b[13]].join("");
if(g.MK(g.ATLAS_ENV("isPartner413084"))){q+=b[30]
}q+=b[245];
if(g.MJ(g.MB(a[77]))){q+=[b[18],g.MC(a[101]),b[52]].join("")
}else{if(((g.MJ(g.MC(a[106]))&&g.MK(g.MB(a[77])))&&((g.MJ(g.ATLAS_STAGE("ebcOLaCbJFYSaBdBaHIKKDfXYZBdRHe"))&&g.MJ(g.ATLAS_TRACK("ebcOLaCbJFYSaBdBaHIKKDfXYZBdRHe")))||(g.MJ(g.ATLAS_STAGE("ebcBUVKQPbBbLOMSVEKXRPLRBdSMHET"))&&g.MJ(g.ATLAS_TRACK("ebcBUVKQPbBbLOMSVEKXRPLRBdSMHET")))))){q+=b[79];
if((g.MJ(/213|220|228|230|232/.test(g.MC(a[108]))))){q+=[b[80],g.MB(a[107]),b[75],g.MC(a[35]),b[18]].join("")
}else{if((g.MJ(/201|219|230|229/.test(g.MC(a[108]))))){q+=[b[80],g.MB(a[109]),b[75],g.MC(a[35]),b[18]].join("")
}else{q+=[b[80],g.MB(a[110]),b[75],g.MC(a[35]),b[18]].join("")
}}q+=b[3]
}else{q+=[b[18],g.MC(a[101]),b[3]].join("")
}}q+=b[246]
}q+=b[242]
}q+=[b[247],g.MC(a[27]),b[248],g.MB(a[13]),b[249]].join("");
