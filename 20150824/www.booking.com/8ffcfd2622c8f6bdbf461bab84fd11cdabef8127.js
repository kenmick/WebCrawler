...
}}}if(booking.track.getVariant("PVdIdAHEZWDTPVJTTC")||(booking.env.search_input_prefill_tracking===1&&booking.track.getVariant("PVDdWLeJIRBHT")==1)){b("#ac-suggestion a").live("mouseover",function(){b("#destination").data("autocomplete").menu.deactivate()
});
...
}}(function(l){if(!l||!booking.track.getVariant("PVdIYROdVaDXKe")){return
...
}if(l===2){var Y=booking.jstmpl.translations("search_box_opt_in_alternative_search",null,{search_term2:v.__suggestions__.initial_term});
...
}b("#ac-suggestion.search-spellcheck a").bind("click",function(){n._search(v.__suggestions__.initial_term,{v_out:0});
b("ul.ui-autocomplete").delegate(".partition_item","click.search_rewrite",function(){t.val(0)
})
})
...
return
}if(booking.track.getVariant("PVdIdAHEZWDTPVJTTC")||(booking.env.search_input_prefill_tracking===1&&booking.track.getVariant("PVDdWLeJIRBHT")==1)){if(this.menu.first()&&/^previous/.test(e)||this.menu.last()&&/^next/.test(e)){if(!booking.track.getVariant("PVDdWLeJIRBHT")){this.element.val(this.term);
this.menu.deactivate();
return
}}}this.menu[e](d)
...
g(s,function(u){if(u&&u.length){u=u.filter(function(w){var v={ChIJ2xJC2SwmsUcRBqiHnUEubtY:1};
...
return{upa:true,ss_raw:s.term,latitude:w.latitude,longitude:w.longitude,dest_type:i(w.types||v.types),dest_id:v.place_id,place_id:v.place_id,place_types:w.place_types,label:v.description,label_highlighted:l(v.description,v.matched_substrings),nr_hotels:0,nr_hotels_25:0}
});
r.__js_upa__=true
}}t(r)
})
};
var o=$("input#destination").bind("keyup",function(){if(!B.track.getVariant(e)){return
}if(!B.atlas){return
}m()
});
...
v=p[r]={name:u.name,latitude:u.geometry.location.lat(),longitude:u.geometry.location.lng(),place_types:u.types,dest_type:i(u.types)}
}if(s){s(v)
}})
...
}else{if(window.ga){window.ga.send("send","event","ac_google_places",s.ss_raw,r,s.position)
...
booking[sNSExperiments].search_autocomplete={priority:9,cur_ui_item:{},init:function(){var f=this,d=$("#destination"),e=d.closest("form"),c=false;
d.one("focus",function(){booking.track.exp("OQJbQMASHDSeEGGO");
...
d.bind("blur",function(j){if(booking.track.getVariant("PVdIVUKeYEGeZOeUPfBHT")=="1"){$(".autocomplete_onboarding").hide()
}if(booking.track.getVariant("PVDdWLeJIRBHT")=="1"){var h=d.data("autocomplete").menu.active;
if(h&&h.length&&h.data("item.autocomplete")){f.cur_ui_item=h.data("item.autocomplete");
var i="<div>"+f.cur_ui_item.label+"</div>";
d.val($(i).text())
}}});
d.keydown(function(h){if(h.keyCode!=13){f.cur_ui_item={};
e.find("input[name='place_id']").remove();
e.find("input[name='place_id_lat']").remove();
e.find("input[name='place_id_lon']").remove()
}});
...
var b=200;
var g=2;
d.autocomplete({source:a,extraParams:booking.env.search_autocomplete_params,minLength:g,delay:booking.env.search_autocomplete_delay||b}).bind("autocompleteselect",function(i,j){if(j.item){c=true;
f.cur_ui_item=j.item;
d.trigger("auto_completed");
...
if(booking.env.b_action=="searchresults"){var h=booking.track.getVariant("OQJbQMASHDSeEGGO");
if(h!==false){if(h==1){$("#frm").submit()
}}}}}).bind("autocompleteopen",function(i,k){if(booking.track.getVariant("PVdIVUKeYEGeZOeUPfBHT")=="1"){$(".autocomplete_onboarding").hide()
...
}}}}}).bind("autocompletesearch",function(h,i){if(B.env.b_action=="country"){var j=d.data("autocomplete").options.extraParams;
if($("#limit-search-area").is(":checked")){j.e_acf_i=B.env.b_country_id;
j.e_acf_t="country"
}else{j.e_acf_i="";
j.e_acf_t=""
}}if(B.env.fesp_autocomplete_excluded_countries){var j=d.data("autocomplete").options.extraParams;
j.fesp_acf_i=B.env.fesp_autocomplete_excluded_countries
}});
if(B.env.b_action=="country"){$("#limit-search-area").change(function(){d.autocomplete("search",d.val());
d.focus()
})
}d.bind("focus",function(h,i){if(B.env.b_action=="country"){}else{return false
}});
if(booking.env.autocomplete_first_query&&booking.env.search_input_prefill_tracking==1&&booking.track.getVariant("PVDdWLeJIRBHT")==1){d.bind("autocompleteclose",function(i,k){if($.isEmptyObject(f.cur_ui_item)&&d.val().length>1){var h=d.data("autocomplete").menu.active;
if(h&&h.length&&h.data("item.autocomplete")){f.cur_ui_item=h.data("item.autocomplete");
var j="<div>"+f.cur_ui_item.label+"</div>";
d.val($(j).text())
}}})
}e.bind("submit",function(j,l){booking.env.ac_close_on_enter_hide=false;
if(booking.env.ac_close_on_enter_keypress_enter===true&&e.find("#sb_enter_key_ac_request").val()<=0){e.find("#sb_enter_key_ac_request").val(1)
}if(d.val()===d.attr("data-wishlist")){d.val(d.attr("data-hotels"));
e.append($("<input>",{type:"hidden",name:"si",value:"ho"}));
e.append($("<input>",{type:"hidden",name:"wl_name",value:d.attr("data-wishlist")}))
}if(booking.track.getVariant("XCePEQYfEbVFRTKe")&&d.attr("data-recent-title")===d.val()){e.append($("<input>",{type:"hidden",name:"si",value:"ho"}));
e.append($("<input>",{type:"hidden",name:"recent_viewed_title",value:d.attr("data-recent-title")}))
}var i=f.cur_ui_item;
if(i.dest_type){var k={dest_type:i.dest_type,dest_id:i.dest_id,ac_pageview_id:booking.env.pageview_id,ac_position:i.position,ac_langcode:i.lc,ac_suggestion_list_length:i.array_length,ss_short:i.ss_short,place_id:i.place_id,place_id_lat:i.place_id_lat,place_id_lon:i.place_id_lon,place_types:i.place_types||""};
if(booking.env.autocomplete_bbtoollocations){k.bbtoollocation=(i.dest_type==="bbtoollocation")?1:null
}e.find("input").filter(function(){if(k.hasOwnProperty(this.name)){return true
}}).remove();
if(k.place_id){k.dest_id=null
}for(var h in k){if(k[h]===null||k[h]===undefined){continue
}if(!k.hasOwnProperty(h)){continue
}e.append($("<input>",{type:"hidden",name:h,value:k[h]}))
}}else{if(d.val()==booking.env.sess_dest_fullname&&booking.env.sess_dest_id&&booking.env.sess_dest_type&&booking.env.normalize_sbox_value){e.find("input[name='dest_type'], input[name='dest_id'], input[name='ac_pageview_id'], input[name='ac_position'], input[name='ac_langcode'], input[name='ac_suggestion_list_length']").remove();
e.append($("<input>",{type:"hidden",name:"dest_type",value:booking.env.sess_dest_type}));
e.append($("<input>",{type:"hidden",name:"dest_id",value:booking.env.sess_dest_id}))
...
e.find("input[name='ac_close_on_enter']").remove();
e.append($("<input>",{type:"hidden",name:"ac_close_on_enter",value:"1"}))
}if(e.find("#b_searchbox_errors [rel='searchstring_not_found']").length&&booking.track.getVariant("bLNZFKbeVEYUaDEDAJFbLEDJNMO")&&booking.env.b_not_found_searchstring){e.find("input[name='ssne'], input[name='ssne_untouched']").remove()
