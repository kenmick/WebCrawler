...
function sohuad_buttom_n(){
    ...
    var browserId = userAgent.match(/(firefox|chrome|safari|opera|msie|mozilla)/)[1];
...
//	});
	...
	jQuery('#bottom_replay').live('click',function(){
		sohuad_bottom_replay = 1;
		sohuad_buttom_b();
        jQuery("#LIUMEITI2").css({"display":"block"});
		jQuery("#_LIUMEITI2").css({"display":"block"});
		setTimeout(function() {
            jQuery("#LIUMEITI2").css({"display":"none"});
			jQuery("#_LIUMEITI2").css({"display":"none"});
		},8000)
	});
	//big
	...
	var control_list2 = '<div id="bottom_close2" style="width:72px;height:16px;position:absolute;top:0;right:0;"><a><img src="http://images.sohu.com/bill/tongyong/close0910.png" style="height:20px;"></a></div>';
	
		jQuery('#SohuBottomAd2').append(jQuery(control_list));
	
	
	jQuery('#bottom_close2').live('click',function(){
		// clearTimeout(sohuad_bottom_time);
		// sohuad_buttom_s();
		jQuery("#SohuBottomAdWrapper2").css({"display":"none"});
	});
