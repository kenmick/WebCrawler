...
B.define("et_debug_level",function(){var a=B.env;
return(a.b_dev_server&&a.et&&a.et.debug_level)?parseInt(a.et.debug_level,10):0
});
booking.define("et_items_to_track",function(){return{experiment:"e",stage:"s",goal:"g",custom_goal:"cg"}
});
booking.define("et_view_tracking_events",function(){return["GENERAL:dom_changed","GENERAL:layout_changed","GENERAL:throttled_scroll","GENERAL:throttled_resize","tab-opened","user_access_menu_register_tab","user_access_menu_login_tab","uc_popover_showed","rt-lightbox-open","rt-lightbox-closed"]
});
booking.define("et_debug",["et_debug_level","et_debug_events"],function(d,b){var c;
function a(f,h){try{if(!c){c=booking.require("et_event_emitter")
}c.trigger.apply(c,[].slice.call(arguments))
}catch(g){if(d){console.log("Error when trying to report et events",g)
}}}return{level:d,events:b,report:a}
});
...
break;
case i.stage:s.push(o.hash+"|"+o.id);
break;
...
}if(!h){b.level&&b.report(b.events.SEND_BEACON,k.slice(0));
q=d();
h=true;
if(!c){c=f(["b_action","b_sid","b_aid","b_lang_for_url","pageview_id","b_site_type_id"])
...
booking.define("et",["send_beacon","is_visible","et_debug","request_info","et_items_to_track","et_view_tracking_events"],function(q,w,t,y,v,g){var j=booking.jset,s={},r={},e=300,k={},n=[],i=false,C=false,I,D=booking.env||{};
if(!j){j={}
}if(!j.r){j.r={}
}if(!j.f){j.f={}
}if(!j.t){j.t={}
}function c(L,K,M){var J;
if(L===v.experiment||L===v.goal){J=[L,K]
}else{J=[L,K,M]
}return J.join("-")
}function b(L){if(!D.et||!D.et.new_js_hashing){return L
}if(r[L]){return r[L]
}var M=2166136261;
for(var J=0,K=L.length;
J<K;
++J){M+=(M<<1)+(M<<4)+(M<<7)+(M<<8)+(M<<24);
M^=L.charCodeAt(J)
}return(r[L]=(M>>>0).toString(16))
}function l(K,J,L){t.level&&t.report(t.events.TRACKING_ATTEMPT,{what:K,hash:J,id:L,variant:(K===v.experiment||K===v.stage)&&A(J)});
if(a(K,J,L)){switch(K){case v.experiment:d(v.experiment,J);
y.populate(J);
q(v.experiment,J);
return A(J);
case v.stage:d(v.stage,J,L);
y.populate(J+"|"+L);
q(v.stage,J,L);
return false;
case v.goal:d(v.goal,J);
q(v.goal,J);
return false;
case v.custom_goal:d(v.custom_goal,J,L);
q(v.custom_goal,J,L);
return false;
default:t.level&&t.report(t.events.TRACK_UNKNOWN_ITEM,K,J,L);
return false
}}else{return false
}}function d(K,J,L){k[c(K,J,L)]=true
}function a(M,L,N){t.level&&t.report(t.events.SHOULD_TRACK,M,L,N);
if(k[c(M,L,N)]){t.level&&t.report(t.events.NOT_TRACKING_WAS_TRACKED,M,L,N);
return false
}else{if(M===v.experiment||M===v.stage){var K=(N||0)+1,J=b(L);
if(j.r[J]===undefined){t.level&&t.report(t.events.NOT_TRACKING_NOT_RUNNING,M,(N||0),L,(N||0));
return false
}else{if(j.f[J]){t.level&&t.report(t.events.NOT_TRACKING_FULLON,M,L,N);
return false
}else{if(j.t[J]&K){d(M,L,N);
t.level&&t.report(t.events.NOT_TRACKING_WAS_TRACKED,M,L,N);
return false
}}}}}return true
}function f(J,L,O,N,P){t.level&&t.report(t.events.ADD_EVENT_LISTENER,J,L,O,N,P);
if(window.jQuery){if(!I){I=jQuery(window)
}var M=jQuery(L).eq(0);
if(!M.length){t.level&&t.report(t.events.NOT_EXISTING_ELEMENT_WONT_ADD_LISTENER,O,N,P);
return
}if(J==="view"){z(M,O,N,P)
}else{M.one(J,function K(){l(O,N,P)
})
}}}function z(K,M,L,N){t.level&&t.report(t.events.ADD_DEBOUNCED_VIEW_HANDLER,K,M,L,N);
var J=c(M,L,N);
if(k[J]){return false
}n.push([K,M,L,N]);
if(!i){t.level&&t.report(t.events.ATTACH_VIEW_LISTENER,n);
I.bind("scroll resize",G);
I.bind(g.join(" "),G);
i=true
}}function G(){if(C){return
}C=true;
var M=[],O=I.scrollTop(),N=I.height();
t.level&&t.report(t.events.CHECK_IF_VISIBLE,n);
var K;
for(var L=0,J=n.length;
L<J;
++L){K=n[L];
if(K&&w(K[0],O,N)){t.level&&t.report(t.events.ONVIEW_TRACKING_TRIGGERED,K[1],K[2],K[3]);
l(K[1],K[2],K[3])
}else{M.push(K)
}}n=M;
t.level&&t.report(t.events.VISIBLE_CHECK_FINISHED,n);
if(n.length===0){i=false;
I.unbind("scroll resize",G);
I.unbind(g.join(" "),G);
t.level&&t.report(t.events.DETACH_VIEW_LISTENER)
}window.setTimeout(function(){C=false
},e)
}function x(J,K,L){return function(M,N){if(a(L,M,N)){f(J,K,L,M,N)
}else{t.level&&t.report(t.events.WONT_ATTACH_EVENT_TRACKING,J,K,L,M,N)
}}
}function m(J,K){return{track:x(J,K,v.experiment),stage:x(J,K,v.stage),goal:x(J,K,v.goal),custom_goal:x(J,K,v.custom_goal)}
}function h(J){return function(K){return{exp:function(L){return m(J,K).track(L)
},stage:function(L,M){return m(J,K).stage(L,M)
},goal:function(L){return m(J,K).goal(L)
},custom_goal:function(L,M){return m(J,K).custom_goal(L,M)
}}
}
}function A(J){var K=b(J);
return j.r[K]||j.f[K]||0
}function u(J){if(arguments.length>1){throw"Track only accept one parameter"
}if(!J){return 0
}return l(v.experiment,J)
}function H(K,J){if(!K){return false
}if(J===undefined){return false
}if(J===0){return u(K)
}if(!/^[1-9]$/.test(J)){return false
}return l(v.stage,K,J)
}function p(J){if(!J){return false
}return l(v.goal,J)
}function o(K,J){if(!K||!J||!/^[1-5]$/.test(J)){return false
}return l(v.custom_goal,K,J)
}function F(){}F.prototype.variant=u;
F.prototype.track=u;
F.prototype.stage=H;
F.prototype.goal=p;
F.prototype.custom_goal=o;
F.prototype.on=m;
F.prototype.compat=h;
F.prototype.debug_level=A("GCbGTMEbZDVHT");
var E=new F();
if(A("GCbGTdeVSKe")){t.level&&t.report("using compatibility mode");
B.jst&&Object.keys(B.jst).forEach(function(J){j.r[b(J)]=B.jst[J]
});
