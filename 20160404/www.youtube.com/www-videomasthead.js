(function(){var m,n=this;function r(a){return void 0!==a}
...
m.unsubscribe=function(a,b,c){if(a=this.o[a]){var d=this.f;if(a=va(a,function(a){return d[a+1]==b&&d[a+2]==c}))return this.X(a)}return!1};
m.X=function(a){var b=this.f[a];if(b){var c=this.o[b];if(0!=this.j)this.h.push(a),this.f[a+1]=aa;else{if(c){var d=sa(c,a);0<=d&&Array.prototype.splice.call(c,d,1)}delete this.f[a];delete this.f[a+1];delete this.f[a+2]}}return!!b};
...
m.clear=function(a){if(a){var b=this.o[a];b&&(y(b,this.X,this),delete this.o[a])}else this.f.length=0,this.o={}};
m.na=function(a){if(a){var b=this.o[a];return b?b.length:0}a=0;for(b in this.o)a+=this.na(b);return a};
m.G=function(){C.W.G.call(this);this.clear();this.h.length=0};var tb=window.yt&&window.yt.config_||window.ytcfg&&window.ytcfg.data_||{};t("yt.config_",tb,void 0);t("yt.tokens_",window.yt&&window.yt.tokens_||{},void 0);var ub=window.yt&&window.yt.msgs_||u("window.ytcfg.msgs")||{};t("yt.msgs_",ub,void 0);function vb(a){var b=arguments;if(1<b.length){var c=b[0];tb[c]=b[1]}else for(c in b=b[0],b)tb[c]=b[c]}
...
function xb(a,b){var c=u("yt.logging.errors.log");c?c(a,b,void 0,void 0):(c=D("ERRORS",[]),c.push([a,b,void 0,void 0]),vb("ERRORS",c))}
...
m.round=function(){this.width=Math.round(this.width);this.height=Math.round(this.height);return this};var ec=z("Opera")||z("OPR"),G=z("Trident")||z("MSIE"),fc=z("Edge"),gc=z("Gecko")&&!(-1!=Ma.toLowerCase().indexOf("webkit")&&!z("Edge"))&&!(z("Trident")||z("MSIE"))&&!z("Edge"),hc=-1!=Ma.toLowerCase().indexOf("webkit")&&!z("Edge"),ic=z("Macintosh"),jc=z("Windows");function kc(){var a=n.document;return a?a.documentMode:void 0}
...
var ld=/\s*;\s*/;m=kd.prototype;m.set=function(a,b,c,d,e,f){if(/[;=\s]/.test(a))throw Error('Invalid cookie name "'+a+'"');if(/[;\r\n]/.test(b))throw Error('Invalid cookie value "'+b+'"');r(c)||(c=-1);e=e?";domain="+e:"";d=d?";path="+d:"";f=f?";secure":"";c=0>c?"":0==c?";expires="+(new Date(1970,1,1)).toUTCString():";expires="+(new Date(na()+1E3*c)).toUTCString();this.f.cookie=a+"="+b+e+d+c+f};
m.get=function(a,b){for(var c=a+"=",d=(this.f.cookie||"").split(ld),e=0,f;f=d[e];e++){if(0==f.lastIndexOf(c,0))return f.substr(c.length);if(f==a)return""}return b};
...
m=zd.prototype;m.getHeight=function(){return this.bottom-this.top};
m.clone=function(){return new zd(this.top,this.right,this.bottom,this.left)};
m.contains=function(a){return this&&a?a instanceof zd?a.left>=this.left&&a.right<=this.right&&a.top>=this.top&&a.bottom<=this.bottom:a.x>=this.left&&a.x<=this.right&&a.y>=this.top&&a.y<=this.bottom:!1};
...
m.round=function(){this.top=Math.round(this.top);this.right=Math.round(this.right);this.bottom=Math.round(this.bottom);this.left=Math.round(this.left);return this};function Ad(a,b,c,d){this.left=a;this.top=b;this.width=c;this.height=d}
m=Ad.prototype;m.clone=function(){return new Ad(this.left,this.top,this.width,this.height)};
m.contains=function(a){return a instanceof Ad?this.left<=a.left&&this.left+this.width>=a.left+a.width&&this.top<=a.top&&this.top+this.height>=a.top+a.height:a.x>=this.left&&a.x<=this.left+this.width&&a.y>=this.top&&a.y<=this.top+this.height};
...
m.round=function(){this.left=Math.round(this.left);this.top=Math.round(this.top);this.width=Math.round(this.width);this.height=Math.round(this.height);return this};function Bd(a,b){var c=xc(a);return c.defaultView&&c.defaultView.getComputedStyle&&(c=c.defaultView.getComputedStyle(a,null))?c[b]||c.getPropertyValue(b)||"":""}
...
;var Ld=z("Firefox"),Md=(z("Chrome")||z("CriOS"))&&!(z("Opera")||z("OPR"))&&!z("Edge");function Nd(){var a;if(a=md.get("PREF",void 0)){a=unescape(a).split("&");for(var b=0;b<a.length;b++){var c=a[b].split("="),d=c[0];(c=c[1])&&(Od[d]=c.toString())}}}
...
10)||0;a=[c,d,e]}else a=[0,0,0];this.l=b;b=a;this.f=b[0];this.h=b[1];this.j=b[2];if(0>=this.f){var g,h,k,l;if(Ab)try{g=new ActiveXObject("ShockwaveFlash.ShockwaveFlash")}catch(p){g=null}else k=document.body,l=document.createElement("object"),l.setAttribute("type","application/x-shockwave-flash"),g=k.appendChild(l);if(g&&"GetVariable"in g)try{h=g.GetVariable("$version")}catch(p){h=""}k&&l&&k.removeChild(l);(g=h||"")?(g=g.split(" ")[1].split(","),g=[parseInt(g[0],10)||0,parseInt(g[1],10)||0,parseInt(g[2],
...
function Oe(a,b,c){for(var d={0:0},e=0,f=0,g=c.length;f<g;f++){var h=c[f],k=a;h.timing>a&&(k=h.timing);d[k]||(d[k]=0);h=(h.f-h.l)*(h.j-h.h);d[k]+=h;e+=h}f=b.width*b.height;0<f&&(f=.1*Math.max(f-e,0),a in d||(d[a]=0),d[a]+=f,e+=f);a=[];if(e){for(var l in d)b=new Qe(parseFloat(l),d[l]),a.push(b);a.sort(function(a,b){return a.timing-b.timing});
...
x(cf,B);var ef=["drm.unavailable","fmt.noneavailable","html5.missingapi","html5.unsupportedads","html5.unsupportedlive"];m=cf.prototype;m.qa=function(a,b){this.isDisposed()||(ff(this,a),gf(this,b),this.D&&hf(this))};
...
m.wb=function(){if(!this.Y){var a=nf(this);a&&"html5"==of(this)?(this.B="html5",this.D||this.N()):(pf(this),this.B="html5",a&&this.P?(this.l.appendChild(this.P),this.N()):(this.f.loaded=!0,this.R=w(function(){var a=this.l,c=this.f.clone();u("yt.player.Application.create")(a,c);this.N()},this),this.Y=!0,a?this.R():(Mb(this.f.assets.js,this.R),de(this.f.assets.css))))}};
m.vb=function(){var a=this.f.clone();if(!this.C){var b=mf(this);b&&(this.C=document.createElement("span"),this.C.tabIndex=0,te(this.Z,this.C,"focus",this.Ea),this.K=document.createElement("span"),this.K.tabIndex=0,te(this.Z,this.K,"focus",this.Ea),b.parentNode&&b.parentNode.insertBefore(this.C,b),b.parentNode&&b.parentNode.insertBefore(this.K,b.nextSibling))}a.attrs.width=a.attrs.width||"100%";a.attrs.height=a.attrs.height||"100%";if("flash"==of(this))this.B="flash",this.D||$d(a,!1,w(this.N,this));
...
m.A=function(a,b){if(!this.isDisposed()){var c=jf(this,b);if(c){if(!wa(this.Wa,a)&&!this.j[a]){var d=rf(this,a);this.S&&this.S(a,d)}this.U.subscribe(a,c);"onReady"==a&&this.D&&E(ma(c,this.api),0)}}};
m.ub=function(a,b){if(!this.isDisposed()){var c=jf(this,b);c&&this.U.unsubscribe(a,c)}};
...
m.wa=function(a){Jb("a11y-announce",a)};
m.lb=function(a){lf(this,a)};
m.nb=function(a){Jb("WATCH_LATER_VIDEO_ADDED",a)};
m.ob=function(a){Jb("WATCH_LATER_VIDEO_REMOVED",a)};
m.jb=function(){this.la||(Md?(this.ia=Dc(document),te(this.L,window,"scroll",this.tb),te(this.L,this.l,"touchmove",this.rb)):(te(this.L,this.l,"mousewheel",this.Ha),te(this.L,this.l,"wheel",this.Ha)),this.la=!0)};
m.kb=function(){ue(this.L);this.la=!1};
...
m.eb=function(){return this.B||of(this)};
...
h}var h=D("TIMING_INFO",{}),k;for(k in h)e[k]=h[k];k=e.srt;delete e.srt;var l;void 0===d.srt&&(k||0===k||(l=Q.timing||{},k=Math.max(0,l.responseStart-l.navigationStart),isNaN(k)&&e.pt&&(k=e.pt)),k||0===k)&&(e.srt=Math.round(k));e.h5jse&&(h=window.location.protocol+u("ytplayer.config.assets.js"),(h=Q.getEntriesByName?Q.getEntriesByName(h)[0]:null)?e.h5jse=Math.round(e.h5jse-h.responseEnd):delete e.h5jse);d.aft=Ve();h=d._start;if("cold"==e.yt_lt){l||(l=Q.timing||{});var p;a:if(p=l,p.msFirstPaint)p=
Math.max(0,p.msFirstPaint);else{var q=window.chrome;if(q&&(q=q.loadTimes,ea(q))){var q=q(),Aa=1E3*Math.min(q.requestTime||Infinity,q.startLoadTime||Infinity),Aa=Infinity===Aa?0:p.navigationStart-Aa;p=Math.max(0,Math.round(1E3*q.firstPaintTime+Aa)||0);break a}p=0}0<p&&p>h&&(d.fpt=p);p=c||af().span;q=l.redirectEnd-l.redirectStart;0<q&&(p.rtime_=q);q=l.domainLookupEnd-l.domainLookupStart;0<q&&(p.dns_=q);q=l.connectEnd-l.connectStart;0<q&&(p.tcp_=q);q=l.connectEnd-l.secureConnectionStart;l.secureConnectionStart>=
l.navigationStart&&0<q&&(p.stcp_=q);q=l.responseStart-l.requestStart;0<q&&(p.req_=q);q=l.responseEnd-l.responseStart;0<q&&(p.rcv_=q);Q.getEntriesByType&&Ye(d);(l=D("SPEEDINDEX_FOR_ACTIONS",void 0))&&-1<l.indexOf(g)&&(l=zb(),g=Ke(),l=zb()-l,0<g.ga&&(e.si=g.ga,d.vsc=Xe(g.Na),c.sid=l));l=[];if(document.querySelector&&Q&&Q.getEntriesByName){var g={'script[name="player/base"]':"pj",'link[rel="stylesheet"][name="www-player"]':"pc",'script[name="scheduler/scheduler"]':"sj",'script[name="desktop_polymer/desktop_polymer"]':"dpj",
'link[rel="import"][name="desktop_polymer"]':"dph"},Z;for(Z in g)if(p=document.querySelector(Z))q="",Aa=p.nodeName,"SCRIPT"==Aa?q=p.src:"LINK"==Aa&&(q=p.href),q&&(p=Q.getEntriesByName(q))&&p[0]&&l.push(g[Z]+"."+Math.round(p[0].duration))}l.length&&(e.rs=l.join(","))}g=We();Z=g.pbr;l=g.vc;g=g.pbs;Z&&l&&g&&Z<l&&l<g&&1==af().info.yt_vis&&"youtube"==f&&(af().info.yt_lt="hot_bg",Z=d.vc,f=d.pbs,delete d.aft,c.aft=Math.round(f-Z));(Z=D("PREVIOUS_ACTION"))&&(e.pa=Z);e.p=D("CLIENT_PROTOCOL")||"unknown";e.t=
...
x(rg,If);ba(rg);m=rg.prototype;m.register=function(){U(this,"mouseover",this.ea);U(this,"mouseout",this.H);U(this,"focus",this.Aa);U(this,"blur",this.xa);U(this,"click",this.H);U(this,"touchstart",this.Ka);U(this,"touchend",this.ha);U(this,"touchcancel",this.ha)};
m.unregister=function(){V(this,"mouseover",this.ea);V(this,"mouseout",this.H);V(this,"focus",this.Aa);V(this,"blur",this.xa);V(this,"click",this.H);V(this,"touchstart",this.Ka);V(this,"touchend",this.ha);V(this,"touchcancel",this.ha);this.dispose();rg.W.unregister.call(this)};
m.dispose=function(){for(var a in this.h)this.H(this.h[a]);this.h={}};
m.ea=function(a){if(!(this.f&&1E3>na()-this.f)){var b=parseInt(A(a,"tooltip-hide-timer"),10);b&&($a(a,"tooltip-hide-timer"),window.clearTimeout(b));var b=w(function(){sg(this,a);$a(a,"tooltip-show-timer")},this),c=parseInt(A(a,"tooltip-show-delay"),10)||0,b=E(b,c);
Ya(a,"tooltip-show-timer",b.toString());a.title&&(Kf(a,tg(a)),a.title="");b=ga(a).toString();this.h[b]=a}};
m.H=function(a){var b=parseInt(A(a,"tooltip-show-timer"),10);b&&(window.clearTimeout(b),$a(a,"tooltip-show-timer"));b=w(function(){if(a){var b=H(ug(this,a));b&&(vg(b),b&&b.parentNode&&b.parentNode.removeChild(b),$a(a,"content-id"));(b=H(ug(this,a,"arialabel")))&&b.parentNode&&b.parentNode.removeChild(b)}$a(a,"tooltip-hide-timer")},this);
b=E(b,50);Ya(a,"tooltip-hide-timer",b.toString());if(b=A(a,"tooltip-text"))a.title=b;b=ga(a).toString();delete this.h[b]};
m.Aa=function(a){this.f=0;this.ea(a)};
m.xa=function(a){this.f=0;this.H(a)};
m.Ka=function(a,b,c){c.changedTouches&&(this.f=0,a=Hf(b,T(this),c.changedTouches[0].target),this.ea(a))};
m.ha=function(a,b,c){c.changedTouches&&(this.f=na(),a=Hf(b,T(this),c.changedTouches[0].target),this.H(a))};
...
Kc(b),l=ug(a,b,"arialabel"),f=document.createElement("div");Zb(f,T(a,"arialabel"));f.id=l;"rtl"==document.body.getAttribute("dir")?Hc(f,c+" "+k):Hc(f,k+" "+c);b.setAttribute("aria-labelledby",l);k=Sc()||document.body;k.appendChild(f);k.appendChild(d);wg(b,c);(c=parseInt(A(b,"tooltip-max-width"),10))&&e.offsetWidth>c&&(e.style.width=c+"px",Zb(g,T(a,"normal-wrap")));g=Yb(b,T(a,"reverse"));yg(a,b,d,e,h,g)||yg(a,b,d,e,h,!g);var p=T(a,"tip-visible");E(function(){Zb(d,p)},0)}}}}
function yg(a,b,c,d,e,f){cc(c,T(a,"tip-reverse"),f);var g=0;f&&(g=1);a=Fd(b);f=new F((a.width-10)/2,f?a.height:0);var h=xc(b),k=new F(0,0),l;l=h?xc(h):document;l=!G||9<=Number(tc)||Cc(vc(l).f)?l.documentElement:l.body;b!=l&&(l=Dd(b),h=Dc(vc(h).f),k.x=l.left+h.x,k.y=l.top+h.y);f=new F(k.x+f.x,k.y+f.y);f=f.clone();k=(g&8&&"rtl"==Cd(c,"direction")?g^4:g)&-9;g=Fd(c);h=g.clone();l=f.clone();h=h.clone();0!=k&&(k&4?l.x-=h.width+0:k&2&&(l.x-=h.width/2),k&1&&(l.y-=h.height+0));f=new Ad(0,0,0,0);f.left=l.x;
...
h=Id(c,"paddingRight");l=Id(c,"paddingTop");var p=Id(c,"paddingBottom"),k=new zd(l,h,p,k)}else k=Bd(c,"paddingLeft"),h=Bd(c,"paddingRight"),l=Bd(c,"paddingTop"),p=Bd(c,"paddingBottom"),k=new zd(parseFloat(l),parseFloat(h),parseFloat(p),parseFloat(k));if(!G||9<=Number(tc))h=Bd(c,"borderLeftWidth"),l=Bd(c,"borderRightWidth"),p=Bd(c,"borderTopWidth"),q=Bd(c,"borderBottomWidth"),h=new zd(parseFloat(p),parseFloat(l),parseFloat(q),parseFloat(h));else{h=Kd(c,"borderLeft");l=Kd(c,"borderRight");var p=Kd(c,
"borderTop"),q=Kd(c,"borderBottom"),h=new zd(p,l,q,h)}f.pixelWidth=g.width-h.left-k.left-k.right-h.right;f.pixelHeight=g.height-h.top-k.top-k.bottom-h.bottom}else f.pixelWidth=g.width,f.pixelHeight=g.height;f=Bc(window);1==c.nodeType?(c=Dd(c),h=new F(c.left,c.top)):(c=c.changedTouches?c.changedTouches[0]:c,h=new F(c.clientX,c.clientY));c=Fd(d);l=Math.floor(c.width/2);g=!!(f.height<h.y+a.height);a=!!(h.y<a.height);k=!!(h.x<l);f=!!(f.width<h.x+l);h=(c.width+3)/-2- -5;b=A(b,"force-tooltip-direction");
...
m.Ja=function(a,b,c){b?(Ya(a,Ag.ta,"true"),c&&Ya(a,Ag.ua,c)):($a(a,Ag.ta),$a(a,Ag.ua));Cg(a)};
...
m.ab=function(a){var b=!!A(a,"ypc-item-type"),c=!!A(a,"ypc-item-id");!A(a,"ypc-enabled")&&b&&c&&(Zb(a,"ypc-enabled"),Ya(a,Ag.Xa,"true"))};
m.$a=function(a){A(a,"ypc-enabled")&&($b(a,"ypc-enabled"),$a(a,"ypc-enabled"))};
function Dg(a,b){var c=zc(T(a));return ta(c,function(a){return b==A(a,"channel-external-id")},a)}
m.Ya=function(a,b,c){var d=Ea(arguments,2);y(a,function(a){b.apply(this,xa(a,d))},this)};
m.M=function(a,b,c){var d=Dg(this,a),d=xa([d],Ea(arguments,1));this.Ya.apply(this,d)};
