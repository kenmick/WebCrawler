...
d.exports=a.extend({init:function(){this.optionsSpec={delay:{name:"delay",type:Number,defaultValue:300},modelId:{name:"sb-id",type:Number,required:true}};
...
},autocomplete:function(){if(this.input.value){clearTimeout(this.timer);
this.render(null,{loading:true});
this.timer=setTimeout(function(){this.model.getDestinationPredictions(this.input.value,this.render.bind(this))
...
r.__upa__=s.filter(m).slice(0,5).map(function(u){var u=u.get("data");
return{upa:true,nr_hotels:0,nr_hotels_25:0,ss_raw:q.ss,dest_id:u.id,place_id:u.id,dest_type:d[u.types&&u.types[0]]||"landmark",label:u.description,label_blocks:b(u.description,u.matched_substrings),label_highlighted:g(u.description,u.matched_substrings),cjk:(B.env.b_lang==="zh"||B.env.b_lang==="xt"||B.env.b_lang==="ja"||B.env.b_lang==="ko")}
});
r.__js_upa__=true;
t(null,r)
...
q.dest_type=d[r.types&&r.types[0]]||"landmark",s(null,q)
...
}else{j.city=j.city.map(function(l){if(!l.label&&l.labels){l.label=f(l.labels);
